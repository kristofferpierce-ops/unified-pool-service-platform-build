...
SCRIPT = ROOT / 'scripts/phase37_step94_monitored_live_write_ops_final_release_hold_planning_safety_disposition_planning_packet.ps1'
UI = ROOT / 'ui/pages/1910_Phase37_Step94_Monitored_Live_Write_Ops_Final_Release_Hold_Safety_Planning.py'
DOC = ROOT / 'docs/PHASE37_STEP94_MONITORED_LIVE_WRITE_OPS_FINAL_RELEASE_HOLD_PLANNING_SAFETY_DISPOSITION_PLANNING_PACKET.md'
...
def test_phase37_step_94_files_exist() -> None:
    assert SCRIPT.exists()
    assert UI.exists()
    assert DOC.exists()


def test_phase37_step_94_launcher_has_valid_param_block() -> None:
    text = read(SCRIPT)
    assert text.startswith("param(")
    assert not text.startswith("\ufeff")


def test_phase37_step_94_launcher_markers() -> None:
    text = read(SCRIPT)
...
def test_phase37_step_94_ui_page_smoke_markers() -> None:
    text = read(UI)
...
def test_phase37_step_94_doc_mentions_planning_only() -> None:
    text = read(DOC).lower()
    assert "planning-only" in text
    assert "does not activate live writes" in text


def test_phase37_step_94_required_safe_markers_present() -> None:
...
def test_phase37_step_94_forbidden_runtime_markers_not_enabled() -> None:
    compact = (read(SCRIPT) + "\n" + read(UI) + "\n" + read(DOC)).replace(" ", "").lower()
...
def test_phase37_step_94_no_phase38_files_created_by_payload() -> None:
