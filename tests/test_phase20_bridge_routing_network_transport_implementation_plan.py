...
def test_bridge_routing_network_transport_implementation_plan_script_is_get_only() -> None:
    script = read("scripts/phase20_generate_bridge_routing_network_transport_implementation_plan.ps1")
    assert "phase20_bridge_routing_network_transport_implementation_plan_" in script
    assert "bridge_routing_network_transport_implementation_plan_only = $true" in script
    ...
    assert "phase20_bridge_routing_network_transport_implementation_plan_stages.csv" in script
...
def test_bridge_routing_network_transport_implementation_plan_streamlit_page_is_report_only() -> None:
    page = read("ui/pages/65_Bridge_Routing_Network_Transport_Implementation_Plan.py")
    assert "Phase 20 Bridge Routing Network Transport Implementation Plan" in page
    assert "network-transport-implementation-plan-only" in page
    ...
    assert "phase20_bridge_routing_network_transport_implementation_plan_*" in page
    assert "phase20_bridge_routing_network_transport_implementation_plan.json" in page
    assert "requests.get" not in page
    assert "requests.post" not in page
    assert "st.download_button" in page


def test_bridge_routing_network_transport_implementation_plan_docs_capture_boundaries() -> None:
    doc = read("docs/PHASE20_STEP13_BRIDGE_ROUTING_NETWORK_TRANSPORT_IMPLEMENTATION_PLAN.md")
    assert "design-only implementation plan" in doc
    assert "network-transport-implementation-plan-only" in doc
    assert "bridge GET only" in doc
    assert "does not mutate bridge state" in doc
    assert "does not call LACRM" in doc
    assert "Do not stage generated implementation plan reports" in doc
