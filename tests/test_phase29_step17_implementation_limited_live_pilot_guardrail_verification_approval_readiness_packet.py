...
SCRIPT = REPO / "scripts/phase29_step17_implementation_limited_live_pilot_guardrail_verification_approval_readiness_packet.ps1"
UI_PAGE = REPO / "ui/pages/873_Phase29_Step17_Implementation_Limited_Live_Pilot_Guardrail_Verification_Approval_Readiness_Packet.py"
DOC = REPO / "docs/PHASE29_STEP17_IMPLEMENTATION_LIMITED_LIVE_PILOT_GUARDRAIL_VERIFICATION_APPROVAL_READINESS_PACKET.md"
TEST = REPO / "tests/test_phase29_step17_implementation_limited_live_pilot_guardrail_verification_approval_readiness_packet.py"
...
def test_phase29_step17_files_exist():
...
def test_phase29_step17_launcher_supports_optimized_actions():
    script = read(SCRIPT)
    assert 'ValidateSet("status", "apply", "smoke", "packet", "all")' in script
    assert "APPLY PASS: Phase 29 Step 17" in script
    assert "SMOKE TEST PASS: Phase 29 Step 17" in script


def test_phase29_step17_safety_markers_present():
...
def test_phase29_step17_does_not_enable_runtime_or_phase30():
...
def test_phase29_step17_doc_declares_no_phase30_boundary():
    doc = read(DOC).lower()
...
def test_phase29_step17_ui_is_reference_only():
    ui = read(UI_PAGE)
...
def test_phase29_step17_prior_step_is_recorded():
    combined = read(SCRIPT) + read(DOC) + read(UI_PAGE)
    assert "Phase 29 Step 16 - Phase 20 Network Transport Implementation Limited Live Pilot Guardrail Verification Approval Boundary Packet" in combined


def test_phase29_step17_uses_exact_four_step_files():
    script = read(SCRIPT)
    for rel in [
