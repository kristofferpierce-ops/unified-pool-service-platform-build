...
SCRIPT = REPO / "scripts/phase29_step108_implementation_limited_live_pilot_final_release_hold_verification_planning_operator_hold_point_packet.ps1"
UI_PAGE = REPO / "ui/pages/964_Phase29_Step108_Implementation_Limited_Live_Pilot_Final_Release_Hold_Verification_Planning_Operator_Hold_Point_Packet.py"
DOC = REPO / "docs/PHASE29_STEP108_IMPLEMENTATION_LIMITED_LIVE_PILOT_FINAL_RELEASE_HOLD_VERIFICATION_PLANNING_OPERATOR_HOLD_POINT_PACKET.md"
TEST = REPO / "tests/test_phase29_step108_implementation_limited_live_pilot_final_release_hold_verification_planning_operator_hold_point_packet.py"
...
def test_phase29_step108_files_exist():
...
def test_phase29_step108_launcher_supports_optimized_actions():
    script = read(SCRIPT)
    assert 'ValidateSet("status", "apply", "smoke", "packet", "all")' in script
    assert "APPLY PASS: Phase 29 Step 108" in script
    assert "SMOKE TEST PASS: Phase 29 Step 108" in script


def test_phase29_step108_safety_markers_present():
...
def test_phase29_step108_does_not_enable_runtime_or_phase30():
...
def test_phase29_step108_doc_declares_no_phase30_boundary():
    doc = read(DOC).lower()
...
def test_phase29_step108_ui_is_reference_only():
    ui = read(UI_PAGE)
...
def test_phase29_step108_prior_step_is_recorded():
    combined = read(SCRIPT) + read(DOC) + read(UI_PAGE)
    assert "Phase 29 Step 107 - Phase 20 Network Transport Implementation Limited Live Pilot Final Release Hold Verification Planning Approval Readiness Packet" in combined


def test_phase29_step108_uses_exact_four_step_files():
    script = read(SCRIPT)
    for rel in [
