...
SCRIPT = REPO / "scripts/phase25_step95_post_closeout_final_release_hold_planning_safety_disposition_review_packet.ps1"
UI = REPO / "ui/pages/471_Phase25_Step95_Implementation_PostCloseout_Final_Release_Hold_Planning_Safety_Disposition_Review_Packet.py"
DOC = REPO / "docs/PHASE25_STEP95_POST_CLOSEOUT_FINAL_RELEASE_HOLD_PLANNING_SAFETY_DISPOSITION_REVIEW_PACKET.md"
TEST = REPO / "tests/test_phase25_step95_post_closeout_final_release_hold_planning_safety_disposition_review_packet.py"
...
def test_phase25_step95_files_exist():
...
def test_phase25_step95_launcher_markers():
    text = read(SCRIPT).lower()
    assert "phase 25 step 95" in text
    assert "phase25-step95-post-closeout-final-release-hold-planning-safety-disposition-review" in text
...
def test_phase25_step95_planning_only_safety_markers():
...
def test_phase25_step95_does_not_start_runtime_or_next_phase():
...
def test_phase25_step95_complexity_gate_allows_standard_planning_only_batch():
    text = compact(read(SCRIPT) + read(DOC) + read(UI))
    assert "complexity_batch_gate" in text
...
def test_phase25_step95_docs_are_operator_readable():
    text = read(DOC)
    assert "Phase 25 Step 95 - Phase 20 Network Transport Implementation Post-Closeout Final Release Hold Planning Safety Disposition Review Packet" in text
...
def test_phase25_step95_ui_is_streamlit_reference_only_page():
    text = read(UI)
...
def test_phase25_step95_test_file_self_identifies_step():
    text = read(TEST).lower()
    assert "step95" in text or "step 95" in text
