...
SCRIPT = REPO / "scripts/phase36_step55_controlled_live_write_expansion_no_write_dry_run_result_review_planning_safety_review_packet.ps1"
UI = REPO / "ui/pages/1751_Phase36_Step55_Live_Write_Expansion_Result_Review_Safety_Review.py"
DOC = REPO / "docs/PHASE36_STEP55_CONTROLLED_LIVE_WRITE_EXPANSION_NO_WRITE_DRY_RUN_RESULT_REVIEW_PLANNING_SAFETY_REVIEW_PACKET.md"
TEST = REPO / "tests/test_phase36_step55_controlled_live_write_expansion_no_write_dry_run_result_review_planning_safety_review_packet.py"
...
def test_phase36_step_55_files_exist() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert path.exists(), f"missing expected file: {path}"


def test_phase36_step_55_launcher_has_required_safe_markers() -> None:
    text = read(SCRIPT)
    for marker in [
        "planning_only = $true",
...
def test_phase36_step_55_operational_files_do_not_enable_forbidden_markers() -> None:
    forbidden_keys = [
        "phase37_start",
...
def test_phase36_step_55_ui_and_doc_reference_dry_run_no_apply() -> None:
    text = read(UI) + "\n" + read(DOC)
...
def test_phase36_step_55_launcher_apply_passes() -> None:
...
def test_phase36_step_55_launcher_smoke_passes() -> None:
    result = subprocess.run(
