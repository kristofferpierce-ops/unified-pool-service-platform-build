...
SCRIPT = REPO_ROOT / "scripts/phase26_step6_implementation_transition_readiness_approval_boundary_packet.ps1"
PAGE = REPO_ROOT / "ui/pages/502_Phase26_Step6_Implementation_Transition_Readiness_Approval_Boundary_Packet.py"
DOC = REPO_ROOT / "docs/PHASE26_STEP6_IMPLEMENTATION_TRANSITION_READINESS_APPROVAL_BOUNDARY_PACKET.md"
TEST = REPO_ROOT / "tests/test_phase26_step6_implementation_transition_readiness_approval_boundary_packet.py"
...
def test_phase26_step6_files_are_present():
    for path in (SCRIPT, PAGE, DOC, TEST):
        assert path.exists(), f"missing {path}"


def test_phase26_step6_launcher_markers_are_present():
    text = read(SCRIPT).lower()
    assert "phase 26 step 6" in text
    assert "phase26-step6-implementation-transition-readiness-approval-boundary" in text
...
def test_phase26_step6_planning_only_safety_markers():
...
def test_phase26_step6_does_not_start_runtime_or_next_phase():
    text = operational_text()
    forbidden = [
        "phase27_start=true",
...
def test_phase26_step6_batch_risk_review_is_chat_only_not_powershell_prompt():
    text = operational_text()
    assert "batch_risk_review_location" in text
...
def test_phase26_step6_launcher_supports_optimized_actions_and_known_fixes():
    script = read(SCRIPT)
    assert "ValidateSet(\"status\", \"apply\", \"smoke\", \"packet\", \"all\")" in script
    assert "APPLY PASS: Phase 26 Step 6" in script
    assert "SMOKE TEST PASS: Phase 26 Step 6" in script
    assert "CHECK: packet_json=" in script
    assert "$File:" not in script
    assert "Join-Path $RepoRoot \"scripts/phase26_step6_implementation_transition_readiness_approval_boundary_packet.ps1\"," not in script
    assert "System.Object[]" not in script


def test_phase26_step6_docs_are_operator_readable():
    text = read(DOC)
    assert "Phase 26 Step 6 - Phase 20 Network Transport Implementation Transition Readiness Approval Boundary Packet" in text
...
def test_phase26_step6_ui_is_streamlit_reference_only_page():
...
def test_phase26_step6_test_file_self_identifies_step():
    text = read(TEST).lower()
    assert "step6" in text or "step 6" in text
