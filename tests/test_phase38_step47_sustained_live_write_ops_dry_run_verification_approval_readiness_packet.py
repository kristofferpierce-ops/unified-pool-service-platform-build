...
SCRIPT = ROOT / 'scripts/phase38_step47_sustained_live_write_ops_dry_run_verification_approval_readiness_packet.ps1'
UI = ROOT / 'ui/pages/1983_Phase38_Step47_Sustained_Live_Write_Ops_Dry_Run_Verification_Approval_Readiness.py'
DOC = ROOT / 'docs/PHASE38_STEP47_SUSTAINED_LIVE_WRITE_OPS_DRY_RUN_VERIFICATION_APPROVAL_READINESS_PACKET.md'
TEST = ROOT / 'tests/test_phase38_step47_sustained_live_write_ops_dry_run_verification_approval_readiness_packet.py'
...
def test_phase38_step47_files_exist() -> None:
    for path in [SCRIPT, UI, DOC, TEST]:
        assert path.exists(), path


def test_phase38_step47_launcher_has_valid_param_block() -> None:
    text = read(SCRIPT)
    assert text.lstrip().startswith("param(")
    assert "\ufeff" not in text[:1]


def test_phase38_step47_launcher_runs_all() -> None:
...
def test_phase38_step47_ui_compiles() -> None:
    compile(read(UI), str(UI), "exec")


def test_phase38_step47_doc_has_phase_and_step() -> None:
    text = read(DOC)
    assert "Phase: 38" in text
    assert "Step: 47" in text
    assert "phase39_start=false" in text.replace(" ", "").lower()


def test_phase38_step47_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase38_step47_required_safe_markers_present() -> None:
    compact = compact_text(SCRIPT, UI, DOC)
    required = [
        "planning_only=true",
...
def test_phase38_step47_no_phase39_files_created_by_payload() -> None:
