...
def test_bridge_routing_network_transport_interface_scaffold_operator_signoff_script_is_get_only() -> None:
    script = read("scripts/phase20_generate_bridge_routing_network_transport_interface_scaffold_operator_signoff.ps1")
    assert "phase20_bridge_routing_network_transport_interface_scaffold_operator_signoff_" in script
    assert "bridge_routing_network_transport_interface_scaffold_operator_signoff_only = $true" in script
    ...
    assert "phase20_bridge_routing_network_transport_interface_scaffold_preflight_matrix_*" in script
...
def test_bridge_routing_network_transport_interface_scaffold_operator_signoff_streamlit_page_is_report_only() -> None:
    page = read("ui/pages/69_Bridge_Routing_Network_Transport_Interface_Scaffold_Operator_Signoff.py")
    assert "Phase 20 Bridge Routing Network Transport Interface Scaffold Operator Signoff" in page
    assert "network-transport-interface-scaffold-operator-signoff-only" in page
    ...
    assert "phase20_bridge_routing_network_transport_interface_scaffold_operator_signoff_*" in page
    assert "phase20_bridge_routing_network_transport_interface_scaffold_operator_signoff.json" in page
    assert "requests.get" not in page
    assert "requests.post" not in page
    assert "st.download_button" in page


def test_bridge_routing_network_transport_interface_scaffold_operator_signoff_docs_capture_boundaries() -> None:
    doc = read("docs/PHASE20_STEP17_BRIDGE_ROUTING_NETWORK_TRANSPORT_INTERFACE_SCAFFOLD_OPERATOR_SIGNOFF.md")
    ...
    assert "network-transport-interface-scaffold-operator-signoff-only" in doc
