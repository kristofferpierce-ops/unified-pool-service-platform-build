...
def test_bridge_routing_network_transport_dry_run_invocation_path_validation_script_is_get_only() -> None:
    script = read("scripts/phase20_validate_bridge_routing_network_transport_dry_run_invocation_path.ps1")
    assert "phase20_bridge_routing_network_transport_dry_run_invocation_path_validation_" in script
    assert "bridge_routing_network_transport_dry_run_invocation_path_validation_only = $true" in script
...
def test_bridge_routing_network_transport_dry_run_invocation_path_validation_streamlit_page_is_report_only() -> None:
    page = read("ui/pages/72_Bridge_Routing_Network_Transport_Dry_Run_Invocation_Path_Validation.py")
    assert "Phase 20 Bridge Routing Network Transport Dry Run Invocation Path Validation" in page
    assert "network-transport-dry-run-invocation-path-validation-only" in page
    ...
    assert "phase20_bridge_routing_network_transport_dry_run_invocation_path_validation_*" in page
    assert "phase20_bridge_routing_network_transport_dry_run_invocation_path_validation.json" in page
    assert "requests.get" not in page
    assert "requests.post" not in page
    assert "st.download_button" in page


def test_bridge_routing_network_transport_dry_run_invocation_path_validation_docs_capture_boundaries() -> None:
    doc = read("docs/PHASE20_STEP20_BRIDGE_ROUTING_NETWORK_TRANSPORT_DRY_RUN_INVOCATION_PATH_VALIDATION.md")
    assert "validates the Phase 20 Step 19 bridge routing network transport dry-run invocation path" in doc
    assert "network-transport-dry-run-invocation-path-validation-only" in doc
    ...
    assert "Do not stage generated dry-run invocation path validation reports" in doc
