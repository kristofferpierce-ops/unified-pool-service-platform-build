...
SCRIPT = REPO / "scripts/phase25_step97_post_closeout_final_release_hold_planning_approval_readiness_packet.ps1"
UI = REPO / "ui/pages/473_Phase25_Step97_Implementation_PostCloseout_Final_Release_Hold_Planning_Approval_Readiness_Packet.py"
DOC = REPO / "docs/PHASE25_STEP97_POST_CLOSEOUT_FINAL_RELEASE_HOLD_PLANNING_APPROVAL_READINESS_PACKET.md"
TEST = REPO / "tests/test_phase25_step97_post_closeout_final_release_hold_planning_approval_readiness_packet.py"
...
def test_phase25_step97_files_exist():
...
def test_phase25_step97_launcher_markers():
    text = read(SCRIPT).lower()
    assert "phase 25 step 97" in text
    assert "phase25-step97-post-closeout-final-release-hold-planning-approval-readiness" in text
...
def test_phase25_step97_planning_only_safety_markers():
...
def test_phase25_step97_does_not_start_runtime_or_next_phase():
...
def test_phase25_step97_complexity_gate_allows_standard_planning_only_batch():
    text = compact(read(SCRIPT) + read(DOC) + read(UI))
    assert "complexity_batch_gate" in text
...
def test_phase25_step97_docs_are_operator_readable():
    text = read(DOC)
    assert "Phase 25 Step 97 - Phase 20 Network Transport Implementation Post-Closeout Final Release Hold Planning Approval Readiness Packet" in text
...
def test_phase25_step97_ui_is_streamlit_reference_only_page():
    text = read(UI)
...
def test_phase25_step97_test_file_self_identifies_step():
    text = read(TEST).lower()
    assert "step97" in text or "step 97" in text
