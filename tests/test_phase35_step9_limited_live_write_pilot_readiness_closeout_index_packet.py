...
SCRIPT = REPO / "scripts/phase35_step9_limited_live_write_pilot_readiness_closeout_index_packet.ps1"
UI_PAGE = REPO / "ui/pages/1585_Phase35_Step9_Limited_Live_Write_Pilot_Readiness_Closeout_Index.py"
DOC = REPO / "docs/PHASE35_STEP9_LIMITED_LIVE_WRITE_PILOT_READINESS_CLOSEOUT_INDEX_PACKET.md"
TEST = REPO / "tests/test_phase35_step9_limited_live_write_pilot_readiness_closeout_index_packet.py"
...
def test_phase35_step9_files_exist():
...
def test_phase35_step9_launcher_supports_optimized_actions():
    script = read(SCRIPT)
    assert 'ValidateSet("status", "apply", "smoke", "packet", "all")' in script
    assert "APPLY PASS: Phase 35 Step 9" in script
    assert "SMOKE TEST PASS: Phase 35 Step 9" in script


def test_phase35_step9_safety_markers_present():
...
def test_phase35_step9_does_not_enable_runtime_or_phase36():
...
def test_phase35_step9_doc_declares_no_phase36_boundary():
    doc = read(DOC).lower()
...
def test_phase35_step9_ui_is_reference_only():
    ui = read(UI_PAGE)
...
def test_phase35_step9_prior_step_is_recorded():
    combined = read(SCRIPT) + read(DOC) + read(UI_PAGE)
    assert "Phase 35 Step 8 - Phase 20 Network Transport Implementation Trusted Production Limited Live Write Pilot Readiness Operator Hold Point Packet" in combined


def test_phase35_step9_uses_exact_four_step_files():
    script = read(SCRIPT)
    for rel in [
