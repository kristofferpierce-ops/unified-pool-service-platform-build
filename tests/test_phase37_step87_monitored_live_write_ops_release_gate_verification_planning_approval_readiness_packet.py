...
SCRIPT = REPO / "scripts/phase37_step87_monitored_live_write_ops_release_gate_verification_planning_approval_readiness_packet.ps1"
UI = REPO / "ui/pages/1903_Phase37_Step87_Monitored_Live_Write_Ops_Release_Gate_Verification_Approval_Readiness.py"
DOC = REPO / "docs/PHASE37_STEP87_MONITORED_LIVE_WRITE_OPS_RELEASE_GATE_VERIFICATION_PLANNING_APPROVAL_READINESS_PACKET.md"
TEST = REPO / "tests/test_phase37_step87_monitored_live_write_ops_release_gate_verification_planning_approval_readiness_packet.py"
...
def test_phase37_step_87_files_exist() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert path.exists(), f"Missing expected file: {path}"


def test_phase37_step_87_launcher_has_valid_param_block() -> None:
    text = read(SCRIPT)
...
def test_phase37_step_87_payloads_are_bom_free() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert not path.read_bytes().startswith(b"\xef\xbb\xbf"), f"BOM detected in {path}"


def test_phase37_step_87_safety_markers_present() -> None:
    text = read(SCRIPT) + "\n" + read(UI) + "\n" + read(DOC)
    required = [
        "planning_only=true",
...
def test_phase37_step_87_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase37_step_87_launcher_smoke_passes() -> None:
