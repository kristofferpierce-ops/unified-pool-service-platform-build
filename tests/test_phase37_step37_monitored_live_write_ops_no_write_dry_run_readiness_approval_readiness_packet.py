...
SCRIPT = REPO / "scripts/phase37_step37_monitored_live_write_ops_no_write_dry_run_readiness_approval_readiness_packet.ps1"
UI = REPO / "ui/pages/1853_Phase37_Step37_Monitored_Live_Write_Ops_No_Write_Dry_Run_Readiness_Approval_Readiness.py"
DOC = REPO / "docs/PHASE37_STEP37_MONITORED_LIVE_WRITE_OPS_NO_WRITE_DRY_RUN_READINESS_APPROVAL_READINESS_PACKET.md"
TEST = REPO / "tests/test_phase37_step37_monitored_live_write_ops_no_write_dry_run_readiness_approval_readiness_packet.py"
...
def test_phase37_step_37_files_exist() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert path.exists(), f"Missing expected file: {path}"


def test_phase37_step_37_launcher_has_valid_param_block() -> None:
    text = read(SCRIPT)
...
def test_phase37_step_37_safety_markers_present() -> None:
    text = read(SCRIPT) + "\n" + read(UI) + "\n" + read(DOC)
    required = [
...
def test_phase37_step_37_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase37_step_37_launcher_smoke_passes() -> None:
