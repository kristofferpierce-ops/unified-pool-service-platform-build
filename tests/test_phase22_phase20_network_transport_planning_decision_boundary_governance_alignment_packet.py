...
STEP_SCRIPT = ROOT / "scripts" / "phase22_generate_phase20_network_transport_planning_decision_boundary_governance_alignment_packet.ps1"
STEP_UI = ROOT / "ui" / "pages" / "136_Phase20_Network_Transport_Planning_Decision_Boundary_Governance_Alignment_Packet.py"
STEP_DOC = ROOT / "docs" / "PHASE22_STEP30_PHASE20_NETWORK_TRANSPORT_PLANNING_DECISION_BOUNDARY_GOVERNANCE_ALIGNMENT_PACKET.md"
STEP_TEST = ROOT / "tests" / "test_phase22_phase20_network_transport_planning_decision_boundary_governance_alignment_packet.py"
...
def test_phase22_step30_files_exist():
    missing = [str(path) for path in STEP_FILES if not path.exists()]
    assert not missing


def test_phase22_step30_document_labels_phase_and_step():
    text = read(STEP_DOC)
    assert "Phase 22 Step 30" in text
    assert "Decision Boundary Governance Alignment Packet" in text
    assert "planning-only" in text


def test_phase22_step30_launcher_supports_optimized_actions():
...
def test_phase22_step30_launcher_is_path_safe_and_idempotent():
    text = read(STEP_SCRIPT)
...
def test_phase22_step30_safety_flags_present():
...
def test_phase22_step30_no_forbidden_runtime_tokens():
...
def test_phase22_step30_packet_outputs_expected_checks():
    text = read(STEP_SCRIPT)
    expected = [
...
def test_phase22_step30_source_bucket_alignment_preserved():
    combined = "\n".join(read(path) for path in STEP_FILES)
...
def test_phase22_step30_decision_boundary_is_planning_only():
    combined = "\n".join(read(path) for path in STEP_FILES)
    assert "decision_boundary_stage" in combined
    assert "planning_alignment_only" in combined
    assert "decision_boundary_governance_runtime" in combined
...
def test_phase22_step30_governance_prerequisites_are_recorded():
    combined = "\n".join(read(path) for path in STEP_FILES)
    for token in [
...
def test_phase22_step30_ui_is_planning_page_only():
    text = read(STEP_UI)
...
def test_phase22_step30_branch_and_prior_step_are_recorded():
    text = read(STEP_SCRIPT)
    assert "phase22-step30-phase20-network-transport-planning-decision-boundary-governance-alignment-packet" in text
    assert "Phase 22 Step 29 - Phase 20 Network Transport Planning Recommendation Engine Alignment Packet" in text
