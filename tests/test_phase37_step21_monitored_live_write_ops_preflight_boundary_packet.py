...
SCRIPT = REPO / "scripts/phase37_step21_monitored_live_write_ops_preflight_boundary_packet.ps1"
UI = REPO / "ui/pages/1837_Phase37_Step21_Monitored_Live_Write_Ops_Preflight_Boundary.py"
DOC = REPO / "docs/PHASE37_STEP21_MONITORED_LIVE_WRITE_OPS_PREFLIGHT_BOUNDARY_PACKET.md"
TEST = REPO / "tests/test_phase37_step21_monitored_live_write_ops_preflight_boundary_packet.py"
...
def test_phase37_step_21_files_exist() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert path.exists(), f"Missing expected file: {path}"


def test_phase37_step_21_launcher_has_valid_param_block() -> None:
    text = read(SCRIPT)
...
def test_phase37_step_21_safety_markers_present() -> None:
    text = read(SCRIPT) + "\n" + read(UI) + "\n" + read(DOC)
    required = [
...
def test_phase37_step_21_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase37_step_21_launcher_smoke_passes() -> None:
