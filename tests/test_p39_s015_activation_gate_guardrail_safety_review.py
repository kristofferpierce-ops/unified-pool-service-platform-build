...
SCRIPT = ROOT / "scripts/p39_s015_activation_gate_guardrail_safety_review.ps1"
UI = ROOT / "ui/pages/2071_Phase39_Step15_Activation_Gate_Guardrail_Safety_Review.py"
DOC = ROOT / "docs/P39_S015_ACTIVATION_GATE_GUARDRAIL_SAFETY_REVIEW.md"
...
def test_phase39_step_15_files_exist() -> None:
    assert SCRIPT.exists()
    assert UI.exists()
    assert DOC.exists()


def test_phase39_step_15_launcher_has_valid_param_block() -> None:
    assert read(SCRIPT).lstrip().startswith("param(")


def test_phase39_step_15_safety_markers_present() -> None:
...
def test_phase39_step_15_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase39_step_15_no_phase40_files_created_by_payload() -> None:
...
def test_phase39_step_15_launcher_smoke_passes() -> None:
...
def test_phase39_step_15_ui_is_planning_only() -> None:
    text = read(UI)
...
def test_phase39_step_15_doc_records_activation_gate_hold() -> None:
    text = read(DOC).lower()
    assert "live-write activation gate remains unarmed" in text
    assert "phase40_boundary_creation=false" in text.replace(" ", "")


def test_phase39_step_15_no_server_or_transport_runtime() -> None:
