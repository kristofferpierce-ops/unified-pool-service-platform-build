...
SCRIPT = REPO / "scripts/phase35_step1_limited_live_write_pilot_readiness_boundary_packet.ps1"
UI_PAGE = REPO / "ui/pages/1577_Phase35_Step1_Limited_Live_Write_Pilot_Readiness_Boundary.py"
DOC = REPO / "docs/PHASE35_STEP1_LIMITED_LIVE_WRITE_PILOT_READINESS_BOUNDARY_PACKET.md"
TEST = REPO / "tests/test_phase35_step1_limited_live_write_pilot_readiness_boundary_packet.py"
...
def test_phase35_step1_files_exist():
...
def test_phase35_step1_launcher_supports_optimized_actions():
    script = read(SCRIPT)
    assert 'ValidateSet("status", "apply", "smoke", "packet", "all")' in script
    assert "APPLY PASS: Phase 35 Step 1" in script
    assert "SMOKE TEST PASS: Phase 35 Step 1" in script


def test_phase35_step1_safety_markers_present():
...
def test_phase35_step1_does_not_enable_runtime_or_phase36():
...
def test_phase35_step1_doc_declares_no_phase36_boundary():
    doc = read(DOC).lower()
...
def test_phase35_step1_ui_is_reference_only():
    ui = read(UI_PAGE)
...
def test_phase35_step1_prior_step_is_recorded():
    combined = read(SCRIPT) + read(DOC) + read(UI_PAGE)
    assert "Phase 34 Step 120 - Phase 20 Network Transport Implementation Phase 34 Final No-Write Closeout Hold Packet" in combined


def test_phase35_step1_uses_exact_four_step_files():
    script = read(SCRIPT)
    for rel in [
