...
SCRIPT_REL = "scripts/phase23_step8_implementation_readiness_intake_closeout_index_packet.ps1"
PAGE_REL = "ui/pages/184_Phase23_Step8_Implementation_Readiness_Intake_Closeout_Index_Packet.py"
DOC_REL = "docs/PHASE23_STEP8_IMPLEMENTATION_READINESS_INTAKE_CLOSEOUT_INDEX_PACKET.md"
TEST_REL = "tests/test_phase23_step8_implementation_readiness_intake_closeout_index_packet.py"
...
def test_phase23_step8_files_exist():
    for path in PUBLIC_FILES:
        assert path.exists(), path


def test_phase23_step8_script_mentions_phase_and_step():
    text = read(SCRIPT)
    assert "Phase 23 Step 8" in text
    assert "$PhaseNumber = 23" in text
    assert "$StepNumber = 8" in text
    assert "Implementation Readiness Intake Closeout Index Packet" in text


def test_phase23_step8_page_number_is_correct():
    assert PAGE.as_posix().endswith(
        "ui/pages/184_Phase23_Step8_Implementation_Readiness_Intake_Closeout_Index_Packet.py"
    )


def test_phase23_step8_doc_title_is_correct():
    text = read(DOC)
    assert "# Phase 23 Step 8 - Phase 20 Network Transport Implementation Readiness Intake Closeout Index Packet" in text


def test_phase23_step8_prior_step_is_phase23_step7():
    text = read(SCRIPT) + "\n" + read(DOC)
    ...
    assert "CHECK: prior_step=Phase 23 Step 7" in text


def test_phase23_step8_launcher_supports_action_all():
    text = read(SCRIPT)
...
def test_phase23_step8_uses_literal_and_safe_paths():
    text = read(SCRIPT)
...
def test_phase23_step8_is_planning_only():
...
def test_phase23_step8_opens_phase23_as_intake_only():
    text = combined_public_text()
    required = [
        "phase23_context",
...
def test_phase23_step8_disables_implementation_and_runtime():
    text = combined_public_text()
    required = [
...
def test_phase23_step8_disables_cross_repo_mutation():
    text = combined_public_text()
    required = [
        "cross_repo_write",
        "cross_repo_mutation",
...
def test_phase23_step8_disables_decisions_and_approvals():
    text = combined_public_text()
    required = [
        "readiness_intake_closeout_index_mode",
        "readiness_intake_closeout_index_write",
        "readiness_intake_closeout_index_record_creation",
...
def test_phase23_step8_keeps_lacrm_dry_run():
    text = combined_public_text()
...
def test_phase23_step8_contains_parent_workspace_commands():
    text = read(DOC)
    ...
    assert "INSTALL_PHASE23_STEP8_SINGLE_FILE" in text


def test_phase23_step8_menu_has_expected_options():
    text = read(SCRIPT)
    assert "Phase 23 Step 8 menu" in text
    assert "Show status / verify paths" in text
    assert "Smoke test Phase 23 Step 8" in text
    assert "Show server start placeholder only" in text
    assert "Generate Phase 23 Step 8 implementation readiness intake closeout index packet" in text


def test_phase23_step8_expected_smoke_success_text_present():
    text = read(SCRIPT)
    expected = (
        "SMOKE TEST PASS: Phase 23 Step 8 Phase 20 Network Transport Implementation "
        "Readiness Intake Closeout Index Packet is present and planning-only."
    )
    assert expected in text


def test_phase23_step8_expected_packet_output_in_doc_and_script():
    combined = read(DOC) + "\n" + read(SCRIPT)
    required = [
        ...
        "PASS: readiness_intake_closeout_index_mode=reference_only",
        "PASS: readiness_intake_closeout_index_write=false",
        "PASS: readiness_intake_closeout_index_record_creation=false",
        "PASS: readiness_intake_decision_creation=false",
        "PASS: readiness_intake_approval_creation=false",
        "PASS: phase22_reopen=false",
        "CHECK: prior_step=Phase 23 Step 7",
...
def test_phase23_step8_packet_metadata_is_correct():
    text = read(SCRIPT)
    assert "phase = 23" in text
    assert "step = 8" in text
    assert "phase23_step8_implementation_readiness_intake_closeout_index_packet" in text


def test_phase23_step8_does_not_start_implementation_or_runtime():
...
def test_phase23_step8_does_not_enable_network_or_bridge_runtime():
    text = combined_runtime_text().lower()
    forbidden = [
...
def test_phase23_step8_does_not_write_to_external_repos():
    text = combined_runtime_text().lower()
    forbidden = [
...
def test_phase23_step8_does_not_create_decisions_approvals_or_signoffs():
    text = combined_runtime_text().lower()
    forbidden = [
...
def test_phase23_step8_packet_generates_json_under_backups():
    text = read(SCRIPT)
...
def test_phase23_step8_expected_branch_is_short():
    text = read(SCRIPT)
    branch = "phase23-step8-readiness-intake-closeout-index"
    assert branch in text
    assert len(branch) < 80


def test_phase23_step8_source_zip_filename_not_required_by_launcher():
    text = read(SCRIPT)
    assert "phase23_step8_source(1).zip" not in text
    assert "phase23_step8_source.zip" not in text
    assert "single-file installer" in text.lower()


def test_phase23_step8_installer_paths_are_short():
    for rel in [SCRIPT_REL, PAGE_REL, DOC_REL, TEST_REL]:
        assert len(rel) < 150


def test_phase23_step8_no_server_start_claim():
