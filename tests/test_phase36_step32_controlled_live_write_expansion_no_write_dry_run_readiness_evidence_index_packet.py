...
SCRIPT = REPO / "scripts/phase36_step32_controlled_live_write_expansion_no_write_dry_run_readiness_evidence_index_packet.ps1"
UI = REPO / "ui/pages/1728_Phase36_Step32_Live_Write_Expansion_Dry_Run_Readiness_Evidence_Index.py"
DOC = REPO / "docs/PHASE36_STEP32_CONTROLLED_LIVE_WRITE_EXPANSION_DRY_RUN_READINESS_EVIDENCE_INDEX_PACKET.md"
TEST = REPO / "tests/test_phase36_step32_controlled_live_write_expansion_no_write_dry_run_readiness_evidence_index_packet.py"
...
def test_phase36_step_32_files_exist() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert path.exists(), f"missing expected file: {path}"


def test_phase36_step_32_launcher_has_required_safe_markers() -> None:
    text = read(SCRIPT)
    for marker in [
        "planning_only = $true",
...
def test_phase36_step_32_operational_files_do_not_enable_forbidden_markers() -> None:
    forbidden_keys = [
        "phase37_start",
...
def test_phase36_step_32_ui_and_doc_reference_dry_run_no_apply() -> None:
    text = read(UI) + "\n" + read(DOC)
...
def test_phase36_step_32_launcher_apply_passes() -> None:
...
def test_phase36_step_32_launcher_smoke_passes() -> None:
    result = subprocess.run(
