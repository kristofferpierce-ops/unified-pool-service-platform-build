...
SCRIPT = REPO / "scripts/phase29_step104_implementation_limited_live_pilot_final_release_hold_verification_planning_safety_disposition_planning_packet.ps1"
UI_PAGE = REPO / "ui/pages/960_Phase29_Step104_Implementation_Limited_Live_Pilot_Final_Release_Hold_Verification_Planning_Safety_Disposition_Planning_Packet.py"
DOC = REPO / "docs/PHASE29_STEP104_IMPLEMENTATION_LIMITED_LIVE_PILOT_FINAL_RELEASE_HOLD_VERIFICATION_PLANNING_SAFETY_DISPOSITION_PLANNING_PACKET.md"
TEST = REPO / "tests/test_phase29_step104_implementation_limited_live_pilot_final_release_hold_verification_planning_safety_disposition_planning_packet.py"
...
def test_phase29_step104_files_exist():
...
def test_phase29_step104_launcher_supports_optimized_actions():
    script = read(SCRIPT)
    assert 'ValidateSet("status", "apply", "smoke", "packet", "all")' in script
    assert "APPLY PASS: Phase 29 Step 104" in script
    assert "SMOKE TEST PASS: Phase 29 Step 104" in script


def test_phase29_step104_safety_markers_present():
...
def test_phase29_step104_does_not_enable_runtime_or_phase30():
...
def test_phase29_step104_doc_declares_no_phase30_boundary():
    doc = read(DOC).lower()
...
def test_phase29_step104_ui_is_reference_only():
    ui = read(UI_PAGE)
...
def test_phase29_step104_prior_step_is_recorded():
    combined = read(SCRIPT) + read(DOC) + read(UI_PAGE)
    assert "Phase 29 Step 103 - Phase 20 Network Transport Implementation Limited Live Pilot Final Release Hold Verification Planning Evidence Gap Review Packet" in combined


def test_phase29_step104_uses_exact_four_step_files():
    script = read(SCRIPT)
    for rel in [
