...
SCRIPT = REPO / "scripts/phase37_step32_monitored_live_write_ops_no_write_dry_run_readiness_evidence_index_packet.ps1"
UI = REPO / "ui/pages/1848_Phase37_Step32_Monitored_Live_Write_Ops_No_Write_Dry_Run_Readiness_Evidence_Index.py"
DOC = REPO / "docs/PHASE37_STEP32_MONITORED_LIVE_WRITE_OPS_NO_WRITE_DRY_RUN_READINESS_EVIDENCE_INDEX_PACKET.md"
TEST = REPO / "tests/test_phase37_step32_monitored_live_write_ops_no_write_dry_run_readiness_evidence_index_packet.py"
...
def test_phase37_step_32_files_exist() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert path.exists(), f"Missing expected file: {path}"


def test_phase37_step_32_launcher_has_valid_param_block() -> None:
    text = read(SCRIPT)
...
def test_phase37_step_32_safety_markers_present() -> None:
    text = read(SCRIPT) + "\n" + read(UI) + "\n" + read(DOC)
    required = [
...
def test_phase37_step_32_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase37_step_32_launcher_smoke_passes() -> None:
