...
SCRIPT = REPO_ROOT / "scripts/phase25_step102_post_closeout_final_release_hold_verification_planning_evidence_index_packet.ps1"
PAGE = REPO_ROOT / "ui/pages/478_Phase25_Step102_Implementation_PostCloseout_Final_Release_Hold_Verification_Planning_Evidence_Index_Packet.py"
DOC = REPO_ROOT / "docs/PHASE25_STEP102_POST_CLOSEOUT_FINAL_RELEASE_HOLD_VERIFICATION_PLANNING_EVIDENCE_INDEX_PACKET.md"
TEST = REPO_ROOT / "tests/test_phase25_step102_post_closeout_final_release_hold_verification_planning_evidence_index_packet.py"
...
def test_phase25_step102_files_are_present():
    for path in (SCRIPT, PAGE, DOC, TEST):
        assert path.exists(), f"missing {path}"


def test_phase25_step102_planning_only_markers_are_present():
...
def test_phase25_step102_does_not_enable_runtime_or_next_phase():
    text = operational_text()
    forbidden = [
        "phase26_start" + "=true",
...
def test_phase25_step102_launcher_supports_optimized_actions():
    script = read(SCRIPT)
    assert "ValidateSet(\"status\", \"apply\", \"smoke\", \"packet\", \"all\")" in script
    assert "APPLY PASS: Phase 25 Step 102" in script
    assert "SMOKE TEST PASS: Phase 25 Step 102" in script
    assert "CHECK: packet_json=" in script


def test_phase25_step102_known_powershell_generation_bugs_are_absent():
    script = read(SCRIPT)
    assert "$File:" not in script
    assert "Join-Path $RepoRoot \"scripts/phase25_step102_post_closeout_final_release_hold_verification_planning_evidence_index_packet.ps1\"," not in script
    assert "System.Object[]" not in script


def test_phase25_step102_ui_page_is_reference_only():
...
def test_phase25_step102_doc_records_no_write_safety():
    doc = read(DOC).lower()
...
def test_phase25_step102_file_names_match_step_number():
    assert "step102" in SCRIPT.name
    assert "Step102" in PAGE.name
    assert "STEP102" in DOC.name
    assert "step102" in TEST.name
