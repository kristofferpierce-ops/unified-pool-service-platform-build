...
SCRIPT = REPO / "scripts/phase37_step22_monitored_live_write_ops_preflight_evidence_index_packet.ps1"
UI = REPO / "ui/pages/1838_Phase37_Step22_Monitored_Live_Write_Ops_Preflight_Evidence_Index.py"
DOC = REPO / "docs/PHASE37_STEP22_MONITORED_LIVE_WRITE_OPS_PREFLIGHT_EVIDENCE_INDEX_PACKET.md"
TEST = REPO / "tests/test_phase37_step22_monitored_live_write_ops_preflight_evidence_index_packet.py"
...
def test_phase37_step_22_files_exist() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert path.exists(), f"Missing expected file: {path}"


def test_phase37_step_22_launcher_has_valid_param_block() -> None:
    text = read(SCRIPT)
...
def test_phase37_step_22_safety_markers_present() -> None:
    text = read(SCRIPT) + "\n" + read(UI) + "\n" + read(DOC)
    required = [
...
def test_phase37_step_22_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase37_step_22_launcher_smoke_passes() -> None:
