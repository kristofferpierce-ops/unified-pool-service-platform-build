...
def test_bridge_routing_network_transport_preflight_matrix_script_is_get_only() -> None:
    script = read("scripts/phase20_generate_bridge_routing_network_transport_preflight_matrix.ps1")
    assert "phase20_bridge_routing_network_transport_preflight_matrix_" in script
    assert "bridge_routing_network_transport_preflight_matrix_only = $true" in script
...
def test_bridge_routing_network_transport_preflight_matrix_streamlit_page_is_report_only() -> None:
    page = read("ui/pages/62_Bridge_Routing_Network_Transport_Preflight_Matrix.py")
    assert "Phase 20 Bridge Routing Network Transport Preflight Matrix" in page
    assert "network-transport-preflight-matrix-only" in page
    ...
    assert "phase20_bridge_routing_network_transport_preflight_matrix_*" in page
    assert "phase20_bridge_routing_network_transport_preflight_matrix.json" in page
    assert "requests.get" not in page
    assert "requests.post" not in page
    assert "st.download_button" in page


def test_bridge_routing_network_transport_preflight_matrix_docs_capture_boundaries() -> None:
    doc = read("docs/PHASE20_STEP10_BRIDGE_ROUTING_NETWORK_TRANSPORT_PREFLIGHT_MATRIX.md")
    assert "no-socket preflight matrix" in doc
    assert "network-transport-preflight-matrix-only" in doc
