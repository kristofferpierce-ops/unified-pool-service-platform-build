...
SCRIPT = ROOT / "scripts/phase38_step17_sustained_live_write_ops_guardrail_verification_approval_readiness_packet.ps1"
UI = ROOT / "ui/pages/1953_Phase38_Step17_Sustained_Live_Write_Ops_Guardrail_Verification_Approval_Readiness.py"
DOC = ROOT / "docs/PHASE38_STEP17_SUSTAINED_LIVE_WRITE_OPS_GUARDRAIL_VERIFICATION_APPROVAL_READINESS_PACKET.md"
TEST = ROOT / "tests/test_phase38_step17_sustained_live_write_ops_guardrail_verification_approval_readiness_packet.py"
...
def test_phase38_step17_files_exist() -> None:
...
def test_phase38_step17_launcher_has_valid_param_block() -> None:
    text = read(SCRIPT)
...
def test_phase38_step17_required_safety_markers_present() -> None:
...
def test_phase38_step17_next_phase_guards_present() -> None:
...
def test_phase38_step17_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase38_step17_literal_pass_markers_present() -> None:
    text = read(SCRIPT)
    assert "APPLY PASS" in text
    assert "SMOKE TEST PASS" in text


def test_phase38_step17_ui_is_planning_only_page() -> None:
    text = read(UI)
...
def test_phase38_step17_doc_records_no_runtime_boundary() -> None:
    text = read(DOC)
...
def test_phase38_step17_no_phase39_files_created_by_payload() -> None:
