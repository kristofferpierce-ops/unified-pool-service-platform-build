...
def test_phase20_network_transport_final_cleanup_review_packet_script_is_get_only_and_no_write() -> None:
    script = read("scripts/phase21_generate_phase20_network_transport_final_cleanup_review_packet.ps1")
    assert "phase21_phase20_network_transport_final_cleanup_review_packet_" in script
    assert "phase20_network_transport_final_cleanup_review_packet_only = $true" in script
    assert "final_cleanup_review_only = $true" in script
...
def test_phase20_network_transport_final_cleanup_review_packet_streamlit_page_is_report_only() -> None:
    page = read("ui/pages/105_Phase20_Network_Transport_Final_Cleanup_Review_Packet.py")
    assert "Phase 21 Step 13 - Phase 20 Network Transport Final Cleanup Review Packet" in page
    assert "Phase 20 network transport final-cleanup-review-only" in page
    ...
    assert "phase21_phase20_network_transport_final_cleanup_review_packet_*" in page
    assert "phase21_phase20_network_transport_final_cleanup_review_packet.json" in page
    assert "requests.get" not in page
    assert "requests.post" not in page
    assert "st.download_button" in page


def test_phase20_network_transport_final_cleanup_review_packet_docs_capture_boundaries() -> None:
    doc = read("docs/PHASE21_STEP13_PHASE20_NETWORK_TRANSPORT_FINAL_CLEANUP_REVIEW_PACKET.md")
    assert "no-write final cleanup review packet" in doc
    assert "Phase 20 network transport final-cleanup-review-only" in doc
    ...
    assert "Stage only the four Phase 21 Step 13 files" in doc
