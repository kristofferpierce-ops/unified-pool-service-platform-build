...
def test_bridge_routing_network_transport_operator_confirmation_gate_design_script_is_get_only_and_no_write() -> None:
    script = read("scripts/phase20_generate_bridge_routing_network_transport_operator_confirmation_gate_design.ps1")
    assert "phase20_bridge_routing_network_transport_operator_confirmation_gate_design_" in script
    assert "bridge_routing_network_transport_operator_confirmation_gate_design_only = $true" in script
    ...
    assert "can_create_confirmation_records_now = $false" in script
    assert "transport_design_confirmation_phrase" in script
    assert "live_write_confirmation_phrase" in script
    assert "rollback_confirmation_phrase" in script
    assert "request_hash_confirmation_required" in script
    assert "two_step_confirmation_required" in script
...
def test_bridge_routing_network_transport_operator_confirmation_gate_design_streamlit_page_is_report_only() -> None:
    page = read("ui/pages/79_Bridge_Routing_Network_Transport_Operator_Confirmation_Gate_Design.py")
    assert "Phase 20 Bridge Routing Network Transport Operator Confirmation Gate Design" in page
    assert "network-transport-operator-confirmation-gate-design-only" in page
    ...
    assert "phase20_bridge_routing_network_transport_operator_confirmation_gate_design_*" in page
    assert "phase20_bridge_routing_network_transport_operator_confirmation_gate_design.json" in page
    assert "requests.get" not in page
    assert "requests.post" not in page
    assert "st.download_button" in page


def test_bridge_routing_network_transport_operator_confirmation_gate_design_docs_capture_boundaries() -> None:
    doc = read("docs/PHASE20_STEP27_BRIDGE_ROUTING_NETWORK_TRANSPORT_OPERATOR_CONFIRMATION_GATE_DESIGN.md")
    assert "no-write operator confirmation gate design" in doc
    assert "network-transport-operator-confirmation-gate-design-only" in doc
    ...
    assert "Do not stage generated operator confirmation reports" in doc
