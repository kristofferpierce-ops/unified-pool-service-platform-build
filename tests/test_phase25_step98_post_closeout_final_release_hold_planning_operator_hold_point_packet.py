...
SCRIPT = REPO / "scripts/phase25_step98_post_closeout_final_release_hold_planning_operator_hold_point_packet.ps1"
UI = REPO / "ui/pages/474_Phase25_Step98_Implementation_PostCloseout_Final_Release_Hold_Planning_Operator_Hold_Point_Packet.py"
DOC = REPO / "docs/PHASE25_STEP98_POST_CLOSEOUT_FINAL_RELEASE_HOLD_PLANNING_OPERATOR_HOLD_POINT_PACKET.md"
TEST = REPO / "tests/test_phase25_step98_post_closeout_final_release_hold_planning_operator_hold_point_packet.py"
...
def test_phase25_step98_files_exist():
...
def test_phase25_step98_launcher_markers():
    text = read(SCRIPT).lower()
    assert "phase 25 step 98" in text
    assert "phase25-step98-post-closeout-final-release-hold-planning-operator-hold-point" in text
...
def test_phase25_step98_planning_only_safety_markers():
...
def test_phase25_step98_does_not_start_runtime_or_next_phase():
...
def test_phase25_step98_complexity_gate_allows_standard_planning_only_batch():
    text = compact(read(SCRIPT) + read(DOC) + read(UI))
    assert "complexity_batch_gate" in text
...
def test_phase25_step98_docs_are_operator_readable():
    text = read(DOC)
    assert "Phase 25 Step 98 - Phase 20 Network Transport Implementation Post-Closeout Final Release Hold Planning Operator Hold Point Packet" in text
...
def test_phase25_step98_ui_is_streamlit_reference_only_page():
    text = read(UI)
...
def test_phase25_step98_test_file_self_identifies_step():
    text = read(TEST).lower()
    assert "step98" in text or "step 98" in text
