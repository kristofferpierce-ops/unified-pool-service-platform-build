...
SCRIPT = REPO / "scripts/phase25_step93_post_closeout_final_release_hold_planning_evidence_gap_review_packet.ps1"
UI = REPO / "ui/pages/469_Phase25_Step93_Implementation_PostCloseout_Final_Release_Hold_Planning_Evidence_Gap_Review_Packet.py"
DOC = REPO / "docs/PHASE25_STEP93_POST_CLOSEOUT_FINAL_RELEASE_HOLD_PLANNING_EVIDENCE_GAP_REVIEW_PACKET.md"
TEST = REPO / "tests/test_phase25_step93_post_closeout_final_release_hold_planning_evidence_gap_review_packet.py"
...
def test_phase25_step93_files_exist():
...
def test_phase25_step93_launcher_markers():
    text = read(SCRIPT).lower()
    assert "phase 25 step 93" in text
    assert "phase25-step93-post-closeout-final-release-hold-planning-evidence-gap-review" in text
...
def test_phase25_step93_planning_only_safety_markers():
...
def test_phase25_step93_does_not_start_runtime_or_next_phase():
...
def test_phase25_step93_complexity_gate_allows_standard_planning_only_batch():
    text = compact(read(SCRIPT) + read(DOC) + read(UI))
    assert "complexity_batch_gate" in text
...
def test_phase25_step93_docs_are_operator_readable():
    text = read(DOC)
    assert "Phase 25 Step 93 - Phase 20 Network Transport Implementation Post-Closeout Final Release Hold Planning Evidence Gap Review Packet" in text
...
def test_phase25_step93_ui_is_streamlit_reference_only_page():
    text = read(UI)
...
def test_phase25_step93_test_file_self_identifies_step():
    text = read(TEST).lower()
    assert "step93" in text or "step 93" in text
