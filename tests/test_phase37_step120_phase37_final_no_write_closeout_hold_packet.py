...
SCRIPT = ROOT / "scripts/phase37_step120_phase37_final_no_write_closeout_hold_packet.ps1"
UI = ROOT / "ui/pages/1936_Phase37_Step120_Phase37_Final_No_Write_Closeout_Hold.py"
DOC = ROOT / "docs/PHASE37_STEP120_PHASE37_FINAL_NO_WRITE_CLOSEOUT_HOLD_PACKET.md"
...
def test_phase37_step_120_files_exist() -> None:
    assert SCRIPT.exists()
    assert UI.exists()
    assert DOC.exists()


def test_phase37_step_120_launcher_has_valid_param_block() -> None:
    assert read(SCRIPT).lstrip().startswith("param(")


def test_phase37_step_120_payloads_are_bom_free() -> None:
    for path in [SCRIPT, UI, DOC]:
        data = path.read_bytes()
        assert not data.startswith(b"\xef\xbb\xbf")


def test_phase37_step_120_safe_markers_present() -> None:
...
def test_phase37_step_120_forbidden_runtime_markers_not_enabled() -> None:
    compact = (read(SCRIPT) + "\n" + read(UI) + "\n" + read(DOC)).replace(" ", "").lower()
    forbidden_true = [
        "phase38_start=true",
...
def test_phase37_step_120_no_phase38_files_created_by_payload() -> None:
...
def test_phase37_step_120_launcher_smoke_passes() -> None:
...
def test_phase37_step_120_ui_is_short_named() -> None:
