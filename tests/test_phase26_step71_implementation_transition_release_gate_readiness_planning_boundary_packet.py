...
SCRIPT = REPO_ROOT / "scripts/phase26_step71_implementation_transition_release_gate_readiness_planning_boundary_packet.ps1"
PAGE = REPO_ROOT / "ui/pages/567_Phase26_Step71_Implementation_Transition_Release_Gate_Readiness_Planning_Boundary_Packet.py"
DOC = REPO_ROOT / "docs/PHASE26_STEP71_IMPLEMENTATION_TRANSITION_RELEASE_GATE_READINESS_PLANNING_BOUNDARY_PACKET.md"
TEST = REPO_ROOT / "tests/test_phase26_step71_implementation_transition_release_gate_readiness_planning_boundary_packet.py"
...
def test_phase26_step71_files_are_present():
    for path in (SCRIPT, PAGE, DOC, TEST):
        assert path.exists(), f"missing {path}"


def test_phase26_step71_launcher_markers_are_present():
    text = read(SCRIPT).lower()
    assert "phase 26 step 71" in text
    assert "phase26-step71-implementation-transition-release-gate-readiness-planning-boundary" in text
...
def test_phase26_step71_planning_only_safety_markers():
...
def test_phase26_step71_does_not_start_runtime_or_next_phase():
    text = operational_text()
    forbidden = [
        "phase27_start=true",
...
def test_phase26_step71_batch_risk_review_is_chat_only_not_powershell_prompt():
    text = operational_text()
    assert "batch_risk_review_location" in text
...
def test_phase26_step71_launcher_supports_optimized_actions_and_known_fixes():
    script = read(SCRIPT)
    assert "ValidateSet(\"status\", \"apply\", \"smoke\", \"packet\", \"all\")" in script
    assert "APPLY PASS: Phase 26 Step 71" in script
    assert "SMOKE TEST PASS: Phase 26 Step 71" in script
    assert "CHECK: packet_json=" in script
    assert "$File:" not in script
    assert "Join-Path $RepoRoot \"scripts/phase26_step71_implementation_transition_release_gate_readiness_planning_boundary_packet.ps1\"," not in script
    assert "System.Object[]" not in script


def test_phase26_step71_docs_are_operator_readable():
    text = read(DOC)
    assert "Phase 26 Step 71 - Phase 20 Network Transport Implementation Transition Release Gate Readiness Planning Boundary Packet" in text
...
def test_phase26_step71_ui_is_streamlit_reference_only_page():
...
def test_phase26_step71_test_file_self_identifies_step():
    text = read(TEST).lower()
    assert "step71" in text or "step 71" in text
