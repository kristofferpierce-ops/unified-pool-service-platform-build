...
SCRIPT = REPO / "scripts/phase38_step71_sustained_live_write_ops_release_gate_readiness_planning_boundary_packet.ps1"
UI = REPO / "ui/pages/2007_Phase38_Step71_Sustained_Live_Write_Ops_Release_Gate_Readiness_Boundary.py"
DOC = REPO / "docs/PHASE38_STEP71_SUSTAINED_LIVE_WRITE_OPS_RELEASE_GATE_READINESS_PLANNING_BOUNDARY_PACKET.md"
TEST = REPO / "tests/test_phase38_step71_sustained_live_write_ops_release_gate_readiness_planning_boundary_packet.py"
...
def test_phase38_step_71_files_exist() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert path.exists(), f"Missing expected file: {path}"


def test_phase38_step_71_launcher_has_valid_param_block() -> None:
    text = read(SCRIPT)
...
def test_phase38_step_71_payloads_are_bom_free() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert not path.read_bytes().startswith(b"\xef\xbb\xbf"), f"BOM detected in {path}"


def test_phase38_step_71_safety_markers_present() -> None:
    text = read(SCRIPT) + "\n" + read(UI) + "\n" + read(DOC)
    required = [
        "planning_only=true",
...
def test_phase38_step_71_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase38_step_71_launcher_smoke_passes() -> None:
