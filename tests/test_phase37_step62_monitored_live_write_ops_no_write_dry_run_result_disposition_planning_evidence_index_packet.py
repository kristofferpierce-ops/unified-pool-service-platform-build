...
SCRIPT = REPO / "scripts/phase37_step62_monitored_live_write_ops_no_write_dry_run_result_disposition_planning_evidence_index_packet.ps1"
UI = REPO / "ui/pages/1878_Phase37_Step62_Monitored_Live_Write_Ops_Result_Disposition_Evidence_Index.py"
DOC = REPO / "docs/PHASE37_STEP62_MONITORED_LIVE_WRITE_OPS_NO_WRITE_DRY_RUN_RESULT_DISPOSITION_PLANNING_EVIDENCE_INDEX_PACKET.md"
TEST = REPO / "tests/test_phase37_step62_monitored_live_write_ops_no_write_dry_run_result_disposition_planning_evidence_index_packet.py"
...
def test_phase37_step_62_files_exist() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert path.exists(), f"Missing expected file: {path}"


def test_phase37_step_62_launcher_has_valid_param_block() -> None:
    text = read(SCRIPT)
...
def test_phase37_step_62_payloads_are_bom_free() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert not path.read_bytes().startswith(b"\xef\xbb\xbf"), f"BOM detected in {path}"


def test_phase37_step_62_safety_markers_present() -> None:
    text = read(SCRIPT) + "\n" + read(UI) + "\n" + read(DOC)
    required = [
        "planning_only=true",
...
def test_phase37_step_62_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase37_step_62_launcher_smoke_passes() -> None:
