...
SCRIPT = REPO / "scripts/phase25_step94_post_closeout_final_release_hold_planning_safety_disposition_planning_packet.ps1"
UI = REPO / "ui/pages/470_Phase25_Step94_Implementation_PostCloseout_Final_Release_Hold_Planning_Safety_Disposition_Planning_Packet.py"
DOC = REPO / "docs/PHASE25_STEP94_POST_CLOSEOUT_FINAL_RELEASE_HOLD_PLANNING_SAFETY_DISPOSITION_PLANNING_PACKET.md"
TEST = REPO / "tests/test_phase25_step94_post_closeout_final_release_hold_planning_safety_disposition_planning_packet.py"
...
def test_phase25_step94_files_exist():
...
def test_phase25_step94_launcher_markers():
    text = read(SCRIPT).lower()
    assert "phase 25 step 94" in text
    assert "phase25-step94-post-closeout-final-release-hold-planning-safety-disposition-planning" in text
...
def test_phase25_step94_planning_only_safety_markers():
...
def test_phase25_step94_does_not_start_runtime_or_next_phase():
...
def test_phase25_step94_complexity_gate_allows_standard_planning_only_batch():
    text = compact(read(SCRIPT) + read(DOC) + read(UI))
    assert "complexity_batch_gate" in text
...
def test_phase25_step94_docs_are_operator_readable():
    text = read(DOC)
    assert "Phase 25 Step 94 - Phase 20 Network Transport Implementation Post-Closeout Final Release Hold Planning Safety Disposition Planning Packet" in text
...
def test_phase25_step94_ui_is_streamlit_reference_only_page():
    text = read(UI)
...
def test_phase25_step94_test_file_self_identifies_step():
    text = read(TEST).lower()
    assert "step94" in text or "step 94" in text
