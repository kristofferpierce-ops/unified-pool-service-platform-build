...
SCRIPT = REPO / "scripts/phase25_step99_post_closeout_final_release_hold_planning_closeout_index_packet.ps1"
UI = REPO / "ui/pages/475_Phase25_Step99_Implementation_PostCloseout_Final_Release_Hold_Planning_Closeout_Index_Packet.py"
DOC = REPO / "docs/PHASE25_STEP99_POST_CLOSEOUT_FINAL_RELEASE_HOLD_PLANNING_CLOSEOUT_INDEX_PACKET.md"
TEST = REPO / "tests/test_phase25_step99_post_closeout_final_release_hold_planning_closeout_index_packet.py"
...
def test_phase25_step99_files_exist():
...
def test_phase25_step99_launcher_markers():
    text = read(SCRIPT).lower()
    assert "phase 25 step 99" in text
    assert "phase25-step99-post-closeout-final-release-hold-planning-closeout-index" in text
...
def test_phase25_step99_planning_only_safety_markers():
...
def test_phase25_step99_does_not_start_runtime_or_next_phase():
...
def test_phase25_step99_complexity_gate_allows_standard_planning_only_batch():
    text = compact(read(SCRIPT) + read(DOC) + read(UI))
    assert "complexity_batch_gate" in text
...
def test_phase25_step99_docs_are_operator_readable():
    text = read(DOC)
    assert "Phase 25 Step 99 - Phase 20 Network Transport Implementation Post-Closeout Final Release Hold Planning Closeout Index Packet" in text
...
def test_phase25_step99_ui_is_streamlit_reference_only_page():
    text = read(UI)
...
def test_phase25_step99_test_file_self_identifies_step():
    text = read(TEST).lower()
    assert "step99" in text or "step 99" in text
