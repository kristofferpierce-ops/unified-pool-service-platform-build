...
SCRIPT = ROOT / "scripts/p39_s004_activation_gate_readiness_safety_planning.ps1"
UI = ROOT / "ui/pages/2060_Phase39_Step4_Activation_Gate_Readiness_Safety_Planning.py"
DOC = ROOT / "docs/P39_S004_ACTIVATION_GATE_READINESS_SAFETY_PLANNING.md"
...
def test_phase39_step_4_files_exist() -> None:
    assert SCRIPT.exists()
    assert UI.exists()
    assert DOC.exists()


def test_phase39_step_4_launcher_has_valid_param_block() -> None:
    text = read(SCRIPT)
    assert text.startswith("param(")


def test_phase39_step_4_launcher_runs_all() -> None:
...
def test_phase39_step_4_ui_compiles() -> None:
    py_compile.compile(str(UI), doraise=True)


def test_phase39_step_4_doc_has_required_safe_markers() -> None:
...
def test_phase39_step_4_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase39_step_4_no_phase40_files_created_by_payload() -> None:
...
def test_phase39_step_4_literal_pass_markers_present() -> None:
    text = read(SCRIPT)
