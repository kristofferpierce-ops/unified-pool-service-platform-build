...
SCRIPT = REPO / "scripts/phase37_step25_monitored_live_write_ops_preflight_safety_review_packet.ps1"
UI = REPO / "ui/pages/1841_Phase37_Step25_Monitored_Live_Write_Ops_Preflight_Safety_Review.py"
DOC = REPO / "docs/PHASE37_STEP25_MONITORED_LIVE_WRITE_OPS_PREFLIGHT_SAFETY_REVIEW_PACKET.md"
TEST = REPO / "tests/test_phase37_step25_monitored_live_write_ops_preflight_safety_review_packet.py"
...
def test_phase37_step_25_files_exist() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert path.exists(), f"Missing expected file: {path}"


def test_phase37_step_25_launcher_has_valid_param_block() -> None:
    text = read(SCRIPT)
...
def test_phase37_step_25_safety_markers_present() -> None:
    text = read(SCRIPT) + "\n" + read(UI) + "\n" + read(DOC)
    required = [
...
def test_phase37_step_25_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase37_step_25_launcher_smoke_passes() -> None:
