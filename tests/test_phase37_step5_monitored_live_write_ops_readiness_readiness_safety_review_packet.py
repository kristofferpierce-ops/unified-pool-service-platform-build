...
SCRIPT = REPO / "scripts/phase37_step5_monitored_live_write_ops_readiness_readiness_safety_review_packet.ps1"
UI = REPO / "ui/pages/1821_Phase37_Step5_Monitored_Live_Write_Ops_Readiness_Safety_Review.py"
DOC = REPO / "docs/PHASE37_STEP5_MONITORED_LIVE_WRITE_OPS_READINESS_READINESS_SAFETY_REVIEW_PACKET.md"
TEST = REPO / "tests/test_phase37_step5_monitored_live_write_ops_readiness_readiness_safety_review_packet.py"
...
def test_phase37_step5_files_exist():
...
def test_phase37_step5_launcher_has_valid_param_block():
    text = _read(SCRIPT)
...
def test_phase37_step5_required_safety_markers_present():
...
def test_phase37_step5_next_phase_guards_present():
    combined = "\n".join(_read(path) for path in (SCRIPT, UI, DOC))
    assert "phase38_start" in combined
    assert "phase38_boundary_creation" in combined


def test_phase37_step5_forbidden_true_markers_absent():
...
def test_phase37_step5_literal_pass_markers_present():
    text = _read(SCRIPT)
    assert "APPLY PASS" in text
    assert "SMOKE TEST PASS" in text


def test_phase37_step5_ui_is_planning_only_page():
    text = _read(UI)
...
def test_phase37_step5_doc_records_no_runtime_boundary():
    text = _read(DOC)
