...
SCRIPT = ROOT / "scripts/p39_s025_activation_gate_preflight_safety_review.ps1"
UI = ROOT / "ui/pages/2081_Phase39_Step25_Activation_Gate_Preflight_Safety_Review.py"
DOC = ROOT / "docs/P39_S025_ACTIVATION_GATE_PREFLIGHT_SAFETY_REVIEW.md"
...
def test_phase39_step_25_files_exist() -> None:
    assert SCRIPT.exists()
    assert UI.exists()
    assert DOC.exists()


def test_phase39_step_25_launcher_has_valid_param_block() -> None:
    assert read(SCRIPT).lstrip().startswith("param(")


def test_phase39_step_25_safety_markers_present() -> None:
...
def test_phase39_step_25_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase39_step_25_no_phase40_files_created_by_payload() -> None:
...
def test_phase39_step_25_launcher_smoke_passes() -> None:
...
def test_phase39_step_25_ui_is_planning_only() -> None:
    text = read(UI)
...
def test_phase39_step_25_doc_records_activation_gate_hold() -> None:
    text = read(DOC).lower()
    assert "live-write activation gate remains unarmed" in text
    assert "phase40_boundary_creation=false" in text.replace(" ", "")


def test_phase39_step_25_no_server_or_transport_runtime() -> None:
