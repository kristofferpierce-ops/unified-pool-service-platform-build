...
SCRIPT = REPO / "scripts/phase33_step97_trusted_production_rollout_final_release_hold_planning_approval_readiness_packet.ps1"
UI_PAGE = REPO / "ui/pages/1433_Phase33_Step97_Implementation_Trusted_Production_Rollout_Final_Release_Hold_Planning_Approval_Readiness_Packet.py"
DOC = REPO / "docs/PHASE33_STEP97_TRUSTED_PRODUCTION_ROLLOUT_FINAL_RELEASE_HOLD_PLANNING_APPROVAL_READINESS_PACKET.md"
TEST = REPO / "tests/test_phase33_step97_trusted_production_rollout_final_release_hold_planning_approval_readiness_packet.py"
...
def test_phase33_step97_files_exist():
...
def test_phase33_step97_launcher_supports_optimized_actions():
    script = read(SCRIPT)
    assert 'ValidateSet("status", "apply", "smoke", "packet", "all")' in script
    assert "APPLY PASS: Phase 33 Step 97" in script
    assert "SMOKE TEST PASS: Phase 33 Step 97" in script


def test_phase33_step97_safety_markers_present():
...
def test_phase33_step97_does_not_enable_runtime_or_phase34():
...
def test_phase33_step97_doc_declares_no_phase34_boundary():
    doc = read(DOC).lower()
...
def test_phase33_step97_ui_is_reference_only():
    ui = read(UI_PAGE)
...
def test_phase33_step97_prior_step_is_recorded():
    combined = read(SCRIPT) + read(DOC) + read(UI_PAGE)
    assert "Phase 33 Step 96 - Phase 20 Network Transport Implementation Trusted Production Rollout Final Release Hold Planning Approval Boundary Packet" in combined


def test_phase33_step97_uses_exact_four_step_files():
    script = read(SCRIPT)
    for rel in [
