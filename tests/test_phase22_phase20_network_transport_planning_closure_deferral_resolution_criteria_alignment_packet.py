...
STEP_SCRIPT = REPO_ROOT / "scripts/phase22_generate_phase20_network_transport_planning_closure_deferral_resolution_criteria_alignment_packet.ps1"
UI_PAGE = REPO_ROOT / "ui/pages/149_Phase20_Network_Transport_Planning_Closure_Deferral_Resolution_Criteria_Alignment_Packet.py"
DOC_FILE = REPO_ROOT / "docs/PHASE22_STEP43_PHASE20_NETWORK_TRANSPORT_PLANNING_CLOSURE_DEFERRAL_RESOLUTION_CRITERIA_ALIGNMENT_PACKET.md"
TEST_FILE = REPO_ROOT / "tests/test_phase22_phase20_network_transport_planning_closure_deferral_resolution_criteria_alignment_packet.py"
...
def test_phase22_step43_files_exist():
    for path in STEP_FILES:
        assert path.exists(), f"Missing {path}"


def test_phase22_step43_launcher_has_expected_title_and_branch():
    text = read(STEP_SCRIPT)
    assert "Phase 22 Step 43 - Phase 20 Network Transport Planning Closure Deferral Resolution Criteria Alignment Packet" in text
    assert "phase22-step43-phase20-network-transport-planning-closure-deferral-resolution-criteria-alignment-packet" in text
    assert "Phase 22 Step 42" in text


def test_phase22_step43_launcher_supports_optimized_actions():
    text = read(STEP_SCRIPT)
...
def test_phase22_step43_launcher_is_path_safe():
    text = read(STEP_SCRIPT)
...
def test_phase22_step43_launcher_has_planning_safety_flags():
    text = read(STEP_SCRIPT)
    required = [
        "planning_only = $true",
...
def test_phase22_step43_launcher_blocks_approval_closure_resolution_and_queue_creation():
    text = read(STEP_SCRIPT)
    blocked = [
...
def test_phase22_step43_launcher_generates_packet_under_backups():
    text = read(STEP_SCRIPT)
    assert "backups" in text
    assert "phase22_phase20_network_transport_planning_closure_deferral_resolution_criteria_alignment_packet.json" in text
    assert "ConvertTo-Json -Depth 12" in text
    assert "CHECK: packet_json=$PacketPath" in text


def test_phase22_step43_smoke_success_text_is_present():
    text = read(STEP_SCRIPT)
    assert "SMOKE TEST PASS: Phase 22 Step 43 Phase 20 Network Transport Planning Closure Deferral Resolution Criteria Alignment Packet is present and planning-only." in text


def test_phase22_step43_expected_packet_checks_are_present():
    text = read(STEP_SCRIPT)
    checks = [
...
def test_phase22_step43_forbidden_tokens_are_split_in_launcher():
    text = read(STEP_SCRIPT)
    assert '("lacrm_live" + "_write=true")' in text
    assert '("closure_decision" + "_record_creation=true")' in text
    assert '("closure_deferral_resolution" + "_record_creation=true")' in text
    assert '("closure_deferral_resolution" + "_queue_creation=true")' in text
    assert "Forbidden token found in ${RelativePath}: $Token" in text


def test_phase22_step43_doc_mentions_resolution_criteria_not_record_creation():
    text = read(DOC_FILE)
    assert "Closure deferral resolution criteria rule" in text
    assert "does not resolve the deferred closure decision" in text
    assert "Blocked outputs" in text


def test_phase22_step43_doc_contains_rollout_alignment():
    text = read(DOC_FILE)
...
def test_phase22_step43_ui_is_read_only_and_planning_only():
    text = read(UI_PAGE)
    assert "read-only" in text
    assert "planning_only" in text
    assert "closure_deferral_resolution_criteria_planned" in text
    assert "does not create closure records" in text


def test_phase22_step43_ui_blocks_runtime_and_writes():
    text = read(UI_PAGE)
    blocked = [
        ...
        "closure_deferral_resolution_record_creation",
        "closure_deferral_resolution_queue_creation",
...
def test_phase22_step43_no_forbidden_runtime_tokens_in_public_files():
...
def test_phase22_step43_step_file_list_matches_expected_paths():
    text = read(STEP_SCRIPT)
    for rel in ["scripts/phase22_generate_phase20_network_transport_planning_closure_deferral_resolution_criteria_alignment_packet.ps1", "ui/pages/149_Phase20_Network_Transport_Planning_Closure_Deferral_Resolution_Criteria_Alignment_Packet.py", "docs/PHASE22_STEP43_PHASE20_NETWORK_TRANSPORT_PLANNING_CLOSURE_DEFERRAL_RESOLUTION_CRITERIA_ALIGNMENT_PACKET.md", "tests/test_phase22_phase20_network_transport_planning_closure_deferral_resolution_criteria_alignment_packet.py"]:
        assert rel in text


def test_phase22_step43_manual_menu_is_preserved():
    text = read(STEP_SCRIPT)
    assert "Phase 22 Step 43 menu" in text
    assert "1. Show status / verify paths" in text
    assert "2. Apply Phase 22 Step 43" in text
    assert "3. Smoke test Phase 22 Step 43" in text
    assert "4. Show server start placeholder only" in text
    assert "5. Generate Phase 20 network transport planning closure deferral resolution criteria alignment packet" in text
    assert "6. Exit" in text


def test_phase22_step43_resolution_criteria_are_planning_reference_only():
    ...
    assert "future evidence requirements" in text
