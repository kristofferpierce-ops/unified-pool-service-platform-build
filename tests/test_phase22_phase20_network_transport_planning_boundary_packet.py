...
def test_phase20_network_transport_planning_boundary_packet_script_is_get_only_and_no_write() -> None:
    script = read("scripts/phase22_generate_phase20_network_transport_planning_boundary_packet.ps1")
    assert "phase22_phase20_network_transport_planning_boundary_packet_" in script
    assert "phase20_network_transport_planning_boundary_packet_only = $true" in script
    assert "planning_boundary_only = $true" in script
    ...
    assert "phase22_phase20_network_transport_planning_entry_packet_*" in script
    assert "phase21_phase20_network_transport_cleanup_exit_packet_*" in script
    assert "phase21_phase20_network_transport_final_cleanup_review_packet_*" in script
...
def test_phase20_network_transport_planning_boundary_packet_streamlit_page_is_report_only() -> None:
    page = read("ui/pages/108_Phase20_Network_Transport_Planning_Boundary_Packet.py")
    assert "Phase 22 Step 2 - Phase 20 Network Transport Planning Boundary Packet" in page
    assert "Phase 20 network transport planning-boundary-only" in page
    ...
    assert "phase22_phase20_network_transport_planning_boundary_packet_*" in page
    assert "phase22_phase20_network_transport_planning_boundary_packet.json" in page
    assert "requests.get" not in page
    assert "requests.post" not in page
    assert "st.download_button" in page


def test_phase20_network_transport_planning_boundary_packet_docs_capture_boundaries() -> None:
    doc = read("docs/PHASE22_STEP2_PHASE20_NETWORK_TRANSPORT_PLANNING_BOUNDARY_PACKET.md")
    assert "no-write planning boundary packet" in doc
    assert "Phase 20 network transport planning-boundary-only" in doc
    ...
    assert "Stage only the four Phase 22 Step 2 files" in doc
