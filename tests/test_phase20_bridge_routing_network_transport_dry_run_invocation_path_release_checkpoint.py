...
def test_bridge_routing_network_transport_dry_run_invocation_path_release_checkpoint_script_is_get_only() -> None:
    script = read("scripts/phase20_generate_bridge_routing_network_transport_dry_run_invocation_path_release_checkpoint.ps1")
    assert "phase20_bridge_routing_network_transport_dry_run_invocation_path_release_checkpoint_" in script
    assert "bridge_routing_network_transport_dry_run_invocation_path_release_checkpoint_only = $true" in script
    ...
    assert "phase20_bridge_routing_network_transport_dry_run_invocation_path_release_checkpoint_artifacts.csv" in script
...
def test_bridge_routing_network_transport_dry_run_invocation_path_release_checkpoint_streamlit_page_is_report_only() -> None:
    page = read("ui/pages/75_Bridge_Routing_Network_Transport_Dry_Run_Invocation_Path_Release_Checkpoint.py")
    assert "Phase 20 Bridge Routing Network Transport Dry Run Invocation Path Release Checkpoint" in page
    assert "network-transport-dry-run-invocation-path-release-checkpoint-only" in page
    ...
    assert "phase20_bridge_routing_network_transport_dry_run_invocation_path_release_checkpoint_*" in page
    assert "phase20_bridge_routing_network_transport_dry_run_invocation_path_release_checkpoint.json" in page
    assert "requests.get" not in page
    assert "requests.post" not in page
    assert "st.download_button" in page


def test_bridge_routing_network_transport_dry_run_invocation_path_release_checkpoint_docs_capture_boundaries() -> None:
    doc = read("docs/PHASE20_STEP23_BRIDGE_ROUTING_NETWORK_TRANSPORT_DRY_RUN_INVOCATION_PATH_RELEASE_CHECKPOINT.md")
    assert "no-socket release checkpoint" in doc
    assert "network-transport-dry-run-invocation-path-release-checkpoint-only" in doc
