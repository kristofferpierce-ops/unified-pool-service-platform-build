...
SCRIPT = REPO / "scripts/phase35_step25_limited_live_write_pilot_preflight_planning_safety_disposition_review_packet.ps1"
UI_PAGE = REPO / "ui/pages/1601_Phase35_Step25_Live_Write_Pilot_Preflight_Safety_Review.py"
DOC = REPO / "docs/PHASE35_STEP25_LIMITED_LIVE_WRITE_PILOT_PREFLIGHT_PLANNING_SAFETY_DISPOSITION_REVIEW_PACKET.md"
TEST = REPO / "tests/test_phase35_step25_limited_live_write_pilot_preflight_planning_safety_disposition_review_packet.py"
...
def test_phase35_step25_files_exist():
...
def test_phase35_step25_launcher_supports_optimized_actions():
    script = read(SCRIPT)
    assert 'ValidateSet("status", "apply", "smoke", "packet", "all")' in script
    assert "APPLY PASS: Phase 35 Step 25" in script
    assert "SMOKE TEST PASS: Phase 35 Step 25" in script


def test_phase35_step25_safety_markers_present():
...
def test_phase35_step25_does_not_enable_runtime_or_phase36():
...
def test_phase35_step25_doc_declares_no_phase36_boundary():
    doc = read(DOC).lower()
...
def test_phase35_step25_ui_is_reference_only():
    ui = read(UI_PAGE)
...
def test_phase35_step25_prior_step_is_recorded():
    combined = read(SCRIPT) + read(DOC) + read(UI_PAGE)
    assert "Phase 35 Step 24 - Phase 20 Network Transport Implementation Trusted Production Limited Live Write Pilot Preflight Planning Safety Disposition Planning Packet" in combined


def test_phase35_step25_uses_exact_four_step_files():
    script = read(SCRIPT)
    for rel in [
