...
SCRIPT = REPO / "scripts/phase37_step27_monitored_live_write_ops_preflight_approval_readiness_packet.ps1"
UI = REPO / "ui/pages/1843_Phase37_Step27_Monitored_Live_Write_Ops_Preflight_Approval_Readiness.py"
DOC = REPO / "docs/PHASE37_STEP27_MONITORED_LIVE_WRITE_OPS_PREFLIGHT_APPROVAL_READINESS_PACKET.md"
TEST = REPO / "tests/test_phase37_step27_monitored_live_write_ops_preflight_approval_readiness_packet.py"
...
def test_phase37_step_27_files_exist() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert path.exists(), f"Missing expected file: {path}"


def test_phase37_step_27_launcher_has_valid_param_block() -> None:
    text = read(SCRIPT)
...
def test_phase37_step_27_safety_markers_present() -> None:
    text = read(SCRIPT) + "\n" + read(UI) + "\n" + read(DOC)
    required = [
...
def test_phase37_step_27_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase37_step_27_launcher_smoke_passes() -> None:
