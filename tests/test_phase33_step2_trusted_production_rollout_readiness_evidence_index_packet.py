...
SCRIPT = REPO / "scripts/phase33_step2_trusted_production_rollout_readiness_evidence_index_packet.ps1"
UI_PAGE = REPO / "ui/pages/1338_Phase33_Step2_Implementation_Trusted_Production_Rollout_Readiness_Evidence_Index_Packet.py"
DOC = REPO / "docs/PHASE33_STEP2_TRUSTED_PRODUCTION_ROLLOUT_READINESS_EVIDENCE_INDEX_PACKET.md"
TEST = REPO / "tests/test_phase33_step2_trusted_production_rollout_readiness_evidence_index_packet.py"
...
def test_phase33_step2_files_exist():
...
def test_phase33_step2_launcher_supports_optimized_actions():
    script = read(SCRIPT)
    assert 'ValidateSet("status", "apply", "smoke", "packet", "all")' in script
    assert "APPLY PASS: Phase 33 Step 2" in script
    assert "SMOKE TEST PASS: Phase 33 Step 2" in script


def test_phase33_step2_safety_markers_present():
...
def test_phase33_step2_does_not_enable_runtime_or_phase34():
...
def test_phase33_step2_doc_declares_no_phase34_boundary():
    doc = read(DOC).lower()
...
def test_phase33_step2_ui_is_reference_only():
    ui = read(UI_PAGE)
...
def test_phase33_step2_prior_step_is_recorded():
    combined = read(SCRIPT) + read(DOC) + read(UI_PAGE)
    assert "Phase 33 Step 1 - Phase 20 Network Transport Implementation Trusted Production Rollout Readiness Boundary Packet" in combined


def test_phase33_step2_uses_exact_four_step_files():
    script = read(SCRIPT)
    for rel in [
