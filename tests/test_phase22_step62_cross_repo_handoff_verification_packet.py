...
SCRIPT_REL = "scripts/phase22_step62_cross_repo_handoff_verification_packet.ps1"
PAGE_REL = "ui/pages/168_Phase20_Network_Transport_Planning_Cross_Repo_Handoff_Verification_Packet.py"
DOC_REL = "docs/PHASE22_STEP62_CROSS_REPO_HANDOFF_VERIFICATION_PACKET.md"
TEST_REL = "tests/test_phase22_step62_cross_repo_handoff_verification_packet.py"
...
def test_step62_files_exist():
    for path in PUBLIC_FILES:
        assert path.exists(), path


def test_step62_script_mentions_phase_and_step():
    text = read(SCRIPT)
    assert "Phase 22 Step 62" in text
    assert "Cross-Repo Handoff Verification Packet" in text


def test_step62_page_number_is_correct():
    assert PAGE.as_posix().endswith(
        "ui/pages/168_Phase20_Network_Transport_Planning_Cross_Repo_Handoff_Verification_Packet.py"
    )


def test_step62_doc_title_is_correct():
    text = read(DOC)
    assert "# Phase 22 Step 62 - Phase 20 Network Transport Planning Cross-Repo Handoff Verification Packet" in text


def test_step62_launcher_supports_action_all():
    text = read(SCRIPT)
...
def test_step62_uses_literal_and_safe_paths():
    text = read(SCRIPT)
...
def test_step62_is_planning_only():
...
def test_step62_keeps_phase23_closed():
    text = combined_public_text()
    required = [
        "phase23_start",
...
def test_step62_disables_cross_repo_mutation():
    text = combined_public_text()
    required = [
        "cross_repo_write",
        "cross_repo_mutation",
...
def test_step62_keeps_lacrm_dry_run():
    text = combined_public_text()
...
def test_step62_contains_parent_workspace_commands():
    text = read(DOC)
    ...
    assert "INSTALL_PHASE22_STEP62_SINGLE_FILE" in text


def test_step62_menu_has_expected_options():
    text = read(SCRIPT)
    assert "Phase 22 Step 62 menu" in text
    assert "Show status / verify paths" in text
    assert "Smoke test Phase 22 Step 62" in text
    assert "Show server start placeholder only" in text
    assert "Generate Phase 22 Step 62 cross-repo handoff verification packet" in text


def test_step62_expected_smoke_success_text_present():
    text = read(SCRIPT)
    expected = (
        "SMOKE TEST PASS: Phase 22 Step 62 Phase 20 Network Transport Planning "
        "Cross-Repo Handoff Verification Packet is present and planning-only."
    )
    assert expected in text


def test_step62_expected_packet_output_in_doc_and_script():
    combined = read(DOC) + "\n" + read(SCRIPT)
    required = [
...
def test_step62_does_not_create_phase23_or_implementation():
...
def test_step62_does_not_enable_network_or_bridge_runtime():
    text = combined_runtime_text().lower()
    forbidden = [
...
def test_step62_does_not_write_to_external_repos():
    text = combined_runtime_text().lower()
    forbidden = [
...
def test_step62_does_not_create_approvals_or_closure_records():
    text = combined_runtime_text().lower()
    forbidden = [
...
def test_step62_packet_generates_json_under_backups():
    text = read(SCRIPT)
    assert "backups" in text
    assert "phase22_step62_cross_repo_handoff_verification_packet" in text
    assert "ConvertTo-Json" in text
    assert "Set-Content -LiteralPath" in text


def test_step62_expected_branch_is_short():
    text = read(SCRIPT)
...
def test_step62_source_zip_filename_not_required_by_launcher():
    text = read(SCRIPT)
    assert "phase22_step62_source(1).zip" not in text
    assert "phase22_step62_source.zip" not in text
    assert "single-file installer" in text.lower()


def test_step62_installer_paths_are_short():
    for rel in [SCRIPT_REL, PAGE_REL, DOC_REL, TEST_REL]:
        assert len(rel) < 150


def test_step62_no_server_start_claim():
