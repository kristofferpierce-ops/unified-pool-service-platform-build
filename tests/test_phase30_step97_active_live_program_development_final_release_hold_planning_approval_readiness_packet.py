...
SCRIPT = REPO / "scripts/phase30_step97_active_live_program_development_final_release_hold_planning_approval_readiness_packet.ps1"
UI_PAGE = REPO / "ui/pages/1073_Phase30_Step97_Implementation_Active_Live_Program_Development_Final_Release_Hold_Planning_Approval_Readiness_Packet.py"
DOC = REPO / "docs/PHASE30_STEP97_ACTIVE_LIVE_PROGRAM_DEVELOPMENT_FINAL_RELEASE_HOLD_PLANNING_APPROVAL_READINESS_PACKET.md"
TEST = REPO / "tests/test_phase30_step97_active_live_program_development_final_release_hold_planning_approval_readiness_packet.py"
...
def test_phase30_step97_files_exist():
...
def test_phase30_step97_launcher_supports_optimized_actions():
    script = read(SCRIPT)
    assert 'ValidateSet("status", "apply", "smoke", "packet", "all")' in script
    assert "APPLY PASS: Phase 30 Step 97" in script
    assert "SMOKE TEST PASS: Phase 30 Step 97" in script


def test_phase30_step97_safety_markers_present():
...
def test_phase30_step97_does_not_enable_runtime_or_phase31():
...
def test_phase30_step97_doc_declares_no_phase31_boundary():
    doc = read(DOC).lower()
...
def test_phase30_step97_ui_is_reference_only():
    ui = read(UI_PAGE)
...
def test_phase30_step97_prior_step_is_recorded():
    combined = read(SCRIPT) + read(DOC) + read(UI_PAGE)
    assert "Phase 30 Step 96 - Phase 20 Network Transport Implementation Active Live Program Development Final Release Hold Planning Approval Boundary Packet" in combined


def test_phase30_step97_uses_exact_four_step_files():
    script = read(SCRIPT)
    for rel in [
