...
SCRIPT = ROOT / "scripts/phase37_step118_monitored_live_write_ops_final_result_review_operator_hold_packet.ps1"
UI = ROOT / "ui/pages/1934_Phase37_Step118_Monitored_Live_Write_Ops_Final_Result_Review_Operator_Hold.py"
DOC = ROOT / "docs/PHASE37_STEP118_MONITORED_LIVE_WRITE_OPS_FINAL_RESULT_REVIEW_OPERATOR_HOLD_PACKET.md"
...
def test_phase37_step_118_files_exist() -> None:
    assert SCRIPT.exists()
    assert UI.exists()
    assert DOC.exists()


def test_phase37_step_118_launcher_has_valid_param_block() -> None:
    assert read(SCRIPT).lstrip().startswith("param(")


def test_phase37_step_118_payloads_are_bom_free() -> None:
    for path in [SCRIPT, UI, DOC]:
        data = path.read_bytes()
        assert not data.startswith(b"\xef\xbb\xbf")


def test_phase37_step_118_safe_markers_present() -> None:
...
def test_phase37_step_118_forbidden_runtime_markers_not_enabled() -> None:
    compact = (read(SCRIPT) + "\n" + read(UI) + "\n" + read(DOC)).replace(" ", "").lower()
    forbidden_true = [
        "phase38_start=true",
...
def test_phase37_step_118_no_phase38_files_created_by_payload() -> None:
...
def test_phase37_step_118_launcher_smoke_passes() -> None:
...
def test_phase37_step_118_ui_is_short_named() -> None:
