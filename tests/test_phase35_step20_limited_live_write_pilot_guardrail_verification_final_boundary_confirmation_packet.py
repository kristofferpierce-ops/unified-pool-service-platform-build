...
SCRIPT = REPO / "scripts/phase35_step20_limited_live_write_pilot_guardrail_verification_final_boundary_confirmation_packet.ps1"
UI_PAGE = REPO / "ui/pages/1596_Phase35_Step20_Live_Write_Pilot_Guardrail_Final_Boundary.py"
DOC = REPO / "docs/PHASE35_STEP20_LIMITED_LIVE_WRITE_PILOT_GUARDRAIL_VERIFICATION_FINAL_BOUNDARY_PACKET.md"
TEST = REPO / "tests/test_phase35_step20_limited_live_write_pilot_guardrail_verification_final_boundary_confirmation_packet.py"
...
def test_phase35_step20_files_exist():
...
def test_phase35_step20_launcher_supports_optimized_actions():
    script = read(SCRIPT)
    assert 'ValidateSet("status", "apply", "smoke", "packet", "all")' in script
    assert "APPLY PASS: Phase 35 Step 20" in script
    assert "SMOKE TEST PASS: Phase 35 Step 20" in script


def test_phase35_step20_safety_markers_present():
...
def test_phase35_step20_does_not_enable_runtime_or_phase36():
...
def test_phase35_step20_doc_declares_no_phase36_boundary():
    doc = read(DOC).lower()
...
def test_phase35_step20_ui_is_reference_only():
    ui = read(UI_PAGE)
...
def test_phase35_step20_prior_step_is_recorded():
    combined = read(SCRIPT) + read(DOC) + read(UI_PAGE)
    assert "Phase 35 Step 19 - Phase 20 Network Transport Implementation Trusted Production Limited Live Write Pilot Guardrail Verification Closeout Index Packet" in combined


def test_phase35_step20_uses_exact_four_step_files():
    script = read(SCRIPT)
    for rel in [
