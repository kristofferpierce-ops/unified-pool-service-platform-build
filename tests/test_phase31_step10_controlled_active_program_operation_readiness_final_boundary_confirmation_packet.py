...
SCRIPT = REPO / "scripts/phase31_step10_controlled_active_program_operation_readiness_final_boundary_confirmation_packet.ps1"
UI_PAGE = REPO / "ui/pages/1106_Phase31_Step10_Implementation_Controlled_Active_Program_Operation_Readiness_Final_Boundary_Confirmation_Packet.py"
DOC = REPO / "docs/PHASE31_STEP10_CONTROLLED_ACTIVE_PROGRAM_OPERATION_READINESS_FINAL_BOUNDARY_CONFIRMATION_PACKET.md"
TEST = REPO / "tests/test_phase31_step10_controlled_active_program_operation_readiness_final_boundary_confirmation_packet.py"
...
def test_phase31_step10_files_exist():
...
def test_phase31_step10_launcher_supports_optimized_actions():
    script = read(SCRIPT)
    assert 'ValidateSet("status", "apply", "smoke", "packet", "all")' in script
    assert "APPLY PASS: Phase 31 Step 10" in script
    assert "SMOKE TEST PASS: Phase 31 Step 10" in script


def test_phase31_step10_safety_markers_present():
...
def test_phase31_step10_does_not_enable_runtime_or_phase32():
...
def test_phase31_step10_doc_declares_no_phase32_boundary():
    doc = read(DOC).lower()
...
def test_phase31_step10_ui_is_reference_only():
    ui = read(UI_PAGE)
...
def test_phase31_step10_prior_step_is_recorded():
    combined = read(SCRIPT) + read(DOC) + read(UI_PAGE)
    assert "Phase 31 Step 9 - Phase 20 Network Transport Implementation Controlled Active Program Operation Readiness Closeout Index Packet" in combined


def test_phase31_step10_uses_exact_four_step_files():
    script = read(SCRIPT)
    for rel in [
