...
SCRIPT_REL = "scripts/phase23_step11_implementation_guardrail_scope_boundary_packet.ps1"
PAGE_REL = "ui/pages/187_Phase23_Step11_Implementation_Guardrail_Scope_Boundary_Packet.py"
DOC_REL = "docs/PHASE23_STEP11_IMPLEMENTATION_GUARDRAIL_SCOPE_BOUNDARY_PACKET.md"
TEST_REL = "tests/test_phase23_step11_implementation_guardrail_scope_boundary_packet.py"
...
def test_phase23_step11_files_exist():
    for path in PUBLIC_FILES:
        assert path.exists(), path


def test_phase23_step11_script_mentions_phase_and_step():
    text = read(SCRIPT)
    assert "Phase 23 Step 11" in text
    assert "$PhaseNumber = 23" in text
    assert "$StepNumber = 11" in text
    assert "Implementation Guardrail Scope Boundary Packet" in text


def test_phase23_step11_page_number_is_correct():
    assert PAGE.as_posix().endswith(
        "ui/pages/187_Phase23_Step11_Implementation_Guardrail_Scope_Boundary_Packet.py"
    )


def test_phase23_step11_doc_title_is_correct():
    text = read(DOC)
    assert "# Phase 23 Step 11 - Phase 20 Network Transport Implementation Guardrail Scope Boundary Packet" in text


def test_phase23_step11_prior_step_is_phase23_step9():
    text = read(SCRIPT) + "\n" + read(DOC)
    assert "Phase 23 Step 10 - Phase 20 Network Transport Implementation Guardrail Baseline Packet" in text
    assert "CHECK: prior_step=Phase 23 Step 10" in text


def test_phase23_step11_launcher_supports_action_all():
    text = read(SCRIPT)
...
def test_phase23_step11_uses_literal_and_safe_paths():
    text = read(SCRIPT)
...
def test_phase23_step11_is_planning_only():
...
def test_phase23_step11_opens_phase23_as_intake_only():
    text = combined_public_text()
    required = [
        "phase23_context",
        "implementation_guardrail_planning_only",
        "phase23_planning_boundary",
        "guardrail_scope_boundary_opened_by_packet",
...
def test_phase23_step11_disables_implementation_and_runtime():
    text = combined_public_text()
    required = [
...
def test_phase23_step11_disables_cross_repo_mutation():
    text = combined_public_text()
    required = [
        "cross_repo_write",
        "cross_repo_mutation",
...
def test_phase23_step11_disables_decisions_and_approvals():
    text = combined_public_text()
    required = [
        "implementation_guardrail_scope_boundary_mode",
        "implementation_guardrail_scope_boundary_write",
        "implementation_guardrail_scope_boundary_record_creation",
...
def test_phase23_step11_keeps_lacrm_dry_run():
    text = combined_public_text()
...
def test_phase23_step11_contains_parent_workspace_commands():
    text = read(DOC)
    ...
    assert "INSTALL_PHASE23_STEP11_SINGLE_FILE" in text


def test_phase23_step11_menu_has_expected_options():
    text = read(SCRIPT)
    assert "Phase 23 Step 11 menu" in text
    assert "Show status / verify paths" in text
    assert "Smoke test Phase 23 Step 11" in text
    assert "Show server start placeholder only" in text
    assert "Generate Phase 23 Step 11 implementation guardrail scope boundary packet" in text


def test_phase23_step11_expected_smoke_success_text_present():
    text = read(SCRIPT)
    expected = (
        "SMOKE TEST PASS: Phase 23 Step 11 Phase 20 Network Transport Implementation "
        "Guardrail Scope Boundary Packet is present and planning-only."
    )
    assert expected in text


def test_phase23_step11_expected_packet_output_in_doc_and_script():
    combined = read(DOC) + "\n" + read(SCRIPT)
    required = [
...
def test_phase23_step11_packet_metadata_is_correct():
    text = read(SCRIPT)
    assert "phase = 23" in text
    assert "step = 11" in text
    assert "phase23_step11_implementation_guardrail_scope_boundary_packet" in text


def test_phase23_step11_does_not_start_implementation_or_runtime():
...
def test_phase23_step11_does_not_enable_network_or_bridge_runtime():
    text = combined_runtime_text().lower()
    forbidden = [
...
def test_phase23_step11_does_not_write_to_external_repos():
    text = combined_runtime_text().lower()
    forbidden = [
...
def test_phase23_step11_does_not_create_decisions_approvals_or_signoffs():
    text = combined_runtime_text().lower()
    forbidden = [
...
def test_phase23_step11_packet_generates_json_under_backups():
    text = read(SCRIPT)
...
def test_phase23_step11_expected_branch_is_short():
    text = read(SCRIPT)
...
def test_phase23_step11_source_zip_filename_not_required_by_launcher():
    text = read(SCRIPT)
    assert "phase23_step11_source(1).zip" not in text
    assert "phase23_step11_source.zip" not in text
    assert "single-file installer" in text.lower()


def test_phase23_step11_installer_paths_are_short():
    for rel in [SCRIPT_REL, PAGE_REL, DOC_REL, TEST_REL]:
        assert len(rel) < 150


def test_phase23_step11_no_server_start_claim():
