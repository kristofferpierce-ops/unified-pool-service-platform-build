...
SCRIPT = REPO / "scripts/phase37_step88_monitored_live_write_ops_release_gate_verification_planning_operator_hold_point_packet.ps1"
UI = REPO / "ui/pages/1904_Phase37_Step88_Monitored_Live_Write_Ops_Release_Gate_Verification_Operator_Hold.py"
DOC = REPO / "docs/PHASE37_STEP88_MONITORED_LIVE_WRITE_OPS_RELEASE_GATE_VERIFICATION_PLANNING_OPERATOR_HOLD_POINT_PACKET.md"
TEST = REPO / "tests/test_phase37_step88_monitored_live_write_ops_release_gate_verification_planning_operator_hold_point_packet.py"
...
def test_phase37_step_88_files_exist() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert path.exists(), f"Missing expected file: {path}"


def test_phase37_step_88_launcher_has_valid_param_block() -> None:
    text = read(SCRIPT)
...
def test_phase37_step_88_payloads_are_bom_free() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert not path.read_bytes().startswith(b"\xef\xbb\xbf"), f"BOM detected in {path}"


def test_phase37_step_88_safety_markers_present() -> None:
    text = read(SCRIPT) + "\n" + read(UI) + "\n" + read(DOC)
    required = [
        "planning_only=true",
...
def test_phase37_step_88_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase37_step_88_launcher_smoke_passes() -> None:
