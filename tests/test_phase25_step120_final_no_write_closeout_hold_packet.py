...
SCRIPT = REPO_ROOT / "scripts/phase25_step120_final_no_write_closeout_hold_packet.ps1"
PAGE = REPO_ROOT / "ui/pages/496_Phase25_Step120_Final_No_Write_Closeout_Hold_Packet.py"
DOC = REPO_ROOT / "docs/PHASE25_STEP120_FINAL_NO_WRITE_CLOSEOUT_HOLD_PACKET.md"
TEST = REPO_ROOT / "tests/test_phase25_step120_final_no_write_closeout_hold_packet.py"
...
def test_phase25_step120_files_are_present():
    for path in (SCRIPT, PAGE, DOC, TEST):
        assert path.exists(), f"missing {path}"


def test_phase25_step120_launcher_markers_are_present():
    text = read(SCRIPT).lower()
    assert "phase 25 step 120" in text
    assert "phase25-step120-final-no-write-closeout-hold" in text
...
def test_phase25_step120_planning_only_safety_markers():
...
def test_phase25_step120_does_not_start_runtime_or_next_phase():
    text = operational_text()
    forbidden = [
        "phase26_start=true",
...
def test_phase25_step120_batch_risk_review_is_chat_only_not_powershell_prompt():
    text = operational_text()
    assert "batch_risk_review_location" in text
...
def test_phase25_step120_launcher_supports_optimized_actions_and_known_fixes():
    script = read(SCRIPT)
    assert "ValidateSet(\"status\", \"apply\", \"smoke\", \"packet\", \"all\")" in script
    assert "APPLY PASS: Phase 25 Step 120" in script
    assert "SMOKE TEST PASS: Phase 25 Step 120" in script
    assert "CHECK: packet_json=" in script
    assert "$File:" not in script
    assert "Join-Path $RepoRoot \"scripts/phase25_step120_final_no_write_closeout_hold_packet.ps1\"," not in script
    assert "System.Object[]" not in script


def test_phase25_step120_docs_are_operator_readable():
    text = read(DOC)
    assert "Phase 25 Step 120 - Phase 20 Network Transport Implementation Phase 25 Final No-Write Closeout Hold Packet" in text
...
def test_phase25_step120_ui_is_streamlit_reference_only_page():
...
def test_phase25_step120_test_file_self_identifies_step():
    text = read(TEST).lower()
    assert "step120" in text or "step 120" in text
