...
SCRIPT = REPO / "scripts/phase37_step29_monitored_live_write_ops_preflight_closeout_index_packet.ps1"
UI = REPO / "ui/pages/1845_Phase37_Step29_Monitored_Live_Write_Ops_Preflight_Closeout_Index.py"
DOC = REPO / "docs/PHASE37_STEP29_MONITORED_LIVE_WRITE_OPS_PREFLIGHT_CLOSEOUT_INDEX_PACKET.md"
TEST = REPO / "tests/test_phase37_step29_monitored_live_write_ops_preflight_closeout_index_packet.py"
...
def test_phase37_step_29_files_exist() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert path.exists(), f"Missing expected file: {path}"


def test_phase37_step_29_launcher_has_valid_param_block() -> None:
    text = read(SCRIPT)
...
def test_phase37_step_29_safety_markers_present() -> None:
    text = read(SCRIPT) + "\n" + read(UI) + "\n" + read(DOC)
    required = [
...
def test_phase37_step_29_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase37_step_29_launcher_smoke_passes() -> None:
