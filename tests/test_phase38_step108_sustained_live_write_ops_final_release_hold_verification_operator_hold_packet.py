...
SCRIPT = ROOT / "scripts/phase38_step108_sustained_live_write_ops_final_release_hold_verification_operator_hold_packet.ps1"
UI = ROOT / "ui/pages/2044_Phase38_Step108_Sustained_Live_Write_Ops_Final_Release_Hold_Verification_Operator_Hold.py"
DOC = ROOT / "docs/PHASE38_STEP108_SUSTAINED_LIVE_WRITE_OPS_FINAL_RELEASE_HOLD_VERIFICATION_OPERATOR_HOLD_PACKET.md"
...
def test_phase38_step_108_files_exist() -> None:
    assert SCRIPT.exists()
    assert UI.exists()
    assert DOC.exists()


def test_phase38_step_108_launcher_has_valid_param_block() -> None:
    text = read(SCRIPT)
    assert text.startswith("param(")


def test_phase38_step_108_launcher_runs_all() -> None:
...
def test_phase38_step_108_ui_compiles() -> None:
    py_compile.compile(str(UI), doraise=True)


def test_phase38_step_108_doc_has_required_safe_markers() -> None:
...
def test_phase38_step_108_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase38_step_108_no_phase39_files_created_by_payload() -> None:
...
def test_phase38_step_108_literal_pass_markers_present() -> None:
    text = read(SCRIPT)
