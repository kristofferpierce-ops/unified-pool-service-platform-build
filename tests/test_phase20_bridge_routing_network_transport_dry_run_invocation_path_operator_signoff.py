...
def test_bridge_routing_network_transport_dry_run_invocation_path_operator_signoff_script_is_get_only() -> None:
    script = read("scripts/phase20_generate_bridge_routing_network_transport_dry_run_invocation_path_operator_signoff.ps1")
    assert "phase20_bridge_routing_network_transport_dry_run_invocation_path_operator_signoff_" in script
    assert "bridge_routing_network_transport_dry_run_invocation_path_operator_signoff_only = $true" in script
    ...
    assert "approved_to_add_bridge_post = $false" in script
...
def test_bridge_routing_network_transport_dry_run_invocation_path_operator_signoff_streamlit_page_is_report_only() -> None:
    page = read("ui/pages/74_Bridge_Routing_Network_Transport_Dry_Run_Invocation_Path_Operator_Signoff.py")
    assert "Phase 20 Bridge Routing Network Transport Dry Run Invocation Path Operator Signoff" in page
    assert "network-transport-dry-run-invocation-path-operator-signoff-only" in page
    ...
    assert "phase20_bridge_routing_network_transport_dry_run_invocation_path_operator_signoff_*" in page
    assert "phase20_bridge_routing_network_transport_dry_run_invocation_path_operator_signoff.json" in page
    assert "requests.get" not in page
    assert "requests.post" not in page
    assert "st.download_button" in page


def test_bridge_routing_network_transport_dry_run_invocation_path_operator_signoff_docs_capture_boundaries() -> None:
    doc = read("docs/PHASE20_STEP22_BRIDGE_ROUTING_NETWORK_TRANSPORT_DRY_RUN_INVOCATION_PATH_OPERATOR_SIGNOFF.md")
    assert "operator signoff dossier" in doc
    assert "does not authorize live bridge routing writes, real bridge network transport, bridge POST calls, or opening network sockets" in doc
    assert "network-transport-dry-run-invocation-path-operator-signoff-only" in doc
