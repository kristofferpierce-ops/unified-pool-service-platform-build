...
SCRIPT = REPO / "scripts/phase37_step43_monitored_live_write_ops_no_write_dry_run_verification_planning_evidence_gap_review_packet.ps1"
UI = REPO / "ui/pages/1859_Phase37_Step43_Monitored_Live_Write_Ops_No_Write_Dry_Run_Verification_Planning_Evidence_Gap_Review.py"
DOC = REPO / "docs/PHASE37_STEP43_MONITORED_LIVE_WRITE_OPS_NO_WRITE_DRY_RUN_VERIFICATION_PLANNING_EVIDENCE_GAP_REVIEW_PACKET.md"
TEST = REPO / "tests/test_phase37_step43_monitored_live_write_ops_no_write_dry_run_verification_planning_evidence_gap_review_packet.py"
...
def test_phase37_step_43_files_exist() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert path.exists(), f"Missing expected file: {path}"


def test_phase37_step_43_launcher_has_valid_param_block() -> None:
    text = read(SCRIPT)
...
def test_phase37_step_43_safety_markers_present() -> None:
    text = read(SCRIPT) + "\n" + read(UI) + "\n" + read(DOC)
    required = [
...
def test_phase37_step_43_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase37_step_43_launcher_smoke_passes() -> None:
