...
SCRIPT = ROOT / "scripts/phase38_step8_sustained_live_write_ops_readiness_operator_hold_packet.ps1"
UI = ROOT / "ui/pages/1944_Phase38_Step8_Sustained_Live_Write_Ops_Readiness_Operator_Hold.py"
DOC = ROOT / "docs/PHASE38_STEP8_SUSTAINED_LIVE_WRITE_OPS_READINESS_OPERATOR_HOLD_PACKET.md"
TEST = ROOT / "tests/test_phase38_step8_sustained_live_write_ops_readiness_operator_hold_packet.py"
...
def test_phase38_step8_files_exist() -> None:
...
def test_phase38_step8_launcher_has_valid_param_block() -> None:
    text = read(SCRIPT)
...
def test_phase38_step8_required_safety_markers_present() -> None:
...
def test_phase38_step8_next_phase_guards_present() -> None:
    combined = "\n".join(read(path) for path in (SCRIPT, UI, DOC))
    assert "phase39_start" in combined
    assert "phase39_boundary_creation" in combined


def test_phase38_step8_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase38_step8_literal_pass_markers_present() -> None:
    text = read(SCRIPT)
    assert "APPLY PASS" in text
    assert "SMOKE TEST PASS" in text


def test_phase38_step8_ui_is_planning_only_page() -> None:
    text = read(UI)
...
def test_phase38_step8_no_phase39_files_created_by_payload() -> None:
