...
SCRIPT = REPO / "scripts/phase37_step4_monitored_live_write_ops_readiness_readiness_safety_planning_packet.ps1"
UI = REPO / "ui/pages/1820_Phase37_Step4_Monitored_Live_Write_Ops_Readiness_Safety_Planning.py"
DOC = REPO / "docs/PHASE37_STEP4_MONITORED_LIVE_WRITE_OPS_READINESS_READINESS_SAFETY_PLANNING_PACKET.md"
TEST = REPO / "tests/test_phase37_step4_monitored_live_write_ops_readiness_readiness_safety_planning_packet.py"
...
def test_phase37_step4_files_exist():
...
def test_phase37_step4_launcher_has_valid_param_block():
    text = _read(SCRIPT)
...
def test_phase37_step4_required_safety_markers_present():
...
def test_phase37_step4_next_phase_guards_present():
    combined = "\n".join(_read(path) for path in (SCRIPT, UI, DOC))
    assert "phase38_start" in combined
    assert "phase38_boundary_creation" in combined


def test_phase37_step4_forbidden_true_markers_absent():
...
def test_phase37_step4_literal_pass_markers_present():
    text = _read(SCRIPT)
    assert "APPLY PASS" in text
    assert "SMOKE TEST PASS" in text


def test_phase37_step4_ui_is_planning_only_page():
    text = _read(UI)
...
def test_phase37_step4_doc_records_no_runtime_boundary():
    text = _read(DOC)
