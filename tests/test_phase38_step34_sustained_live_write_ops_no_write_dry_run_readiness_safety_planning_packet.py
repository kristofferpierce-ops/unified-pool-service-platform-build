...
SCRIPT = ROOT / "scripts/phase38_step34_sustained_live_write_ops_no_write_dry_run_readiness_safety_planning_packet.ps1"
UI = ROOT / "ui/pages/1970_Phase38_Step34_Sustained_Live_Write_Ops_No_Write_Dry_Run_Readiness_Safety_Planning.py"
DOC = ROOT / "docs/PHASE38_STEP34_SUSTAINED_LIVE_WRITE_OPS_NO_WRITE_DRY_RUN_READINESS_SAFETY_PLANNING_PACKET.md"
TEST = ROOT / "tests/test_phase38_step34_sustained_live_write_ops_no_write_dry_run_readiness_safety_planning_packet.py"
...
def test_phase38_step_34_files_exist() -> None:
...
def test_phase38_step_34_launcher_has_valid_param_block() -> None:
    text = read(SCRIPT)
    assert text.startswith("param(")
    assert not text.startswith("\ufeff")


def test_phase38_step_34_launcher_runs_all() -> None:
...
def test_phase38_step_34_safety_markers_present() -> None:
...
def test_phase38_step_34_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase38_step_34_no_phase39_files_created_by_payload() -> None:
...
def test_phase38_step_34_ui_page_is_short_named_and_safe() -> None:
...
def test_phase38_step_34_doc_names_four_file_packet() -> None:
    text = read(DOC)
    assert "scripts/phase38_step34_sustained_live_write_ops_no_write_dry_run_readiness_safety_planning_packet.ps1" in text
    assert "ui/pages/1970_Phase38_Step34_Sustained_Live_Write_Ops_No_Write_Dry_Run_Readiness_Safety_Planning.py" in text
    assert "docs/PHASE38_STEP34_SUSTAINED_LIVE_WRITE_OPS_NO_WRITE_DRY_RUN_READINESS_SAFETY_PLANNING_PACKET.md" in text
    assert "tests/test_phase38_step34_sustained_live_write_ops_no_write_dry_run_readiness_safety_planning_packet.py" in text
