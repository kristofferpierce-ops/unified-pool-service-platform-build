...
SCRIPT = REPO / "scripts/phase28_step27_implementation_sandbox_pilot_preflight_planning_approval_readiness_packet.ps1"
UI_PAGE = REPO / "ui/pages/763_Phase28_Step27_Implementation_Sandbox_Pilot_Preflight_Planning_Approval_Readiness_Packet.py"
DOC = REPO / "docs/PHASE28_STEP27_IMPLEMENTATION_SANDBOX_PILOT_PREFLIGHT_PLANNING_APPROVAL_READINESS_PACKET.md"
TEST = REPO / "tests/test_phase28_step27_implementation_sandbox_pilot_preflight_planning_approval_readiness_packet.py"
...
def test_phase28_step27_files_exist():
...
def test_phase28_step27_launcher_supports_optimized_actions():
    script = read(SCRIPT)
    assert 'ValidateSet("status", "apply", "smoke", "packet", "all")' in script
    assert "APPLY PASS: Phase 28 Step 27" in script
    assert "SMOKE TEST PASS: Phase 28 Step 27" in script


def test_phase28_step27_safety_markers_present():
...
def test_phase28_step27_does_not_enable_runtime_or_phase29():
...
def test_phase28_step27_doc_declares_no_phase29_boundary():
    doc = read(DOC).lower()
...
def test_phase28_step27_ui_is_reference_only():
    ui = read(UI_PAGE)
...
def test_phase28_step27_prior_step_is_recorded():
    combined = read(SCRIPT) + read(DOC) + read(UI_PAGE)
    assert "Phase 28 Step 26 - Phase 20 Network Transport Implementation Sandbox Pilot Preflight Planning Approval Boundary Packet" in combined


def test_phase28_step27_uses_exact_four_step_files():
    script = read(SCRIPT)
    for rel in [
