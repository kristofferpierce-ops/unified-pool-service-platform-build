...
SCRIPT = ROOT / "scripts/phase38_step6_sustained_live_write_ops_readiness_approval_boundary_packet.ps1"
UI = ROOT / "ui/pages/1942_Phase38_Step6_Sustained_Live_Write_Ops_Readiness_Approval_Boundary.py"
DOC = ROOT / "docs/PHASE38_STEP6_SUSTAINED_LIVE_WRITE_OPS_READINESS_APPROVAL_BOUNDARY_PACKET.md"
TEST = ROOT / "tests/test_phase38_step6_sustained_live_write_ops_readiness_approval_boundary_packet.py"
...
def test_phase38_step6_files_exist() -> None:
...
def test_phase38_step6_launcher_has_valid_param_block() -> None:
    text = read(SCRIPT)
...
def test_phase38_step6_required_safety_markers_present() -> None:
...
def test_phase38_step6_next_phase_guards_present() -> None:
    combined = "\n".join(read(path) for path in (SCRIPT, UI, DOC))
    assert "phase39_start" in combined
    assert "phase39_boundary_creation" in combined


def test_phase38_step6_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase38_step6_literal_pass_markers_present() -> None:
    text = read(SCRIPT)
    assert "APPLY PASS" in text
    assert "SMOKE TEST PASS" in text


def test_phase38_step6_ui_is_planning_only_page() -> None:
    text = read(UI)
...
def test_phase38_step6_no_phase39_files_created_by_payload() -> None:
