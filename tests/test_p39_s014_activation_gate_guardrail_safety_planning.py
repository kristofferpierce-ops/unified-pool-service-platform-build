...
SCRIPT = ROOT / "scripts/p39_s014_activation_gate_guardrail_safety_planning.ps1"
UI = ROOT / "ui/pages/2070_Phase39_Step14_Activation_Gate_Guardrail_Safety_Planning.py"
DOC = ROOT / "docs/P39_S014_ACTIVATION_GATE_GUARDRAIL_SAFETY_PLANNING.md"
...
def test_phase39_step_14_files_exist() -> None:
    assert SCRIPT.exists()
    assert UI.exists()
    assert DOC.exists()


def test_phase39_step_14_launcher_has_valid_param_block() -> None:
    assert read(SCRIPT).lstrip().startswith("param(")


def test_phase39_step_14_safety_markers_present() -> None:
...
def test_phase39_step_14_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase39_step_14_no_phase40_files_created_by_payload() -> None:
...
def test_phase39_step_14_launcher_smoke_passes() -> None:
...
def test_phase39_step_14_ui_is_planning_only() -> None:
    text = read(UI)
...
def test_phase39_step_14_doc_records_activation_gate_hold() -> None:
    text = read(DOC).lower()
    assert "live-write activation gate remains unarmed" in text
    assert "phase40_boundary_creation=false" in text.replace(" ", "")


def test_phase39_step_14_no_server_or_transport_runtime() -> None:
