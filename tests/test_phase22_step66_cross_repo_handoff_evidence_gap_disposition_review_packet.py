...
SCRIPT_REL = "scripts/phase22_step66_cross_repo_handoff_evidence_gap_disposition_review_packet.ps1"
PAGE_REL = "ui/pages/172_Phase20_Network_Transport_Planning_Cross_Repo_Handoff_Evidence_Gap_Disposition_Review_Packet.py"
DOC_REL = "docs/PHASE22_STEP66_CROSS_REPO_HANDOFF_EVIDENCE_GAP_DISPOSITION_REVIEW_PACKET.md"
TEST_REL = "tests/test_phase22_step66_cross_repo_handoff_evidence_gap_disposition_review_packet.py"
...
def test_step66_files_exist():
    for path in PUBLIC_FILES:
        assert path.exists(), path


def test_step66_script_mentions_phase_and_step():
    text = read(SCRIPT)
    assert "Phase 22 Step 66" in text
    assert "$StepNumber = 66" in text
    assert "Cross-Repo Handoff Evidence Gap Disposition Review Packet" in text


def test_step66_page_number_is_correct():
...
def test_step66_doc_title_is_correct():
    text = read(DOC)
...
def test_step66_prior_step_is_step65():
    text = read(SCRIPT) + "\n" + read(DOC)
    ...
    assert "CHECK: prior_step=Phase 22 Step 65" in text


def test_step66_launcher_supports_action_all():
    text = read(SCRIPT)
...
def test_step66_uses_literal_and_safe_paths():
    text = read(SCRIPT)
...
def test_step66_is_planning_only():
...
def test_step66_keeps_phase23_closed():
    text = combined_public_text()
    required = [
        "phase23_start",
...
def test_step66_disables_cross_repo_mutation():
    text = combined_public_text()
    required = [
        "cross_repo_write",
        "cross_repo_mutation",
...
def test_step66_disables_evidence_collection_and_decisions():
    text = combined_public_text()
    required = [
        "evidence_gap_disposition_review_mode",
        "handoff_evidence_gap_disposition_review_record_creation",
        "handoff_evidence_collection_execution",
        "handoff_evidence_gap_disposition_review_write",
...
def test_step66_keeps_lacrm_dry_run():
    text = combined_public_text()
...
def test_step66_contains_parent_workspace_commands():
    text = read(DOC)
    ...
    assert "INSTALL_PHASE22_STEP66_SINGLE_FILE" in text


def test_step66_menu_has_expected_options():
    text = read(SCRIPT)
    assert "Phase 22 Step 66 menu" in text
    assert "Show status / verify paths" in text
    assert "Smoke test Phase 22 Step 66" in text
    assert "Show server start placeholder only" in text
    assert "Generate Phase 22 Step 66 cross-repo handoff evidence gap disposition review packet" in text


def test_step66_expected_smoke_success_text_present():
    text = read(SCRIPT)
    expected = (
        "SMOKE TEST PASS: Phase 22 Step 66 Phase 20 Network Transport Planning "
        "Cross-Repo Handoff Evidence Gap Disposition Review Packet is present and planning-only."
    )
    assert expected in text


def test_step66_expected_packet_output_in_doc_and_script():
    combined = read(DOC) + "\n" + read(SCRIPT)
    required = [
...
def test_step66_does_not_create_phase23_or_implementation():
...
def test_step66_does_not_enable_network_or_bridge_runtime():
    text = combined_runtime_text().lower()
    forbidden = [
...
def test_step66_does_not_write_to_external_repos():
    text = combined_runtime_text().lower()
    forbidden = [
...
def test_step66_does_not_collect_decide_or_remediate_evidence():
    text = combined_runtime_text().lower()
    forbidden = [
        "handoff_evidence_collection_execution: true",
        "handoff_evidence_collection_execution = true",
        "handoff_evidence_gap_disposition_review_write: true",
        "handoff_evidence_gap_disposition_review_write = true",
        ...
        "evidence_gap_resolution_execution = true",
        "handoff_evidence_gap_disposition_review_record_creation: true",
        "handoff_evidence_gap_disposition_review_record_creation = true",
    ]
    for marker in forbidden:
        assert marker not in text
...
def test_step66_does_not_create_approvals_or_closure_records():
    text = combined_runtime_text().lower()
    forbidden = [
...
def test_step66_packet_generates_json_under_backups():
    text = read(SCRIPT)
    assert "backups" in text
    assert "phase22_step66_cross_repo_handoff_evidence_gap_disposition_review_packet" in text
    assert "ConvertTo-Json" in text
    assert "Set-Content -LiteralPath" in text


def test_step66_expected_branch_is_short():
    text = read(SCRIPT)
...
def test_step66_source_zip_filename_not_required_by_launcher():
    text = read(SCRIPT)
    assert "phase22_step66_source(1).zip" not in text
    assert "phase22_step66_source.zip" not in text
    assert "single-file installer" in text.lower()


def test_step66_installer_paths_are_short():
    for rel in [SCRIPT_REL, PAGE_REL, DOC_REL, TEST_REL]:
        assert len(rel) < 150


def test_step66_no_server_start_claim():
