...
SCRIPT = REPO / "scripts/phase30_step26_active_live_program_development_preflight_planning_approval_boundary_packet.ps1"
UI_PAGE = REPO / "ui/pages/1002_Phase30_Step26_Implementation_Active_Live_Program_Development_Preflight_Planning_Approval_Boundary_Packet.py"
DOC = REPO / "docs/PHASE30_STEP26_ACTIVE_LIVE_PROGRAM_DEVELOPMENT_PREFLIGHT_PLANNING_APPROVAL_BOUNDARY_PACKET.md"
TEST = REPO / "tests/test_phase30_step26_active_live_program_development_preflight_planning_approval_boundary_packet.py"
...
def test_phase30_step26_files_exist():
...
def test_phase30_step26_launcher_supports_optimized_actions():
    script = read(SCRIPT)
    assert 'ValidateSet("status", "apply", "smoke", "packet", "all")' in script
    assert "APPLY PASS: Phase 30 Step 26" in script
    assert "SMOKE TEST PASS: Phase 30 Step 26" in script


def test_phase30_step26_safety_markers_present():
...
def test_phase30_step26_does_not_enable_runtime_or_phase31():
...
def test_phase30_step26_doc_declares_no_phase31_boundary():
    doc = read(DOC).lower()
...
def test_phase30_step26_ui_is_reference_only():
    ui = read(UI_PAGE)
...
def test_phase30_step26_prior_step_is_recorded():
    combined = read(SCRIPT) + read(DOC) + read(UI_PAGE)
    assert "Phase 30 Step 25 - Phase 20 Network Transport Implementation Active Live Program Development Preflight Planning Safety Disposition Review Packet" in combined


def test_phase30_step26_uses_exact_four_step_files():
    script = read(SCRIPT)
    for rel in [
