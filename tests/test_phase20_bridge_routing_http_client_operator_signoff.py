...
def test_bridge_routing_http_client_operator_signoff_script_is_get_only() -> None:
    script = read("scripts/phase20_generate_bridge_routing_http_client_operator_signoff.ps1")
    assert "phase20_bridge_routing_http_client_operator_signoff_" in script
    assert "bridge_routing_http_client_operator_signoff_only = $true" in script
...
def test_bridge_routing_http_client_operator_signoff_streamlit_page_is_report_only() -> None:
    page = read("ui/pages/57_Bridge_Routing_HTTP_Client_Operator_Signoff.py")
    assert "Phase 20 Bridge Routing HTTP Client Operator Signoff" in page
    assert "HTTP-client-operator-signoff-only" in page
    ...
    assert "phase20_bridge_routing_http_client_operator_signoff_*" in page
    assert "phase20_bridge_routing_http_client_operator_signoff.json" in page
    assert "requests.get" not in page
    assert "requests.post" not in page
    assert "st.download_button" in page


def test_bridge_routing_http_client_operator_signoff_docs_capture_boundaries() -> None:
    doc = read("docs/PHASE20_STEP5_BRIDGE_ROUTING_HTTP_CLIENT_OPERATOR_SIGNOFF.md")
    assert "operator signoff dossier" in doc
    assert "does not authorize live bridge routing writes or real bridge network transport" in doc
    assert "HTTP-client-operator-signoff-only" in doc
