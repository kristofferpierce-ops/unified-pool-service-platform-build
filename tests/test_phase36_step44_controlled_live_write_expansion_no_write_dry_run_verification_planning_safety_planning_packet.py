...
SCRIPT = REPO / "scripts/phase36_step44_controlled_live_write_expansion_no_write_dry_run_verification_planning_safety_planning_packet.ps1"
UI = REPO / "ui/pages/1740_Phase36_Step44_Live_Write_Expansion_Dry_Run_Verification_Safety_Planning.py"
DOC = REPO / "docs/PHASE36_STEP44_CONTROLLED_LIVE_WRITE_EXPANSION_DRY_RUN_VERIFICATION_PLANNING_SAFETY_PLANNING_PACKET.md"
TEST = REPO / "tests/test_phase36_step44_controlled_live_write_expansion_no_write_dry_run_verification_planning_safety_planning_packet.py"
...
def test_phase36_step_44_files_exist() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert path.exists(), f"missing expected file: {path}"


def test_phase36_step_44_launcher_has_required_safe_markers() -> None:
    text = read(SCRIPT)
    for marker in [
        "planning_only = $true",
...
def test_phase36_step_44_operational_files_do_not_enable_forbidden_markers() -> None:
    forbidden_keys = [
        "phase37_start",
...
def test_phase36_step_44_ui_and_doc_reference_dry_run_no_apply() -> None:
    text = read(UI) + "\n" + read(DOC)
...
def test_phase36_step_44_launcher_apply_passes() -> None:
...
def test_phase36_step_44_launcher_smoke_passes() -> None:
    result = subprocess.run(
