...
SCRIPT = ROOT / "scripts/phase38_step26_sustained_live_write_ops_preflight_planning_approval_boundary_packet.ps1"
UI = ROOT / "ui/pages/1962_Phase38_Step26_Sustained_Live_Write_Ops_Preflight_Approval_Boundary.py"
DOC = ROOT / "docs/PHASE38_STEP26_SUSTAINED_LIVE_WRITE_OPS_PREFLIGHT_PLANNING_APPROVAL_BOUNDARY_PACKET.md"
TEST = ROOT / "tests/test_phase38_step26_sustained_live_write_ops_preflight_planning_approval_boundary_packet.py"
...
def test_phase38_step_26_files_exist() -> None:
...
def test_phase38_step_26_launcher_has_valid_param_block() -> None:
    text = read(SCRIPT)
    assert text.startswith("param(")
    assert not text.startswith("\ufeff")


def test_phase38_step_26_launcher_runs_all() -> None:
...
def test_phase38_step_26_safety_markers_present() -> None:
...
def test_phase38_step_26_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase38_step_26_no_phase39_files_created_by_payload() -> None:
...
def test_phase38_step_26_ui_page_is_short_named_and_safe() -> None:
...
def test_phase38_step_26_doc_names_four_file_packet() -> None:
    text = read(DOC)
    assert "scripts/phase38_step26_sustained_live_write_ops_preflight_planning_approval_boundary_packet.ps1" in text
    assert "ui/pages/1962_Phase38_Step26_Sustained_Live_Write_Ops_Preflight_Approval_Boundary.py" in text
    assert "docs/PHASE38_STEP26_SUSTAINED_LIVE_WRITE_OPS_PREFLIGHT_PLANNING_APPROVAL_BOUNDARY_PACKET.md" in text
    assert "tests/test_phase38_step26_sustained_live_write_ops_preflight_planning_approval_boundary_packet.py" in text
