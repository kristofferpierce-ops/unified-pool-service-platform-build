...
SCRIPT = REPO / "scripts/phase36_step75_controlled_live_write_expansion_release_gate_readiness_planning_safety_review_packet.ps1"
UI = REPO / "ui/pages/1771_Phase36_Step75_Live_Write_Expansion_Release_Gate_Readiness_Safety_Review.py"
DOC = REPO / "docs/PHASE36_STEP75_CONTROLLED_LIVE_WRITE_EXPANSION_RELEASE_GATE_READINESS_PLANNING_SAFETY_REVIEW_PACKET.md"
TEST = REPO / "tests/test_phase36_step75_controlled_live_write_expansion_release_gate_readiness_planning_safety_review_packet.py"
...
def test_phase36_step_75_files_exist() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert path.exists(), f"missing expected file: {path}"


def test_phase36_step_75_launcher_has_required_safe_markers() -> None:
    text = read(SCRIPT)
    for marker in [
        "planning_only = $true",
...
def test_phase36_step_75_operational_files_do_not_enable_forbidden_markers() -> None:
    forbidden_keys = [
        "phase37_start",
...
def test_phase36_step_75_ui_and_doc_reference_dry_run_no_apply() -> None:
    text = read(UI) + "\n" + read(DOC)
...
def test_phase36_step_75_launcher_apply_passes() -> None:
...
def test_phase36_step_75_launcher_smoke_passes() -> None:
    result = subprocess.run(
