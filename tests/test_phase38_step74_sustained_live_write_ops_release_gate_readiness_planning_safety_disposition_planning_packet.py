...
SCRIPT = REPO / "scripts/phase38_step74_sustained_live_write_ops_release_gate_readiness_planning_safety_disposition_planning_packet.ps1"
UI = REPO / "ui/pages/2010_Phase38_Step74_Sustained_Live_Write_Ops_Release_Gate_Readiness_Safety_Planning.py"
DOC = REPO / "docs/PHASE38_STEP74_SUSTAINED_LIVE_WRITE_OPS_RELEASE_GATE_READINESS_PLANNING_SAFETY_DISPOSITION_PLANNING_PACKET.md"
TEST = REPO / "tests/test_phase38_step74_sustained_live_write_ops_release_gate_readiness_planning_safety_disposition_planning_packet.py"
...
def test_phase38_step_74_files_exist() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert path.exists(), f"Missing expected file: {path}"


def test_phase38_step_74_launcher_has_valid_param_block() -> None:
    text = read(SCRIPT)
...
def test_phase38_step_74_payloads_are_bom_free() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert not path.read_bytes().startswith(b"\xef\xbb\xbf"), f"BOM detected in {path}"


def test_phase38_step_74_safety_markers_present() -> None:
    text = read(SCRIPT) + "\n" + read(UI) + "\n" + read(DOC)
    required = [
        "planning_only=true",
...
def test_phase38_step_74_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase38_step_74_launcher_smoke_passes() -> None:
