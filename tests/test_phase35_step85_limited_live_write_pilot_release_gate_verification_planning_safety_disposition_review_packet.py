...
SCRIPT = REPO / "scripts/phase35_step85_limited_live_write_pilot_release_gate_verification_planning_safety_disposition_review_packet.ps1"
UI_PAGE = REPO / "ui/pages/1661_Phase35_Step85_Live_Write_Pilot_Release_Gate_Verification_Safety_Review.py"
DOC = REPO / "docs/PHASE35_STEP85_LIMITED_LIVE_WRITE_PILOT_RELEASE_GATE_VERIFICATION_PLANNING_SAFETY_DISPOSITION_REVIEW_PACKET.md"
TEST = REPO / "tests/test_phase35_step85_limited_live_write_pilot_release_gate_verification_planning_safety_disposition_review_packet.py"
...
def test_phase35_step85_files_exist():
...
def test_phase35_step85_launcher_supports_optimized_actions():
    script = read(SCRIPT)
    assert 'ValidateSet("status", "apply", "smoke", "packet", "all")' in script
    assert "APPLY PASS: Phase 35 Step 85" in script
    assert "SMOKE TEST PASS: Phase 35 Step 85" in script


def test_phase35_step85_safety_markers_present():
...
def test_phase35_step85_does_not_enable_runtime_or_phase36():
...
def test_phase35_step85_doc_declares_no_phase36_boundary():
    doc = read(DOC).lower()
...
def test_phase35_step85_ui_is_reference_only():
    ui = read(UI_PAGE)
...
def test_phase35_step85_prior_step_is_recorded():
    combined = read(SCRIPT) + read(DOC) + read(UI_PAGE)
    assert "Phase 35 Step 84 - Phase 20 Network Transport Implementation Trusted Production Limited Live Write Pilot Release Gate Verification Planning Safety Disposition Planning Packet" in combined


def test_phase35_step85_uses_exact_four_step_files():
    script = read(SCRIPT)
    for rel in [
