...
SCRIPT = REPO / "scripts/phase36_step113_final_result_review_evidence_gap_review_packet.ps1"
UI = REPO / "ui/pages/1809_Phase36_Step113_Live_Write_Expansion_Final_Result_Review_Evidence_Gap_Review.py"
DOC = REPO / "docs/PHASE36_STEP113_CONTROLLED_LIVE_WRITE_EXPANSION_FINAL_RESULT_REVIEW_EVIDENCE_GAP_REVIEW_PACKET.md"
TEST = REPO / "tests/test_phase36_step113_final_result_review_evidence_gap_review_packet.py"
...
def test_phase36_step_113_files_exist() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert path.exists(), f"missing expected file: {path}"


def test_phase36_step_113_launcher_has_required_safe_markers() -> None:
    text = read(SCRIPT)
    for marker in [
        "planning_only = $true",
...
def test_phase36_step_113_operational_files_do_not_enable_forbidden_markers() -> None:
    forbidden_keys = [
        "phase37_start",
...
def test_phase36_step_113_ui_and_doc_reference_dry_run_no_apply() -> None:
    text = read(UI) + "\n" + read(DOC)
...
def test_phase36_step_113_launcher_apply_passes() -> None:
...
def test_phase36_step_113_launcher_smoke_passes() -> None:
    result = subprocess.run(
