...
SCRIPT = REPO / "scripts/phase33_step7_trusted_production_rollout_readiness_approval_readiness_packet.ps1"
UI_PAGE = REPO / "ui/pages/1343_Phase33_Step7_Implementation_Trusted_Production_Rollout_Readiness_Approval_Readiness_Packet.py"
DOC = REPO / "docs/PHASE33_STEP7_TRUSTED_PRODUCTION_ROLLOUT_READINESS_APPROVAL_READINESS_PACKET.md"
TEST = REPO / "tests/test_phase33_step7_trusted_production_rollout_readiness_approval_readiness_packet.py"
...
def test_phase33_step7_files_exist():
...
def test_phase33_step7_launcher_supports_optimized_actions():
    script = read(SCRIPT)
    assert 'ValidateSet("status", "apply", "smoke", "packet", "all")' in script
    assert "APPLY PASS: Phase 33 Step 7" in script
    assert "SMOKE TEST PASS: Phase 33 Step 7" in script


def test_phase33_step7_safety_markers_present():
...
def test_phase33_step7_does_not_enable_runtime_or_phase34():
...
def test_phase33_step7_doc_declares_no_phase34_boundary():
    doc = read(DOC).lower()
...
def test_phase33_step7_ui_is_reference_only():
    ui = read(UI_PAGE)
...
def test_phase33_step7_prior_step_is_recorded():
    combined = read(SCRIPT) + read(DOC) + read(UI_PAGE)
    assert "Phase 33 Step 6 - Phase 20 Network Transport Implementation Trusted Production Rollout Readiness Approval Boundary Packet" in combined


def test_phase33_step7_uses_exact_four_step_files():
    script = read(SCRIPT)
    for rel in [
