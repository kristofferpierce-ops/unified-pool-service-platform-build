...
function looksLikeAceNoise(text) {
  const t = String(text || "").trim();
  if (!t) return true;
  if (/^\d+(\.\d+)?$/.test(t)) return true;
  if (/^[a-z]-[a-z0-9-]{8,}$/i.test(t)) return true;
  if (/^p-[a-z0-9-]+$/i.test(t)) return true;
  if (/^[\d.\-]+$/.test(t) && !/[A-Za-z]/.test(t)) return true;
  if (/^\d{1,2}:\d{2}(\.\d+)?$/.test(t)) return true;
  if (/^\d{1,2}:\d{2}:\d{2}(\.\d+)?$/.test(t)) return true;
  if (t.length < 3 && !/[A-Za-z]/.test(t)) return true;
  return false;
}

function cleanInsightItems(items) {
  const out = [];
  for (const raw of items || []) {
    const cleaned = stripHtml(raw);
    if (!cleaned) continue;
    if (looksLikeAceNoise(cleaned)) continue;
...
function flattenInsightText(value, acc = []) {
  ...
  if (typeof value === "string" || typeof value === "number" || typeof value === "boolean") {
    ...
    if (cleaned && !looksLikeAceNoise(cleaned)) acc.push(cleaned);
    return acc;
  }
...
function splitTextToItems(text) {
  ...
  const parsed = lines.length > 1
    ? lines
    : cleaned.split(/(?:\s*;\s*|\s*\|\s*|\s{2,})/).map(x => x.trim()).filter(Boolean);

  return cleanInsightItems(parsed);
}
...
document.addEventListener("click", (event) => {
  ...
  const type = btn.getAttribute("data-quick-type") || "calls";
  openTaskModal(type).then(() => {
