...
def test_phase20_network_transport_commit_guard_packet_script_is_get_only_and_no_write() -> None:
    script = read("scripts/phase21_generate_phase20_network_transport_commit_guard_packet.ps1")
    assert "phase21_phase20_network_transport_commit_guard_packet_" in script
    assert "phase20_network_transport_commit_guard_packet_only = $true" in script
    assert "commit_guard_only = $true" in script
...
def test_phase20_network_transport_commit_guard_packet_streamlit_page_is_report_only() -> None:
    page = read("ui/pages/99_Phase20_Network_Transport_Commit_Guard_Packet.py")
    assert "Phase 21 Step 7 - Phase 20 Network Transport Commit Guard Packet" in page
    assert "Phase 20 network transport commit-guard-only" in page
    ...
    assert "phase21_phase20_network_transport_commit_guard_packet_*" in page
    assert "phase21_phase20_network_transport_commit_guard_packet.json" in page
    assert "requests.get" not in page
    assert "requests.post" not in page
    assert "st.download_button" in page


def test_phase20_network_transport_commit_guard_packet_docs_capture_boundaries() -> None:
    doc = read("docs/PHASE21_STEP7_PHASE20_NETWORK_TRANSPORT_COMMIT_GUARD_PACKET.md")
    assert "no-write commit guard" in doc
    assert "Phase 20 network transport commit-guard-only" in doc
    ...
    assert "Stage only the four Phase 21 Step 7 files" in doc
