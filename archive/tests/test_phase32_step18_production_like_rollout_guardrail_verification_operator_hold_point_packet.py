...
SCRIPT = REPO / "scripts/phase32_step18_production_like_rollout_guardrail_verification_operator_hold_point_packet.ps1"
UI_PAGE = REPO / "ui/pages/1234_Phase32_Step18_Implementation_Production_Like_Rollout_Guardrail_Verification_Operator_Hold_Point_Packet.py"
DOC = REPO / "docs/PHASE32_STEP18_PRODUCTION_LIKE_ROLLOUT_GUARDRAIL_VERIFICATION_OPERATOR_HOLD_POINT_PACKET.md"
TEST = REPO / "tests/test_phase32_step18_production_like_rollout_guardrail_verification_operator_hold_point_packet.py"
...
def test_phase32_step18_files_exist():
...
def test_phase32_step18_launcher_supports_optimized_actions():
    script = read(SCRIPT)
    assert 'ValidateSet("status", "apply", "smoke", "packet", "all")' in script
    assert "APPLY PASS: Phase 32 Step 18" in script
    assert "SMOKE TEST PASS: Phase 32 Step 18" in script


def test_phase32_step18_safety_markers_present():
...
def test_phase32_step18_does_not_enable_runtime_or_phase33():
...
def test_phase32_step18_doc_declares_no_phase33_boundary():
    doc = read(DOC).lower()
...
def test_phase32_step18_ui_is_reference_only():
    ui = read(UI_PAGE)
...
def test_phase32_step18_prior_step_is_recorded():
    combined = read(SCRIPT) + read(DOC) + read(UI_PAGE)
    assert "Phase 32 Step 17 - Phase 20 Network Transport Implementation Production-Like Rollout Guardrail Verification Approval Readiness Packet" in combined


def test_phase32_step18_uses_exact_four_step_files():
    script = read(SCRIPT)
    for rel in [
