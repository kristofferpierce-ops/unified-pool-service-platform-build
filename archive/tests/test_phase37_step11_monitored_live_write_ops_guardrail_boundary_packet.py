...
SCRIPT = REPO / "scripts/phase37_step11_monitored_live_write_ops_guardrail_boundary_packet.ps1"
UI = REPO / "ui/pages/1827_Phase37_Step11_Monitored_Live_Write_Ops_Guardrail_Boundary.py"
DOC = REPO / "docs/PHASE37_STEP11_MONITORED_LIVE_WRITE_OPS_GUARDRAIL_BOUNDARY_PACKET.md"
TEST = REPO / "tests/test_phase37_step11_monitored_live_write_ops_guardrail_boundary_packet.py"
...
def test_phase37_step11_files_exist():
...
def test_phase37_step11_launcher_has_valid_param_block():
    text = _read(SCRIPT)
...
def test_phase37_step11_required_safety_markers_present():
...
def test_phase37_step11_forbidden_true_markers_absent():
...
def test_phase37_step11_literal_pass_markers_present():
    text = _read(SCRIPT)
    assert "APPLY PASS" in text
    assert "SMOKE TEST PASS" in text


def test_phase37_step11_launcher_smoke_passes():
