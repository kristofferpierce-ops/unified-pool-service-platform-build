...
SCRIPT_REL = "scripts/phase22_step70_final_closeout_phase23_boundary_packet.ps1"
PAGE_REL = "ui/pages/176_Phase20_Network_Transport_Planning_Final_Closeout_Phase23_Boundary_Packet.py"
DOC_REL = "docs/PHASE22_STEP70_FINAL_CLOSEOUT_PHASE23_BOUNDARY_PACKET.md"
TEST_REL = "tests/test_phase22_step70_final_closeout_phase23_boundary_packet.py"
...
def test_step70_files_exist():
    for path in PUBLIC_FILES:
        assert path.exists(), path


def test_step70_script_mentions_phase_and_step():
    text = read(SCRIPT)
    assert "Phase 22 Step 70" in text
    assert "$StepNumber = 70" in text
    assert "Final Closeout Phase 23 Boundary Packet" in text


def test_step70_page_number_is_correct():
    assert PAGE.as_posix().endswith(
        "ui/pages/176_Phase20_Network_Transport_Planning_Final_Closeout_Phase23_Boundary_Packet.py"
    )


def test_step70_doc_title_is_correct():
    text = read(DOC)
    assert "# Phase 22 Step 70 - Phase 20 Network Transport Planning Final Closeout Phase 23 Boundary Packet" in text


def test_step70_prior_step_is_step69():
    text = read(SCRIPT) + "\n" + read(DOC)
    assert "Phase 22 Step 69 - Phase 20 Network Transport Planning Final Cross-Repo Handoff Closeout Index Packet" in text
    assert "CHECK: prior_step=Phase 22 Step 69" in text


def test_step70_launcher_supports_action_all():
    text = read(SCRIPT)
...
def test_step70_uses_literal_and_safe_paths():
    text = read(SCRIPT)
...
def test_step70_is_planning_only():
...
def test_step70_keeps_phase23_closed():
    text = combined_public_text()
    required = [
        "phase23_start",
...
def test_step70_disables_cross_repo_mutation():
    text = combined_public_text()
    required = [
        "cross_repo_write",
        "cross_repo_mutation",
...
def test_step70_disables_evidence_collection_decisions_and_approvals():
    text = combined_public_text()
    required = [
        "final_closeout_phase23_boundary_mode",
        "final_closeout_phase23_boundary_record_creation",
        "handoff_evidence_collection_execution",
        "final_closeout_phase23_boundary_write",
...
def test_step70_keeps_lacrm_dry_run():
    text = combined_public_text()
...
def test_step70_contains_parent_workspace_commands():
    text = read(DOC)
    ...
    assert "INSTALL_PHASE22_STEP70_SINGLE_FILE" in text


def test_step70_menu_has_expected_options():
    text = read(SCRIPT)
    assert "Phase 22 Step 70 menu" in text
    assert "Show status / verify paths" in text
    assert "Smoke test Phase 22 Step 70" in text
    assert "Show server start placeholder only" in text
    assert "Generate Phase 22 Step 70 final closeout phase 23 boundary packet" in text


def test_step70_expected_smoke_success_text_present():
    text = read(SCRIPT)
    expected = (
        "SMOKE TEST PASS: Phase 22 Step 70 Phase 20 Network Transport Planning "
        "Final Closeout Phase 23 Boundary Packet is present and planning-only."
    )
    assert expected in text


def test_step70_expected_packet_output_in_doc_and_script():
    combined = read(DOC) + "\n" + read(SCRIPT)
    required = [
        "PASS: cross_repo_write=false",
        "PASS: cross_repo_mutation=false",
        "PASS: external_repo_push=false",
        "PASS: final_closeout_phase23_boundary_mode=reference_only",
        "PASS: final_closeout_phase23_boundary_write=false",
        ...
        "CHECK: prior_step=Phase 22 Step 69",
        "CHECK: phase23_start=not_started",
        "CHECK: final_closeout_phase23_boundary_mode=reference_only",
...
def test_step70_does_not_create_phase23_or_implementation():
...
def test_step70_does_not_enable_network_or_bridge_runtime():
    text = combined_runtime_text().lower()
    forbidden = [
...
def test_step70_does_not_write_to_external_repos():
    text = combined_runtime_text().lower()
    forbidden = [
...
def test_step70_does_not_collect_decide_approve_or_remediate_evidence():
    text = combined_runtime_text().lower()
    forbidden = [
        "handoff_evidence_collection_execution: true",
        "handoff_evidence_collection_execution = true",
        "final_closeout_phase23_boundary_write: true",
        "final_closeout_phase23_boundary_write = true",
        ...
        "evidence_gap_resolution_execution = true",
        "final_closeout_phase23_boundary_record_creation: true",
        "final_closeout_phase23_boundary_record_creation = true",
    ]
    for marker in forbidden:
        assert marker not in text
...
def test_step70_does_not_create_approvals_or_closure_records():
    text = combined_runtime_text().lower()
    forbidden = [
...
def test_step70_packet_generates_json_under_backups():
    text = read(SCRIPT)
    assert "backups" in text
    assert "phase22_step70_final_closeout_phase23_boundary_packet" in text
    assert "ConvertTo-Json" in text
    assert "Set-Content -LiteralPath" in text


def test_step70_expected_branch_is_short():
    text = read(SCRIPT)
    branch = "phase22-step70-final-closeout-phase23-boundary"
    assert branch in text
    assert len(branch) < 80


def test_step70_source_zip_filename_not_required_by_launcher():
    text = read(SCRIPT)
    assert "phase22_step70_source(1).zip" not in text
    assert "phase22_step70_source.zip" not in text
    assert "single-file installer" in text.lower()


def test_step70_installer_paths_are_short():
    for rel in [SCRIPT_REL, PAGE_REL, DOC_REL, TEST_REL]:
        assert len(rel) < 150


def test_step70_no_server_start_claim():
