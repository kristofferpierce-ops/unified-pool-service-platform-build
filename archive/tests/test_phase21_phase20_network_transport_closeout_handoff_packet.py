...
def test_phase20_network_transport_closeout_handoff_packet_script_is_get_only_and_no_write() -> None:
    script = read("scripts/phase21_generate_phase20_network_transport_closeout_handoff_packet.ps1")
    assert "phase21_phase20_network_transport_closeout_handoff_packet_" in script
    assert "phase20_network_transport_closeout_handoff_packet_only = $true" in script
    assert "closeout_handoff_only = $true" in script
    ...
    assert "phase21_phase20_network_transport_commit_guard_packet_*" in script
    assert "phase21_phase20_network_transport_cleanup_manifest_packet_*" in script
    assert "phase21_phase20_network_transport_workspace_hygiene_packet_*" in script
...
def test_phase20_network_transport_closeout_handoff_packet_streamlit_page_is_report_only() -> None:
    page = read("ui/pages/100_Phase20_Network_Transport_Closeout_Handoff_Packet.py")
    assert "Phase 21 Step 8 - Phase 20 Network Transport Closeout Handoff Packet" in page
    assert "Phase 20 network transport closeout-handoff-only" in page
    ...
    assert "phase21_phase20_network_transport_closeout_handoff_packet_*" in page
    assert "phase21_phase20_network_transport_closeout_handoff_packet.json" in page
    assert "requests.get" not in page
    assert "requests.post" not in page
    assert "st.download_button" in page


def test_phase20_network_transport_closeout_handoff_packet_docs_capture_boundaries() -> None:
    doc = read("docs/PHASE21_STEP8_PHASE20_NETWORK_TRANSPORT_CLOSEOUT_HANDOFF_PACKET.md")
    assert "no-write closeout handoff" in doc
    assert "Phase 20 network transport closeout-handoff-only" in doc
    ...
    assert "Stage only the four Phase 21 Step 8 files" in doc
