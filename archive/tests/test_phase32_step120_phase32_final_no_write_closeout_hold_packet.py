...
SCRIPT = REPO / "scripts/phase32_step120_phase32_final_no_write_closeout_hold_packet.ps1"
UI_PAGE = REPO / "ui/pages/1336_Phase32_Step120_Phase32_Final_No_Write_Closeout_Hold_Packet.py"
DOC = REPO / "docs/PHASE32_STEP120_PHASE32_FINAL_NO_WRITE_CLOSEOUT_HOLD_PACKET.md"
TEST = REPO / "tests/test_phase32_step120_phase32_final_no_write_closeout_hold_packet.py"
...
def test_phase32_step120_files_exist():
...
def test_phase32_step120_launcher_supports_optimized_actions():
    script = read(SCRIPT)
    assert 'ValidateSet("status", "apply", "smoke", "packet", "all")' in script
    assert "APPLY PASS: Phase 32 Step 120" in script
    assert "SMOKE TEST PASS: Phase 32 Step 120" in script


def test_phase32_step120_safety_markers_present():
...
def test_phase32_step120_does_not_enable_runtime_or_phase33():
...
def test_phase32_step120_doc_declares_no_phase33_boundary():
    doc = read(DOC).lower()
...
def test_phase32_step120_ui_is_reference_only():
    ui = read(UI_PAGE)
...
def test_phase32_step120_prior_step_is_recorded():
    combined = read(SCRIPT) + read(DOC) + read(UI_PAGE)
    assert "Phase 32 Step 119 - Phase 20 Network Transport Implementation Production-Like Rollout Final Release Result Review Planning Closeout Index Packet" in combined


def test_phase32_step120_uses_exact_four_step_files():
    script = read(SCRIPT)
    for rel in [
