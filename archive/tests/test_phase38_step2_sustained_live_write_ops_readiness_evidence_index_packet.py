...
SCRIPT = ROOT / "scripts/phase38_step2_sustained_live_write_ops_readiness_evidence_index_packet.ps1"
UI = ROOT / "ui/pages/1938_Phase38_Step2_Sustained_Live_Write_Ops_Readiness_Evidence_Index.py"
DOC = ROOT / "docs/PHASE38_STEP2_SUSTAINED_LIVE_WRITE_OPS_READINESS_EVIDENCE_INDEX_PACKET.md"
TEST = ROOT / "tests/test_phase38_step2_sustained_live_write_ops_readiness_evidence_index_packet.py"
...
def test_phase38_step2_files_exist() -> None:
...
def test_phase38_step2_launcher_has_valid_param_block() -> None:
    text = read(SCRIPT)
...
def test_phase38_step2_required_safety_markers_present() -> None:
...
def test_phase38_step2_next_phase_guards_present() -> None:
    combined = "\n".join(read(path) for path in (SCRIPT, UI, DOC))
    assert "phase39_start" in combined
    assert "phase39_boundary_creation" in combined


def test_phase38_step2_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase38_step2_literal_pass_markers_present() -> None:
    text = read(SCRIPT)
    assert "APPLY PASS" in text
    assert "SMOKE TEST PASS" in text


def test_phase38_step2_ui_is_planning_only_page() -> None:
    text = read(UI)
...
def test_phase38_step2_no_phase39_files_created_by_payload() -> None:
