...
def test_phase20_network_transport_cleanup_exit_packet_script_is_get_only_and_no_write() -> None:
    script = read("scripts/phase21_generate_phase20_network_transport_cleanup_exit_packet.ps1")
    assert "phase21_phase20_network_transport_cleanup_exit_packet_" in script
    assert "phase20_network_transport_cleanup_exit_packet_only = $true" in script
    assert "cleanup_exit_only = $true" in script
...
def test_phase20_network_transport_cleanup_exit_packet_streamlit_page_is_report_only() -> None:
    page = read("ui/pages/106_Phase20_Network_Transport_Cleanup_Exit_Packet.py")
    assert "Phase 21 Step 14 - Phase 20 Network Transport Cleanup Exit Packet" in page
    assert "Phase 20 network transport cleanup-exit-only" in page
    ...
    assert "phase21_phase20_network_transport_cleanup_exit_packet_*" in page
    assert "phase21_phase20_network_transport_cleanup_exit_packet.json" in page
    assert "requests.get" not in page
    assert "requests.post" not in page
    assert "st.download_button" in page


def test_phase20_network_transport_cleanup_exit_packet_docs_capture_boundaries() -> None:
    doc = read("docs/PHASE21_STEP14_PHASE20_NETWORK_TRANSPORT_CLEANUP_EXIT_PACKET.md")
    assert "no-write cleanup exit packet" in doc
    assert "Phase 20 network transport cleanup-exit-only" in doc
    ...
    assert "Stage only the four Phase 21 Step 14 files" in doc
