...
def test_phase20_network_transport_workspace_hygiene_packet_script_is_get_only_and_no_write() -> None:
    script = read("scripts/phase21_generate_phase20_network_transport_workspace_hygiene_packet.ps1")
    assert "phase21_phase20_network_transport_workspace_hygiene_packet_" in script
    assert "phase20_network_transport_workspace_hygiene_packet_only = $true" in script
    assert "workspace_hygiene_only = $true" in script
    ...
    assert "git -C $PlatformDir branch --show-current" in script
    assert "current_step_candidate" in script
    assert "do_not_stage" in script
...
def test_phase20_network_transport_workspace_hygiene_packet_streamlit_page_is_report_only() -> None:
    page = read("ui/pages/98_Phase20_Network_Transport_Workspace_Hygiene_Packet.py")
    assert "Phase 21 Step 6 - Phase 20 Network Transport Workspace Hygiene Packet" in page
    assert "Phase 20 network transport workspace-hygiene-only" in page
    ...
    assert "phase21_phase20_network_transport_workspace_hygiene_packet_*" in page
    assert "phase21_phase20_network_transport_workspace_hygiene_packet.json" in page
    assert "requests.get" not in page
    assert "requests.post" not in page
    assert "st.download_button" in page


def test_phase20_network_transport_workspace_hygiene_packet_docs_capture_boundaries() -> None:
    doc = read("docs/PHASE21_STEP6_PHASE20_NETWORK_TRANSPORT_WORKSPACE_HYGIENE_PACKET.md")
    assert "no-write workspace hygiene packet" in doc
    assert "Phase 20 network transport workspace-hygiene-only" in doc
    ...
    assert "Stage only the four Phase 21 Step 6 files" in doc
