...
def test_phase20_network_transport_artifact_gap_packet_script_is_get_only_and_no_write() -> None:
    script = read("scripts/phase21_generate_phase20_network_transport_artifact_gap_packet.ps1")
    assert "phase21_phase20_network_transport_artifact_gap_packet_" in script
    assert "phase20_network_transport_artifact_gap_packet_only = $true" in script
    assert "artifact_gap_packet_only = $true" in script
    ...
    assert "phase21_phase20_network_transport_review_board_packet_*" in script
    assert "phase21_phase20_network_transport_consolidated_closeout_packet_*" in script
...
def test_phase20_network_transport_artifact_gap_packet_streamlit_page_is_report_only() -> None:
    page = read("ui/pages/96_Phase20_Network_Transport_Artifact_Gap_Packet.py")
    assert "Phase 21 Step 4 - Phase 20 Network Transport Artifact Gap Packet" in page
    assert "Phase 20 network transport artifact-gap-only" in page
    ...
    assert "phase21_phase20_network_transport_artifact_gap_packet_*" in page
    assert "phase21_phase20_network_transport_artifact_gap_packet.json" in page
    assert "requests.get" not in page
    assert "requests.post" not in page
    assert "st.download_button" in page


def test_phase20_network_transport_artifact_gap_packet_docs_capture_boundaries() -> None:
    doc = read("docs/PHASE21_STEP4_PHASE20_NETWORK_TRANSPORT_ARTIFACT_GAP_PACKET.md")
    assert "no-write artifact gap packet" in doc
    assert "Phase 20 network transport artifact-gap-only" in doc
    ...
    assert "Stage only the four Phase 21 Step 4 files" in doc
