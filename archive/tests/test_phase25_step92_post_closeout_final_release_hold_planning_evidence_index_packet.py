...
SCRIPT = REPO / "scripts/phase25_step92_post_closeout_final_release_hold_planning_evidence_index_packet.ps1"
UI = REPO / "ui/pages/468_Phase25_Step92_Implementation_PostCloseout_Final_Release_Hold_Planning_Evidence_Index_Packet.py"
DOC = REPO / "docs/PHASE25_STEP92_POST_CLOSEOUT_FINAL_RELEASE_HOLD_PLANNING_EVIDENCE_INDEX_PACKET.md"
TEST = REPO / "tests/test_phase25_step92_post_closeout_final_release_hold_planning_evidence_index_packet.py"
...
def test_phase25_step92_files_exist():
...
def test_phase25_step92_launcher_markers():
    text = read(SCRIPT).lower()
    assert "phase 25 step 92" in text
    assert "phase25-step92-post-closeout-final-release-hold-planning-evidence-index" in text
...
def test_phase25_step92_planning_only_safety_markers():
...
def test_phase25_step92_does_not_start_runtime_or_next_phase():
...
def test_phase25_step92_complexity_gate_allows_standard_planning_only_batch():
    text = compact(read(SCRIPT) + read(DOC) + read(UI))
    assert "complexity_batch_gate" in text
...
def test_phase25_step92_docs_are_operator_readable():
    text = read(DOC)
    assert "Phase 25 Step 92 - Phase 20 Network Transport Implementation Post-Closeout Final Release Hold Planning Evidence Index Packet" in text
...
def test_phase25_step92_ui_is_streamlit_reference_only_page():
    text = read(UI)
...
def test_phase25_step92_test_file_self_identifies_step():
    text = read(TEST).lower()
    assert "step92" in text or "step 92" in text
