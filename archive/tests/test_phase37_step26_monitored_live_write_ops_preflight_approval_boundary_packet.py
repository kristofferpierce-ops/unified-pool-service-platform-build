...
SCRIPT = REPO / "scripts/phase37_step26_monitored_live_write_ops_preflight_approval_boundary_packet.ps1"
UI = REPO / "ui/pages/1842_Phase37_Step26_Monitored_Live_Write_Ops_Preflight_Approval_Boundary.py"
DOC = REPO / "docs/PHASE37_STEP26_MONITORED_LIVE_WRITE_OPS_PREFLIGHT_APPROVAL_BOUNDARY_PACKET.md"
TEST = REPO / "tests/test_phase37_step26_monitored_live_write_ops_preflight_approval_boundary_packet.py"
...
def test_phase37_step_26_files_exist() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert path.exists(), f"Missing expected file: {path}"


def test_phase37_step_26_launcher_has_valid_param_block() -> None:
    text = read(SCRIPT)
...
def test_phase37_step_26_safety_markers_present() -> None:
    text = read(SCRIPT) + "\n" + read(UI) + "\n" + read(DOC)
    required = [
...
def test_phase37_step_26_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase37_step_26_launcher_smoke_passes() -> None:
