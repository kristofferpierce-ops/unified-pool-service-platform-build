...
def test_phase20_network_transport_planning_implementation_gate_packet_script_is_get_only_and_no_write() -> None:
    script = read("scripts/phase22_generate_phase20_network_transport_planning_implementation_gate_packet.ps1")
    assert "phase22_phase20_network_transport_planning_implementation_gate_packet_" in script
    assert "phase20_network_transport_planning_implementation_gate_packet_only = $true" in script
    assert "planning_implementation_gate_only = $true" in script
    assert "planning_only = $true" in script
    assert "bridge_get_only = $true" in script
    assert "implementation_gate_record_created = $false" in script
    ...
    assert "can_record_operator_approval_now = $false" in script
    assert "can_record_final_approval_now = $false" in script
    assert "can_record_operator_signoff_now = $false" in script
    assert "can_record_implementation_gate_now = $false" in script
    assert "can_create_design_closure_record_now = $false" in script
    ...
    assert "phase22_phase20_network_transport_planning_transition_packet_*" in script
    assert "phase22_phase20_network_transport_planning_exit_packet_*" in script
...
def test_phase20_network_transport_planning_implementation_gate_packet_streamlit_page_is_report_only() -> None:
    page = read("ui/pages/121_Phase20_Network_Transport_Planning_Implementation_Gate_Packet.py")
    assert "Phase 22 Step 15 - Phase 20 Network Transport Planning Implementation Gate Packet" in page
    assert "Phase 20 network transport planning-implementation-gate-only" in page
    assert "does not create implementation-gate records" in page
    ...
    assert "phase22_phase20_network_transport_planning_implementation_gate_packet_*" in page
    assert "phase22_phase20_network_transport_planning_implementation_gate_packet.json" in page
    assert "requests.get" not in page
    assert "requests.post" not in page
    assert "st.download_button" in page


def test_phase20_network_transport_planning_implementation_gate_packet_docs_capture_boundaries() -> None:
    doc = read("docs/PHASE22_STEP15_PHASE20_NETWORK_TRANSPORT_PLANNING_IMPLEMENTATION_GATE_PACKET.md")
    assert "no-write planning implementation gate packet" in doc
    assert "Phase 20 network transport planning-implementation-gate-only" in doc
    assert "does not create implementation-gate records" in doc
    ...
    assert "Stage only the four Phase 22 Step 15 files" in doc
