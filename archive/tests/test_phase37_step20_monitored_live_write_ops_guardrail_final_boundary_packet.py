...
SCRIPT = REPO / "scripts/phase37_step20_monitored_live_write_ops_guardrail_final_boundary_packet.ps1"
UI = REPO / "ui/pages/1836_Phase37_Step20_Monitored_Live_Write_Ops_Guardrail_Final_Boundary.py"
DOC = REPO / "docs/PHASE37_STEP20_MONITORED_LIVE_WRITE_OPS_GUARDRAIL_FINAL_BOUNDARY_PACKET.md"
TEST = REPO / "tests/test_phase37_step20_monitored_live_write_ops_guardrail_final_boundary_packet.py"
...
def test_phase37_step20_files_exist():
...
def test_phase37_step20_launcher_has_valid_param_block():
    text = _read(SCRIPT)
...
def test_phase37_step20_required_safety_markers_present():
...
def test_phase37_step20_forbidden_true_markers_absent():
...
def test_phase37_step20_literal_pass_markers_present():
    text = _read(SCRIPT)
    assert "APPLY PASS" in text
    assert "SMOKE TEST PASS" in text


def test_phase37_step20_launcher_smoke_passes():
