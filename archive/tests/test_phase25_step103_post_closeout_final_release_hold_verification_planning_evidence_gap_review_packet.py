...
SCRIPT = REPO_ROOT / "scripts/phase25_step103_post_closeout_final_release_hold_verification_planning_evidence_gap_review_packet.ps1"
PAGE = REPO_ROOT / "ui/pages/479_Phase25_Step103_Implementation_PostCloseout_Final_Release_Hold_Verification_Planning_Evidence_Gap_Review_Packet.py"
DOC = REPO_ROOT / "docs/PHASE25_STEP103_POST_CLOSEOUT_FINAL_RELEASE_HOLD_VERIFICATION_PLANNING_EVIDENCE_GAP_REVIEW_PACKET.md"
TEST = REPO_ROOT / "tests/test_phase25_step103_post_closeout_final_release_hold_verification_planning_evidence_gap_review_packet.py"
...
def test_phase25_step103_files_are_present():
    for path in (SCRIPT, PAGE, DOC, TEST):
        assert path.exists(), f"missing {path}"


def test_phase25_step103_planning_only_markers_are_present():
...
def test_phase25_step103_does_not_enable_runtime_or_next_phase():
    text = operational_text()
    forbidden = [
        "phase26_start" + "=true",
...
def test_phase25_step103_launcher_supports_optimized_actions():
    script = read(SCRIPT)
    assert "ValidateSet(\"status\", \"apply\", \"smoke\", \"packet\", \"all\")" in script
    assert "APPLY PASS: Phase 25 Step 103" in script
    assert "SMOKE TEST PASS: Phase 25 Step 103" in script
    assert "CHECK: packet_json=" in script


def test_phase25_step103_known_powershell_generation_bugs_are_absent():
    script = read(SCRIPT)
    assert "$File:" not in script
    assert "Join-Path $RepoRoot \"scripts/phase25_step103_post_closeout_final_release_hold_verification_planning_evidence_gap_review_packet.ps1\"," not in script
    assert "System.Object[]" not in script


def test_phase25_step103_ui_page_is_reference_only():
...
def test_phase25_step103_doc_records_no_write_safety():
    doc = read(DOC).lower()
...
def test_phase25_step103_file_names_match_step_number():
    assert "step103" in SCRIPT.name
    assert "Step103" in PAGE.name
    assert "STEP103" in DOC.name
    assert "step103" in TEST.name
