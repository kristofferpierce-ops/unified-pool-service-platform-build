...
SCRIPT = REPO / "scripts/phase37_step16_monitored_live_write_ops_guardrail_approval_boundary_packet.ps1"
UI = REPO / "ui/pages/1832_Phase37_Step16_Monitored_Live_Write_Ops_Guardrail_Approval_Boundary.py"
DOC = REPO / "docs/PHASE37_STEP16_MONITORED_LIVE_WRITE_OPS_GUARDRAIL_APPROVAL_BOUNDARY_PACKET.md"
TEST = REPO / "tests/test_phase37_step16_monitored_live_write_ops_guardrail_approval_boundary_packet.py"
...
def test_phase37_step16_files_exist():
...
def test_phase37_step16_launcher_has_valid_param_block():
    text = _read(SCRIPT)
...
def test_phase37_step16_required_safety_markers_present():
...
def test_phase37_step16_forbidden_true_markers_absent():
...
def test_phase37_step16_literal_pass_markers_present():
    text = _read(SCRIPT)
    assert "APPLY PASS" in text
    assert "SMOKE TEST PASS" in text


def test_phase37_step16_launcher_smoke_passes():
