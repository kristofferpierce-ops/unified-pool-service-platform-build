...
SCRIPT = REPO / "scripts/phase25_step91_post_closeout_final_release_hold_planning_boundary_packet.ps1"
UI = REPO / "ui/pages/467_Phase25_Step91_Implementation_PostCloseout_Final_Release_Hold_Planning_Boundary_Packet.py"
DOC = REPO / "docs/PHASE25_STEP91_POST_CLOSEOUT_FINAL_RELEASE_HOLD_PLANNING_BOUNDARY_PACKET.md"
TEST = REPO / "tests/test_phase25_step91_post_closeout_final_release_hold_planning_boundary_packet.py"
...
def test_phase25_step91_files_exist():
...
def test_phase25_step91_launcher_markers():
    text = read(SCRIPT).lower()
    assert "phase 25 step 91" in text
    assert "phase25-step91-post-closeout-final-release-hold-planning-boundary" in text
...
def test_phase25_step91_planning_only_safety_markers():
...
def test_phase25_step91_does_not_start_runtime_or_next_phase():
...
def test_phase25_step91_complexity_gate_allows_standard_planning_only_batch():
    text = compact(read(SCRIPT) + read(DOC) + read(UI))
    assert "complexity_batch_gate" in text
...
def test_phase25_step91_docs_are_operator_readable():
    text = read(DOC)
    assert "Phase 25 Step 91 - Phase 20 Network Transport Implementation Post-Closeout Final Release Hold Planning Boundary Packet" in text
...
def test_phase25_step91_ui_is_streamlit_reference_only_page():
    text = read(UI)
...
def test_phase25_step91_test_file_self_identifies_step():
    text = read(TEST).lower()
    assert "step91" in text or "step 91" in text
