...
SCRIPT = REPO / "scripts/phase31_step37_controlled_active_program_operation_no_write_dry_run_readiness_approval_readiness_packet.ps1"
UI_PAGE = REPO / "ui/pages/1133_Phase31_Step37_Implementation_Controlled_Active_Program_Operation_No_Write_Dry_Run_Readiness_Approval_Readiness_Packet.py"
DOC = REPO / "docs/PHASE31_STEP37_CONTROLLED_ACTIVE_PROGRAM_OPERATION_NO_WRITE_DRY_RUN_READINESS_APPROVAL_READINESS_PACKET.md"
TEST = REPO / "tests/test_phase31_step37_controlled_active_program_operation_no_write_dry_run_readiness_approval_readiness_packet.py"
...
def test_phase31_step37_files_exist():
...
def test_phase31_step37_launcher_supports_optimized_actions():
    script = read(SCRIPT)
    assert 'ValidateSet("status", "apply", "smoke", "packet", "all")' in script
    assert "APPLY PASS: Phase 31 Step 37" in script
    assert "SMOKE TEST PASS: Phase 31 Step 37" in script


def test_phase31_step37_safety_markers_present():
...
def test_phase31_step37_does_not_enable_runtime_or_phase32():
...
def test_phase31_step37_doc_declares_no_phase32_boundary():
    doc = read(DOC).lower()
...
def test_phase31_step37_ui_is_reference_only():
    ui = read(UI_PAGE)
...
def test_phase31_step37_prior_step_is_recorded():
    combined = read(SCRIPT) + read(DOC) + read(UI_PAGE)
    assert "Phase 31 Step 36 - Phase 20 Network Transport Implementation Controlled Active Program Operation No-Write Dry Run Readiness Approval Boundary Packet" in combined


def test_phase31_step37_uses_exact_four_step_files():
    script = read(SCRIPT)
    for rel in [
