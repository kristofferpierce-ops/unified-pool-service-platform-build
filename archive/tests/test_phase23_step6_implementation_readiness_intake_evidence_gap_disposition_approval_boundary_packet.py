...
SCRIPT_REL = "scripts/phase23_step6_implementation_readiness_intake_evidence_gap_disposition_approval_boundary_packet.ps1"
PAGE_REL = "ui/pages/182_Phase23_Step6_Implementation_Readiness_Intake_Evidence_Gap_Disposition_Approval_Boundary_Packet.py"
DOC_REL = "docs/PHASE23_STEP6_IMPLEMENTATION_READINESS_INTAKE_EVIDENCE_GAP_DISPOSITION_APPROVAL_BOUNDARY_PACKET.md"
TEST_REL = "tests/test_phase23_step6_implementation_readiness_intake_evidence_gap_disposition_approval_boundary_packet.py"
...
def test_phase23_step6_files_exist():
    for path in PUBLIC_FILES:
        assert path.exists(), path


def test_phase23_step6_script_mentions_phase_and_step():
    text = read(SCRIPT)
    assert "Phase 23 Step 6" in text
    assert "$PhaseNumber = 23" in text
    assert "$StepNumber = 6" in text
    assert "Implementation Readiness Intake Evidence Gap Disposition Approval Boundary Packet" in text


def test_phase23_step6_page_number_is_correct():
...
def test_phase23_step6_doc_title_is_correct():
    text = read(DOC)
...
def test_phase23_step6_prior_step_is_phase23_step5():
    text = read(SCRIPT) + "\n" + read(DOC)
    ...
    assert "CHECK: prior_step=Phase 23 Step 5" in text


def test_phase23_step6_launcher_supports_action_all():
    text = read(SCRIPT)
...
def test_phase23_step6_uses_literal_and_safe_paths():
    text = read(SCRIPT)
...
def test_phase23_step6_is_planning_only():
...
def test_phase23_step6_opens_phase23_as_intake_only():
    text = combined_public_text()
    required = [
        "phase23_context",
...
def test_phase23_step6_disables_implementation_and_runtime():
    text = combined_public_text()
    required = [
...
def test_phase23_step6_disables_cross_repo_mutation():
    text = combined_public_text()
    required = [
        "cross_repo_write",
        "cross_repo_mutation",
...
def test_phase23_step6_disables_decisions_and_approvals():
    text = combined_public_text()
    required = [
        "readiness_intake_evidence_gap_disposition_approval_boundary_mode",
        "readiness_intake_evidence_gap_disposition_approval_boundary_write",
        "readiness_intake_evidence_gap_disposition_approval_boundary_record_creation",
...
def test_phase23_step6_keeps_lacrm_dry_run():
    text = combined_public_text()
...
def test_phase23_step6_contains_parent_workspace_commands():
    text = read(DOC)
    ...
    assert "INSTALL_PHASE23_STEP6_SINGLE_FILE" in text


def test_phase23_step6_menu_has_expected_options():
    text = read(SCRIPT)
    assert "Phase 23 Step 6 menu" in text
    assert "Show status / verify paths" in text
    assert "Smoke test Phase 23 Step 6" in text
    assert "Show server start placeholder only" in text
    assert "Generate Phase 23 Step 6 implementation readiness intake evidence gap disposition approval boundary packet" in text


def test_phase23_step6_expected_smoke_success_text_present():
    text = read(SCRIPT)
    expected = (
        "SMOKE TEST PASS: Phase 23 Step 6 Phase 20 Network Transport Implementation "
        "Readiness Intake Evidence Gap Disposition Approval Boundary Packet is present and planning-only."
    )
    assert expected in text


def test_phase23_step6_expected_packet_output_in_doc_and_script():
    combined = read(DOC) + "\n" + read(SCRIPT)
    required = [
        ...
        "PASS: readiness_intake_evidence_gap_disposition_approval_boundary_mode=reference_only",
        "PASS: readiness_intake_evidence_gap_disposition_approval_boundary_write=false",
        "PASS: readiness_intake_evidence_gap_disposition_approval_boundary_record_creation=false",
        "PASS: readiness_intake_decision_creation=false",
        "PASS: readiness_intake_approval_creation=false",
        "PASS: phase22_reopen=false",
        "CHECK: prior_step=Phase 23 Step 5",
...
def test_phase23_step6_packet_metadata_is_correct():
    text = read(SCRIPT)
    assert "phase = 23" in text
    assert "step = 6" in text
    assert "phase23_step6_implementation_readiness_intake_evidence_gap_disposition_approval_boundary_packet" in text


def test_phase23_step6_does_not_start_implementation_or_runtime():
...
def test_phase23_step6_does_not_enable_network_or_bridge_runtime():
    text = combined_runtime_text().lower()
    forbidden = [
...
def test_phase23_step6_does_not_write_to_external_repos():
    text = combined_runtime_text().lower()
    forbidden = [
...
def test_phase23_step6_does_not_create_decisions_approvals_or_signoffs():
    text = combined_runtime_text().lower()
    forbidden = [
...
def test_phase23_step6_packet_generates_json_under_backups():
    text = read(SCRIPT)
...
def test_phase23_step6_expected_branch_is_short():
    text = read(SCRIPT)
    branch = "phase23-step6-readiness-intake-evidence-gap-disposition-approval-boundary"
    assert branch in text
    assert len(branch) < 80


def test_phase23_step6_source_zip_filename_not_required_by_launcher():
    text = read(SCRIPT)
    assert "phase23_step6_source(1).zip" not in text
    assert "phase23_step6_source.zip" not in text
    assert "single-file installer" in text.lower()


def test_phase23_step6_installer_paths_are_short():
    for rel in [SCRIPT_REL, PAGE_REL, DOC_REL, TEST_REL]:
        assert len(rel) < 150


def test_phase23_step6_no_server_start_claim():
