...
SCRIPT = REPO / "scripts/phase37_step30_monitored_live_write_ops_preflight_final_boundary_packet.ps1"
UI = REPO / "ui/pages/1846_Phase37_Step30_Monitored_Live_Write_Ops_Preflight_Final_Boundary.py"
DOC = REPO / "docs/PHASE37_STEP30_MONITORED_LIVE_WRITE_OPS_PREFLIGHT_FINAL_BOUNDARY_PACKET.md"
TEST = REPO / "tests/test_phase37_step30_monitored_live_write_ops_preflight_final_boundary_packet.py"
...
def test_phase37_step_30_files_exist() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert path.exists(), f"Missing expected file: {path}"


def test_phase37_step_30_launcher_has_valid_param_block() -> None:
    text = read(SCRIPT)
...
def test_phase37_step_30_safety_markers_present() -> None:
    text = read(SCRIPT) + "\n" + read(UI) + "\n" + read(DOC)
    required = [
...
def test_phase37_step_30_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase37_step_30_launcher_smoke_passes() -> None:
