...
SCRIPT = REPO / "scripts/phase36_step111_final_result_review_boundary_packet.ps1"
UI = REPO / "ui/pages/1807_Phase36_Step111_Live_Write_Expansion_Final_Result_Review_Boundary.py"
DOC = REPO / "docs/PHASE36_STEP111_CONTROLLED_LIVE_WRITE_EXPANSION_FINAL_RESULT_REVIEW_BOUNDARY_PACKET.md"
TEST = REPO / "tests/test_phase36_step111_final_result_review_boundary_packet.py"
...
def test_phase36_step_111_files_exist() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert path.exists(), f"missing expected file: {path}"


def test_phase36_step_111_launcher_has_required_safe_markers() -> None:
    text = read(SCRIPT)
    for marker in [
        "planning_only = $true",
...
def test_phase36_step_111_operational_files_do_not_enable_forbidden_markers() -> None:
    forbidden_keys = [
        "phase37_start",
...
def test_phase36_step_111_ui_and_doc_reference_dry_run_no_apply() -> None:
    text = read(UI) + "\n" + read(DOC)
...
def test_phase36_step_111_launcher_apply_passes() -> None:
...
def test_phase36_step_111_launcher_smoke_passes() -> None:
    result = subprocess.run(
