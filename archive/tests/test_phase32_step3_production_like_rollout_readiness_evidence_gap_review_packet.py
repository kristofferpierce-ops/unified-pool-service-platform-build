...
SCRIPT = REPO / "scripts/phase32_step3_production_like_rollout_readiness_evidence_gap_review_packet.ps1"
UI_PAGE = REPO / "ui/pages/1219_Phase32_Step3_Implementation_Production_Like_Rollout_Readiness_Evidence_Gap_Review_Packet.py"
DOC = REPO / "docs/PHASE32_STEP3_PRODUCTION_LIKE_ROLLOUT_READINESS_EVIDENCE_GAP_REVIEW_PACKET.md"
TEST = REPO / "tests/test_phase32_step3_production_like_rollout_readiness_evidence_gap_review_packet.py"
...
def test_phase32_step3_files_exist():
...
def test_phase32_step3_launcher_supports_optimized_actions():
    script = read(SCRIPT)
    assert 'ValidateSet("status", "apply", "smoke", "packet", "all")' in script
    assert "APPLY PASS: Phase 32 Step 3" in script
    assert "SMOKE TEST PASS: Phase 32 Step 3" in script


def test_phase32_step3_safety_markers_present():
...
def test_phase32_step3_does_not_enable_runtime_or_phase33():
...
def test_phase32_step3_doc_declares_no_phase33_boundary():
    doc = read(DOC).lower()
...
def test_phase32_step3_ui_is_reference_only():
    ui = read(UI_PAGE)
...
def test_phase32_step3_prior_step_is_recorded():
    combined = read(SCRIPT) + read(DOC) + read(UI_PAGE)
    assert "Phase 32 Step 2 - Phase 20 Network Transport Implementation Production-Like Rollout Readiness Evidence Index Packet" in combined


def test_phase32_step3_uses_exact_four_step_files():
    script = read(SCRIPT)
    for rel in [
