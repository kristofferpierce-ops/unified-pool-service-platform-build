...
def test_phase20_network_transport_consolidated_closeout_packet_script_is_get_only_and_no_write() -> None:
    script = read("scripts/phase21_generate_phase20_network_transport_consolidated_closeout_packet.ps1")
    assert "phase21_phase20_network_transport_consolidated_closeout_packet_" in script
    assert "phase20_network_transport_consolidated_closeout_packet_only = $true" in script
    assert "consolidated_closeout_packet_only = $true" in script
...
def test_phase20_network_transport_consolidated_closeout_packet_streamlit_page_is_report_only() -> None:
    page = read("ui/pages/94_Phase20_Network_Transport_Consolidated_Closeout_Packet.py")
    assert "Phase 21 Step 2 - Phase 20 Network Transport Consolidated Closeout Packet" in page
    assert "Phase 20 network transport consolidated-closeout-only" in page
    ...
    assert "phase21_phase20_network_transport_consolidated_closeout_packet_*" in page
    assert "phase21_phase20_network_transport_consolidated_closeout_packet.json" in page
    assert "requests.get" not in page
    assert "requests.post" not in page
    assert "st.download_button" in page


def test_phase20_network_transport_consolidated_closeout_packet_docs_capture_boundaries() -> None:
    doc = read("docs/PHASE21_STEP2_PHASE20_NETWORK_TRANSPORT_CONSOLIDATED_CLOSEOUT_PACKET.md")
    assert "no-write consolidated closeout packet" in doc
    assert "Phase 20 network transport consolidated-closeout-only" in doc
    ...
    assert "Stage only the four Phase 21 Step 2 files" in doc
