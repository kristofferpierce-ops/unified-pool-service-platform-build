...
def test_phase20_network_transport_planning_scope_packet_script_is_get_only_and_no_write() -> None:
    script = read("scripts/phase22_generate_phase20_network_transport_planning_scope_packet.ps1")
    assert "phase22_phase20_network_transport_planning_scope_packet_" in script
    assert "phase20_network_transport_planning_scope_packet_only = $true" in script
    assert "planning_scope_only = $true" in script
...
def test_phase20_network_transport_planning_scope_packet_streamlit_page_is_report_only() -> None:
    page = read("ui/pages/109_Phase20_Network_Transport_Planning_Scope_Packet.py")
    assert "Phase 22 Step 3 - Phase 20 Network Transport Planning Scope Packet" in page
    assert "Phase 20 network transport planning-scope-only" in page
    ...
    assert "phase22_phase20_network_transport_planning_scope_packet_*" in page
    assert "phase22_phase20_network_transport_planning_scope_packet.json" in page
    assert "requests.get" not in page
    assert "requests.post" not in page
    assert "st.download_button" in page


def test_phase20_network_transport_planning_scope_packet_docs_capture_boundaries() -> None:
    doc = read("docs/PHASE22_STEP3_PHASE20_NETWORK_TRANSPORT_PLANNING_SCOPE_PACKET.md")
    assert "no-write planning scope packet" in doc
    assert "Phase 20 network transport planning-scope-only" in doc
    ...
    assert "Stage only the four Phase 22 Step 3 files" in doc
