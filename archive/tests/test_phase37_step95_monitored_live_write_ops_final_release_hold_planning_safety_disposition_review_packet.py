...
SCRIPT = ROOT / 'scripts/phase37_step95_monitored_live_write_ops_final_release_hold_planning_safety_disposition_review_packet.ps1'
UI = ROOT / 'ui/pages/1911_Phase37_Step95_Monitored_Live_Write_Ops_Final_Release_Hold_Safety_Review.py'
DOC = ROOT / 'docs/PHASE37_STEP95_MONITORED_LIVE_WRITE_OPS_FINAL_RELEASE_HOLD_PLANNING_SAFETY_DISPOSITION_REVIEW_PACKET.md'
...
def test_phase37_step_95_files_exist() -> None:
    assert SCRIPT.exists()
    assert UI.exists()
    assert DOC.exists()


def test_phase37_step_95_launcher_has_valid_param_block() -> None:
    text = read(SCRIPT)
    assert text.startswith("param(")
    assert not text.startswith("\ufeff")


def test_phase37_step_95_launcher_markers() -> None:
    text = read(SCRIPT)
...
def test_phase37_step_95_ui_page_smoke_markers() -> None:
    text = read(UI)
...
def test_phase37_step_95_doc_mentions_planning_only() -> None:
    text = read(DOC).lower()
    assert "planning-only" in text
    assert "does not activate live writes" in text


def test_phase37_step_95_required_safe_markers_present() -> None:
...
def test_phase37_step_95_forbidden_runtime_markers_not_enabled() -> None:
    compact = (read(SCRIPT) + "\n" + read(UI) + "\n" + read(DOC)).replace(" ", "").lower()
...
def test_phase37_step_95_no_phase38_files_created_by_payload() -> None:
