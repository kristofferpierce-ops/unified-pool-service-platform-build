...
SCRIPT = REPO / "scripts/phase37_step24_monitored_live_write_ops_preflight_safety_planning_packet.ps1"
UI = REPO / "ui/pages/1840_Phase37_Step24_Monitored_Live_Write_Ops_Preflight_Safety_Planning.py"
DOC = REPO / "docs/PHASE37_STEP24_MONITORED_LIVE_WRITE_OPS_PREFLIGHT_SAFETY_PLANNING_PACKET.md"
TEST = REPO / "tests/test_phase37_step24_monitored_live_write_ops_preflight_safety_planning_packet.py"
...
def test_phase37_step_24_files_exist() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert path.exists(), f"Missing expected file: {path}"


def test_phase37_step_24_launcher_has_valid_param_block() -> None:
    text = read(SCRIPT)
...
def test_phase37_step_24_safety_markers_present() -> None:
    text = read(SCRIPT) + "\n" + read(UI) + "\n" + read(DOC)
    required = [
...
def test_phase37_step_24_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase37_step_24_launcher_smoke_passes() -> None:
