...
SCRIPT = ROOT / "scripts/p38_s120_phase38_final_no_write_closeout_hold.ps1"
UI = ROOT / "ui/pages/2056_Phase38_Step120_Phase38_Final_No_Write_Closeout_Hold.py"
DOC = ROOT / "docs/P38_S120_PHASE38_FINAL_NO_WRITE_CLOSEOUT_HOLD.md"
...
def test_phase38_step_120_files_exist() -> None:
    assert SCRIPT.exists()
    assert UI.exists()
    assert DOC.exists()


def test_phase38_step_120_launcher_has_valid_param_block() -> None:
    text = read(SCRIPT)
    assert text.startswith("param(")


def test_phase38_step_120_launcher_runs_all() -> None:
...
def test_phase38_step_120_ui_compiles() -> None:
    py_compile.compile(str(UI), doraise=True)


def test_phase38_step_120_doc_has_required_safe_markers() -> None:
...
def test_phase38_step_120_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase38_step_120_no_phase39_files_created_by_payload() -> None:
...
def test_phase38_step_120_literal_pass_markers_present() -> None:
    text = read(SCRIPT)
