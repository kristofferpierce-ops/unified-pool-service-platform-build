...
SCRIPT = REPO / "scripts/phase30_step19_active_live_program_development_guardrail_verification_closeout_index_packet.ps1"
UI_PAGE = REPO / "ui/pages/995_Phase30_Step19_Implementation_Active_Live_Program_Development_Guardrail_Verification_Closeout_Index_Packet.py"
DOC = REPO / "docs/PHASE30_STEP19_ACTIVE_LIVE_PROGRAM_DEVELOPMENT_GUARDRAIL_VERIFICATION_CLOSEOUT_INDEX_PACKET.md"
TEST = REPO / "tests/test_phase30_step19_active_live_program_development_guardrail_verification_closeout_index_packet.py"
...
def test_phase30_step19_files_exist():
...
def test_phase30_step19_launcher_supports_optimized_actions():
    script = read(SCRIPT)
    assert 'ValidateSet("status", "apply", "smoke", "packet", "all")' in script
    assert "APPLY PASS: Phase 30 Step 19" in script
    assert "SMOKE TEST PASS: Phase 30 Step 19" in script


def test_phase30_step19_safety_markers_present():
...
def test_phase30_step19_does_not_enable_runtime_or_phase31():
...
def test_phase30_step19_doc_declares_no_phase31_boundary():
    doc = read(DOC).lower()
...
def test_phase30_step19_ui_is_reference_only():
    ui = read(UI_PAGE)
...
def test_phase30_step19_prior_step_is_recorded():
    combined = read(SCRIPT) + read(DOC) + read(UI_PAGE)
    assert "Phase 30 Step 18 - Phase 20 Network Transport Implementation Active Live Program Development Guardrail Verification Operator Hold Point Packet" in combined


def test_phase30_step19_uses_exact_four_step_files():
    script = read(SCRIPT)
    for rel in [
