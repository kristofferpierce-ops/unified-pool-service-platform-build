...
SCRIPT = REPO_ROOT / "scripts/phase25_step113_post_closeout_final_release_result_review_planning_evidence_gap_review_packet.ps1"
PAGE = REPO_ROOT / "ui/pages/489_Phase25_Step113_Implementation_PostCloseout_Final_Release_Result_Review_Planning_Evidence_Gap_Review_Packet.py"
DOC = REPO_ROOT / "docs/PHASE25_STEP113_POST_CLOSEOUT_FINAL_RELEASE_RESULT_REVIEW_PLANNING_EVIDENCE_GAP_REVIEW_PACKET.md"
TEST = REPO_ROOT / "tests/test_phase25_step113_post_closeout_final_release_result_review_planning_evidence_gap_review_packet.py"
...
def test_phase25_step113_files_are_present():
    for path in (SCRIPT, PAGE, DOC, TEST):
        assert path.exists(), f"missing {path}"


def test_phase25_step113_launcher_markers_are_present():
    text = read(SCRIPT).lower()
    assert "phase 25 step 113" in text
    assert "phase25-step113-post-closeout-final-release-result-review-planning-evidence-gap-review" in text
...
def test_phase25_step113_planning_only_safety_markers():
...
def test_phase25_step113_does_not_start_runtime_or_next_phase():
    text = operational_text()
    forbidden = [
        "phase26_start=true",
...
def test_phase25_step113_batch_risk_review_is_chat_only_not_powershell_prompt():
    text = operational_text()
    assert "batch_risk_review_location" in text
...
def test_phase25_step113_launcher_supports_optimized_actions_and_known_fixes():
    script = read(SCRIPT)
    assert "ValidateSet(\"status\", \"apply\", \"smoke\", \"packet\", \"all\")" in script
    assert "APPLY PASS: Phase 25 Step 113" in script
    assert "SMOKE TEST PASS: Phase 25 Step 113" in script
    assert "CHECK: packet_json=" in script
    assert "$File:" not in script
    assert "Join-Path $RepoRoot \"scripts/phase25_step113_post_closeout_final_release_result_review_planning_evidence_gap_review_packet.ps1\"," not in script
    assert "System.Object[]" not in script


def test_phase25_step113_docs_are_operator_readable():
    text = read(DOC)
    assert "Phase 25 Step 113 - Phase 20 Network Transport Implementation Post-Closeout Final Release Result Review Planning Evidence Gap Review Packet" in text
...
def test_phase25_step113_ui_is_streamlit_reference_only_page():
...
def test_phase25_step113_test_file_self_identifies_step():
    text = read(TEST).lower()
    assert "step113" in text or "step 113" in text
