...
SCRIPT = REPO / "scripts/phase37_step103_monitored_live_write_ops_final_release_hold_verification_planning_evidence_gap_review_packet.ps1"
UI = REPO / "ui/pages/1919_Phase37_Step103_Monitored_Live_Write_Ops_Final_Release_Hold_Verification_Evidence_Gap_Review.py"
DOC = REPO / "docs/PHASE37_STEP103_MONITORED_LIVE_WRITE_OPS_FINAL_RELEASE_HOLD_VERIFICATION_PLANNING_EVIDENCE_GAP_REVIEW_PACKET.md"
TEST = REPO / "tests/test_phase37_step103_monitored_live_write_ops_final_release_hold_verification_planning_evidence_gap_review_packet.py"
...
def test_phase37_step_103_files_exist() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert path.exists(), f"Missing expected file: {path}"


def test_phase37_step_103_launcher_has_valid_param_block() -> None:
    text = read(SCRIPT)
...
def test_phase37_step_103_payloads_are_bom_free() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert not path.read_bytes().startswith(b"\xef\xbb\xbf"), f"BOM detected in {path}"


def test_phase37_step_103_safety_markers_present() -> None:
    text = read(SCRIPT) + "\n" + read(UI) + "\n" + read(DOC)
    required = [
        "planning_only=true",
...
def test_phase37_step_103_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase37_step_103_launcher_smoke_passes() -> None:
