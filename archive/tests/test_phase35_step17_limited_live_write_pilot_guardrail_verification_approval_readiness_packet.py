...
SCRIPT = REPO / "scripts/phase35_step17_limited_live_write_pilot_guardrail_verification_approval_readiness_packet.ps1"
UI_PAGE = REPO / "ui/pages/1593_Phase35_Step17_Live_Write_Pilot_Guardrail_Approval_Readiness.py"
DOC = REPO / "docs/PHASE35_STEP17_LIMITED_LIVE_WRITE_PILOT_GUARDRAIL_VERIFICATION_APPROVAL_READINESS_PACKET.md"
TEST = REPO / "tests/test_phase35_step17_limited_live_write_pilot_guardrail_verification_approval_readiness_packet.py"
...
def test_phase35_step17_files_exist():
...
def test_phase35_step17_launcher_supports_optimized_actions():
    script = read(SCRIPT)
    assert 'ValidateSet("status", "apply", "smoke", "packet", "all")' in script
    assert "APPLY PASS: Phase 35 Step 17" in script
    assert "SMOKE TEST PASS: Phase 35 Step 17" in script


def test_phase35_step17_safety_markers_present():
...
def test_phase35_step17_does_not_enable_runtime_or_phase36():
...
def test_phase35_step17_doc_declares_no_phase36_boundary():
    doc = read(DOC).lower()
...
def test_phase35_step17_ui_is_reference_only():
    ui = read(UI_PAGE)
...
def test_phase35_step17_prior_step_is_recorded():
    combined = read(SCRIPT) + read(DOC) + read(UI_PAGE)
    assert "Phase 35 Step 16 - Phase 20 Network Transport Implementation Trusted Production Limited Live Write Pilot Guardrail Verification Approval Boundary Packet" in combined


def test_phase35_step17_uses_exact_four_step_files():
    script = read(SCRIPT)
    for rel in [
