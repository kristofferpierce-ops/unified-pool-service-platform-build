...
SCRIPT = REPO / "scripts/phase37_step104_monitored_live_write_ops_final_release_hold_verification_planning_safety_disposition_planning_packet.ps1"
UI = REPO / "ui/pages/1920_Phase37_Step104_Monitored_Live_Write_Ops_Final_Release_Hold_Verification_Safety_Planning.py"
DOC = REPO / "docs/PHASE37_STEP104_MONITORED_LIVE_WRITE_OPS_FINAL_RELEASE_HOLD_VERIFICATION_PLANNING_SAFETY_DISPOSITION_PLANNING_PACKET.md"
TEST = REPO / "tests/test_phase37_step104_monitored_live_write_ops_final_release_hold_verification_planning_safety_disposition_planning_packet.py"
...
def test_phase37_step_104_files_exist() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert path.exists(), f"Missing expected file: {path}"


def test_phase37_step_104_launcher_has_valid_param_block() -> None:
    text = read(SCRIPT)
...
def test_phase37_step_104_payloads_are_bom_free() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert not path.read_bytes().startswith(b"\xef\xbb\xbf"), f"BOM detected in {path}"


def test_phase37_step_104_safety_markers_present() -> None:
    text = read(SCRIPT) + "\n" + read(UI) + "\n" + read(DOC)
    required = [
        "planning_only=true",
...
def test_phase37_step_104_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase37_step_104_launcher_smoke_passes() -> None:
