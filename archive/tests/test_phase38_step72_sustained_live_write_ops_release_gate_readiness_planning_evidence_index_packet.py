...
SCRIPT = REPO / "scripts/phase38_step72_sustained_live_write_ops_release_gate_readiness_planning_evidence_index_packet.ps1"
UI = REPO / "ui/pages/2008_Phase38_Step72_Sustained_Live_Write_Ops_Release_Gate_Readiness_Evidence_Index.py"
DOC = REPO / "docs/PHASE38_STEP72_SUSTAINED_LIVE_WRITE_OPS_RELEASE_GATE_READINESS_PLANNING_EVIDENCE_INDEX_PACKET.md"
TEST = REPO / "tests/test_phase38_step72_sustained_live_write_ops_release_gate_readiness_planning_evidence_index_packet.py"
...
def test_phase38_step_72_files_exist() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert path.exists(), f"Missing expected file: {path}"


def test_phase38_step_72_launcher_has_valid_param_block() -> None:
    text = read(SCRIPT)
...
def test_phase38_step_72_payloads_are_bom_free() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert not path.read_bytes().startswith(b"\xef\xbb\xbf"), f"BOM detected in {path}"


def test_phase38_step_72_safety_markers_present() -> None:
    text = read(SCRIPT) + "\n" + read(UI) + "\n" + read(DOC)
    required = [
        "planning_only=true",
...
def test_phase38_step_72_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase38_step_72_launcher_smoke_passes() -> None:
