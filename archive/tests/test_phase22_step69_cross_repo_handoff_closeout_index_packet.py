...
SCRIPT_REL = "scripts/phase22_step69_cross_repo_handoff_closeout_index_packet.ps1"
PAGE_REL = "ui/pages/175_Phase20_Network_Transport_Planning_Final_Cross_Repo_Handoff_Closeout_Index_Packet.py"
DOC_REL = "docs/PHASE22_STEP69_FINAL_CROSS_REPO_HANDOFF_CLOSEOUT_INDEX_PACKET.md"
TEST_REL = "tests/test_phase22_step69_cross_repo_handoff_closeout_index_packet.py"
...
def test_step69_files_exist():
    for path in PUBLIC_FILES:
        assert path.exists(), path


def test_step69_script_mentions_phase_and_step():
    text = read(SCRIPT)
    assert "Phase 22 Step 69" in text
    assert "$StepNumber = 69" in text
    assert "Final Cross-Repo Handoff Closeout Index Packet" in text


def test_step69_page_number_is_correct():
    assert PAGE.as_posix().endswith(
        "ui/pages/175_Phase20_Network_Transport_Planning_Final_Cross_Repo_Handoff_Closeout_Index_Packet.py"
    )


def test_step69_doc_title_is_correct():
    text = read(DOC)
    assert "# Phase 22 Step 69 - Phase 20 Network Transport Planning Final Cross-Repo Handoff Closeout Index Packet" in text


def test_step69_prior_step_is_step68():
    text = read(SCRIPT) + "\n" + read(DOC)
    ...
    assert "CHECK: prior_step=Phase 22 Step 68" in text


def test_step69_launcher_supports_action_all():
    text = read(SCRIPT)
...
def test_step69_uses_literal_and_safe_paths():
    text = read(SCRIPT)
...
def test_step69_is_planning_only():
...
def test_step69_keeps_phase23_closed():
    text = combined_public_text()
    required = [
        "phase23_start",
...
def test_step69_disables_cross_repo_mutation():
    text = combined_public_text()
    required = [
        "cross_repo_write",
        "cross_repo_mutation",
...
def test_step69_disables_evidence_collection_decisions_and_approvals():
    text = combined_public_text()
    required = [
        "final_cross_repo_handoff_closeout_index_mode",
        "handoff_closeout_index_record_creation",
        "handoff_evidence_collection_execution",
        "handoff_closeout_index_write",
...
def test_step69_keeps_lacrm_dry_run():
    text = combined_public_text()
...
def test_step69_contains_parent_workspace_commands():
    text = read(DOC)
    ...
    assert "INSTALL_PHASE22_STEP69_SINGLE_FILE" in text


def test_step69_menu_has_expected_options():
    text = read(SCRIPT)
    assert "Phase 22 Step 69 menu" in text
    assert "Show status / verify paths" in text
    assert "Smoke test Phase 22 Step 69" in text
    assert "Show server start placeholder only" in text
    assert "Generate Phase 22 Step 69 final cross-repo handoff closeout index packet" in text


def test_step69_expected_smoke_success_text_present():
    text = read(SCRIPT)
    expected = (
        "SMOKE TEST PASS: Phase 22 Step 69 Phase 20 Network Transport Planning "
        "Final Cross-Repo Handoff Closeout Index Packet is present and planning-only."
    )
    assert expected in text


def test_step69_expected_packet_output_in_doc_and_script():
    combined = read(DOC) + "\n" + read(SCRIPT)
    required = [
        "PASS: cross_repo_write=false",
        "PASS: cross_repo_mutation=false",
        "PASS: external_repo_push=false",
        "PASS: final_cross_repo_handoff_closeout_index_mode=reference_only",
        "PASS: handoff_closeout_index_write=false",
        ...
        "CHECK: prior_step=Phase 22 Step 68",
        "CHECK: phase23_start=not_started",
        "CHECK: final_cross_repo_handoff_closeout_index_mode=reference_only",
...
def test_step69_does_not_create_phase23_or_implementation():
...
def test_step69_does_not_enable_network_or_bridge_runtime():
    text = combined_runtime_text().lower()
    forbidden = [
...
def test_step69_does_not_write_to_external_repos():
    text = combined_runtime_text().lower()
    forbidden = [
...
def test_step69_does_not_collect_decide_approve_or_remediate_evidence():
    text = combined_runtime_text().lower()
    forbidden = [
        "handoff_evidence_collection_execution: true",
        "handoff_evidence_collection_execution = true",
        "handoff_closeout_index_write: true",
        "handoff_closeout_index_write = true",
        ...
        "evidence_gap_resolution_execution = true",
        "handoff_closeout_index_record_creation: true",
        "handoff_closeout_index_record_creation = true",
    ]
    for marker in forbidden:
        assert marker not in text
...
def test_step69_does_not_create_approvals_or_closure_records():
    text = combined_runtime_text().lower()
    forbidden = [
...
def test_step69_packet_generates_json_under_backups():
    text = read(SCRIPT)
    assert "backups" in text
    assert "phase22_step69_cross_repo_handoff_closeout_index_packet" in text
    assert "ConvertTo-Json" in text
    assert "Set-Content -LiteralPath" in text


def test_step69_expected_branch_is_short():
    text = read(SCRIPT)
    branch = "phase22-step69-final-handoff-closeout-index"
    assert branch in text
    assert len(branch) < 80


def test_step69_source_zip_filename_not_required_by_launcher():
    text = read(SCRIPT)
    assert "phase22_step69_source(1).zip" not in text
    assert "phase22_step69_source.zip" not in text
    assert "single-file installer" in text.lower()


def test_step69_installer_paths_are_short():
    for rel in [SCRIPT_REL, PAGE_REL, DOC_REL, TEST_REL]:
        assert len(rel) < 150


def test_step69_no_server_start_claim():
