...
STEP_FILES = [
    REPO_ROOT / "scripts/phase22_generate_phase20_network_transport_planning_closure_deferral_resolution_review_alignment_packet.ps1",
    REPO_ROOT / "ui/pages/151_Phase20_Network_Transport_Planning_Closure_Deferral_Resolution_Review_Alignment_Packet.py",
    REPO_ROOT / "docs/PHASE22_STEP45_PHASE20_NETWORK_TRANSPORT_PLANNING_CLOSURE_DEFERRAL_RESOLUTION_REVIEW_ALIGNMENT_PACKET.md",
    REPO_ROOT / "tests/test_phase22_phase20_network_transport_planning_closure_deferral_resolution_review_alignment_packet.py",
]
...
def test_phase22_step45_files_exist():
    for path in STEP_FILES:
        assert path.exists(), path


def test_phase22_step45_doc_identifies_phase_and_step():
    text = read(DOC_FILE)
    assert "Phase 22 Step 45" in text
    assert "Closure Deferral Resolution Review Alignment Packet" in text


def test_phase22_step45_ui_identifies_planning_only_status():
    text = read(UI_PAGE)
    assert "Planning-only alignment packet" in text
    assert "closure-deferral resolution" in text


def test_phase22_step45_launcher_supports_action_all():
    text = read(STEP_SCRIPT)
    assert '[ValidateSet("menu", "status", "apply", "smoke", "packet", "all")]' in text
    assert '"all"' in text


def test_phase22_step45_launcher_uses_literal_path_handling():
    text = read(STEP_SCRIPT)
...
def test_phase22_step45_launcher_is_idempotent_for_same_source_and_target():
    text = read(STEP_SCRIPT)
    assert "source and target are the same file" in text
    assert "Test-SameFile" in text


def test_phase22_step45_planning_only_safety_flags_are_present():
...
def test_phase22_step45_network_transport_remains_disabled():
    text = read(STEP_SCRIPT) + read(DOC_FILE)
...
def test_phase22_step45_closure_and_approval_records_are_not_created():
    text = read(STEP_SCRIPT) + read(DOC_FILE)
...
def test_phase22_step45_lacrm_write_lane_is_unarmed():
    text = read(STEP_SCRIPT) + read(DOC_FILE)
...
def test_phase22_step45_packet_generation_is_backup_only():
    text = read(STEP_SCRIPT)
    assert "backups\\phase22_phase20_network_transport_planning_closure_deferral_resolution_review_alignment_packet_" in text
    assert "packet_json=" in text


def test_phase22_step45_source_bucket_alignment_is_documented():
    text = read(STEP_SCRIPT) + read(DOC_FILE)
    assert "raw_normalized_matched_approved_applied" in text
    assert "source buckets" in text or "source_bucket" in text


def test_phase22_step45_bridge_absorption_guardrail_is_documented():
    text = read(STEP_SCRIPT) + read(DOC_FILE)
    assert "connector_package_not_separate_product" in text or "connector-package" in text


def test_phase22_step45_resolution_review_is_planned_only():
    text = read(STEP_SCRIPT) + read(DOC_FILE)
    assert "closure_deferral_resolution_review" in text
    assert "planned_only" in text
    assert "closure_resolution_review_creation" in text


def test_phase22_step45_does_not_create_backlog_or_applied_layer_mutation():
    text = read(STEP_SCRIPT) + read(DOC_FILE)
    assert "closure_deferral_backlog_mutation" in text
    assert "applied_layer_mutation" in text


def test_phase22_step45_test_file_tracks_all_public_files():
    text = read(TEST_FILE)
    for expected in [
...
def test_phase22_step45_public_files_do_not_contain_forbidden_runtime_tokens():
    forbidden = [
        ...
        "closure_resolution_review" + "_creation=true",
...
def test_phase22_step45_does_not_reference_runtime_server_start():
    text = read(STEP_SCRIPT) + read(DOC_FILE)
