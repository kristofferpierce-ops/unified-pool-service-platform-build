...
SCRIPT = REPO / "scripts/phase35_step90_limited_live_write_pilot_release_gate_verification_planning_final_boundary_confirmation_packet.ps1"
UI_PAGE = REPO / "ui/pages/1666_Phase35_Step90_Live_Write_Pilot_Release_Gate_Verification_Final_Boundary.py"
DOC = REPO / "docs/PHASE35_STEP90_LIMITED_LIVE_WRITE_PILOT_RELEASE_GATE_VERIFICATION_PLANNING_FINAL_BOUNDARY_CONFIRMATION_PACKET.md"
TEST = REPO / "tests/test_phase35_step90_limited_live_write_pilot_release_gate_verification_planning_final_boundary_confirmation_packet.py"
...
def test_phase35_step90_files_exist():
...
def test_phase35_step90_launcher_supports_optimized_actions():
    script = read(SCRIPT)
    assert 'ValidateSet("status", "apply", "smoke", "packet", "all")' in script
    assert "APPLY PASS: Phase 35 Step 90" in script
    assert "SMOKE TEST PASS: Phase 35 Step 90" in script


def test_phase35_step90_safety_markers_present():
...
def test_phase35_step90_does_not_enable_runtime_or_phase36():
...
def test_phase35_step90_doc_declares_no_phase36_boundary():
    doc = read(DOC).lower()
...
def test_phase35_step90_ui_is_reference_only():
    ui = read(UI_PAGE)
...
def test_phase35_step90_prior_step_is_recorded():
    combined = read(SCRIPT) + read(DOC) + read(UI_PAGE)
    assert "Phase 35 Step 89 - Phase 20 Network Transport Implementation Trusted Production Limited Live Write Pilot Release Gate Verification Planning Closeout Index Packet" in combined


def test_phase35_step90_uses_exact_four_step_files():
    script = read(SCRIPT)
    for rel in [
