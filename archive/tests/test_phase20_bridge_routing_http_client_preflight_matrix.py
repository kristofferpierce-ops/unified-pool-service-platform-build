...
def test_bridge_routing_http_client_preflight_matrix_script_is_get_only() -> None:
    script = read("scripts/phase20_generate_bridge_routing_http_client_preflight_matrix.ps1")
    assert "phase20_bridge_routing_http_client_preflight_matrix_" in script
    assert "bridge_routing_http_client_preflight_matrix_only = $true" in script
...
def test_bridge_routing_http_client_preflight_matrix_streamlit_page_is_report_only() -> None:
    page = read("ui/pages/56_Bridge_Routing_HTTP_Client_Preflight_Matrix.py")
    assert "Phase 20 Bridge Routing HTTP Client Preflight Matrix" in page
    assert "HTTP-client-preflight-matrix-only" in page
    ...
    assert "phase20_bridge_routing_http_client_preflight_matrix_*" in page
    assert "phase20_bridge_routing_http_client_preflight_matrix.json" in page
    assert "requests.get" not in page
    assert "requests.post" not in page
    assert "st.download_button" in page


def test_bridge_routing_http_client_preflight_matrix_docs_capture_boundaries() -> None:
    doc = read("docs/PHASE20_STEP4_BRIDGE_ROUTING_HTTP_CLIENT_PREFLIGHT_MATRIX.md")
    assert "no-network preflight matrix" in doc
    assert "HTTP-client-preflight-matrix-only" in doc
