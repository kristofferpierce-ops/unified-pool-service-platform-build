...
SCRIPT = REPO / "scripts/phase36_step108_controlled_live_write_expansion_final_release_hold_verification_operator_hold_packet.ps1"
UI = REPO / "ui/pages/1804_Phase36_Step108_Live_Write_Expansion_Final_Release_Hold_Verification_Operator_Hold.py"
DOC = REPO / "docs/PHASE36_STEP108_CONTROLLED_LIVE_WRITE_EXPANSION_FINAL_RELEASE_HOLD_VERIFICATION_OPERATOR_HOLD_PACKET.md"
TEST = REPO / "tests/test_phase36_step108_controlled_live_write_expansion_final_release_hold_verification_operator_hold_packet.py"
...
def test_phase36_step_108_files_exist() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert path.exists(), f"missing expected file: {path}"


def test_phase36_step_108_launcher_has_required_safe_markers() -> None:
    text = read(SCRIPT)
    for marker in [
        "planning_only = $true",
...
def test_phase36_step_108_operational_files_do_not_enable_forbidden_markers() -> None:
    forbidden_keys = [
        "phase37_start",
...
def test_phase36_step_108_ui_and_doc_reference_dry_run_no_apply() -> None:
    text = read(UI) + "\n" + read(DOC)
...
def test_phase36_step_108_launcher_apply_passes() -> None:
...
def test_phase36_step_108_launcher_smoke_passes() -> None:
    result = subprocess.run(
