...
def test_phase20_network_transport_planning_sequence_packet_script_is_get_only_and_no_write() -> None:
    script = read("scripts/phase22_generate_phase20_network_transport_planning_sequence_packet.ps1")
    assert "phase22_phase20_network_transport_planning_sequence_packet_" in script
    assert "phase20_network_transport_planning_sequence_packet_only = $true" in script
    assert "planning_sequence_only = $true" in script
...
def test_phase20_network_transport_planning_sequence_packet_streamlit_page_is_report_only() -> None:
    page = read("ui/pages/112_Phase20_Network_Transport_Planning_Sequence_Packet.py")
    assert "Phase 22 Step 6 - Phase 20 Network Transport Planning Sequence Packet" in page
    assert "Phase 20 network transport planning-sequence-only" in page
    ...
    assert "phase22_phase20_network_transport_planning_sequence_packet_*" in page
    assert "phase22_phase20_network_transport_planning_sequence_packet.json" in page
    assert "requests.get" not in page
    assert "requests.post" not in page
    assert "st.download_button" in page


def test_phase20_network_transport_planning_sequence_packet_docs_capture_boundaries() -> None:
    doc = read("docs/PHASE22_STEP6_PHASE20_NETWORK_TRANSPORT_PLANNING_SEQUENCE_PACKET.md")
    assert "no-write planning sequence packet" in doc
    assert "Phase 20 network transport planning-sequence-only" in doc
    ...
    assert "Stage only the four Phase 22 Step 6 files" in doc
