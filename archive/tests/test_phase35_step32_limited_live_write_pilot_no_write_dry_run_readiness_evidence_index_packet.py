...
SCRIPT = REPO / "scripts/phase35_step32_limited_live_write_pilot_no_write_dry_run_readiness_evidence_index_packet.ps1"
UI_PAGE = REPO / "ui/pages/1608_Phase35_Step32_Live_Write_Pilot_No_Write_Dry_Run_Readiness_Evidence_Index.py"
DOC = REPO / "docs/PHASE35_STEP32_LIMITED_LIVE_WRITE_PILOT_NO_WRITE_DRY_RUN_READINESS_EVIDENCE_INDEX_PACKET.md"
TEST = REPO / "tests/test_phase35_step32_limited_live_write_pilot_no_write_dry_run_readiness_evidence_index_packet.py"
...
def test_phase35_step32_files_exist():
...
def test_phase35_step32_launcher_supports_optimized_actions():
    script = read(SCRIPT)
    assert 'ValidateSet("status", "apply", "smoke", "packet", "all")' in script
    assert "APPLY PASS: Phase 35 Step 32" in script
    assert "SMOKE TEST PASS: Phase 35 Step 32" in script


def test_phase35_step32_safety_markers_present():
...
def test_phase35_step32_does_not_enable_runtime_or_phase36():
...
def test_phase35_step32_doc_declares_no_phase36_boundary():
    doc = read(DOC).lower()
...
def test_phase35_step32_ui_is_reference_only():
    ui = read(UI_PAGE)
...
def test_phase35_step32_prior_step_is_recorded():
    combined = read(SCRIPT) + read(DOC) + read(UI_PAGE)
    assert "Phase 35 Step 31 - Phase 20 Network Transport Implementation Trusted Production Limited Live Write Pilot No-Write Dry Run Readiness Boundary Packet" in combined


def test_phase35_step32_uses_exact_four_step_files():
    script = read(SCRIPT)
    for rel in [
