...
SCRIPT = REPO_ROOT / "scripts/phase25_step105_post_closeout_final_release_hold_verification_planning_safety_disposition_review_packet.ps1"
PAGE = REPO_ROOT / "ui/pages/481_Phase25_Step105_Implementation_PostCloseout_Final_Release_Hold_Verification_Planning_Safety_Disposition_Review_Packet.py"
DOC = REPO_ROOT / "docs/PHASE25_STEP105_POST_CLOSEOUT_FINAL_RELEASE_HOLD_VERIFICATION_PLANNING_SAFETY_DISPOSITION_REVIEW_PACKET.md"
TEST = REPO_ROOT / "tests/test_phase25_step105_post_closeout_final_release_hold_verification_planning_safety_disposition_review_packet.py"
...
def test_phase25_step105_files_are_present():
    for path in (SCRIPT, PAGE, DOC, TEST):
        assert path.exists(), f"missing {path}"


def test_phase25_step105_planning_only_markers_are_present():
...
def test_phase25_step105_does_not_enable_runtime_or_next_phase():
    text = operational_text()
    forbidden = [
        "phase26_start" + "=true",
...
def test_phase25_step105_launcher_supports_optimized_actions():
    script = read(SCRIPT)
    assert "ValidateSet(\"status\", \"apply\", \"smoke\", \"packet\", \"all\")" in script
    assert "APPLY PASS: Phase 25 Step 105" in script
    assert "SMOKE TEST PASS: Phase 25 Step 105" in script
    assert "CHECK: packet_json=" in script


def test_phase25_step105_known_powershell_generation_bugs_are_absent():
    script = read(SCRIPT)
    assert "$File:" not in script
    assert "Join-Path $RepoRoot \"scripts/phase25_step105_post_closeout_final_release_hold_verification_planning_safety_disposition_review_packet.ps1\"," not in script
    assert "System.Object[]" not in script


def test_phase25_step105_ui_page_is_reference_only():
...
def test_phase25_step105_doc_records_no_write_safety():
    doc = read(DOC).lower()
...
def test_phase25_step105_file_names_match_step_number():
    assert "step105" in SCRIPT.name
    assert "Step105" in PAGE.name
    assert "STEP105" in DOC.name
    assert "step105" in TEST.name
