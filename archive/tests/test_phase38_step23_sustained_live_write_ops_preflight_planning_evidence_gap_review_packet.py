...
SCRIPT = ROOT / "scripts/phase38_step23_sustained_live_write_ops_preflight_planning_evidence_gap_review_packet.ps1"
UI = ROOT / "ui/pages/1959_Phase38_Step23_Sustained_Live_Write_Ops_Preflight_Evidence_Gap_Review.py"
DOC = ROOT / "docs/PHASE38_STEP23_SUSTAINED_LIVE_WRITE_OPS_PREFLIGHT_PLANNING_EVIDENCE_GAP_REVIEW_PACKET.md"
TEST = ROOT / "tests/test_phase38_step23_sustained_live_write_ops_preflight_planning_evidence_gap_review_packet.py"
...
def test_phase38_step_23_files_exist() -> None:
...
def test_phase38_step_23_launcher_has_valid_param_block() -> None:
    text = read(SCRIPT)
    assert text.startswith("param(")
    assert not text.startswith("\ufeff")


def test_phase38_step_23_launcher_runs_all() -> None:
...
def test_phase38_step_23_safety_markers_present() -> None:
...
def test_phase38_step_23_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase38_step_23_no_phase39_files_created_by_payload() -> None:
...
def test_phase38_step_23_ui_page_is_short_named_and_safe() -> None:
...
def test_phase38_step_23_doc_names_four_file_packet() -> None:
    text = read(DOC)
    assert "scripts/phase38_step23_sustained_live_write_ops_preflight_planning_evidence_gap_review_packet.ps1" in text
    assert "ui/pages/1959_Phase38_Step23_Sustained_Live_Write_Ops_Preflight_Evidence_Gap_Review.py" in text
    assert "docs/PHASE38_STEP23_SUSTAINED_LIVE_WRITE_OPS_PREFLIGHT_PLANNING_EVIDENCE_GAP_REVIEW_PACKET.md" in text
    assert "tests/test_phase38_step23_sustained_live_write_ops_preflight_planning_evidence_gap_review_packet.py" in text
