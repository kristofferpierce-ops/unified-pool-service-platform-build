...
def test_phase20_network_transport_archive_readiness_packet_script_is_get_only_and_no_write() -> None:
    script = read("scripts/phase21_generate_phase20_network_transport_archive_readiness_packet.ps1")
    assert "phase21_phase20_network_transport_archive_readiness_packet_" in script
    assert "phase20_network_transport_archive_readiness_packet_only = $true" in script
    assert "archive_readiness_only = $true" in script
...
def test_phase20_network_transport_archive_readiness_packet_streamlit_page_is_report_only() -> None:
    page = read("ui/pages/101_Phase20_Network_Transport_Archive_Readiness_Packet.py")
    assert "Phase 21 Step 9 - Phase 20 Network Transport Archive Readiness Packet" in page
    assert "Phase 20 network transport archive-readiness-only" in page
    ...
    assert "phase21_phase20_network_transport_archive_readiness_packet_*" in page
    assert "phase21_phase20_network_transport_archive_readiness_packet.json" in page
    assert "requests.get" not in page
    assert "requests.post" not in page
    assert "st.download_button" in page


def test_phase20_network_transport_archive_readiness_packet_docs_capture_boundaries() -> None:
    doc = read("docs/PHASE21_STEP9_PHASE20_NETWORK_TRANSPORT_ARCHIVE_READINESS_PACKET.md")
    assert "no-write archive readiness packet" in doc
    assert "Phase 20 network transport archive-readiness-only" in doc
    ...
    assert "Stage only the four Phase 21 Step 9 files" in doc
