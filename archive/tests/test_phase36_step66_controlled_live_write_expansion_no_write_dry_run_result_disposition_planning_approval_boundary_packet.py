...
SCRIPT = REPO / "scripts/phase36_step66_controlled_live_write_expansion_no_write_dry_run_result_disposition_planning_approval_boundary_packet.ps1"
UI = REPO / "ui/pages/1762_Phase36_Step66_Live_Write_Expansion_Result_Disposition_Approval_Boundary.py"
DOC = REPO / "docs/PHASE36_STEP66_CONTROLLED_LIVE_WRITE_EXPANSION_NO_WRITE_DRY_RUN_RESULT_DISPOSITION_PLANNING_APPROVAL_BOUNDARY_PACKET.md"
TEST = REPO / "tests/test_phase36_step66_controlled_live_write_expansion_no_write_dry_run_result_disposition_planning_approval_boundary_packet.py"
...
def test_phase36_step_66_files_exist() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert path.exists(), f"missing expected file: {path}"


def test_phase36_step_66_launcher_has_required_safe_markers() -> None:
    text = read(SCRIPT)
    for marker in [
        "planning_only = $true",
...
def test_phase36_step_66_operational_files_do_not_enable_forbidden_markers() -> None:
    forbidden_keys = [
        "phase37_start",
...
def test_phase36_step_66_ui_and_doc_reference_dry_run_no_apply() -> None:
    text = read(UI) + "\n" + read(DOC)
...
def test_phase36_step_66_launcher_apply_passes() -> None:
...
def test_phase36_step_66_launcher_smoke_passes() -> None:
    result = subprocess.run(
