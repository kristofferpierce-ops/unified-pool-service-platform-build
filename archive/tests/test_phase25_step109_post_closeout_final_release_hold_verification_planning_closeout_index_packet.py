...
SCRIPT = REPO_ROOT / "scripts/phase25_step109_post_closeout_final_release_hold_verification_planning_closeout_index_packet.ps1"
PAGE = REPO_ROOT / "ui/pages/485_Phase25_Step109_Implementation_PostCloseout_Final_Release_Hold_Verification_Planning_Closeout_Index_Packet.py"
DOC = REPO_ROOT / "docs/PHASE25_STEP109_POST_CLOSEOUT_FINAL_RELEASE_HOLD_VERIFICATION_PLANNING_CLOSEOUT_INDEX_PACKET.md"
TEST = REPO_ROOT / "tests/test_phase25_step109_post_closeout_final_release_hold_verification_planning_closeout_index_packet.py"
...
def test_phase25_step109_files_are_present():
    for path in (SCRIPT, PAGE, DOC, TEST):
        assert path.exists(), f"missing {path}"


def test_phase25_step109_planning_only_markers_are_present():
...
def test_phase25_step109_does_not_enable_runtime_or_next_phase():
    text = operational_text()
    forbidden = [
        "phase26_start" + "=true",
...
def test_phase25_step109_launcher_supports_optimized_actions():
    script = read(SCRIPT)
    assert "ValidateSet(\"status\", \"apply\", \"smoke\", \"packet\", \"all\")" in script
    assert "APPLY PASS: Phase 25 Step 109" in script
    assert "SMOKE TEST PASS: Phase 25 Step 109" in script
    assert "CHECK: packet_json=" in script


def test_phase25_step109_known_powershell_generation_bugs_are_absent():
    script = read(SCRIPT)
    assert "$File:" not in script
    assert "Join-Path $RepoRoot \"scripts/phase25_step109_post_closeout_final_release_hold_verification_planning_closeout_index_packet.ps1\"," not in script
    assert "System.Object[]" not in script


def test_phase25_step109_ui_page_is_reference_only():
...
def test_phase25_step109_doc_records_no_write_safety():
    doc = read(DOC).lower()
...
def test_phase25_step109_file_names_match_step_number():
    assert "step109" in SCRIPT.name
    assert "Step109" in PAGE.name
    assert "STEP109" in DOC.name
    assert "step109" in TEST.name
