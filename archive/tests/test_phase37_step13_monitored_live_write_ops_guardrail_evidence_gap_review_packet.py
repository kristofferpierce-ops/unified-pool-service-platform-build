...
SCRIPT = REPO / "scripts/phase37_step13_monitored_live_write_ops_guardrail_evidence_gap_review_packet.ps1"
UI = REPO / "ui/pages/1829_Phase37_Step13_Monitored_Live_Write_Ops_Guardrail_Evidence_Gap_Review.py"
DOC = REPO / "docs/PHASE37_STEP13_MONITORED_LIVE_WRITE_OPS_GUARDRAIL_EVIDENCE_GAP_REVIEW_PACKET.md"
TEST = REPO / "tests/test_phase37_step13_monitored_live_write_ops_guardrail_evidence_gap_review_packet.py"
...
def test_phase37_step13_files_exist():
...
def test_phase37_step13_launcher_has_valid_param_block():
    text = _read(SCRIPT)
...
def test_phase37_step13_required_safety_markers_present():
...
def test_phase37_step13_forbidden_true_markers_absent():
...
def test_phase37_step13_literal_pass_markers_present():
    text = _read(SCRIPT)
    assert "APPLY PASS" in text
    assert "SMOKE TEST PASS" in text


def test_phase37_step13_launcher_smoke_passes():
