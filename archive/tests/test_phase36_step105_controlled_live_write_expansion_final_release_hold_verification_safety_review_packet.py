...
SCRIPT = REPO / "scripts/phase36_step105_controlled_live_write_expansion_final_release_hold_verification_safety_review_packet.ps1"
UI = REPO / "ui/pages/1801_Phase36_Step105_Live_Write_Expansion_Final_Release_Hold_Verification_Safety_Review.py"
DOC = REPO / "docs/PHASE36_STEP105_CONTROLLED_LIVE_WRITE_EXPANSION_FINAL_RELEASE_HOLD_VERIFICATION_SAFETY_REVIEW_PACKET.md"
TEST = REPO / "tests/test_phase36_step105_controlled_live_write_expansion_final_release_hold_verification_safety_review_packet.py"
...
def test_phase36_step_105_files_exist() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert path.exists(), f"missing expected file: {path}"


def test_phase36_step_105_launcher_has_required_safe_markers() -> None:
    text = read(SCRIPT)
    for marker in [
        "planning_only = $true",
...
def test_phase36_step_105_operational_files_do_not_enable_forbidden_markers() -> None:
    forbidden_keys = [
        "phase37_start",
...
def test_phase36_step_105_ui_and_doc_reference_dry_run_no_apply() -> None:
    text = read(UI) + "\n" + read(DOC)
...
def test_phase36_step_105_launcher_apply_passes() -> None:
...
def test_phase36_step_105_launcher_smoke_passes() -> None:
    result = subprocess.run(
