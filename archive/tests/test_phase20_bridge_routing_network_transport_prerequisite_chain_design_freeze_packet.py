...
def test_bridge_routing_network_transport_prerequisite_chain_design_freeze_packet_script_is_get_only_and_no_write() -> None:
    script = read("scripts/phase20_generate_bridge_routing_network_transport_prerequisite_chain_design_freeze_packet.ps1")
    assert "phase20_bridge_routing_network_transport_prerequisite_chain_design_freeze_packet_" in script
    assert "bridge_routing_network_transport_prerequisite_chain_design_freeze_packet_only = $true" in script
    assert "bridge_get_only = $true" in script
    assert "design_freeze_packet_only = $true" in script
    ...
    assert "can_create_design_freeze_record_now = $false" in script
...
def test_bridge_routing_network_transport_prerequisite_chain_design_freeze_packet_streamlit_page_is_report_only() -> None:
    page = read("ui/pages/84_Bridge_Routing_Network_Transport_Prerequisite_Chain_Design_Freeze_Packet.py")
    assert "Phase 20 Bridge Routing Network Transport Prerequisite Chain Design Freeze Packet" in page
    assert "network-transport-prerequisite-chain-design-freeze-packet-only" in page
    ...
    assert "phase20_bridge_routing_network_transport_prerequisite_chain_design_freeze_packet_*" in page
    assert "phase20_bridge_routing_network_transport_prerequisite_chain_design_freeze_packet.json" in page
    assert "requests.get" not in page
    assert "requests.post" not in page
    assert "st.download_button" in page


def test_bridge_routing_network_transport_prerequisite_chain_design_freeze_packet_docs_capture_boundaries() -> None:
    doc = read("docs/PHASE20_STEP32_BRIDGE_ROUTING_NETWORK_TRANSPORT_PREREQUISITE_CHAIN_DESIGN_FREEZE_PACKET.md")
    assert "no-write design freeze packet" in doc
    assert "network-transport-prerequisite-chain-design-freeze-packet-only" in doc
    ...
    assert "Do not stage generated prerequisite chain design freeze reports" in doc
