...
SCRIPT = ROOT / "scripts/phase38_step28_sustained_live_write_ops_preflight_planning_operator_hold_packet.ps1"
UI = ROOT / "ui/pages/1964_Phase38_Step28_Sustained_Live_Write_Ops_Preflight_Operator_Hold.py"
DOC = ROOT / "docs/PHASE38_STEP28_SUSTAINED_LIVE_WRITE_OPS_PREFLIGHT_PLANNING_OPERATOR_HOLD_PACKET.md"
TEST = ROOT / "tests/test_phase38_step28_sustained_live_write_ops_preflight_planning_operator_hold_packet.py"
...
def test_phase38_step_28_files_exist() -> None:
...
def test_phase38_step_28_launcher_has_valid_param_block() -> None:
    text = read(SCRIPT)
    assert text.startswith("param(")
    assert not text.startswith("\ufeff")


def test_phase38_step_28_launcher_runs_all() -> None:
...
def test_phase38_step_28_safety_markers_present() -> None:
...
def test_phase38_step_28_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase38_step_28_no_phase39_files_created_by_payload() -> None:
...
def test_phase38_step_28_ui_page_is_short_named_and_safe() -> None:
...
def test_phase38_step_28_doc_names_four_file_packet() -> None:
    text = read(DOC)
    assert "scripts/phase38_step28_sustained_live_write_ops_preflight_planning_operator_hold_packet.ps1" in text
    assert "ui/pages/1964_Phase38_Step28_Sustained_Live_Write_Ops_Preflight_Operator_Hold.py" in text
    assert "docs/PHASE38_STEP28_SUSTAINED_LIVE_WRITE_OPS_PREFLIGHT_PLANNING_OPERATOR_HOLD_PACKET.md" in text
    assert "tests/test_phase38_step28_sustained_live_write_ops_preflight_planning_operator_hold_packet.py" in text
