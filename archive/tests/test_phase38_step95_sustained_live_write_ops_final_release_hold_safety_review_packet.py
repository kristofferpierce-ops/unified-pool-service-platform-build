...
SCRIPT = ROOT / "scripts/phase38_step95_sustained_live_write_ops_final_release_hold_safety_review_packet.ps1"
UI = ROOT / "ui/pages/2031_Phase38_Step95_Sustained_Live_Write_Ops_Final_Release_Hold_Safety_Review.py"
DOC = ROOT / "docs/PHASE38_STEP95_SUSTAINED_LIVE_WRITE_OPS_FINAL_RELEASE_HOLD_SAFETY_REVIEW_PACKET.md"
...
def test_phase38_step_95_files_exist() -> None:
    assert SCRIPT.exists()
    assert UI.exists()
    assert DOC.exists()


def test_phase38_step_95_launcher_has_valid_param_block() -> None:
    text = read(SCRIPT)
    assert text.startswith("param(")


def test_phase38_step_95_launcher_runs_all() -> None:
...
def test_phase38_step_95_ui_compiles() -> None:
    py_compile.compile(str(UI), doraise=True)


def test_phase38_step_95_doc_has_required_safe_markers() -> None:
...
def test_phase38_step_95_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase38_step_95_no_phase39_files_created_by_payload() -> None:
...
def test_phase38_step_95_literal_pass_markers_present() -> None:
    text = read(SCRIPT)
