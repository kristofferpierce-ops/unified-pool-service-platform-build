...
SCRIPT = REPO / "scripts/phase37_step28_monitored_live_write_ops_preflight_operator_hold_packet.ps1"
UI = REPO / "ui/pages/1844_Phase37_Step28_Monitored_Live_Write_Ops_Preflight_Operator_Hold.py"
DOC = REPO / "docs/PHASE37_STEP28_MONITORED_LIVE_WRITE_OPS_PREFLIGHT_OPERATOR_HOLD_PACKET.md"
TEST = REPO / "tests/test_phase37_step28_monitored_live_write_ops_preflight_operator_hold_packet.py"
...
def test_phase37_step_28_files_exist() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert path.exists(), f"Missing expected file: {path}"


def test_phase37_step_28_launcher_has_valid_param_block() -> None:
    text = read(SCRIPT)
...
def test_phase37_step_28_safety_markers_present() -> None:
    text = read(SCRIPT) + "\n" + read(UI) + "\n" + read(DOC)
    required = [
...
def test_phase37_step_28_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase37_step_28_launcher_smoke_passes() -> None:
