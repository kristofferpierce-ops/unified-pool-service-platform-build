...
SCRIPT = REPO / "scripts/phase37_step38_monitored_live_write_ops_no_write_dry_run_readiness_operator_hold_packet.ps1"
UI = REPO / "ui/pages/1854_Phase37_Step38_Monitored_Live_Write_Ops_No_Write_Dry_Run_Readiness_Operator_Hold.py"
DOC = REPO / "docs/PHASE37_STEP38_MONITORED_LIVE_WRITE_OPS_NO_WRITE_DRY_RUN_READINESS_OPERATOR_HOLD_PACKET.md"
TEST = REPO / "tests/test_phase37_step38_monitored_live_write_ops_no_write_dry_run_readiness_operator_hold_packet.py"
...
def test_phase37_step_38_files_exist() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert path.exists(), f"Missing expected file: {path}"


def test_phase37_step_38_launcher_has_valid_param_block() -> None:
    text = read(SCRIPT)
...
def test_phase37_step_38_safety_markers_present() -> None:
    text = read(SCRIPT) + "\n" + read(UI) + "\n" + read(DOC)
    required = [
...
def test_phase37_step_38_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase37_step_38_launcher_smoke_passes() -> None:
