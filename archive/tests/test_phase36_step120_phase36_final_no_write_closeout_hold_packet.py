...
SCRIPT = REPO / "scripts/phase36_step120_phase36_final_no_write_closeout_hold_packet.ps1"
UI = REPO / "ui/pages/1816_Phase36_Step120_Phase36_Final_No_Write_Closeout_Hold.py"
DOC = REPO / "docs/PHASE36_STEP120_PHASE36_FINAL_NO_WRITE_CLOSEOUT_HOLD_PACKET.md"
TEST = REPO / "tests/test_phase36_step120_phase36_final_no_write_closeout_hold_packet.py"
...
def test_phase36_step_120_files_exist() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert path.exists(), f"missing expected file: {path}"


def test_phase36_step_120_launcher_has_required_safe_markers() -> None:
    text = read(SCRIPT)
    for marker in [
        "planning_only = $true",
...
def test_phase36_step_120_operational_files_do_not_enable_forbidden_markers() -> None:
    forbidden_keys = [
        "phase37_start",
...
def test_phase36_step_120_ui_and_doc_reference_dry_run_no_apply() -> None:
    text = read(UI) + "\n" + read(DOC)
...
def test_phase36_step_120_launcher_apply_passes() -> None:
...
def test_phase36_step_120_launcher_smoke_passes() -> None:
    result = subprocess.run(
