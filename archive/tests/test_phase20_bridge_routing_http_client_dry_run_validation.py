...
def test_bridge_routing_http_client_dry_run_validation_script_is_get_only() -> None:
    script = read("scripts/phase20_validate_bridge_routing_http_client_dry_run.ps1")
    assert "phase20_bridge_routing_http_client_dry_run_validation_" in script
    assert "bridge_routing_http_client_dry_run_validation_only = $true" in script
...
def test_bridge_routing_http_client_dry_run_validation_streamlit_page_is_report_only() -> None:
    page = read("ui/pages/55_Bridge_Routing_HTTP_Client_Dry_Run_Validation.py")
    assert "Phase 20 Bridge Routing HTTP Client Dry Run Validation" in page
    assert "HTTP-client-dry-run-validation-only" in page
    ...
    assert "phase20_bridge_routing_http_client_dry_run_validation_*" in page
    assert "phase20_bridge_routing_http_client_dry_run_validation.json" in page
    assert "requests.get" not in page
    assert "requests.post" not in page
    assert "st.download_button" in page


def test_bridge_routing_http_client_dry_run_validation_docs_capture_boundaries() -> None:
    doc = read("docs/PHASE20_STEP3_BRIDGE_ROUTING_HTTP_CLIENT_DRY_RUN_VALIDATION.md")
    assert "validates the Phase 20 Step 2 bridge routing HTTP client dry-run transport simulation" in doc
    assert "HTTP-client-dry-run-validation-only" in doc
