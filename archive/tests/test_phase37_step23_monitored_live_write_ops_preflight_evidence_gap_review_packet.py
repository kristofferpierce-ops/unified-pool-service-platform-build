...
SCRIPT = REPO / "scripts/phase37_step23_monitored_live_write_ops_preflight_evidence_gap_review_packet.ps1"
UI = REPO / "ui/pages/1839_Phase37_Step23_Monitored_Live_Write_Ops_Preflight_Evidence_Gap_Review.py"
DOC = REPO / "docs/PHASE37_STEP23_MONITORED_LIVE_WRITE_OPS_PREFLIGHT_EVIDENCE_GAP_REVIEW_PACKET.md"
TEST = REPO / "tests/test_phase37_step23_monitored_live_write_ops_preflight_evidence_gap_review_packet.py"
...
def test_phase37_step_23_files_exist() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert path.exists(), f"Missing expected file: {path}"


def test_phase37_step_23_launcher_has_valid_param_block() -> None:
    text = read(SCRIPT)
...
def test_phase37_step_23_safety_markers_present() -> None:
    text = read(SCRIPT) + "\n" + read(UI) + "\n" + read(DOC)
    required = [
...
def test_phase37_step_23_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase37_step_23_launcher_smoke_passes() -> None:
