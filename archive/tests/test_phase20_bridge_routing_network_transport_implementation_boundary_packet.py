...
def test_bridge_routing_network_transport_implementation_boundary_packet_script_is_get_only_and_no_write() -> None:
    script = read("scripts/phase20_generate_bridge_routing_network_transport_implementation_boundary_packet.ps1")
    assert "phase20_bridge_routing_network_transport_implementation_boundary_packet_" in script
    assert "bridge_routing_network_transport_implementation_boundary_packet_only = $true" in script
    assert "bridge_get_only = $true" in script
    assert "implementation_boundary_packet_only = $true" in script
    assert "implementation_code_created = $false" in script
    ...
    assert "design_freeze_record_created = $false" in script
    ...
    assert "can_create_implementation_code_now = $false" in script
    assert "requests/httpx/socket network transport" in script
    assert "Interface stubs with explicit NotImplementedError" in script or "interface stubs with explicit NotImplementedError" in script
    assert "phase20_bridge_routing_network_transport_prerequisite_chain_design_freeze_packet_*" in script
...
def test_bridge_routing_network_transport_implementation_boundary_packet_streamlit_page_is_report_only() -> None:
    page = read("ui/pages/85_Bridge_Routing_Network_Transport_Implementation_Boundary_Packet.py")
    assert "Phase 20 Bridge Routing Network Transport Implementation Boundary Packet" in page
    assert "network-transport-implementation-boundary-packet-only" in page
    assert "does not create implementation code" in page
    ...
    assert "phase20_bridge_routing_network_transport_implementation_boundary_packet_*" in page
    assert "phase20_bridge_routing_network_transport_implementation_boundary_packet.json" in page
    assert "requests.get" not in page
    assert "requests.post" not in page
    assert "st.download_button" in page


def test_bridge_routing_network_transport_implementation_boundary_packet_docs_capture_boundaries() -> None:
    doc = read("docs/PHASE20_STEP33_BRIDGE_ROUTING_NETWORK_TRANSPORT_IMPLEMENTATION_BOUNDARY_PACKET.md")
    assert "no-write implementation boundary packet" in doc
    assert "network-transport-implementation-boundary-packet-only" in doc
    assert "bridge GET only" in doc
    assert "does not create implementation code" in doc
    ...
    assert "Do not stage generated implementation boundary reports" in doc
