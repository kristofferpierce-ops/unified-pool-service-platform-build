...
SCRIPT = ROOT / "scripts/p39_s013_activation_gate_guardrail_evidence_gap_review.ps1"
UI = ROOT / "ui/pages/2069_Phase39_Step13_Activation_Gate_Guardrail_Evidence_Gap_Review.py"
DOC = ROOT / "docs/P39_S013_ACTIVATION_GATE_GUARDRAIL_EVIDENCE_GAP_REVIEW.md"
...
def test_phase39_step_13_files_exist() -> None:
    assert SCRIPT.exists()
    assert UI.exists()
    assert DOC.exists()


def test_phase39_step_13_launcher_has_valid_param_block() -> None:
    assert read(SCRIPT).lstrip().startswith("param(")


def test_phase39_step_13_safety_markers_present() -> None:
...
def test_phase39_step_13_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase39_step_13_no_phase40_files_created_by_payload() -> None:
...
def test_phase39_step_13_launcher_smoke_passes() -> None:
...
def test_phase39_step_13_ui_is_planning_only() -> None:
    text = read(UI)
...
def test_phase39_step_13_doc_records_activation_gate_hold() -> None:
    text = read(DOC).lower()
    assert "live-write activation gate remains unarmed" in text
    assert "phase40_boundary_creation=false" in text.replace(" ", "")


def test_phase39_step_13_no_server_or_transport_runtime() -> None:
