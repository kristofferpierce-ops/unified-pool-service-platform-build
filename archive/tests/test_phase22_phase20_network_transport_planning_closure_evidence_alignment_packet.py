...
SCRIPT = REPO_ROOT / "scripts" / "phase22_generate_phase20_network_transport_planning_closure_evidence_alignment_packet.ps1"
UI_PAGE = REPO_ROOT / "ui" / "pages" / "144_Phase20_Network_Transport_Planning_Closure_Evidence_Alignment_Packet.py"
DOC = REPO_ROOT / "docs" / "PHASE22_STEP38_PHASE20_NETWORK_TRANSPORT_PLANNING_CLOSURE_EVIDENCE_ALIGNMENT_PACKET.md"
...
def test_phase22_step38_files_exist() -> None:
    for path in STEP_FILES:
        assert path.exists(), f"Missing expected file: {path}"


def test_phase22_step38_title_and_number_are_declared() -> None:
    combined = combined_public_text()
    assert "Phase 22 Step 38" in combined
    assert "Closure Evidence Alignment Packet" in combined


def test_phase22_step38_launcher_supports_optimized_action_all() -> None:
...
def test_phase22_step38_launcher_is_path_safe_and_idempotent() -> None:
    script_text = read(SCRIPT)
...
def test_phase22_step38_planning_only_guardrails_are_declared() -> None:
    combined = combined_public_text()
    required = [
        "planning_only",
...
def test_phase22_step38_closure_evidence_base_guards_are_plan_only() -> None:
    combined = combined_public_text()
    for token in [
        ...
        "closure_evidence_compiled_for_review_not_approved",
    ]:
        assert token in combined


def test_phase22_step38_prior_gate_chain_is_preserved() -> None:
    combined = combined_public_text()
    for token in [
        "Phase 22 Step 30",
        "Phase 22 Step 31",
...
def test_phase22_step38_rollout_alignment_is_preserved() -> None:
    combined = combined_public_text()
...
def test_phase22_step38_forbidden_runtime_tokens_absent_from_public_files() -> None:
    forbidden = [
        "requests.post(",
        "requests.get(",
        "httpx.",
...
def test_phase22_step38_packet_generation_mentions_expected_pass_checks() -> None:
    script_text = read(SCRIPT)
    expected = [
...
def test_phase22_step38_streamlit_page_is_read_only() -> None:
...
def test_phase22_step38_document_lists_all_added_files() -> None:
    doc_text = read(DOC)
    for rel_path in [
        r"scripts\phase22_generate_phase20_network_transport_planning_closure_evidence_alignment_packet.ps1",
        r"ui\pages\144_Phase20_Network_Transport_Planning_Closure_Evidence_Alignment_Packet.py",
        r"docs\PHASE22_STEP38_PHASE20_NETWORK_TRANSPORT_PLANNING_CLOSURE_EVIDENCE_ALIGNMENT_PACKET.md",
        r"tests\test_phase22_phase20_network_transport_planning_closure_evidence_alignment_packet.py",
    ]:
        assert rel_path in doc_text


def test_phase22_step38_no_approval_creation_or_execution_start() -> None:
    combined = combined_public_text()
    checks = [
...
def test_phase22_step38_launcher_avoids_variable_colon_parser_bug() -> None:
    script_text = read(SCRIPT)
...
def test_phase22_step38_closure_evidence_is_plan_only() -> None:
    combined = combined_public_text()
    for token in [
        "planning_closure_record_creation",
        "closure_evidence_record_creation",
        "closure_evidence_approval",
        "closure_evidence_application",
        "implementation_release_authorization",
        "planning_closure_evidence_plan_only",
        "closure_evidence_compiled_for_review_not_approved",
        "not_granted",
    ]:
...
def test_phase22_step38_expected_branch_and_step_number_are_correct() -> None:
    script_text = read(SCRIPT)
    assert "$StepNumber = 38" in script_text
    assert "$PhaseNumber = 22" in script_text
    assert "phase22-step38-phase20-network-transport-planning-closure-evidence-alignment-packet" in script_text
    assert "Phase 22 Step 37 - Phase 20 Network Transport Planning Handoff Dossier Alignment Packet" in script_text
