...
def test_phase20_network_transport_planning_transition_packet_script_is_get_only_and_no_write() -> None:
    script = read("scripts/phase22_generate_phase20_network_transport_planning_transition_packet.ps1")
    assert "phase22_phase20_network_transport_planning_transition_packet_" in script
    assert "phase20_network_transport_planning_transition_packet_only = $true" in script
    assert "planning_transition_only = $true" in script
    ...
    assert "phase22_phase20_network_transport_planning_closure_packet_*" in script
...
def test_phase20_network_transport_planning_transition_packet_streamlit_page_is_report_only() -> None:
    page = read("ui/pages/120_Phase20_Network_Transport_Planning_Transition_Packet.py")
    assert "Phase 22 Step 14 - Phase 20 Network Transport Planning Transition Packet" in page
    assert "Phase 20 network transport planning-transition-only" in page
    ...
    assert "phase22_phase20_network_transport_planning_transition_packet_*" in page
    assert "phase22_phase20_network_transport_planning_transition_packet.json" in page
    assert "requests.get" not in page
    assert "requests.post" not in page
    assert "st.download_button" in page


def test_phase20_network_transport_planning_transition_packet_docs_capture_boundaries() -> None:
    doc = read("docs/PHASE22_STEP14_PHASE20_NETWORK_TRANSPORT_PLANNING_TRANSITION_PACKET.md")
    assert "no-write planning transition packet" in doc
    assert "Phase 20 network transport planning-transition-only" in doc
    ...
    assert "Stage only the four Phase 22 Step 14 files" in doc
