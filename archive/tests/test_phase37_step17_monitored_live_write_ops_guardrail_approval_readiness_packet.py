...
SCRIPT = REPO / "scripts/phase37_step17_monitored_live_write_ops_guardrail_approval_readiness_packet.ps1"
UI = REPO / "ui/pages/1833_Phase37_Step17_Monitored_Live_Write_Ops_Guardrail_Approval_Readiness.py"
DOC = REPO / "docs/PHASE37_STEP17_MONITORED_LIVE_WRITE_OPS_GUARDRAIL_APPROVAL_READINESS_PACKET.md"
TEST = REPO / "tests/test_phase37_step17_monitored_live_write_ops_guardrail_approval_readiness_packet.py"
...
def test_phase37_step17_files_exist():
...
def test_phase37_step17_launcher_has_valid_param_block():
    text = _read(SCRIPT)
...
def test_phase37_step17_required_safety_markers_present():
...
def test_phase37_step17_forbidden_true_markers_absent():
...
def test_phase37_step17_literal_pass_markers_present():
    text = _read(SCRIPT)
    assert "APPLY PASS" in text
    assert "SMOKE TEST PASS" in text


def test_phase37_step17_launcher_smoke_passes():
