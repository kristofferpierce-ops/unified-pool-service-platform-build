...
SCRIPT = REPO / "scripts/phase37_step44_monitored_live_write_ops_no_write_dry_run_verification_planning_safety_planning_packet.ps1"
UI = REPO / "ui/pages/1860_Phase37_Step44_Monitored_Live_Write_Ops_No_Write_Dry_Run_Verification_Planning_Safety_Planning.py"
DOC = REPO / "docs/PHASE37_STEP44_MONITORED_LIVE_WRITE_OPS_NO_WRITE_DRY_RUN_VERIFICATION_PLANNING_SAFETY_PLANNING_PACKET.md"
TEST = REPO / "tests/test_phase37_step44_monitored_live_write_ops_no_write_dry_run_verification_planning_safety_planning_packet.py"
...
def test_phase37_step_44_files_exist() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert path.exists(), f"Missing expected file: {path}"


def test_phase37_step_44_launcher_has_valid_param_block() -> None:
    text = read(SCRIPT)
...
def test_phase37_step_44_safety_markers_present() -> None:
    text = read(SCRIPT) + "\n" + read(UI) + "\n" + read(DOC)
    required = [
...
def test_phase37_step_44_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase37_step_44_launcher_smoke_passes() -> None:
