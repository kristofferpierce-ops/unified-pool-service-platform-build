...
SCRIPT = ROOT / "scripts/phase38_step1_sustained_live_write_ops_readiness_boundary_packet.ps1"
UI = ROOT / "ui/pages/1937_Phase38_Step1_Sustained_Live_Write_Ops_Readiness_Boundary.py"
DOC = ROOT / "docs/PHASE38_STEP1_SUSTAINED_LIVE_WRITE_OPS_READINESS_BOUNDARY_PACKET.md"
TEST = ROOT / "tests/test_phase38_step1_sustained_live_write_ops_readiness_boundary_packet.py"
...
def test_phase38_step1_files_exist() -> None:
...
def test_phase38_step1_launcher_has_valid_param_block() -> None:
    text = read(SCRIPT)
...
def test_phase38_step1_required_safety_markers_present() -> None:
...
def test_phase38_step1_next_phase_guards_present() -> None:
    combined = "\n".join(read(path) for path in (SCRIPT, UI, DOC))
    assert "phase39_start" in combined
    assert "phase39_boundary_creation" in combined


def test_phase38_step1_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase38_step1_literal_pass_markers_present() -> None:
    text = read(SCRIPT)
    assert "APPLY PASS" in text
    assert "SMOKE TEST PASS" in text


def test_phase38_step1_ui_is_planning_only_page() -> None:
    text = read(UI)
...
def test_phase38_step1_no_phase39_files_created_by_payload() -> None:
