...
SCRIPT = REPO / "scripts/phase31_step1_controlled_active_program_operation_readiness_boundary_packet.ps1"
UI_PAGE = REPO / "ui/pages/1097_Phase31_Step1_Implementation_Controlled_Active_Program_Operation_Readiness_Boundary_Packet.py"
DOC = REPO / "docs/PHASE31_STEP1_CONTROLLED_ACTIVE_PROGRAM_OPERATION_READINESS_BOUNDARY_PACKET.md"
TEST = REPO / "tests/test_phase31_step1_controlled_active_program_operation_readiness_boundary_packet.py"
...
def test_phase31_step1_files_exist():
...
def test_phase31_step1_launcher_supports_optimized_actions():
    script = read(SCRIPT)
    assert 'ValidateSet("status", "apply", "smoke", "packet", "all")' in script
    assert "APPLY PASS: Phase 31 Step 1" in script
    assert "SMOKE TEST PASS: Phase 31 Step 1" in script


def test_phase31_step1_safety_markers_present():
...
def test_phase31_step1_does_not_enable_runtime_or_phase32():
...
def test_phase31_step1_doc_declares_no_phase32_boundary():
    doc = read(DOC).lower()
...
def test_phase31_step1_ui_is_reference_only():
    ui = read(UI_PAGE)
...
def test_phase31_step1_prior_step_is_recorded():
    combined = read(SCRIPT) + read(DOC) + read(UI_PAGE)
    assert "Phase 30 Step 120 - Phase 20 Network Transport Implementation Phase 30 Final No-Write Closeout Hold Packet" in combined


def test_phase31_step1_uses_exact_four_step_files():
    script = read(SCRIPT)
    for rel in [
