...
SCRIPT = ROOT / "scripts/phase38_step87_sustained_live_write_ops_release_gate_verification_approval_readiness_packet.ps1"
UI = ROOT / "ui/pages/2023_Phase38_Step87_Sustained_Live_Write_Ops_Release_Gate_Verification_Approval_Readiness.py"
DOC = ROOT / "docs/PHASE38_STEP87_SUSTAINED_LIVE_WRITE_OPS_RELEASE_GATE_VERIFICATION_APPROVAL_READINESS_PACKET.md"
...
def test_phase38_step_87_files_exist() -> None:
    assert SCRIPT.exists()
    assert UI.exists()
    assert DOC.exists()


def test_phase38_step_87_launcher_has_valid_param_block() -> None:
    text = read(SCRIPT)
    assert text.startswith("param(")


def test_phase38_step_87_launcher_runs_all() -> None:
...
def test_phase38_step_87_ui_compiles() -> None:
    py_compile.compile(str(UI), doraise=True)


def test_phase38_step_87_doc_has_required_safe_markers() -> None:
...
def test_phase38_step_87_forbidden_runtime_markers_not_enabled() -> None:
...
def test_phase38_step_87_no_phase39_files_created_by_payload() -> None:
...
def test_phase38_step_87_literal_pass_markers_present() -> None:
    text = read(SCRIPT)
