...
SCRIPT = REPO_ROOT / "scripts/phase26_step15_implementation_transition_guardrail_verification_safety_disposition_review_packet.ps1"
PAGE = REPO_ROOT / "ui/pages/511_Phase26_Step15_Implementation_Transition_Guardrail_Verification_Safety_Disposition_Review_Packet.py"
DOC = REPO_ROOT / "docs/PHASE26_STEP15_IMPLEMENTATION_TRANSITION_GUARDRAIL_VERIFICATION_SAFETY_DISPOSITION_REVIEW_PACKET.md"
TEST = REPO_ROOT / "tests/test_phase26_step15_implementation_transition_guardrail_verification_safety_disposition_review_packet.py"
...
def test_phase26_step15_files_are_present():
    for path in (SCRIPT, PAGE, DOC, TEST):
        assert path.exists(), f"missing {path}"


def test_phase26_step15_launcher_markers_are_present():
    text = read(SCRIPT).lower()
    assert "phase 26 step 15" in text
    assert "phase26-step15-implementation-transition-guardrail-verification-safety-disposition-review" in text
...
def test_phase26_step15_planning_only_safety_markers():
...
def test_phase26_step15_does_not_start_runtime_or_next_phase():
    text = operational_text()
    forbidden = [
        "phase27_start=true",
...
def test_phase26_step15_batch_risk_review_is_chat_only_not_powershell_prompt():
    text = operational_text()
    assert "batch_risk_review_location" in text
...
def test_phase26_step15_launcher_supports_optimized_actions_and_known_fixes():
    script = read(SCRIPT)
    assert "ValidateSet(\"status\", \"apply\", \"smoke\", \"packet\", \"all\")" in script
    assert "APPLY PASS: Phase 26 Step 15" in script
    assert "SMOKE TEST PASS: Phase 26 Step 15" in script
    assert "CHECK: packet_json=" in script
    assert "$File:" not in script
    assert "Join-Path $RepoRoot \"scripts/phase26_step15_implementation_transition_guardrail_verification_safety_disposition_review_packet.ps1\"," not in script
    assert "System.Object[]" not in script


def test_phase26_step15_docs_are_operator_readable():
    text = read(DOC)
    assert "Phase 26 Step 15 - Phase 20 Network Transport Implementation Transition Guardrail Verification Safety Disposition Review Packet" in text
...
def test_phase26_step15_ui_is_streamlit_reference_only_page():
...
def test_phase26_step15_test_file_self_identifies_step():
    text = read(TEST).lower()
    assert "step15" in text or "step 15" in text
