...
SCRIPT = REPO / "scripts/phase25_step96_post_closeout_final_release_hold_planning_approval_boundary_packet.ps1"
UI = REPO / "ui/pages/472_Phase25_Step96_Implementation_PostCloseout_Final_Release_Hold_Planning_Approval_Boundary_Packet.py"
DOC = REPO / "docs/PHASE25_STEP96_POST_CLOSEOUT_FINAL_RELEASE_HOLD_PLANNING_APPROVAL_BOUNDARY_PACKET.md"
TEST = REPO / "tests/test_phase25_step96_post_closeout_final_release_hold_planning_approval_boundary_packet.py"
...
def test_phase25_step96_files_exist():
...
def test_phase25_step96_launcher_markers():
    text = read(SCRIPT).lower()
    assert "phase 25 step 96" in text
    assert "phase25-step96-post-closeout-final-release-hold-planning-approval-boundary" in text
...
def test_phase25_step96_planning_only_safety_markers():
...
def test_phase25_step96_does_not_start_runtime_or_next_phase():
...
def test_phase25_step96_complexity_gate_allows_standard_planning_only_batch():
    text = compact(read(SCRIPT) + read(DOC) + read(UI))
    assert "complexity_batch_gate" in text
...
def test_phase25_step96_docs_are_operator_readable():
    text = read(DOC)
    assert "Phase 25 Step 96 - Phase 20 Network Transport Implementation Post-Closeout Final Release Hold Planning Approval Boundary Packet" in text
...
def test_phase25_step96_ui_is_streamlit_reference_only_page():
    text = read(UI)
...
def test_phase25_step96_test_file_self_identifies_step():
    text = read(TEST).lower()
    assert "step96" in text or "step 96" in text
