...
SCRIPT = REPO / "scripts/phase37_step6_monitored_live_write_ops_readiness_readiness_approval_boundary_packet.ps1"
UI = REPO / "ui/pages/1822_Phase37_Step6_Monitored_Live_Write_Ops_Readiness_Approval_Boundary.py"
DOC = REPO / "docs/PHASE37_STEP6_MONITORED_LIVE_WRITE_OPS_READINESS_READINESS_APPROVAL_BOUNDARY_PACKET.md"
TEST = REPO / "tests/test_phase37_step6_monitored_live_write_ops_readiness_readiness_approval_boundary_packet.py"
...
def test_phase37_step6_files_exist():
...
def test_phase37_step6_launcher_has_valid_param_block():
    text = _read(SCRIPT)
...
def test_phase37_step6_required_safety_markers_present():
...
def test_phase37_step6_next_phase_guards_present():
    combined = "\n".join(_read(path) for path in (SCRIPT, UI, DOC))
    assert "phase38_start" in combined
    assert "phase38_boundary_creation" in combined


def test_phase37_step6_forbidden_true_markers_absent():
...
def test_phase37_step6_literal_pass_markers_present():
    text = _read(SCRIPT)
    assert "APPLY PASS" in text
    assert "SMOKE TEST PASS" in text


def test_phase37_step6_ui_is_planning_only_page():
    text = _read(UI)
...
def test_phase37_step6_doc_records_no_runtime_boundary():
    text = _read(DOC)
