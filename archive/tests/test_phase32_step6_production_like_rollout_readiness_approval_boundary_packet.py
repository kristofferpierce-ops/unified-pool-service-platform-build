...
SCRIPT = REPO / "scripts/phase32_step6_production_like_rollout_readiness_approval_boundary_packet.ps1"
UI_PAGE = REPO / "ui/pages/1222_Phase32_Step6_Implementation_Production_Like_Rollout_Readiness_Approval_Boundary_Packet.py"
DOC = REPO / "docs/PHASE32_STEP6_PRODUCTION_LIKE_ROLLOUT_READINESS_APPROVAL_BOUNDARY_PACKET.md"
TEST = REPO / "tests/test_phase32_step6_production_like_rollout_readiness_approval_boundary_packet.py"
...
def test_phase32_step6_files_exist():
...
def test_phase32_step6_launcher_supports_optimized_actions():
    script = read(SCRIPT)
    assert 'ValidateSet("status", "apply", "smoke", "packet", "all")' in script
    assert "APPLY PASS: Phase 32 Step 6" in script
    assert "SMOKE TEST PASS: Phase 32 Step 6" in script


def test_phase32_step6_safety_markers_present():
...
def test_phase32_step6_does_not_enable_runtime_or_phase33():
...
def test_phase32_step6_doc_declares_no_phase33_boundary():
    doc = read(DOC).lower()
...
def test_phase32_step6_ui_is_reference_only():
    ui = read(UI_PAGE)
...
def test_phase32_step6_prior_step_is_recorded():
    combined = read(SCRIPT) + read(DOC) + read(UI_PAGE)
    assert "Phase 32 Step 5 - Phase 20 Network Transport Implementation Production-Like Rollout Readiness Safety Disposition Review Packet" in combined


def test_phase32_step6_uses_exact_four_step_files():
    script = read(SCRIPT)
    for rel in [
