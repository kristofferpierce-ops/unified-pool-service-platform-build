...
def test_phase20_network_transport_review_board_packet_script_is_get_only_and_no_write() -> None:
    script = read("scripts/phase21_generate_phase20_network_transport_review_board_packet.ps1")
    assert "phase21_phase20_network_transport_review_board_packet_" in script
    assert "phase20_network_transport_review_board_packet_only = $true" in script
    assert "review_board_packet_only = $true" in script
...
def test_phase20_network_transport_review_board_packet_streamlit_page_is_report_only() -> None:
    page = read("ui/pages/95_Phase20_Network_Transport_Review_Board_Packet.py")
    assert "Phase 21 Step 3 - Phase 20 Network Transport Review Board Packet" in page
    assert "Phase 20 network transport review-board-only" in page
    ...
    assert "phase21_phase20_network_transport_review_board_packet_*" in page
    assert "phase21_phase20_network_transport_review_board_packet.json" in page
    assert "requests.get" not in page
    assert "requests.post" not in page
    assert "st.download_button" in page


def test_phase20_network_transport_review_board_packet_docs_capture_boundaries() -> None:
    doc = read("docs/PHASE21_STEP3_PHASE20_NETWORK_TRANSPORT_REVIEW_BOARD_PACKET.md")
    assert "no-write review board" in doc
    assert "Phase 20 network transport review-board-only" in doc
    ...
    assert "Stage only the four Phase 21 Step 3 files" in doc
