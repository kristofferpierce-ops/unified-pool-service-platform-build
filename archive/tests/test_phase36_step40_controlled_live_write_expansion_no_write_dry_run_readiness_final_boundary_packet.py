...
SCRIPT = REPO / "scripts/phase36_step40_controlled_live_write_expansion_no_write_dry_run_readiness_final_boundary_packet.ps1"
UI = REPO / "ui/pages/1736_Phase36_Step40_Live_Write_Expansion_Dry_Run_Readiness_Final_Boundary.py"
DOC = REPO / "docs/PHASE36_STEP40_CONTROLLED_LIVE_WRITE_EXPANSION_DRY_RUN_READINESS_FINAL_BOUNDARY_PACKET.md"
TEST = REPO / "tests/test_phase36_step40_controlled_live_write_expansion_no_write_dry_run_readiness_final_boundary_packet.py"
...
def test_phase36_step_40_files_exist() -> None:
    for path in (SCRIPT, UI, DOC, TEST):
        assert path.exists(), f"missing expected file: {path}"


def test_phase36_step_40_launcher_has_required_safe_markers() -> None:
    text = read(SCRIPT)
    for marker in [
        "planning_only = $true",
...
def test_phase36_step_40_operational_files_do_not_enable_forbidden_markers() -> None:
    forbidden_keys = [
        "phase37_start",
...
def test_phase36_step_40_ui_and_doc_reference_dry_run_no_apply() -> None:
    text = read(UI) + "\n" + read(DOC)
...
def test_phase36_step_40_launcher_apply_passes() -> None:
...
def test_phase36_step_40_launcher_smoke_passes() -> None:
    result = subprocess.run(
