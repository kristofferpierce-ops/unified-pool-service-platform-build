...
st.set_page_config(page_title="Phase 20 Network Transport Review Board Packet", page_icon="📌", layout="wide")
...
def latest_reports(limit: int = 20) -> list[Path]:
    ...
    return sorted(
        [
            p
            for p in BACKUP_DIR.glob("phase21_phase20_network_transport_review_board_packet_*")
            if p.is_dir() and (p / "phase21_phase20_network_transport_review_board_packet.json").exists()
        ],
...
st.title("📌 Phase 21 Step 3 - Phase 20 Network Transport Review Board Packet")
st.caption("No-write review board for the Phase 20 bridge routing network transport design chain.")

st.warning(
    "This page is Phase 20 network transport review-board-only. It does not create design closure records, "
...
if not reports:
    st.info(
        "No Phase 20 network transport review board packet found yet. Run "
        "`scripts\\phase21_generate_phase20_network_transport_review_board_packet.ps1` from the platform repo."
    )
    st.stop()
...
json_path = report_dir / "phase21_phase20_network_transport_review_board_packet.json"
sections_csv_path = report_dir / "phase21_phase20_network_transport_review_board_packet_sections.csv"
buckets_csv_path = report_dir / "phase21_phase20_network_transport_review_board_packet_buckets.csv"
artifacts_csv_path = report_dir / "phase21_phase20_network_transport_review_board_packet_artifacts.csv"
source_csv_path = report_dir / "phase21_phase20_network_transport_review_board_packet_source_audits.csv"
checklist_csv_path = report_dir / "phase21_phase20_network_transport_review_board_packet_checklist.csv"
issues_csv_path = report_dir / "phase21_phase20_network_transport_review_board_packet_issues.csv"
md_path = report_dir / "phase21_phase20_network_transport_review_board_packet.md"

report = load_json(json_path)
safety = report.get("safety", {})
packet = report.get("review_board_packet", {})
...
c1.metric("Board status", packet.get("status", "unknown"))
c2.metric("Present artifacts", packet.get("present_artifact_count", 0))
...
if (
    safety.get("phase20_network_transport_review_board_packet_only")
    and safety.get("review_board_packet_only")
    ...
    st.success("Safe review board confirmed: no-write review only, bridge GET only, no approval, no implementation phase, no transport, no bridge POST, no socket.")
else:
    st.error("Review review-board safety flags before continuing.")

if packet.get("status") == "phase20_review_board_packet_blocked":
    st.error("Review board packet is blocked. Resolve blockers before further closeout work.")
elif packet.get("status") == "phase20_review_board_packet_review_required_no_write":
    st.warning("Review board packet has review items. This still does not authorize approvals, implementation, or network transport.")
else:
    st.success("Review board packet is ready for review. This still does not authorize approvals, implementation, or network transport.")

st.subheader("Review board summary")
st.json(packet)

st.subheader("Board sections")
if sections:
    st.dataframe(sections, use_container_width=True, hide_index=True)
else:
    st.info("No board sections were recorded.")
...
if issues:
    ...
    st.success("No review-board issues were recorded.")
...
for label, path, filename in [
    ("Download sections CSV", sections_csv_path, "phase21_phase20_network_transport_review_board_packet_sections.csv"),
    ("Download buckets CSV", buckets_csv_path, "phase21_phase20_network_transport_review_board_packet_buckets.csv"),
    ("Download artifacts CSV", artifacts_csv_path, "phase21_phase20_network_transport_review_board_packet_artifacts.csv"),
    ("Download source audits CSV", source_csv_path, "phase21_phase20_network_transport_review_board_packet_source_audits.csv"),
    ("Download checklist CSV", checklist_csv_path, "phase21_phase20_network_transport_review_board_packet_checklist.csv"),
    ("Download issues CSV", issues_csv_path, "phase21_phase20_network_transport_review_board_packet_issues.csv"),
]:
    if path.exists():
        st.download_button(label, path.read_text(encoding="utf-8-sig"), filename, "text/csv")

st.download_button(
    "Download review board packet JSON",
    json.dumps(report, indent=2, default=str),
    "phase21_phase20_network_transport_review_board_packet.json",
    "application/json",
)

with st.expander("Raw review board packet JSON"):
    st.json(report)
