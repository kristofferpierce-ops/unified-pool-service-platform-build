...
st.set_page_config(page_title="Phase 20 Network Transport Planning Boundary Packet", page_icon="🧱", layout="wide")
...
def latest_reports(limit: int = 20) -> list[Path]:
    ...
    return sorted(
        [
            p
            for p in BACKUP_DIR.glob("phase22_phase20_network_transport_planning_boundary_packet_*")
            if p.is_dir() and (p / "phase22_phase20_network_transport_planning_boundary_packet.json").exists()
        ],
...
st.title("🧱 Phase 22 Step 2 - Phase 20 Network Transport Planning Boundary Packet")
st.caption("No-write planning boundary packet for the Phase 20 bridge routing network transport chain.")

st.warning(
    "This page is Phase 20 network transport planning-boundary-only. It does not create design closure records, "
...
if not reports:
    st.info(
        "No Phase 20 network transport planning boundary packet found yet. Run "
        "`scripts\\phase22_generate_phase20_network_transport_planning_boundary_packet.ps1` from the platform repo."
    )
    st.stop()
...
json_path = report_dir / "phase22_phase20_network_transport_planning_boundary_packet.json"
sections_csv_path = report_dir / "phase22_phase20_network_transport_planning_boundary_packet_sections.csv"
entries_csv_path = report_dir / "phase22_phase20_network_transport_planning_boundary_packet_entries.csv"
boundaries_csv_path = report_dir / "phase22_phase20_network_transport_planning_boundary_packet_boundaries.csv"
decisions_csv_path = report_dir / "phase22_phase20_network_transport_planning_boundary_packet_decisions.csv"
families_csv_path = report_dir / "phase22_phase20_network_transport_planning_boundary_packet_artifact_families.csv"
git_status_csv_path = report_dir / "phase22_phase20_network_transport_planning_boundary_packet_git_status.csv"
staged_csv_path = report_dir / "phase22_phase20_network_transport_planning_boundary_packet_staged.csv"
source_artifacts_csv_path = report_dir / "phase22_phase20_network_transport_planning_boundary_packet_source_artifacts.csv"
checklist_csv_path = report_dir / "phase22_phase20_network_transport_planning_boundary_packet_checklist.csv"
issues_csv_path = report_dir / "phase22_phase20_network_transport_planning_boundary_packet_issues.csv"
md_path = report_dir / "phase22_phase20_network_transport_planning_boundary_packet.md"

report = load_json(json_path)
safety = report.get("safety", {})
packet = report.get("planning_boundary_packet", {})
sections = report.get("boundary_sections", [])
entries = report.get("boundary_entries", [])
boundary_reviews = report.get("boundary_reviews", [])
...
c1.metric("Planning boundary status", packet.get("status", "unknown"))
c2.metric("Boundary review topics", packet.get("boundary_review_count", 0))
c3.metric("Review items", packet.get("review_count", 0))
c4.metric("Staged forbidden", packet.get("staged_forbidden_count", 0))

if (
    safety.get("phase20_network_transport_planning_boundary_packet_only")
    and safety.get("planning_boundary_only")
    ...
    st.success("Safe planning boundary confirmed: no-write planning only, bridge GET only, no approval, no implementation phase, no transport, no bridge POST, no socket.")
else:
    st.error("Review planning-boundary safety flags before continuing.")

if packet.get("status") == "phase20_planning_boundary_packet_blocked":
    st.error("Planning boundary packet is blocked. Resolve blockers before continuing planning.")
elif packet.get("status") == "phase20_planning_boundary_packet_review_required_no_write":
    st.warning("Planning boundary packet has review items. This still does not authorize approvals, implementation, or network transport.")
else:
    st.success("Planning boundary packet is ready for review. This still does not authorize approvals, implementation, or network transport.")

st.subheader("Planning boundary summary")
st.json(packet)

st.subheader("Boundary sections")
if sections:
    st.dataframe(sections, use_container_width=True, hide_index=True)
else:
    st.info("No boundary sections were recorded.")

st.subheader("Boundary entries")
if entries:
    ...
    st.info("No boundary entries were recorded.")

st.subheader("Boundary review topics")
if boundary_reviews:
    st.dataframe(boundary_reviews, use_container_width=True, hide_index=True)
else:
    st.info("No boundary review topics were recorded.")
...
if issues:
    ...
    st.success("No planning-boundary issues were recorded.")
...
for label, path, filename in [
    ("Download sections CSV", sections_csv_path, "phase22_phase20_network_transport_planning_boundary_packet_sections.csv"),
    ("Download entries CSV", entries_csv_path, "phase22_phase20_network_transport_planning_boundary_packet_entries.csv"),
    ("Download boundaries CSV", boundaries_csv_path, "phase22_phase20_network_transport_planning_boundary_packet_boundaries.csv"),
    ("Download decisions CSV", decisions_csv_path, "phase22_phase20_network_transport_planning_boundary_packet_decisions.csv"),
    ("Download artifact families CSV", families_csv_path, "phase22_phase20_network_transport_planning_boundary_packet_artifact_families.csv"),
    ("Download git status CSV", git_status_csv_path, "phase22_phase20_network_transport_planning_boundary_packet_git_status.csv"),
    ("Download staged CSV", staged_csv_path, "phase22_phase20_network_transport_planning_boundary_packet_staged.csv"),
    ("Download source artifacts CSV", source_artifacts_csv_path, "phase22_phase20_network_transport_planning_boundary_packet_source_artifacts.csv"),
    ("Download checklist CSV", checklist_csv_path, "phase22_phase20_network_transport_planning_boundary_packet_checklist.csv"),
    ("Download issues CSV", issues_csv_path, "phase22_phase20_network_transport_planning_boundary_packet_issues.csv"),
]:
    if path.exists():
        st.download_button(label, path.read_text(encoding="utf-8-sig"), filename, "text/csv")

st.download_button(
    "Download planning boundary packet JSON",
    json.dumps(report, indent=2, default=str),
    "phase22_phase20_network_transport_planning_boundary_packet.json",
    "application/json",
)

with st.expander("Raw planning boundary packet JSON"):
    st.json(report)
