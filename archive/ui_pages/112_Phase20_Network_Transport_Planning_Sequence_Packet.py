...
st.set_page_config(page_title="Phase 20 Network Transport Planning Sequence Packet", page_icon="🗺️", layout="wide")
...
def latest_reports(limit: int = 20) -> list[Path]:
    ...
    return sorted(
        [
            p
            for p in BACKUP_DIR.glob("phase22_phase20_network_transport_planning_sequence_packet_*")
            if p.is_dir() and (p / "phase22_phase20_network_transport_planning_sequence_packet.json").exists()
        ],
...
st.title("🗺️ Phase 22 Step 6 - Phase 20 Network Transport Planning Sequence Packet")
st.caption("No-write planning sequence packet for the Phase 20 bridge routing network transport chain.")

st.warning(
    "This page is Phase 20 network transport planning-sequence-only. It does not create design closure records, "
...
if not reports:
    st.info(
        "No Phase 20 network transport planning sequence packet found yet. Run "
        "`scripts\\phase22_generate_phase20_network_transport_planning_sequence_packet.ps1` from the platform repo."
    )
    st.stop()
...
json_path = report_dir / "phase22_phase20_network_transport_planning_sequence_packet.json"
sections_csv_path = report_dir / "phase22_phase20_network_transport_planning_sequence_packet_sections.csv"
entries_csv_path = report_dir / "phase22_phase20_network_transport_planning_sequence_packet_entries.csv"
sequence_csv_path = report_dir / "phase22_phase20_network_transport_planning_sequence_packet_sequence_areas.csv"
decisions_csv_path = report_dir / "phase22_phase20_network_transport_planning_sequence_packet_decisions.csv"
families_csv_path = report_dir / "phase22_phase20_network_transport_planning_sequence_packet_artifact_families.csv"
git_status_csv_path = report_dir / "phase22_phase20_network_transport_planning_sequence_packet_git_status.csv"
staged_csv_path = report_dir / "phase22_phase20_network_transport_planning_sequence_packet_staged.csv"
source_artifacts_csv_path = report_dir / "phase22_phase20_network_transport_planning_sequence_packet_source_artifacts.csv"
checklist_csv_path = report_dir / "phase22_phase20_network_transport_planning_sequence_packet_checklist.csv"
issues_csv_path = report_dir / "phase22_phase20_network_transport_planning_sequence_packet_issues.csv"
md_path = report_dir / "phase22_phase20_network_transport_planning_sequence_packet.md"

report = load_json(json_path)
safety = report.get("safety", {})
packet = report.get("planning_sequence_packet", {})
sections = report.get("planning_sections", [])
entries = report.get("planning_entries", [])
sequence_areas = report.get("sequence_areas", [])
...
c1.metric("Planning sequence status", packet.get("status", "unknown"))
c2.metric("Sequence areas", packet.get("sequence_area_count", 0))
c3.metric("Review items", packet.get("review_count", 0))
c4.metric("Staged forbidden", packet.get("staged_forbidden_count", 0))

if (
    safety.get("phase20_network_transport_planning_sequence_packet_only")
    and safety.get("planning_sequence_only")
    ...
    st.success("Safe planning sequence confirmed: no-write planning only, bridge GET only, no approval, no implementation phase, no transport, no bridge POST, no socket.")
else:
    st.error("Review planning-sequence safety flags before continuing.")

if packet.get("status") == "phase20_planning_sequence_packet_blocked":
    st.error("Planning sequence packet is blocked. Resolve blockers before continuing planning.")
elif packet.get("status") == "phase20_planning_sequence_packet_review_required_no_write":
    st.warning("Planning sequence packet has review items. This still does not authorize approvals, implementation, or network transport.")
else:
    st.success("Planning sequence packet is ready for review. This still does not authorize approvals, implementation, or network transport.")

st.subheader("Planning sequence summary")
st.json(packet)
...
st.subheader("Sequence areas")
if sequence_areas:
    st.dataframe(sequence_areas, use_container_width=True, hide_index=True)
else:
    st.info("No sequence areas were recorded.")
...
if issues:
    ...
    st.success("No planning-sequence issues were recorded.")
...
st.code(
    "\n".join(
        [
            ...
            f"Sequence areas CSV: {sequence_csv_path}",
...
for label, path, filename in [
    ("Download sections CSV", sections_csv_path, "phase22_phase20_network_transport_planning_sequence_packet_sections.csv"),
    ("Download entries CSV", entries_csv_path, "phase22_phase20_network_transport_planning_sequence_packet_entries.csv"),
    ("Download sequence areas CSV", sequence_csv_path, "phase22_phase20_network_transport_planning_sequence_packet_sequence_areas.csv"),
    ("Download decisions CSV", decisions_csv_path, "phase22_phase20_network_transport_planning_sequence_packet_decisions.csv"),
    ("Download artifact families CSV", families_csv_path, "phase22_phase20_network_transport_planning_sequence_packet_artifact_families.csv"),
    ("Download git status CSV", git_status_csv_path, "phase22_phase20_network_transport_planning_sequence_packet_git_status.csv"),
    ("Download staged CSV", staged_csv_path, "phase22_phase20_network_transport_planning_sequence_packet_staged.csv"),
    ("Download source artifacts CSV", source_artifacts_csv_path, "phase22_phase20_network_transport_planning_sequence_packet_source_artifacts.csv"),
    ("Download checklist CSV", checklist_csv_path, "phase22_phase20_network_transport_planning_sequence_packet_checklist.csv"),
    ("Download issues CSV", issues_csv_path, "phase22_phase20_network_transport_planning_sequence_packet_issues.csv"),
]:
    if path.exists():
        st.download_button(label, path.read_text(encoding="utf-8-sig"), filename, "text/csv")

st.download_button(
    "Download planning sequence packet JSON",
    json.dumps(report, indent=2, default=str),
    "phase22_phase20_network_transport_planning_sequence_packet.json",
    "application/json",
)

with st.expander("Raw planning sequence packet JSON"):
    st.json(report)
