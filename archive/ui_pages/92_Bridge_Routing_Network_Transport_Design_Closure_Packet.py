...
st.set_page_config(page_title="Bridge Routing Network Transport Design Closure Packet", page_icon="🔒", layout="wide")
...
def latest_reports(limit: int = 20) -> list[Path]:
    ...
    return sorted(
        [
            p
            for p in BACKUP_DIR.glob("phase20_bridge_routing_network_transport_design_closure_packet_*")
            if p.is_dir() and (p / "phase20_bridge_routing_network_transport_design_closure_packet.json").exists()
        ],
...
st.title("🔒 Phase 20 Bridge Routing Network Transport Design Closure Packet")
st.caption("No-write design closure packet for the Phase 20 bridge routing network transport evidence chain.")

st.warning(
    "This page is network-transport-design-closure-packet-only. It shows design-closure evidence. "
    "It does not create design closure records, does not record final approvals, does not start an implementation phase, "
...
if not reports:
    st.info(
        "No bridge routing network transport design closure packet found yet. Run "
        "`scripts\\phase20_generate_bridge_routing_network_transport_design_closure_packet.ps1` from the platform repo."
    )
    st.stop()
...
json_path = report_dir / "phase20_bridge_routing_network_transport_design_closure_packet.json"
methods_csv_path = report_dir / "phase20_bridge_routing_network_transport_design_closure_packet_methods.csv"
checklist_csv_path = report_dir / "phase20_bridge_routing_network_transport_design_closure_packet_checklist.csv"
artifacts_csv_path = report_dir / "phase20_bridge_routing_network_transport_design_closure_packet_artifacts.csv"
issues_csv_path = report_dir / "phase20_bridge_routing_network_transport_design_closure_packet_issues.csv"
md_path = report_dir / "phase20_bridge_routing_network_transport_design_closure_packet.md"

report = load_json(json_path)
safety = report.get("safety", {})
packet = report.get("design_closure_packet", {})
...
if (
    safety.get("bridge_routing_network_transport_design_closure_packet_only")
    and safety.get("design_closure_packet_only")
    and safety.get("design_closure_preview_created")
    ...
    st.success("Safe design closure packet confirmed: no-write closure only, bridge GET only, no approval, no implementation phase, no transport, no bridge POST, no socket.")
else:
    st.error("Review design closure packet safety flags before continuing.")

if packet.get("status") == "design_closure_packet_blocked":
    st.error("Design closure packet is blocked. Resolve blockers before future closure or implementation planning.")
elif packet.get("status") == "design_closure_packet_review_required_no_write":
    st.warning("Design closure packet has review items. This still does not authorize approvals, implementation, or network transport.")
else:
    st.success("Design closure packet is ready for review. This still does not authorize approvals, implementation, or network transport.")

st.subheader("Design closure packet summary")
st.json(packet)
...
if issues:
    ...
    st.success("No design closure packet issues were recorded.")
...
for label, path, filename in [
    ("Download methods CSV", methods_csv_path, "phase20_bridge_routing_network_transport_design_closure_packet_methods.csv"),
    ("Download checklist CSV", checklist_csv_path, "phase20_bridge_routing_network_transport_design_closure_packet_checklist.csv"),
    ("Download artifacts CSV", artifacts_csv_path, "phase20_bridge_routing_network_transport_design_closure_packet_artifacts.csv"),
    ("Download issues CSV", issues_csv_path, "phase20_bridge_routing_network_transport_design_closure_packet_issues.csv"),
]:
    if path.exists():
        st.download_button(label, path.read_text(encoding="utf-8-sig"), filename, "text/csv")

st.download_button(
    "Download design closure packet JSON",
    json.dumps(report, indent=2, default=str),
    "phase20_bridge_routing_network_transport_design_closure_packet.json",
    "application/json",
)

with st.expander("Raw design closure packet JSON"):
    st.json(report)
