...
$PhaseNumber = 22
$StepNumber = 64
$StepName = "Phase 22 Step 65 - Phase 20 Network Transport Planning Cross-Repo Handoff Evidence Gap Disposition Planning Packet"
$PriorStepName = "Phase 22 Step 64 - Phase 20 Network Transport Planning Cross-Repo Handoff Evidence Gap Review Packet"
$ExpectedBranch = "phase22-step65-cross-repo-handoff-evidence-gap-disposition-planning"

$StepFiles = @(
    "scripts/phase22_step65_cross_repo_handoff_evidence_gap_disposition_planning_packet.ps1",
    "ui/pages/171_Phase20_Network_Transport_Planning_Cross_Repo_Handoff_Evidence_Gap_Disposition_Planning_Packet.py",
    "docs/PHASE22_STEP65_CROSS_REPO_HANDOFF_EVIDENCE_GAP_DISPOSITION_PLANNING_PACKET.md",
    "tests/test_phase22_step65_cross_repo_handoff_evidence_gap_disposition_planning_packet.py"
)
...
function Get-SafetyPosture {
    return [ordered]@{
        ...
        handoff_evidence_gap_disposition_planning_write = $false
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 22 Step 65 files copied or already present."
}
...
function Test-Smoke {
    ...
    if ($safety["handoff_evidence_gap_disposition_planning_write"]) { throw "SMOKE TEST FAIL: handoff evidence gap disposition planning write must remain disabled" }
    ...
    Write-Host "SMOKE TEST PASS: Phase 22 Step 65 Phase 20 Network Transport Planning Cross-Repo Handoff Evidence Gap Disposition Planning Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $packetPath = Join-SafePath -Root $backupDir -RelativePath ("phase22_step65_cross_repo_handoff_evidence_gap_disposition_planning_packet_{0}.json" -f $stamp)
    ...
    $packet = [ordered]@{
        ...
        planning_only = $true
        evidence_gap_disposition_planning_mode = "reference_only"
        handoff_state = "post_step64_gap_review_reference_only"
        phase23_start = "not_started"
        implementation_phase_start = "not_started"
        safety_posture = Get-SafetyPosture
        evidence_gap_disposition_planning_scope = @(
            "Plan reference-only disposition categories for evidence gaps reviewed through Step 64",
    ...
    Write-Host "PASS: evidence_gap_disposition_planning_mode=reference_only"
    Write-Host "PASS: handoff_evidence_gap_disposition_planning_write=false"
    ...
    Write-Host "CHECK: prior_step=Phase 22 Step 64"
    Write-Host "CHECK: phase23_start=not_started"
    Write-Host "CHECK: implementation_phase_start=not_started"
    Write-Host "CHECK: evidence_gap_disposition_planning_mode=reference_only"
...
function Show-ServerPlaceholder {
    ...
    Write-Host "CHECK: Evidence gap disposition planning is reference-only and does not collect, mutate, or validate sibling repositories."
}

function Show-Menu {
    while ($true) {
        Write-Host ""
        Write-Host "Phase 22 Step 65 menu"
        Write-Host "1. Show status / verify paths"
        Write-Host "2. Apply Phase 22 Step 65 Phase 20 Network Transport Planning Cross-Repo Handoff Evidence Gap Disposition Planning Packet files"
        Write-Host "3. Smoke test Phase 22 Step 65"
        Write-Host "4. Show server start placeholder only"
        Write-Host "5. Generate Phase 22 Step 65 cross-repo handoff evidence gap disposition planning packet"
        Write-Host "6. Exit"
