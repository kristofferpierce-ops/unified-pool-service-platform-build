...
$PhaseNumber = 36
$StepNumber = 19
$StepTitle = "Phase 36 Step 19 - Phase 20 Network Transport Implementation Trusted Production Controlled Live Write Expansion Guardrail Verification Closeout Index Packet"
$ExpectedBranch = "phase36-step19-guardrail-verification-closeout-index"
$PriorCompletedStep = "Phase 36 Step 18 - Phase 20 Network Transport Implementation Trusted Production Controlled Live Write Expansion Guardrail Verification Operator Hold Point Packet"
...
$StepFiles = @(
    "scripts/phase36_step19_controlled_live_write_expansion_guardrail_verification_closeout_index_packet.ps1",
    "ui/pages/1715_Phase36_Step19_Guardrail_Verification_Closeout_Index.py",
    "docs/PHASE36_STEP19_CONTROLLED_LIVE_WRITE_EXPANSION_GUARDRAIL_VERIFICATION_CLOSEOUT_INDEX_PACKET.md",
    "tests/test_phase36_step19_controlled_live_write_expansion_guardrail_verification_closeout_index_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase36_boundary = "trusted_production_controlled_live_write_expansion_guardrail_verification_closeout_index_opened_by_packet"
...
function Assert-StepFiles {
    foreach ($Rel in $StepFiles) {
        ...
        if (-not (Test-Path $Path)) {
            throw ("Missing expected Phase 36 Step 19 file: {0}" -f $Rel)
        }
    }
}
...
function Invoke-Apply {
    Assert-StepFiles
    Write-Host "APPLY PASS: Phase 36 Step 19 planning-only packet verified without live-write activation, live-write apply, bridge POST, sockets, DB mutation, server launch, or Phase 37 boundary creation."
}
...
function Invoke-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 36 Step 19 safety markers confirm planning-only/no-write/no-live-write-activation/no-live-write-apply/no-runtime/no-phase37 posture."
}
