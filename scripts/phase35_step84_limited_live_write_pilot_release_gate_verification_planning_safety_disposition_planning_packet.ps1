...
$StepNumber = 84
$StepTitle = "Phase 35 Step 84 - Phase 20 Network Transport Implementation Trusted Production Limited Live Write Pilot Release Gate Verification Planning Safety Disposition Planning Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Trusted Production Limited Live Write Pilot Release Gate Verification Planning Safety Disposition Planning Packet"
$ExpectedBranch = "phase35-step84-limited-live-write-pilot-release-gate-verification-planning-safety-disposition-planning"
$PriorStep = "Phase 35 Step 83 - Phase 20 Network Transport Implementation Trusted Production Limited Live Write Pilot Release Gate Verification Planning Evidence Gap Review Packet"
$Slug = "trusted_production_limited_live_write_pilot_release_gate_verification_planning_safety_disposition_planning"
$ContextKey = "trusted_production_limited_live_write_pilot_release_gate_verification_planning"
$PhaseContext = "trusted_production_limited_live_write_pilot_release_gate_verification_planning_only"

if (-not $RepoRoot) { $RepoRoot = (Resolve-Path (Join-Path $PSScriptRoot ".." )).Path }

$StepFiles = @(
    "scripts/phase35_step84_limited_live_write_pilot_release_gate_verification_planning_safety_disposition_planning_packet.ps1",
    "ui/pages/1660_Phase35_Step84_Live_Write_Pilot_Release_Gate_Verification_Safety_Planning.py",
    "docs/PHASE35_STEP84_LIMITED_LIVE_WRITE_PILOT_RELEASE_GATE_VERIFICATION_PLANNING_SAFETY_DISPOSITION_PLANNING_PACKET.md",
    "tests/test_phase35_step84_limited_live_write_pilot_release_gate_verification_planning_safety_disposition_planning_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase35_boundary = "trusted_production_limited_live_write_pilot_release_gate_verification_planning_safety_disposition_planning_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 35 Step 84 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 35 Step 84 Phase 20 Network Transport Implementation Trusted Production Limited Live Write Pilot Release Gate Verification Planning Safety Disposition Planning Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase35_step84_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase35_step84_{0}_packet.json" -f $Slug)
