...
$Phase = 22
$Step = 28
$StepTitle = "Phase 22 Step 28 - Phase 20 Network Transport Planning Pattern Detection Alignment Packet"
$ExpectedBranch = "phase22-step28-phase20-network-transport-planning-pattern-detection-alignment-packet"
$PriorCompletedStep = "Phase 22 Step 27 - Phase 20 Network Transport Planning Probabilistic Calibration Alignment Packet"
...
$StepFiles = @(
    [ordered]@{
        relative_path = "scripts\phase22_generate_phase20_network_transport_planning_pattern_detection_alignment_packet.ps1"
        description = "Phase 22 Step 28 path-safe planning launcher"
    },
    [ordered]@{
        relative_path = "ui\pages\134_Phase20_Network_Transport_Planning_Pattern_Detection_Alignment_Packet.py"
        description = "Streamlit planning page for pattern detection alignment"
    },
    [ordered]@{
        relative_path = "docs\PHASE22_STEP28_PHASE20_NETWORK_TRANSPORT_PLANNING_PATTERN_DETECTION_ALIGNMENT_PACKET.md"
        description = "Planning documentation for pattern detection alignment"
    },
    [ordered]@{
        relative_path = "tests\test_phase22_phase20_network_transport_planning_pattern_detection_alignment_packet.py"
        description = "Planning-only pytest coverage for Phase 22 Step 28"
    }
)
...
function Resolve-SourceRoot {
    ...
    if ($PSScriptRoot) {
        ...
        $DocPath = Join-PathSafe $ScriptParent "docs\PHASE22_STEP28_PHASE20_NETWORK_TRANSPORT_PLANNING_PATTERN_DETECTION_ALIGNMENT_PACKET.md"
        if (Test-LiteralPathSafe $DocPath) {
            return $ScriptParent
        }
    }

    $RepoDocPath = Join-PathSafe $ResolvedRepoRoot "docs\PHASE22_STEP28_PHASE20_NETWORK_TRANSPORT_PLANNING_PATTERN_DETECTION_ALIGNMENT_PACKET.md"
...
function Copy-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 22 Step 28 files copied or already present."
}
...
function Test-StepSmoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 22 Step 28 Phase 20 Network Transport Planning Pattern Detection Alignment Packet is present and planning-only."
}
...
function New-StepPacket {
    ...
    $OutputDirectory = Join-PathSafe $ResolvedRepoRoot "backups\phase22_phase20_network_transport_planning_pattern_detection_alignment_packet_$Timestamp"
    if (-not (Test-LiteralPathSafe $OutputDirectory)) {
        New-Item -ItemType Directory -Path $OutputDirectory -Force | Out-Null
    }

    $PacketPath = Join-PathSafe $OutputDirectory "phase22_phase20_network_transport_planning_pattern_detection_alignment_packet.json"

    $Packet = [ordered]@{
        packet_id = "phase22_step28_phase20_network_transport_planning_pattern_detection_alignment_packet"
        phase = 22
        step = 28
        title = $StepTitle
        ...
        pattern_detection_scope = [ordered]@{
            profitability_patterns = "planned_only"
            operational_inconsistencies = "planned_only"
            route_density_patterns = "planned_only"
            account_class_patterns = "planned_only"
            seasonal_chemical_patterns = "planned_only"
            labor_variance_patterns = "planned_only"
            vendor_price_variance_patterns = "planned_only"
            callback_risk_patterns = "planned_only"
        }
        non_actions = @(
    ...
    Write-Host "PASS: pattern_detection_runtime=false"
    Write-Host "CHECK: pattern_detection_stage=planning_alignment_only"
    Write-Host "CHECK: anomaly_detection_runtime=false"
    Write-Host "CHECK: recommendation_engine_runtime=false"
...
function Show-ServerPlaceholder {
    ...
    Write-Host "CHECK: Pattern detection, anomaly detection, and recommendation runtime remain disabled."
}
...
function Show-Menu {
    ...
    while ($true) {
        Write-Host ""
        Write-Host "Phase 22 Step 28 menu"
        Write-Host "1. Show status / verify paths"
        Write-Host "2. Apply Phase 22 Step 28 Phase 20 Network Transport Planning Pattern Detection Alignment Packet files"
        Write-Host "3. Smoke test Phase 22 Step 28"
        Write-Host "4. Show server start placeholder only"
        Write-Host "5. Generate Phase 20 network transport planning pattern detection alignment packet"
        Write-Host "6. Exit"
