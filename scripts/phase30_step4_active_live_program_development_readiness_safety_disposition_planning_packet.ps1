...
$StepNumber = 4
$StepTitle = "Phase 30 Step 4 - Phase 20 Network Transport Implementation Active Live Program Development Readiness Safety Disposition Planning Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Active Live Program Development Readiness Safety Disposition Planning Packet"
$ExpectedBranch = "phase30-step4-active-live-program-development-readiness-safety-disposition-planning"
$PriorStep = "Phase 30 Step 3 - Phase 20 Network Transport Implementation Active Live Program Development Readiness Evidence Gap Review Packet"
$Slug = "implementation_active_live_program_development_readiness_safety_disposition_planning"
...
$StepFiles = @(
    "scripts/phase30_step4_active_live_program_development_readiness_safety_disposition_planning_packet.ps1",
    "ui/pages/980_Phase30_Step4_Implementation_Active_Live_Program_Development_Readiness_Safety_Disposition_Planning_Packet.py",
    "docs/PHASE30_STEP4_ACTIVE_LIVE_PROGRAM_DEVELOPMENT_READINESS_SAFETY_DISPOSITION_PLANNING_PACKET.md",
    "tests/test_phase30_step4_active_live_program_development_readiness_safety_disposition_planning_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase30_boundary = "implementation_active_live_program_development_readiness_safety_disposition_planning_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 30 Step 4 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 30 Step 4 Phase 20 Network Transport Implementation Active Live Program Development Readiness Safety Disposition Planning Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase30_step4_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase30_step4_{0}_packet.json" -f $Slug)
