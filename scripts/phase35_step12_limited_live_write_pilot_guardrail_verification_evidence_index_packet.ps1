...
$StepNumber = 12
$StepTitle = "Phase 35 Step 12 - Phase 20 Network Transport Implementation Trusted Production Limited Live Write Pilot Guardrail Verification Evidence Index Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Trusted Production Limited Live Write Pilot Guardrail Verification Evidence Index Packet"
$ExpectedBranch = "phase35-step12-limited-live-write-pilot-guardrail-verification-evidence-index"
$PriorStep = "Phase 35 Step 11 - Phase 20 Network Transport Implementation Trusted Production Limited Live Write Pilot Guardrail Verification Boundary Packet"
$Slug = "trusted_production_limited_live_write_pilot_guardrail_verification_evidence_index"
...
$StepFiles = @(
    "scripts/phase35_step12_limited_live_write_pilot_guardrail_verification_evidence_index_packet.ps1",
    "ui/pages/1588_Phase35_Step12_Live_Write_Pilot_Guardrail_Evidence_Index.py",
    "docs/PHASE35_STEP12_LIMITED_LIVE_WRITE_PILOT_GUARDRAIL_VERIFICATION_EVIDENCE_INDEX_PACKET.md",
    "tests/test_phase35_step12_limited_live_write_pilot_guardrail_verification_evidence_index_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase35_boundary = "trusted_production_limited_live_write_pilot_guardrail_verification_evidence_index_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 35 Step 12 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 35 Step 12 Phase 20 Network Transport Implementation Trusted Production Limited Live Write Pilot Guardrail Verification Evidence Index Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase35_step12_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase35_step12_{0}_packet.json" -f $Slug)
