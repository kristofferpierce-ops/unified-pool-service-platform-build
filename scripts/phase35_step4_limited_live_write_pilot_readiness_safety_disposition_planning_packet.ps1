...
$StepNumber = 4
$StepTitle = "Phase 35 Step 4 - Phase 20 Network Transport Implementation Trusted Production Limited Live Write Pilot Readiness Safety Disposition Planning Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Trusted Production Limited Live Write Pilot Readiness Safety Disposition Planning Packet"
$ExpectedBranch = "phase35-step4-limited-live-write-pilot-readiness-safety-planning"
$PriorStep = "Phase 35 Step 3 - Phase 20 Network Transport Implementation Trusted Production Limited Live Write Pilot Readiness Evidence Gap Review Packet"
$Slug = "trusted_production_limited_live_write_pilot_readiness_safety_disposition_planning"
...
$StepFiles = @(
    "scripts/phase35_step4_limited_live_write_pilot_readiness_safety_disposition_planning_packet.ps1",
    "ui/pages/1580_Phase35_Step4_Limited_Live_Write_Pilot_Readiness_Safety_Planning.py",
    "docs/PHASE35_STEP4_LIMITED_LIVE_WRITE_PILOT_READINESS_SAFETY_PLANNING_PACKET.md",
    "tests/test_phase35_step4_limited_live_write_pilot_readiness_safety_disposition_planning_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase35_boundary = "trusted_production_limited_live_write_pilot_readiness_safety_disposition_planning_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 35 Step 4 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 35 Step 4 Phase 20 Network Transport Implementation Trusted Production Limited Live Write Pilot Readiness Safety Disposition Planning Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase35_step4_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase35_step4_{0}_packet.json" -f $Slug)
