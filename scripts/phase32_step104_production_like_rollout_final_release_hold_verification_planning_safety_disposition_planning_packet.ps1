...
$StepNumber = 104
$StepTitle = "Phase 32 Step 104 - Phase 20 Network Transport Implementation Production-Like Rollout Final Release Hold Verification Planning Safety Disposition Planning Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Production-Like Rollout Final Release Hold Verification Planning Safety Disposition Planning Packet"
$ExpectedBranch = "phase32-step104-production-like-rollout-final-release-hold-verification-planning-safety-disposition-planning"
$PriorStep = "Phase 32 Step 103 - Phase 20 Network Transport Implementation Production-Like Rollout Final Release Hold Verification Planning Evidence Gap Review Packet"
$Slug = "implementation_production_like_rollout_final_release_hold_verification_planning_safety_disposition_planning"
$ContextKey = "implementation_production_like_rollout_final_release_hold_verification_planning"
$PhaseContext = "implementation_production_like_rollout_final_release_hold_verification_planning_planning_only"

if (-not $RepoRoot) { $RepoRoot = (Resolve-Path (Join-Path $PSScriptRoot ".." )).Path }

$StepFiles = @(
    "scripts/phase32_step104_production_like_rollout_final_release_hold_verification_planning_safety_disposition_planning_packet.ps1",
    "ui/pages/1320_Phase32_Step104_Implementation_Production_Like_Rollout_Final_Release_Hold_Verification_Planning_Safety_Disposition_Planning_Packet.py",
    "docs/PHASE32_STEP104_PRODUCTION_LIKE_ROLLOUT_FINAL_RELEASE_HOLD_VERIFICATION_PLANNING_SAFETY_DISPOSITION_PLANNING_PACKET.md",
    "tests/test_phase32_step104_production_like_rollout_final_release_hold_verification_planning_safety_disposition_planning_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase32_boundary = "implementation_production_like_rollout_final_release_hold_verification_planning_safety_disposition_planning_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 32 Step 104 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 32 Step 104 Phase 20 Network Transport Implementation Production-Like Rollout Final Release Hold Verification Planning Safety Disposition Planning Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase32_step104_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase32_step104_{0}_packet.json" -f $Slug)
