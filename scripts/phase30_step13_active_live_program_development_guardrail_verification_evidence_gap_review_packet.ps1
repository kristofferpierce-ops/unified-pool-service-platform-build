...
$StepNumber = 13
$StepTitle = "Phase 30 Step 13 - Phase 20 Network Transport Implementation Active Live Program Development Guardrail Verification Evidence Gap Review Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Active Live Program Development Guardrail Verification Evidence Gap Review Packet"
$ExpectedBranch = "phase30-step13-active-live-program-development-guardrail-verification-evidence-gap-review"
$PriorStep = "Phase 30 Step 12 - Phase 20 Network Transport Implementation Active Live Program Development Guardrail Verification Evidence Index Packet"
$Slug = "implementation_active_live_program_development_guardrail_verification_evidence_gap_review"
...
$StepFiles = @(
    "scripts/phase30_step13_active_live_program_development_guardrail_verification_evidence_gap_review_packet.ps1",
    "ui/pages/989_Phase30_Step13_Implementation_Active_Live_Program_Development_Guardrail_Verification_Evidence_Gap_Review_Packet.py",
    "docs/PHASE30_STEP13_ACTIVE_LIVE_PROGRAM_DEVELOPMENT_GUARDRAIL_VERIFICATION_EVIDENCE_GAP_REVIEW_PACKET.md",
    "tests/test_phase30_step13_active_live_program_development_guardrail_verification_evidence_gap_review_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase30_boundary = "implementation_active_live_program_development_guardrail_verification_evidence_gap_review_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 30 Step 13 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 30 Step 13 Phase 20 Network Transport Implementation Active Live Program Development Guardrail Verification Evidence Gap Review Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase30_step13_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase30_step13_{0}_packet.json" -f $Slug)
