...
$StepNumber = 36
$PhaseNumber = 22
$StepTitle = "Phase 22 Step 37 - Phase 20 Network Transport Planning Handoff Dossier Alignment Packet"
$ExpectedBranch = "phase22-step37-phase20-network-transport-planning-exit-readiness-alignment-packet"
$PriorCompletedStep = "Phase 22 Step 36 - Phase 20 Network Transport Planning Exit Readiness Alignment Packet"
$PacketSlug = "phase22_phase20_network_transport_planning_handoff_dossier_alignment_packet"
...
$HandoffDossierAlignment = [ordered]@{
    handoff_dossier_scope = "planning_packet_only"
    planning_lane_status = "ready_for_handoff_review_not_approved"
...
function Show-Status {
    ...
    Write-Host ""
    Write-Host "Handoff dossier alignment"
    foreach ($Key in $HandoffDossierAlignment.Keys) {
        Write-Host ("  {0}: {1}" -f $Key, $HandoffDossierAlignment[$Key])
    }
    Write-Host ""
...
function Copy-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 22 Step 37 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 22 Step 37 Phase 20 Network Transport Planning Handoff Dossier Alignment Packet is present and planning-only."
}
...
function New-PlanningPacket {
    ...
    $Packet = [ordered]@{
        ...
        live_write_disabled = $true
        live_write_unarmed = $true
        handoff_dossier_alignment = $HandoffDossierAlignment
    ...
    Write-Host "CHECK: planning_lane_status=ready_for_handoff_review_not_approved"
...
function Show-ServerPlaceholder {
    ...
    Write-Host "CHECK: Handoff dossier alignment is documented only and does not approve, queue, or start implementation."
}
...
while ($true) {
    Write-Host ""
    Write-Host "Phase 22 Step 37 menu"
    Write-Host "1. Show status / verify paths"
    Write-Host "2. Apply Phase 22 Step 37 Phase 20 Network Transport Planning Handoff Dossier Alignment Packet files"
    Write-Host "3. Smoke test Phase 22 Step 37"
    Write-Host "4. Show server start placeholder only"
    Write-Host "5. Generate Phase 20 network transport planning handoff dossier alignment packet"
