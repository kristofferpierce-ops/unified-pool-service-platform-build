...
$StepNumber = 6
$StepTitle = "Phase 29 Step 6 - Phase 20 Network Transport Implementation Limited Live Pilot Readiness Approval Boundary Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Limited Live Pilot Readiness Approval Boundary Packet"
$ExpectedBranch = "phase29-step6-implementation-limited-live-pilot-readiness-approval-boundary"
$PriorStep = "Phase 29 Step 5 - Phase 20 Network Transport Implementation Limited Live Pilot Readiness Safety Disposition Review Packet"
$Slug = "implementation_limited_live_pilot_readiness_approval_boundary"

if (-not $RepoRoot) { $RepoRoot = (Resolve-Path (Join-Path $PSScriptRoot ".." )).Path }

$StepFiles = @(
    "scripts/phase29_step6_implementation_limited_live_pilot_readiness_approval_boundary_packet.ps1",
    "ui/pages/862_Phase29_Step6_Implementation_Limited_Live_Pilot_Readiness_Approval_Boundary_Packet.py",
    "docs/PHASE29_STEP6_IMPLEMENTATION_LIMITED_LIVE_PILOT_READINESS_APPROVAL_BOUNDARY_PACKET.md",
    "tests/test_phase29_step6_implementation_limited_live_pilot_readiness_approval_boundary_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase29_boundary = "implementation_limited_live_pilot_readiness_approval_boundary_opened_by_packet"
    ...
    implementation_limited_live_pilot_readiness_approval_boundary_mode = "reference_only"
    implementation_limited_live_pilot_readiness_approval_boundary_write = $false
    implementation_limited_live_pilot_readiness_approval_boundary_record_creation = $false
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 29 Step 6 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 29 Step 6 Phase 20 Network Transport Implementation Limited Live Pilot Readiness Approval Boundary Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase29_step6_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase29_step6_{0}_packet.json" -f $Slug)
    ...
    foreach ($Key in @(
        ...
        "cross_repo_mutation",
        "external_repo_push",
        "implementation_limited_live_pilot_readiness_approval_boundary_mode",
        "implementation_limited_live_pilot_readiness_approval_boundary_write",
        "implementation_limited_live_pilot_readiness_approval_boundary_record_creation",
