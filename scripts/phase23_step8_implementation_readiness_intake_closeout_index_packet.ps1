...
$PhaseNumber = 23
$StepNumber = 8
$StepName = "Phase 23 Step 8 - Phase 20 Network Transport Implementation Readiness Intake Closeout Index Packet"
$PriorStepName = "Phase 23 Step 7 - Phase 20 Network Transport Implementation Readiness Intake Evidence Gap Disposition Approval Readiness Packet"
$ExpectedBranch = "phase23-step8-readiness-intake-closeout-index"

$StepFiles = @(
    "scripts/phase23_step8_implementation_readiness_intake_closeout_index_packet.ps1",
    "ui/pages/184_Phase23_Step8_Implementation_Readiness_Intake_Closeout_Index_Packet.py",
    "docs/PHASE23_STEP8_IMPLEMENTATION_READINESS_INTAKE_CLOSEOUT_INDEX_PACKET.md",
    "tests/test_phase23_step8_implementation_readiness_intake_closeout_index_packet.py"
)
...
function Get-SafetyPosture {
    return [ordered]@{
        ...
        readiness_intake_closeout_index_mode = "reference_only"
        readiness_intake_closeout_index_write = $false
        readiness_intake_closeout_index_record_creation = $false
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 23 Step 8 files copied or already present."
}
...
function Test-Smoke {
    ...
    if ($safety["readiness_intake_closeout_index_write"]) { throw "SMOKE TEST FAIL: readiness intake closeout index writes must remain disabled" }
    if ($safety["readiness_intake_closeout_index_record_creation"]) { throw "SMOKE TEST FAIL: readiness intake closeout index record creation must remain disabled" }
    ...
    Write-Host "SMOKE TEST PASS: Phase 23 Step 8 Phase 20 Network Transport Implementation Readiness Intake Closeout Index Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $packetDir = Join-SafePath -Root $backupRoot -RelativePath ("phase23_step8_implementation_readiness_intake_closeout_index_packet_{0}" -f $stamp)
    if (-not (Test-SafePath -Path $packetDir)) { New-Item -ItemType Directory -Path $packetDir -Force | Out-Null }
    $packetPath = Join-SafePath -Root $packetDir -RelativePath "phase23_step8_implementation_readiness_intake_closeout_index_packet.json"
    $packet = [ordered]@{
        phase = 23
        step = 8
        step_name = $StepName
        ...
        intake_scope = @(
            ...
            "Open Phase 23 as a planning intake closeout index only",
    ...
    Write-Host "PASS: readiness_intake_closeout_index_mode=reference_only"
    Write-Host "PASS: readiness_intake_closeout_index_write=false"
    Write-Host "PASS: readiness_intake_closeout_index_record_creation=false"
    ...
    Write-Host "CHECK: prior_step=Phase 23 Step 7"
...
function Show-ServerPlaceholder {
    ...
    Write-Host "CHECK: Phase 23 Step 8 is planning intake only and does not start network transport implementation."
...
function Show-Menu {
    while ($true) {
        Write-Host ""
        Write-Host "Phase 23 Step 8 menu"
        Write-Host "1. Show status / verify paths"
        Write-Host "2. Apply Phase 23 Step 8 Phase 20 Network Transport Implementation Readiness Intake Closeout Index Packet files"
        Write-Host "3. Smoke test Phase 23 Step 8"
        Write-Host "4. Show server start placeholder only"
        Write-Host "5. Generate Phase 23 Step 8 implementation readiness intake closeout index packet"
        Write-Host "6. Exit"
