...
$StepNumber = 91
$StepTitle = "Phase 32 Step 91 - Phase 20 Network Transport Implementation Production-Like Rollout Final Release Hold Planning Boundary Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Production-Like Rollout Final Release Hold Planning Boundary Packet"
$ExpectedBranch = "phase32-step91-production-like-rollout-final-release-hold-planning-boundary"
$PriorStep = "Phase 32 Step 90 - Phase 20 Network Transport Implementation Production-Like Rollout Release Gate Verification Planning Final Boundary Confirmation Packet"
$Slug = "implementation_production_like_rollout_final_release_hold_planning_boundary"
...
$StepFiles = @(
    "scripts/phase32_step91_production_like_rollout_final_release_hold_planning_boundary_packet.ps1",
    "ui/pages/1307_Phase32_Step91_Implementation_Production_Like_Rollout_Final_Release_Hold_Planning_Boundary_Packet.py",
    "docs/PHASE32_STEP91_PRODUCTION_LIKE_ROLLOUT_FINAL_RELEASE_HOLD_PLANNING_BOUNDARY_PACKET.md",
    "tests/test_phase32_step91_production_like_rollout_final_release_hold_planning_boundary_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase32_boundary = "implementation_production_like_rollout_final_release_hold_planning_boundary_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 32 Step 91 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 32 Step 91 Phase 20 Network Transport Implementation Production-Like Rollout Final Release Hold Planning Boundary Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase32_step91_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase32_step91_{0}_packet.json" -f $Slug)
