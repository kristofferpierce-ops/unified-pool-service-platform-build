param(
    ...
    [string]$PlanningBoundaryPacketDir = "",
    [string]$PlanningEntryPacketDir = "",
...
function Classify-RepoPath {
    ...
    if ($CurrentStepFiles -contains $Path) {
        return [ordered]@{
            ...
            should_stage = $true
            reason = "Current Phase 22 Step 3 file."
        }
    }
...
if ([string]::IsNullOrWhiteSpace($PlanningBoundaryPacketDir)) {
    $PlanningBoundaryPacketDir = Get-LatestArtifactDir -FolderFilter "phase22_phase20_network_transport_planning_boundary_packet_*" -JsonName "phase22_phase20_network_transport_planning_boundary_packet.json" -Required $true
}
if ([string]::IsNullOrWhiteSpace($PlanningEntryPacketDir)) {
    $PlanningEntryPacketDir = Get-LatestArtifactDir -FolderFilter "phase22_phase20_network_transport_planning_entry_packet_*" -JsonName "phase22_phase20_network_transport_planning_entry_packet.json" -Required $false
}
...
$planningBoundaryPath = Join-Path $PlanningBoundaryPacketDir "phase22_phase20_network_transport_planning_boundary_packet.json"
$planningEntryPath = if (![string]::IsNullOrWhiteSpace($PlanningEntryPacketDir)) { Join-Path $PlanningEntryPacketDir "phase22_phase20_network_transport_planning_entry_packet.json" } else { "" }
...
if (!(Test-Path -LiteralPath $planningBoundaryPath)) {
    throw "Planning boundary packet JSON not found: $planningBoundaryPath"
}

$planningBoundary = Read-JsonFile $planningBoundaryPath
$planningEntry = if (![string]::IsNullOrWhiteSpace($planningEntryPath) -and (Test-Path -LiteralPath $planningEntryPath)) { Read-JsonFile $planningEntryPath } else { $null }
...
$scopeSections = New-Object System.Collections.ArrayList
$scopeEntries = New-Object System.Collections.ArrayList
$scopeAreas = New-Object System.Collections.ArrayList
...
Add-ChecklistItem $checklist "artifact" "Planning boundary packet exists" $true $planningBoundaryPath
Add-ChecklistItem $checklist "artifact" "Planning entry packet exists" ($planningEntry -ne $null) $planningEntryPath "recommended"
Add-ChecklistItem $checklist "artifact" "Cleanup exit packet exists" ($cleanupExit -ne $null) $cleanupExitPath "recommended"

Add-ChecklistItem $checklist "planning_boundary" "Planning boundary packet is scope-safe" ($planningBoundary.safety.phase20_network_transport_planning_boundary_packet_only -eq $true) "phase20_network_transport_planning_boundary_packet_only=$($planningBoundary.safety.phase20_network_transport_planning_boundary_packet_only)"
Add-ChecklistItem $checklist "planning_boundary" "Planning boundary created no closure record" ($planningBoundary.safety.design_closure_record_created -eq $false) "design_closure_record_created=$($planningBoundary.safety.design_closure_record_created)"
Add-ChecklistItem $checklist "planning_boundary" "Planning boundary recorded no final approval" ($planningBoundary.safety.final_approval_recorded -eq $false) "final_approval_recorded=$($planningBoundary.safety.final_approval_recorded)"
Add-ChecklistItem $checklist "planning_boundary" "Planning boundary started no implementation phase" ($planningBoundary.safety.implementation_phase_started -eq $false) "implementation_phase_started=$($planningBoundary.safety.implementation_phase_started)"
Add-ChecklistItem $checklist "planning_boundary" "Planning boundary has no execution implementation" ($planningBoundary.safety.execution_implementation_created -eq $false) "execution_implementation_created=$($planningBoundary.safety.execution_implementation_created)"
Add-ChecklistItem $checklist "planning_boundary" "Planning boundary has no network transport" ($planningBoundary.safety.network_transport_implemented -eq $false) "network_transport_implemented=$($planningBoundary.safety.network_transport_implemented)"
Add-ChecklistItem $checklist "planning_boundary" "Planning boundary has no socket opened" ($planningBoundary.safety.network_socket_opened -eq $false) "network_socket_opened=$($planningBoundary.safety.network_socket_opened)"
Add-ChecklistItem $checklist "planning_boundary" "Planning boundary has no bridge POST" ($planningBoundary.safety.bridge_post_called -eq $false -and $planningBoundary.safety.bridge_post_call_implemented -eq $false) "bridge_post_called=$($planningBoundary.safety.bridge_post_called); bridge_post_call_implemented=$($planningBoundary.safety.bridge_post_call_implemented)"
Add-ChecklistItem $checklist "planning_boundary" "Planning boundary has no platform DB mutation" ($planningBoundary.safety.platform_db_mutation_performed -eq $false) "platform_db_mutation_performed=$($planningBoundary.safety.platform_db_mutation_performed)"
Add-ChecklistItem $checklist "planning_boundary" "Planning boundary has no bridge mutation" ($planningBoundary.safety.bridge_mutation_performed -eq $false) "bridge_mutation_performed=$($planningBoundary.safety.bridge_mutation_performed)"
Add-ChecklistItem $checklist "planning_boundary" "Planning boundary has no LACRM call" ($planningBoundary.safety.lacrm_call_performed -eq $false) "lacrm_call_performed=$($planningBoundary.safety.lacrm_call_performed)"
Add-ChecklistItem $checklist "planning_boundary" "Planning boundary has no blockers" ([int]$planningBoundary.planning_boundary_packet.blocker_count -eq 0) "blocker_count=$($planningBoundary.planning_boundary_packet.blocker_count)"

if ($planningEntry -ne $null) {
    Add-ChecklistItem $checklist "planning_entry" "Planning entry packet is planning-only" ($planningEntry.safety.planning_only -eq $true) "planning_only=$($planningEntry.safety.planning_only)" "recommended"
}
...
$boundaryPrefixes = @($planningBoundary.planning_entries | Where-Object { $_.category -eq "do_not_stage" } | ForEach-Object { [string]$_.item })
if (@($boundaryPrefixes).Count -gt 0) {
    $doNotStagePrefixes = $boundaryPrefixes
}

$currentStepFiles = @(
    "scripts/phase22_generate_phase20_network_transport_planning_scope_packet.ps1",
    "ui/pages/109_Phase20_Network_Transport_Planning_Scope_Packet.py",
    "docs/PHASE22_STEP3_PHASE20_NETWORK_TRANSPORT_PLANNING_SCOPE_PACKET.md",
    "tests/test_phase22_phase20_network_transport_planning_scope_packet.py"
)

$currentBranch = (& git -C $PlatformDir branch --show-current 2>$null | Out-String).Trim()

# Planning scope uses git status --short against the platform repo.
...
if ($planningBoundary.artifact_families) {
    foreach ($family in @($planningBoundary.artifact_families)) {
        [void]$artifactFamilies.Add($family)
    }
}
if ($planningBoundary.archive_decisions) {
    foreach ($decision in @($planningBoundary.archive_decisions)) {
        [void]$archiveDecisions.Add($decision)
    }
}

$scopeAreaSpecs = @(
    [ordered]@{ key="execution_boundary"; item="Execution boundary"; summary="Define execution interface and request/response envelopes only."; implementation_allowed=$false; bridge_post_allowed=$false; socket_allowed=$false },
    [ordered]@{ key="transport_adapter_boundary"; item="Transport adapter boundary"; summary="Define adapter seams and dependency injection only."; implementation_allowed=$false; bridge_post_allowed=$false; socket_allowed=$false },
    [ordered]@{ key="bridge_post_boundary"; item="Bridge POST boundary"; summary="Define POST contract and retry expectations only."; implementation_allowed=$false; bridge_post_allowed=$false; socket_allowed=$false },
    [ordered]@{ key="response_capture_boundary"; item="Response capture boundary"; summary="Define response capture shape and persistence prerequisites only."; implementation_allowed=$false; bridge_post_allowed=$false; socket_allowed=$false },
    [ordered]@{ key="cutover_gate_boundary"; item="Cutover gate boundary"; summary="Define cutover prerequisites, approvals, and rollback gates only."; implementation_allowed=$false; bridge_post_allowed=$false; socket_allowed=$false },
    [ordered]@{ key="audit_rollback_boundary"; item="Audit and rollback boundary"; summary="Define audit/rollback record requirements only."; implementation_allowed=$false; bridge_post_allowed=$false; socket_allowed=$false }
)

foreach ($spec in $scopeAreaSpecs) {
    [void]$scopeAreas.Add([ordered]@{
        key = $spec.key
        item = $spec.item
    ...
    [void]$scopeEntries.Add([ordered]@{
        category = "scope_areas"
        action = "review_only"
        item = $spec.item
...
[void]$scopeSections.Add([ordered]@{
    section = "Planning scope state"
    status = "review"
    summary = "Phase 22 Step 3 remains planning-only and converts planning boundaries into explicit review-only scope areas."
})
[void]$scopeSections.Add([ordered]@{
    section = "Scope areas"
    status = "review"
    summary = "Execution, transport adapter, bridge POST, response capture, cutover, and audit/rollback remain review-only."
})
[void]$scopeSections.Add([ordered]@{
...
foreach ($file in $currentStepFiles) {
    [void]$scopeEntries.Add([ordered]@{
        ...
        action = "stage"
        item = $file
        evidence = "Current Phase 22 Step 3 file."
        required = "yes"
    })
}
...
$nextCommitCommands = @(
    ...
    "git status --short",
    "git add scripts/phase22_generate_phase20_network_transport_planning_scope_packet.ps1",
    "git add ui/pages/109_Phase20_Network_Transport_Planning_Scope_Packet.py",
    "git add docs/PHASE22_STEP3_PHASE20_NETWORK_TRANSPORT_PLANNING_SCOPE_PACKET.md",
    "git add tests/test_phase22_phase20_network_transport_planning_scope_packet.py",
    "git diff --cached --name-only",
    'git commit -m "Phase 22 Step 3 phase20 network transport planning scope packet"',
    "git push -u origin phase22-step3-phase20-network-transport-planning-scope-packet"
)

Add-ChecklistItem $checklist "planning_scope" "Current branch detected" (-not [string]::IsNullOrWhiteSpace($currentBranch)) "current_branch=$currentBranch"
Add-ChecklistItem $checklist "planning_scope" "At least one planning-scope section exists" (@($scopeSections).Count -gt 0) "planning_scope_sections=$(@($scopeSections).Count)"
Add-ChecklistItem $checklist "planning_scope" "Exactly four stage-now entries exist" (@($scopeEntries | Where-Object { $_.category -eq "stage_now" }).Count -eq 4) "stage_now_entries=$(@($scopeEntries | Where-Object { $_.category -eq "stage_now" }).Count)"
Add-ChecklistItem $checklist "planning_scope" "Scope areas were generated" (@($scopeAreas).Count -ge 6) "scope_areas=$(@($scopeAreas).Count)" "recommended"
Add-ChecklistItem $checklist "planning_scope" "Commit commands exist" (@($nextCommitCommands).Count -gt 0) "commit_commands=$(@($nextCommitCommands).Count)"
Add-ChecklistItem $checklist "planning_scope" "No forbidden paths are currently staged" ($stagedForbiddenCount -eq 0) "staged_forbidden_count=$stagedForbiddenCount"
Add-ChecklistItem $checklist "planning_scope" "No review-other paths are currently staged" ($stagedReviewOtherCount -eq 0) "staged_review_other_count=$stagedReviewOtherCount" "recommended"
Add-ChecklistItem $checklist "planning_scope" "Workspace review-other files count is zero or acceptable" ($reviewOtherCount -eq 0) "review_other_count=$reviewOtherCount" "recommended"

$planningBoundaryStatus = [string]$planningBoundary.planning_boundary_packet.status
if ($planningBoundaryStatus -eq "phase20_planning_boundary_packet_blocked") {
    Add-Issue $issues "blocker" "planning_boundary_packet_blocked" "Planning boundary packet is blocked." "planning_boundary"
} elseif ($planningBoundaryStatus -eq "phase20_planning_boundary_packet_review_required_no_write") {
    Add-Issue $issues "review" "planning_boundary_packet_review_required" "Planning boundary packet has review items." "planning_boundary"
}

if ([string]::IsNullOrWhiteSpace($currentBranch)) {
    Add-Issue $issues "blocker" "current_branch_missing" "Current branch was not detected for planning scope." "workspace"
}
...
[void]$sourceArtifacts.Add([ordered]@{ key="planning_boundary_packet"; json_path=$planningBoundaryPath; sha256=Get-FileSha256 $planningBoundaryPath })
[void]$sourceArtifacts.Add([ordered]@{ key="planning_entry_packet"; json_path=$planningEntryPath; sha256=Get-FileSha256 $planningEntryPath })
...
$packetStatus = if ($blockerCount -eq 0 -and $reviewCount -eq 0) {
    "phase20_planning_scope_packet_ready_no_write"
} elseif ($blockerCount -eq 0) {
    "phase20_planning_scope_packet_review_required_no_write"
} else {
    "phase20_planning_scope_packet_blocked"
}

if ([string]::IsNullOrWhiteSpace($OutputDir)) {
    $stamp = Get-Date -Format "yyyyMMdd-HHmmss"
    $OutputDir = Join-Path $BackupDir "phase22_phase20_network_transport_planning_scope_packet_$stamp"
}
...
$report = [ordered]@{
    ...
    phase = "Phase 22 Step 3"
    purpose = "Phase 20 bridge routing network transport planning scope packet"
    workspace = $Workspace
    platform_api = $PlatformApi
    bridge_url = $BridgeUrl
    safety = [ordered]@{
        phase20_network_transport_planning_scope_packet_only = $true
    ...
    planning_scope_packet = [ordered]@{
        status = $packetStatus
        blocker_count = $blockerCount
        review_count = $reviewCount
        current_branch = $currentBranch
        section_count = @($scopeSections).Count
        entry_count = @($scopeEntries).Count
        scope_area_count = @($scopeAreas).Count
        ...
        packet_label = "phase22_phase20_network_transport_planning_scope_no_write"
        reason = if ($blockerCount -eq 0) {
            "Phase 22 Step 3 created a no-write planning scope packet for the Phase 20 network transport chain. It does not authorize implementation, transport, sockets, bridge POST, response capture, mutations, LACRM calls, or live writes."
        } else {
            "Phase 22 Step 3 found blockers in the Phase 20 planning scope packet. Resolve blockers before continuing planning."
        }
    }
    planning_sections = @($scopeSections)
    planning_entries = @($scopeEntries)
    ...
    counts = [ordered]@{
        sections = @($scopeSections).Count
        entries = @($scopeEntries).Count
        scope_areas = @($scopeAreas).Count
    ...
    next_recommended_actions = @(
        "Review this planning scope packet before any Phase 22 planning detail packets are drafted.",
        "Keep the latest packet folder in each family and only review older generated folders for manual archive actions.",
        "Use the stage-now entries as the only files to add for the Phase 22 Step 3 commit.",
        "Carry the do-not-stage entries forward as the hard commit boundary.",
        "Do not add requests, httpx, socket use, bridge POST calls, or routing write endpoints from Phase 22 Step 3.",
        "Do not call interface execution methods from Phase 22 Step 3.",
        "Do not record final approvals from Phase 22 Step 3.",
        "Do not start an implementation phase from Phase 22 Step 3.",
        "Review scope areas only: execution boundary, transport adapter boundary, bridge POST boundary, response capture boundary, cutover gate boundary, and audit/rollback boundary."
    )
}

$jsonPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_scope_packet.json"
$sectionsCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_scope_packet_sections.csv"
$entriesCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_scope_packet_entries.csv"
$scopeCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_scope_packet_scope_areas.csv"
$decisionsCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_scope_packet_decisions.csv"
$familiesCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_scope_packet_artifact_families.csv"
$gitStatusCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_scope_packet_git_status.csv"
$stagedCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_scope_packet_staged.csv"
$sourceArtifactsCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_scope_packet_source_artifacts.csv"
$checklistCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_scope_packet_checklist.csv"
$issuesCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_scope_packet_issues.csv"
$mdPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_scope_packet.md"

$report | ConvertTo-Json -Depth 100 | Set-Content -LiteralPath $jsonPath -Encoding UTF8
$scopeSections | Export-Csv -LiteralPath $sectionsCsvPath -NoTypeInformation -Encoding UTF8
$scopeEntries | Export-Csv -LiteralPath $entriesCsvPath -NoTypeInformation -Encoding UTF8
$scopeAreas | Export-Csv -LiteralPath $scopeCsvPath -NoTypeInformation -Encoding UTF8
...
$sectionText = (@($scopeSections) | ForEach-Object { "- $($_.section): $($_.status); $($_.summary)" }) -join "`n"
$entryText = (@($scopeEntries) | ForEach-Object { "- [$($_.category)] $($_.action) $($_.item): $($_.evidence)" }) -join "`n"
$scopeText = (@($scopeAreas) | ForEach-Object { "- $($_.item): status=$($_.status); implementation_allowed=$($_.implementation_allowed); bridge_post_allowed=$($_.bridge_post_allowed)" }) -join "`n"
$decisionText = if (@($archiveDecisions).Count -gt 0) { (@($archiveDecisions) | ForEach-Object { "- $($_.folder_filter): decision=$($_.decision); older_generated_folders=$($_.older_generated_folder_count); archive_order=$($_.archive_order)" }) -join "`n" } else { "- none" }
$familyText = if (@($artifactFamilies).Count -gt 0) { (@($artifactFamilies) | ForEach-Object { "- $($_.folder_filter): folder_count=$($_.folder_count); older_generated_folders=$($_.older_generated_folder_count); latest=$($_.latest_folder)" }) -join "`n" } else { "- none" }
...
$md = @"
# Phase 22 Step 3 - Phase 20 Network Transport Planning Scope Packet

Generated: $($report.generated_at)

Source planning boundary packet:

``````
$planningBoundaryPath
``````

## Safety

- Phase 20 network transport planning scope packet only: true
- Planning scope only: true
- Planning only: true
...
if ($blockerCount -eq 0) {
    Write-Host "PASS | phase20_network_transport_planning_scope_packet=$OutputDir | status=$packetStatus | bridge_post_called=False | execution_implementation_created=False | implementation_phase_started=False"
} else {
    Write-Host "CHECK | phase20_network_transport_planning_scope_packet=$OutputDir | status=$packetStatus | blockers=$blockerCount | bridge_post_called=False | execution_implementation_created=False | implementation_phase_started=False"
}

Write-Host ""
Write-Host "Planning scope packet files:"
