...
$PhaseNumber = 37
$StepNumber = 4
$StepTitle = "Phase 37 Step 4 - Phase 20 Network Transport Implementation Trusted Production Monitored Live Write Operations Readiness Safety Disposition Planning Packet"
$PriorCompletedStep = "Phase 37 Step 3 - Phase 20 Network Transport Implementation Trusted Production Monitored Live Write Operations Readiness Evidence Gap Review Packet"
...
$StepFiles = @(
    "scripts/phase37_step4_monitored_live_write_ops_readiness_readiness_safety_planning_packet.ps1",
    "ui/pages/1820_Phase37_Step4_Monitored_Live_Write_Ops_Readiness_Safety_Planning.py",
    "docs/PHASE37_STEP4_MONITORED_LIVE_WRITE_OPS_READINESS_READINESS_SAFETY_PLANNING_PACKET.md",
    "tests/test_phase37_step4_monitored_live_write_ops_readiness_readiness_safety_planning_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase37_boundary = "trusted_production_monitored_live_write_operations_readiness_safety_planning_opened_by_packet"
...
function Assert-StepFiles {
    foreach ($Rel in $StepFiles) {
        ...
        if (-not (Test-Path $Path)) {
            throw ("Missing expected Phase 37 Step 4 file: {0}" -f $Rel)
        }
    }
}
...
function Invoke-Apply {
    Assert-StepFiles
    Write-Host "APPLY PASS: Phase 37 Step 4 planning-only packet verified without live-write activation, live-write apply, live-user access, bridge POST, sockets, DB mutation, server launch, or Phase 38 boundary creation."
}
...
function Invoke-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 37 Step 4 safety markers confirm planning-only/no-write/no-live-write-activation/no-live-write-apply/no-runtime/no-phase38 posture."
}
