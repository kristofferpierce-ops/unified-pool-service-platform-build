...
$StepNumber = 71
$StepTitle = "Phase 31 Step 71 - Phase 20 Network Transport Implementation Controlled Active Program Operation Release Gate Readiness Planning Boundary Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Controlled Active Program Operation Release Gate Readiness Planning Boundary Packet"
$ExpectedBranch = "phase31-step71-controlled-active-program-operation-release-gate-readiness-planning-boundary"
$PriorStep = "Phase 31 Step 70 - Phase 20 Network Transport Implementation Controlled Active Program Operation No-Write Dry Run Result Disposition Planning Final Boundary Confirmation Packet"
$Slug = "implementation_controlled_active_program_operation_release_gate_readiness_planning_boundary"
...
$StepFiles = @(
    "scripts/phase31_step71_controlled_active_program_operation_release_gate_readiness_planning_boundary_packet.ps1",
    "ui/pages/1167_Phase31_Step71_Implementation_Controlled_Active_Program_Operation_Release_Gate_Readiness_Planning_Boundary_Packet.py",
    "docs/PHASE31_STEP71_CONTROLLED_ACTIVE_PROGRAM_OPERATION_RELEASE_GATE_READINESS_PLANNING_BOUNDARY_PACKET.md",
    "tests/test_phase31_step71_controlled_active_program_operation_release_gate_readiness_planning_boundary_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase31_boundary = "implementation_controlled_active_program_operation_release_gate_readiness_planning_boundary_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 31 Step 71 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 31 Step 71 Phase 20 Network Transport Implementation Controlled Active Program Operation Release Gate Readiness Planning Boundary Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase31_step71_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase31_step71_{0}_packet.json" -f $Slug)
