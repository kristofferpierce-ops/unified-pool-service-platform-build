...
$StepNumber = 17
$StepTitle = "Phase 28 Step 17 - Phase 20 Network Transport Implementation Sandbox Pilot Guardrail Verification Approval Readiness Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Sandbox Pilot Guardrail Verification Approval Readiness Packet"
$ExpectedBranch = "phase28-step17-implementation-sandbox-pilot-guardrail-verification-approval-readiness"
$PriorStep = "Phase 28 Step 16 - Phase 20 Network Transport Implementation Sandbox Pilot Guardrail Verification Approval Boundary Packet"
$Slug = "implementation_sandbox_pilot_guardrail_verification_approval_readiness"

if (-not $RepoRoot) { $RepoRoot = (Resolve-Path (Join-Path $PSScriptRoot ".." )).Path }

$StepFiles = @("scripts/phase28_step17_implementation_sandbox_pilot_guardrail_verification_approval_readiness_packet.ps1", "ui/pages/753_Phase28_Step17_Implementation_Sandbox_Pilot_Guardrail_Verification_Approval_Readiness_Packet.py", "docs/PHASE28_STEP17_IMPLEMENTATION_SANDBOX_PILOT_GUARDRAIL_VERIFICATION_APPROVAL_READINESS_PACKET.md", "tests/test_phase28_step17_implementation_sandbox_pilot_guardrail_verification_approval_readiness_packet.py")
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 28 Step 17 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 28 Step 17 Phase 20 Network Transport Implementation Sandbox Pilot Guardrail Verification Approval Readiness Packet is present and planning-only."
}

function New-Packet {
    $Timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
    $BackupRoot = Join-Path $RepoRoot "backups"
    $PacketDir = Join-Path $BackupRoot ("phase28_step17_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase28_step17_{0}_packet.json" -f $Slug)

    $Payload = [ordered]@{ phase = 28; step = 17; title = $StepTitle; prior_step = $PriorStep; safety_posture = $SafetyPosture; files = $StepFiles; generated_at = (Get-Date).ToString("s") }
    ...
    Write-Host "PASS: phase28_boundary=implementation_sandbox_pilot_guardrail_verification_approval_readiness_opened_by_packet"
    ...
    Write-Host "PASS: implementation_sandbox_pilot_guardrail_verification_approval_readiness_mode=reference_only"
    Write-Host "PASS: implementation_sandbox_pilot_guardrail_verification_approval_readiness_write=false"
    Write-Host "PASS: implementation_sandbox_pilot_guardrail_verification_approval_readiness_record_creation=false"
