...
$StepNumber = 6
$StepTitle = "Phase 31 Step 6 - Phase 20 Network Transport Implementation Controlled Active Program Operation Readiness Approval Boundary Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Controlled Active Program Operation Readiness Approval Boundary Packet"
$ExpectedBranch = "phase31-step6-controlled-active-program-operation-readiness-approval-boundary"
$PriorStep = "Phase 31 Step 5 - Phase 20 Network Transport Implementation Controlled Active Program Operation Readiness Safety Disposition Review Packet"
$Slug = "implementation_controlled_active_program_operation_readiness_approval_boundary"
...
$StepFiles = @(
    "scripts/phase31_step6_controlled_active_program_operation_readiness_approval_boundary_packet.ps1",
    "ui/pages/1102_Phase31_Step6_Implementation_Controlled_Active_Program_Operation_Readiness_Approval_Boundary_Packet.py",
    "docs/PHASE31_STEP6_CONTROLLED_ACTIVE_PROGRAM_OPERATION_READINESS_APPROVAL_BOUNDARY_PACKET.md",
    "tests/test_phase31_step6_controlled_active_program_operation_readiness_approval_boundary_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase31_boundary = "implementation_controlled_active_program_operation_readiness_approval_boundary_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 31 Step 6 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 31 Step 6 Phase 20 Network Transport Implementation Controlled Active Program Operation Readiness Approval Boundary Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase31_step6_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase31_step6_{0}_packet.json" -f $Slug)
