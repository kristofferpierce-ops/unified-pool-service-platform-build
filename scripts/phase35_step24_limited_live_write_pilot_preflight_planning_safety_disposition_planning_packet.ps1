...
$StepNumber = 24
$StepTitle = "Phase 35 Step 24 - Phase 20 Network Transport Implementation Trusted Production Limited Live Write Pilot Preflight Planning Safety Disposition Planning Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Trusted Production Limited Live Write Pilot Preflight Planning Safety Disposition Planning Packet"
$ExpectedBranch = "phase35-step24-limited-live-write-pilot-preflight-planning-safety-planning"
$PriorStep = "Phase 35 Step 23 - Phase 20 Network Transport Implementation Trusted Production Limited Live Write Pilot Preflight Planning Evidence Gap Review Packet"
$Slug = "trusted_production_limited_live_write_pilot_preflight_planning_safety_disposition_planning"
$ContextKey = "trusted_production_limited_live_write_pilot_preflight_planning"
$PhaseContext = "trusted_production_limited_live_write_pilot_preflight_planning_only"

if (-not $RepoRoot) { $RepoRoot = (Resolve-Path (Join-Path $PSScriptRoot ".." )).Path }

$StepFiles = @(
    "scripts/phase35_step24_limited_live_write_pilot_preflight_planning_safety_disposition_planning_packet.ps1",
    "ui/pages/1600_Phase35_Step24_Live_Write_Pilot_Preflight_Safety_Planning.py",
    "docs/PHASE35_STEP24_LIMITED_LIVE_WRITE_PILOT_PREFLIGHT_PLANNING_SAFETY_DISPOSITION_PLANNING_PACKET.md",
    "tests/test_phase35_step24_limited_live_write_pilot_preflight_planning_safety_disposition_planning_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase35_boundary = "trusted_production_limited_live_write_pilot_preflight_planning_safety_disposition_planning_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 35 Step 24 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 35 Step 24 Phase 20 Network Transport Implementation Trusted Production Limited Live Write Pilot Preflight Planning Safety Disposition Planning Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase35_step24_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase35_step24_{0}_packet.json" -f $Slug)
