...
$PhaseNumber = 36
$StepNumber = 6
$StepTitle = "Phase 36 Step 6 - Phase 20 Network Transport Implementation Trusted Production Controlled Live Write Expansion Readiness Approval Boundary Packet"
$ExpectedBranch = "phase36-step6-readiness-approval-boundary"
$PriorCompletedStep = "Phase 36 Step 5 - Phase 20 Network Transport Implementation Trusted Production Controlled Live Write Expansion Readiness Safety Disposition Review Packet"
...
$StepFiles = @(
    "scripts/phase36_step6_controlled_live_write_expansion_readiness_approval_boundary_packet.ps1",
    "ui/pages/1702_Phase36_Step6_Readiness_Approval_Boundary.py",
    "docs/PHASE36_STEP6_CONTROLLED_LIVE_WRITE_EXPANSION_READINESS_APPROVAL_BOUNDARY_PACKET.md",
    "tests/test_phase36_step6_controlled_live_write_expansion_readiness_approval_boundary_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase36_boundary = "trusted_production_controlled_live_write_expansion_readiness_approval_boundary_opened_by_packet"
...
function Assert-StepFiles {
    foreach ($Rel in $StepFiles) {
        ...
        if (-not (Test-Path $Path)) {
            throw ("Missing expected Phase 36 Step 6 file: {0}" -f $Rel)
        }
    }
}
...
function Invoke-Apply {
    Assert-StepFiles
    Write-Host "APPLY PASS: Phase 36 Step 6 planning-only packet verified without live-write activation, live-write apply, bridge POST, sockets, DB mutation, server launch, or Phase 37 boundary creation."
}
...
function Invoke-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 36 Step 6 safety markers confirm planning-only/no-write/no-live-write-activation/no-live-write-apply/no-runtime/no-phase37 posture."
}
