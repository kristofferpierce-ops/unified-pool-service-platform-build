...
$StepNumber = 12
$StepTitle = "Phase 31 Step 12 - Phase 20 Network Transport Implementation Controlled Active Program Operation Guardrail Verification Evidence Index Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Controlled Active Program Operation Guardrail Verification Evidence Index Packet"
$ExpectedBranch = "phase31-step12-controlled-active-program-operation-guardrail-verification-evidence-index"
$PriorStep = "Phase 31 Step 11 - Phase 20 Network Transport Implementation Controlled Active Program Operation Guardrail Verification Boundary Packet"
$Slug = "implementation_controlled_active_program_operation_guardrail_verification_evidence_index"
...
$StepFiles = @(
    "scripts/phase31_step12_controlled_active_program_operation_guardrail_verification_evidence_index_packet.ps1",
    "ui/pages/1108_Phase31_Step12_Implementation_Controlled_Active_Program_Operation_Guardrail_Verification_Evidence_Index_Packet.py",
    "docs/PHASE31_STEP12_CONTROLLED_ACTIVE_PROGRAM_OPERATION_GUARDRAIL_VERIFICATION_EVIDENCE_INDEX_PACKET.md",
    "tests/test_phase31_step12_controlled_active_program_operation_guardrail_verification_evidence_index_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase31_boundary = "implementation_controlled_active_program_operation_guardrail_verification_evidence_index_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 31 Step 12 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 31 Step 12 Phase 20 Network Transport Implementation Controlled Active Program Operation Guardrail Verification Evidence Index Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase31_step12_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase31_step12_{0}_packet.json" -f $Slug)
