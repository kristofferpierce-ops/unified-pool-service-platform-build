...
function Classify-RepoPath {
    ...
    if ($CurrentStepFiles -contains $Path) {
        return [ordered]@{
            ...
            should_stage = $true
            reason = "Current Phase 22 Step 12 file."
        }
    }
...
if ([string]::IsNullOrWhiteSpace($PlanningApprovalPacketDir)) {
    $PlanningApprovalPacketDir = Get-LatestArtifactDir -FolderFilter "phase22_phase20_network_transport_planning_approval_packet_*" -JsonName "phase22_phase20_network_transport_planning_approval_packet.json" -Required $true
}
if ([string]::IsNullOrWhiteSpace($PlanningSignoffPacketDir)) {
    $PlanningSignoffPacketDir = Get-LatestArtifactDir -FolderFilter "phase22_phase20_network_transport_planning_signoff_packet_*" -JsonName "phase22_phase20_network_transport_planning_signoff_packet.json" -Required $false
}
...
$planningApprovalPath = Join-Path $PlanningApprovalPacketDir "phase22_phase20_network_transport_planning_approval_packet.json"
$planningSignoffPath = if (![string]::IsNullOrWhiteSpace($PlanningSignoffPacketDir)) { Join-Path $PlanningSignoffPacketDir "phase22_phase20_network_transport_planning_signoff_packet.json" } else { "" }
...
if (!(Test-Path -LiteralPath $planningApprovalPath)) {
    throw "Planning approval packet JSON not found: $planningApprovalPath"
}

$planningApproval = Read-JsonFile $planningApprovalPath
$planningSignoff = if (![string]::IsNullOrWhiteSpace($planningSignoffPath) -and (Test-Path -LiteralPath $planningSignoffPath)) { Read-JsonFile $planningSignoffPath } else { $null }
...
$closureSections = New-Object System.Collections.ArrayList
$closureEntries = New-Object System.Collections.ArrayList
$closureAreas = New-Object System.Collections.ArrayList
...
Add-ChecklistItem $checklist "artifact" "Planning approval packet exists" $true $planningApprovalPath
Add-ChecklistItem $checklist "artifact" "Planning signoff packet exists" ($planningSignoff -ne $null) $planningSignoffPath "recommended"
Add-ChecklistItem $checklist "artifact" "Cleanup exit packet exists" ($cleanupExit -ne $null) $cleanupExitPath "recommended"

Add-ChecklistItem $checklist "planning_approval" "Planning approval packet is closure-safe" ($planningApproval.safety.phase20_network_transport_planning_approval_packet_only -eq $true) "phase20_network_transport_planning_approval_packet_only=$($planningApproval.safety.phase20_network_transport_planning_approval_packet_only)"
Add-ChecklistItem $checklist "planning_approval" "Planning approval created no closure record" ($planningApproval.safety.design_closure_record_created -eq $false) "design_closure_record_created=$($planningApproval.safety.design_closure_record_created)"
Add-ChecklistItem $checklist "planning_approval" "Planning approval recorded no final approval" ($planningApproval.safety.final_approval_recorded -eq $false) "final_approval_recorded=$($planningApproval.safety.final_approval_recorded)"
Add-ChecklistItem $checklist "planning_approval" "Planning approval recorded no operator approval" ($planningApproval.safety.operator_approval_recorded -eq $false) "operator_approval_recorded=$($planningApproval.safety.operator_approval_recorded)"
Add-ChecklistItem $checklist "planning_approval" "Planning approval started no implementation phase" ($planningApproval.safety.implementation_phase_started -eq $false) "implementation_phase_started=$($planningApproval.safety.implementation_phase_started)"
Add-ChecklistItem $checklist "planning_approval" "Planning approval has no execution implementation" ($planningApproval.safety.execution_implementation_created -eq $false) "execution_implementation_created=$($planningApproval.safety.execution_implementation_created)"
Add-ChecklistItem $checklist "planning_approval" "Planning approval has no network transport" ($planningApproval.safety.network_transport_implemented -eq $false) "network_transport_implemented=$($planningApproval.safety.network_transport_implemented)"
Add-ChecklistItem $checklist "planning_approval" "Planning approval has no socket opened" ($planningApproval.safety.network_socket_opened -eq $false) "network_socket_opened=$($planningApproval.safety.network_socket_opened)"
Add-ChecklistItem $checklist "planning_approval" "Planning approval has no bridge POST" ($planningApproval.safety.bridge_post_called -eq $false -and $planningApproval.safety.bridge_post_call_implemented -eq $false) "bridge_post_called=$($planningApproval.safety.bridge_post_called); bridge_post_call_implemented=$($planningApproval.safety.bridge_post_call_implemented)"
Add-ChecklistItem $checklist "planning_approval" "Planning approval has no platform DB mutation" ($planningApproval.safety.platform_db_mutation_performed -eq $false) "platform_db_mutation_performed=$($planningApproval.safety.platform_db_mutation_performed)"
Add-ChecklistItem $checklist "planning_approval" "Planning approval has no bridge mutation" ($planningApproval.safety.bridge_mutation_performed -eq $false) "bridge_mutation_performed=$($planningApproval.safety.bridge_mutation_performed)"
Add-ChecklistItem $checklist "planning_approval" "Planning approval has no LACRM call" ($planningApproval.safety.lacrm_call_performed -eq $false) "lacrm_call_performed=$($planningApproval.safety.lacrm_call_performed)"
Add-ChecklistItem $checklist "planning_approval" "Planning approval has no blockers" ([int]$planningApproval.planning_approval_packet.blocker_count -eq 0) "blocker_count=$($planningApproval.planning_approval_packet.blocker_count)"

if ($planningSignoff -ne $null) {
    Add-ChecklistItem $checklist "planning_signoff" "Planning signoff packet is planning-only" ($planningSignoff.safety.planning_only -eq $true) "planning_only=$($planningSignoff.safety.planning_only)" "recommended"
}
...
$approvalPrefixes = @($planningApproval.planning_entries | Where-Object { $_.category -eq "do_not_stage" } | ForEach-Object { [string]$_.item })
if (@($approvalPrefixes).Count -gt 0) {
    $doNotStagePrefixes = $approvalPrefixes
}

$currentStepFiles = @(
    "scripts/phase22_generate_phase20_network_transport_planning_closure_packet.ps1",
    "ui/pages/118_Phase20_Network_Transport_Planning_Closure_Packet.py",
    "docs/PHASE22_STEP12_PHASE20_NETWORK_TRANSPORT_PLANNING_CLOSURE_PACKET.md",
    "tests/test_phase22_phase20_network_transport_planning_closure_packet.py"
)
...
if ($planningApproval.artifact_families) {
    foreach ($family in @($planningApproval.artifact_families)) {
        [void]$artifactFamilies.Add($family)
    }
}
if ($planningApproval.archive_decisions) {
    foreach ($decision in @($planningApproval.archive_decisions)) {
        [void]$archiveDecisions.Add($decision)
    }
}

$closureSpecs = @(
    [ordered]@{ key="execution_envelope_closure"; item="Execution envelope closure review"; depends_on="Execution envelope approval review"; summary="Review execution request, result, error, and dry-run envelope completeness as closure-ready planning material only."; implementation_allowed=$false; bridge_post_allowed=$false; socket_allowed=$false; closure_level="planning_only" },
    [ordered]@{ key="transport_injection_closure"; item="Transport injection closure review"; depends_on="Transport injection approval review"; summary="Review dependency injection seams and transport-off defaults as closure-ready planning material only."; implementation_allowed=$false; bridge_post_allowed=$false; socket_allowed=$false; closure_level="planning_only" },
    [ordered]@{ key="bridge_gate_closure"; item="Bridge gate closure review"; depends_on="Bridge gate approval review"; summary="Review confirmation phrase, armed-state, environment gate, and explicit POST precondition coverage as closure-ready planning material only."; implementation_allowed=$false; bridge_post_allowed=$false; socket_allowed=$false; closure_level="planning_only" },
    [ordered]@{ key="response_storage_closure"; item="Response storage closure review"; depends_on="Response storage approval review"; summary="Review response record, storage, and idempotent capture coverage as closure-ready planning material only."; implementation_allowed=$false; bridge_post_allowed=$false; socket_allowed=$false; closure_level="planning_only" },
    [ordered]@{ key="cutover_closure"; item="Cutover closure review"; depends_on="Cutover approval review"; summary="Review cutover prerequisites, verification checkpoints, and rollback review coverage as closure-ready planning material only."; implementation_allowed=$false; bridge_post_allowed=$false; socket_allowed=$false; closure_level="planning_only" },
    [ordered]@{ key="audit_rollback_closure"; item="Audit and rollback closure review"; depends_on="Audit and rollback approval review"; summary="Review audit record, rollback record, and snapshot coverage as closure-ready planning material only."; implementation_allowed=$false; bridge_post_allowed=$false; socket_allowed=$false; closure_level="planning_only" }
)

foreach ($spec in $closureSpecs) {
    [void]$closureAreas.Add([ordered]@{
        key = $spec.key
        item = $spec.item
    ...
    [void]$closureEntries.Add([ordered]@{
        category = "closure_areas"
        action = "review_only"
        item = $spec.item
...
[void]$closureSections.Add([ordered]@{
    section = "Planning closure state"
    status = "review"
    summary = "Phase 22 Step 12 remains planning-only and converts planning approval areas into explicit closure-review areas."
})
[void]$closureSections.Add([ordered]@{
    section = "Closure areas"
    status = "review"
    summary = "Execution envelope, transport injection, bridge gate, response storage, cutover, and audit/rollback closure areas remain review-only."
})
[void]$closureSections.Add([ordered]@{
...
foreach ($file in @($currentStepFiles)) {
    [void]$closureEntries.Add([ordered]@{
        ...
        action = "stage"
        item = $file
        evidence = "Current Phase 22 Step 12 file."
        required = "yes"
    })
}
...
$nextCommitCommands = @(
    ...
    "git status --short",
    "git add scripts/phase22_generate_phase20_network_transport_planning_closure_packet.ps1",
    "git add ui/pages/118_Phase20_Network_Transport_Planning_Closure_Packet.py",
    "git add docs/PHASE22_STEP12_PHASE20_NETWORK_TRANSPORT_PLANNING_CLOSURE_PACKET.md",
    "git add tests/test_phase22_phase20_network_transport_planning_closure_packet.py",
    "git diff --cached --name-only",
    'git commit -m "Phase 22 Step 12 phase20 network transport planning closure packet"',
    "git push -u origin phase22-step12-phase20-network-transport-planning-closure-packet"
)

Add-ChecklistItem $checklist "planning_closure" "Current branch detected" (-not [string]::IsNullOrWhiteSpace($currentBranch)) "current_branch=$currentBranch"
Add-ChecklistItem $checklist "planning_closure" "At least one planning-closure section exists" (@($closureSections).Count -gt 0) "planning_closure_sections=$(@($closureSections).Count)"
Add-ChecklistItem $checklist "planning_closure" "Exactly four stage-now entries exist" (@($closureEntries | Where-Object { $_.category -eq "stage_now" }).Count -eq 4) "stage_now_entries=$(@($closureEntries | Where-Object { $_.category -eq "stage_now" }).Count)"
Add-ChecklistItem $checklist "planning_closure" "Closure areas were generated" (@($closureAreas).Count -ge 6) "closure_areas=$(@($closureAreas).Count)" "recommended"
Add-ChecklistItem $checklist "planning_closure" "Archive decisions were carried forward" (@($archiveDecisions).Count -eq @($artifactFamilies).Count) "archive_decisions=$(@($archiveDecisions).Count); artifact_families=$(@($artifactFamilies).Count)" "recommended"
Add-ChecklistItem $checklist "planning_closure" "Commit commands exist" (@($nextCommitCommands).Count -gt 0) "commit_commands=$(@($nextCommitCommands).Count)"
Add-ChecklistItem $checklist "planning_closure" "No forbidden paths are currently staged" ($stagedForbiddenCount -eq 0) "staged_forbidden_count=$stagedForbiddenCount"
Add-ChecklistItem $checklist "planning_closure" "No review-other paths are currently staged" ($stagedReviewOtherCount -eq 0) "staged_review_other_count=$stagedReviewOtherCount" "recommended"
Add-ChecklistItem $checklist "planning_closure" "Workspace review-other files count is zero or acceptable" ($reviewOtherCount -eq 0) "review_other_count=$reviewOtherCount" "recommended"

$planningApprovalStatus = [string]$planningApproval.planning_approval_packet.status
if ($planningApprovalStatus -eq "phase20_planning_approval_packet_blocked") {
    Add-Issue $issues "blocker" "planning_approval_packet_blocked" "Planning approval packet is blocked." "planning_approval"
} elseif ($planningApprovalStatus -eq "phase20_planning_approval_packet_review_required_no_write") {
    Add-Issue $issues "review" "planning_approval_packet_review_required" "Planning approval packet has review items." "planning_approval"
}

if ([string]::IsNullOrWhiteSpace($currentBranch)) {
    Add-Issue $issues "blocker" "current_branch_missing" "Current branch was not detected for planning closure." "workspace"
}
...
$packetStatus = if ($blockerCount -eq 0 -and $reviewCount -eq 0) {
    "phase20_planning_closure_packet_ready_no_write"
} elseif ($blockerCount -eq 0) {
    "phase20_planning_closure_packet_review_required_no_write"
} else {
    "phase20_planning_closure_packet_blocked"
}

if ([string]::IsNullOrWhiteSpace($OutputDir)) {
    $stamp = Get-Date -Format "yyyyMMdd-HHmmss"
    $OutputDir = Join-Path $BackupDir "phase22_phase20_network_transport_planning_closure_packet_$stamp"
}
...
$report = [ordered]@{
    ...
    phase = "Phase 22 Step 12"
    purpose = "Phase 20 bridge routing network transport planning closure packet"
    workspace = $Workspace
    platform_api = $PlatformApi
    bridge_url = $BridgeUrl
    safety = [ordered]@{
        phase20_network_transport_planning_closure_packet_only = $true
        planning_closure_only = $true
        planning_only = $true
    ...
    planning_closure_packet = [ordered]@{
        status = $packetStatus
        blocker_count = $blockerCount
        review_count = $reviewCount
        current_branch = $currentBranch
        section_count = @($closureSections).Count
        entry_count = @($closureEntries).Count
        closure_area_count = @($closureAreas).Count
        ...
        packet_label = "phase22_phase20_network_transport_planning_closure_no_write"
        reason = if ($blockerCount -eq 0) {
            "Phase 22 Step 12 created a no-write planning closure packet for the Phase 20 network transport chain. It does not authorize closure records, approvals, implementation, transport, sockets, bridge POST, response capture, mutations, LACRM calls, or live writes."
        } else {
            "Phase 22 Step 12 found blockers in the Phase 20 planning closure packet. Resolve blockers before continuing planning."
        }
    }
    planning_sections = @($closureSections)
    planning_entries = @($closureEntries)
    closure_areas = @($closureAreas)
    ...
    counts = [ordered]@{
        sections = @($closureSections).Count
        entries = @($closureEntries).Count
        closure_areas = @($closureAreas).Count
    ...
    next_recommended_actions = @(
        "Review this planning closure packet before any Phase 22 exit or implementation-gate packet is drafted.",
        "Keep the latest packet folder in each family and only review older generated folders for manual archive actions.",
        "Use the stage-now entries as the only files to add for the Phase 22 Step 12 commit.",
        "Carry the do-not-stage entries forward as the hard commit boundary.",
        "Do not add requests, httpx, socket use, bridge POST calls, or routing write endpoints from Phase 22 Step 12.",
        "Do not call interface execution methods from Phase 22 Step 12.",
        "Do not record operator signoffs, operator approvals, or final approvals from Phase 22 Step 12.",
        "Do not start an implementation phase from Phase 22 Step 12.",
        "Review closure-ready planning only: execution envelope, transport injection, bridge gate, response storage, cutover, and audit/rollback closure areas."
    )
}

$jsonPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_closure_packet.json"
$sectionsCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_closure_packet_sections.csv"
$entriesCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_closure_packet_entries.csv"
$areasCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_closure_packet_closure_areas.csv"
$decisionsCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_closure_packet_decisions.csv"
$familiesCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_closure_packet_artifact_families.csv"
$gitStatusCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_closure_packet_git_status.csv"
$stagedCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_closure_packet_staged.csv"
$sourceArtifactsCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_closure_packet_source_artifacts.csv"
$checklistCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_closure_packet_checklist.csv"
$issuesCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_closure_packet_issues.csv"
$mdPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_closure_packet.md"

$report | ConvertTo-Json -Depth 100 | Set-Content -LiteralPath $jsonPath -Encoding UTF8
$closureSections | Export-Csv -LiteralPath $sectionsCsvPath -NoTypeInformation -Encoding UTF8
$closureEntries | Export-Csv -LiteralPath $entriesCsvPath -NoTypeInformation -Encoding UTF8
$closureAreas | Export-Csv -LiteralPath $areasCsvPath -NoTypeInformation -Encoding UTF8
...
$sectionText = (@($closureSections) | ForEach-Object { "- $($_.section): $($_.status); $($_.summary)" }) -join "`n"
$entryText = (@($closureEntries) | ForEach-Object { "- [$($_.category)] $($_.action) $($_.item): $($_.evidence)" }) -join "`n"
$closureText = (@($closureAreas) | ForEach-Object { "- $($_.item): depends_on=$($_.depends_on); status=$($_.status)" }) -join "`n"
...
$md = @"
# Phase 22 Step 12 - Phase 20 Network Transport Planning Closure Packet

Generated: $($report.generated_at)

Source planning approval packet:
...
- Phase 20 network transport planning closure packet only: true
- Planning closure only: true
- Planning only: true
...
- Sections: $(@($closureSections).Count)
- Entries: $(@($closureEntries).Count)
- Closure areas: $(@($closureAreas).Count)
...
if ($blockerCount -eq 0) {
    Write-Host "PASS | phase20_network_transport_planning_closure_packet=$OutputDir | status=$packetStatus | bridge_post_called=False | execution_implementation_created=False | implementation_phase_started=False"
} else {
    Write-Host "CHECK | phase20_network_transport_planning_closure_packet=$OutputDir | status=$packetStatus | blockers=$blockerCount | bridge_post_called=False | execution_implementation_created=False | implementation_phase_started=False"
}

Write-Host ""
Write-Host "Planning closure packet files:"
