...
$PhaseNumber = 23
$StepNumber = 40
$StepName = "Phase 23 Step 40 - Phase 20 Network Transport Implementation Patch Review Planning Approval Boundary Packet"
$PriorStepName = "Phase 23 Step 39 - Phase 20 Network Transport Implementation Patch Review Planning Safety Disposition Review Packet"
$ExpectedBranch = "phase23-step40-implementation-patch-review-planning-approval-boundary"
$ModeKey = "implementation_patch_review_planning_approval_boundary_mode"
$WriteKey = "implementation_patch_review_planning_approval_boundary_write"
$RecordKey = "implementation_patch_review_planning_approval_boundary_record_creation"
$PhaseContext = "implementation_patch_review_planning_approval_boundary_planning_only"
$PacketSlug = "phase23_step40_implementation_patch_review_planning_approval_boundary_packet"

$StepFiles = @(
    "scripts/phase23_step40_implementation_patch_review_planning_approval_boundary_packet.ps1",
    "ui/pages/216_Phase23_Step40_Implementation_Patch_Review_Planning_Approval_Boundary_Packet.py",
    "docs/PHASE23_STEP40_IMPLEMENTATION_PATCH_REVIEW_PLANNING_APPROVAL_BOUNDARY_PACKET.md",
    "tests/test_phase23_step40_implementation_patch_review_planning_approval_boundary_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase23_planning_boundary = "implementation_patch_review_planning_approval_boundary_opened_by_packet"
    phase23_implementation_start = $false
    implementation_phase_start = $false
    cross_repo_write = $false
    cross_repo_mutation = $false
    external_repo_push = $false
    implementation_patch_review_planning_approval_boundary_mode = "reference_only"
    implementation_patch_review_planning_approval_boundary_write = $false
    implementation_patch_review_planning_approval_boundary_record_creation = $false
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 23 Step 40 files copied or already present."
}
...
function Invoke-SmokeTest {
    ...
    $scriptText = Get-Content -LiteralPath (Join-SafePath -Root $Repo -RelativePath "scripts/phase23_step40_implementation_patch_review_planning_approval_boundary_packet.ps1") -Raw -Encoding UTF8
    ...
    Write-Host "SMOKE TEST PASS: Phase 23 Step 40 Implementation Patch Review Planning Approval Boundary Packet is present and planning-only."
}
...
function Write-Packet {
    ...
    Write-Host "PASS: phase23_planning_boundary=implementation_patch_review_planning_approval_boundary_opened_by_packet"
    ...
    Write-Host "PASS: implementation_patch_review_planning_approval_boundary_mode=reference_only"
    Write-Host "PASS: implementation_patch_review_planning_approval_boundary_write=false"
    Write-Host "PASS: implementation_patch_review_planning_approval_boundary_record_creation=false"
...
while ($true) {
    Write-Host ""
    Write-Host "Phase 23 Step 40 menu"
    Write-Host "1. Show status / verify paths"
    Write-Host "2. Apply Phase 23 Step 40 Implementation Patch Review Planning Approval Boundary Packet files"
    Write-Host "3. Smoke test Phase 23 Step 40"
    Write-Host "4. Show server start placeholder only"
    Write-Host "5. Generate Phase 23 Step 40 Implementation Patch Review Planning Approval Boundary Packet"
    Write-Host "6. Exit"
