param(
    ...
    [string]$PlanningReviewPacketDir = "",
...
function Classify-RepoPath {
    ...
    if ($CurrentStepFiles -contains $Path) {
        return [ordered]@{
            ...
            should_stage = $true
            reason = "Current Phase 22 Step 10 file."
        }
    }
...
if ([string]::IsNullOrWhiteSpace($PlanningFreezePacketDir)) {
    $PlanningFreezePacketDir = Get-LatestArtifactDir -FolderFilter "phase22_phase20_network_transport_planning_freeze_packet_*" -JsonName "phase22_phase20_network_transport_planning_freeze_packet.json" -Required $true
}
if ([string]::IsNullOrWhiteSpace($PlanningReviewPacketDir)) {
    $PlanningReviewPacketDir = Get-LatestArtifactDir -FolderFilter "phase22_phase20_network_transport_planning_review_packet_*" -JsonName "phase22_phase20_network_transport_planning_review_packet.json" -Required $false
}
...
$planningFreezePath = Join-Path $PlanningFreezePacketDir "phase22_phase20_network_transport_planning_freeze_packet.json"
$planningReviewPath = if (![string]::IsNullOrWhiteSpace($PlanningReviewPacketDir)) { Join-Path $PlanningReviewPacketDir "phase22_phase20_network_transport_planning_review_packet.json" } else { "" }
...
if (!(Test-Path -LiteralPath $planningFreezePath)) {
    throw "Planning freeze packet JSON not found: $planningFreezePath"
}

$planningFreeze = Read-JsonFile $planningFreezePath
$planningReview = if (![string]::IsNullOrWhiteSpace($planningReviewPath) -and (Test-Path -LiteralPath $planningReviewPath)) { Read-JsonFile $planningReviewPath } else { $null }
...
$signoffSections = New-Object System.Collections.ArrayList
$signoffEntries = New-Object System.Collections.ArrayList
$signoffAreas = New-Object System.Collections.ArrayList
...
Add-ChecklistItem $checklist "artifact" "Planning freeze packet exists" $true $planningFreezePath
Add-ChecklistItem $checklist "artifact" "Planning review packet exists" ($planningReview -ne $null) $planningReviewPath "recommended"
Add-ChecklistItem $checklist "artifact" "Cleanup exit packet exists" ($cleanupExit -ne $null) $cleanupExitPath "recommended"

Add-ChecklistItem $checklist "planning_freeze" "Planning freeze packet is signoff-safe" ($planningFreeze.safety.phase20_network_transport_planning_freeze_packet_only -eq $true) "phase20_network_transport_planning_freeze_packet_only=$($planningFreeze.safety.phase20_network_transport_planning_freeze_packet_only)"
Add-ChecklistItem $checklist "planning_freeze" "Planning freeze created no closure record" ($planningFreeze.safety.design_closure_record_created -eq $false) "design_closure_record_created=$($planningFreeze.safety.design_closure_record_created)"
Add-ChecklistItem $checklist "planning_freeze" "Planning freeze recorded no final approval" ($planningFreeze.safety.final_approval_recorded -eq $false) "final_approval_recorded=$($planningFreeze.safety.final_approval_recorded)"
Add-ChecklistItem $checklist "planning_freeze" "Planning freeze started no implementation phase" ($planningFreeze.safety.implementation_phase_started -eq $false) "implementation_phase_started=$($planningFreeze.safety.implementation_phase_started)"
Add-ChecklistItem $checklist "planning_freeze" "Planning freeze has no execution implementation" ($planningFreeze.safety.execution_implementation_created -eq $false) "execution_implementation_created=$($planningFreeze.safety.execution_implementation_created)"
Add-ChecklistItem $checklist "planning_freeze" "Planning freeze has no network transport" ($planningFreeze.safety.network_transport_implemented -eq $false) "network_transport_implemented=$($planningFreeze.safety.network_transport_implemented)"
Add-ChecklistItem $checklist "planning_freeze" "Planning freeze has no socket opened" ($planningFreeze.safety.network_socket_opened -eq $false) "network_socket_opened=$($planningFreeze.safety.network_socket_opened)"
Add-ChecklistItem $checklist "planning_freeze" "Planning freeze has no bridge POST" ($planningFreeze.safety.bridge_post_called -eq $false -and $planningFreeze.safety.bridge_post_call_implemented -eq $false) "bridge_post_called=$($planningFreeze.safety.bridge_post_called); bridge_post_call_implemented=$($planningFreeze.safety.bridge_post_call_implemented)"
Add-ChecklistItem $checklist "planning_freeze" "Planning freeze has no platform DB mutation" ($planningFreeze.safety.platform_db_mutation_performed -eq $false) "platform_db_mutation_performed=$($planningFreeze.safety.platform_db_mutation_performed)"
Add-ChecklistItem $checklist "planning_freeze" "Planning freeze has no bridge mutation" ($planningFreeze.safety.bridge_mutation_performed -eq $false) "bridge_mutation_performed=$($planningFreeze.safety.bridge_mutation_performed)"
Add-ChecklistItem $checklist "planning_freeze" "Planning freeze has no LACRM call" ($planningFreeze.safety.lacrm_call_performed -eq $false) "lacrm_call_performed=$($planningFreeze.safety.lacrm_call_performed)"
Add-ChecklistItem $checklist "planning_freeze" "Planning freeze has no blockers" ([int]$planningFreeze.planning_freeze_packet.blocker_count -eq 0) "blocker_count=$($planningFreeze.planning_freeze_packet.blocker_count)"

if ($planningReview -ne $null) {
    Add-ChecklistItem $checklist "planning_review" "Planning review packet is planning-only" ($planningReview.safety.planning_only -eq $true) "planning_only=$($planningReview.safety.planning_only)" "recommended"
}
...
$freezePrefixes = @($planningFreeze.planning_entries | Where-Object { $_.category -eq "do_not_stage" } | ForEach-Object { [string]$_.item })
if (@($freezePrefixes).Count -gt 0) {
    $doNotStagePrefixes = $freezePrefixes
}

$currentStepFiles = @(
    "scripts/phase22_generate_phase20_network_transport_planning_signoff_packet.ps1",
    "ui/pages/116_Phase20_Network_Transport_Planning_Signoff_Packet.py",
    "docs/PHASE22_STEP10_PHASE20_NETWORK_TRANSPORT_PLANNING_SIGNOFF_PACKET.md",
    "tests/test_phase22_phase20_network_transport_planning_signoff_packet.py"
)
...
if ($planningFreeze.artifact_families) {
    foreach ($family in @($planningFreeze.artifact_families)) {
        [void]$artifactFamilies.Add($family)
    }
}
if ($planningFreeze.archive_decisions) {
    foreach ($decision in @($planningFreeze.archive_decisions)) {
        [void]$archiveDecisions.Add($decision)
    }
}

$signoffSpecs = @(
    [ordered]@{ key="execution_envelope_signoff"; item="Execution envelope signoff review"; depends_on="Execution envelope freeze review"; summary="Review execution request, result, error, and dry-run envelope completeness as signoff-ready planning material only."; implementation_allowed=$false; bridge_post_allowed=$false; socket_allowed=$false; signoff_level="planning_only" },
    [ordered]@{ key="transport_injection_signoff"; item="Transport injection signoff review"; depends_on="Transport injection freeze review"; summary="Review dependency injection seams and transport-off defaults as signoff-ready planning material only."; implementation_allowed=$false; bridge_post_allowed=$false; socket_allowed=$false; signoff_level="planning_only" },
    [ordered]@{ key="bridge_gate_signoff"; item="Bridge gate signoff review"; depends_on="Bridge gate freeze review"; summary="Review confirmation phrase, armed-state, environment gate, and explicit POST precondition coverage as signoff-ready planning material only."; implementation_allowed=$false; bridge_post_allowed=$false; socket_allowed=$false; signoff_level="planning_only" },
    [ordered]@{ key="response_storage_signoff"; item="Response storage signoff review"; depends_on="Response storage freeze review"; summary="Review response record, storage, and idempotent capture coverage as signoff-ready planning material only."; implementation_allowed=$false; bridge_post_allowed=$false; socket_allowed=$false; signoff_level="planning_only" },
    [ordered]@{ key="cutover_signoff"; item="Cutover signoff review"; depends_on="Cutover freeze review"; summary="Review cutover prerequisites, verification checkpoints, and rollback review coverage as signoff-ready planning material only."; implementation_allowed=$false; bridge_post_allowed=$false; socket_allowed=$false; signoff_level="planning_only" },
    [ordered]@{ key="audit_rollback_signoff"; item="Audit and rollback signoff review"; depends_on="Audit and rollback freeze review"; summary="Review audit record, rollback record, and snapshot coverage as signoff-ready planning material only."; implementation_allowed=$false; bridge_post_allowed=$false; socket_allowed=$false; signoff_level="planning_only" }
)

foreach ($spec in $signoffSpecs) {
    [void]$signoffAreas.Add([ordered]@{
        key = $spec.key
        item = $spec.item
        depends_on = $spec.depends_on
        summary = $spec.summary
        status = "review_only"
        signoff_level = $spec.signoff_level
    ...
    [void]$signoffEntries.Add([ordered]@{
        category = "signoff_areas"
        action = "review_only"
        item = $spec.item
...
[void]$signoffSections.Add([ordered]@{
    section = "Planning signoff state"
    status = "review"
    summary = "Phase 22 Step 10 remains planning-only and converts planning freeze areas into explicit signoff-review areas."
})
[void]$signoffSections.Add([ordered]@{
    section = "Signoff areas"
    status = "review"
    summary = "Execution envelope, transport injection, bridge gate, response storage, cutover, and audit/rollback signoff areas remain review-only."
})
[void]$signoffSections.Add([ordered]@{
...
foreach ($file in @($currentStepFiles)) {
    [void]$signoffEntries.Add([ordered]@{
        ...
        action = "stage"
        item = $file
        evidence = "Current Phase 22 Step 10 file."
        required = "yes"
    })
}
...
$nextCommitCommands = @(
    ...
    "git status --short",
    "git add scripts/phase22_generate_phase20_network_transport_planning_signoff_packet.ps1",
    "git add ui/pages/116_Phase20_Network_Transport_Planning_Signoff_Packet.py",
    "git add docs/PHASE22_STEP10_PHASE20_NETWORK_TRANSPORT_PLANNING_SIGNOFF_PACKET.md",
    "git add tests/test_phase22_phase20_network_transport_planning_signoff_packet.py",
    "git diff --cached --name-only",
    'git commit -m "Phase 22 Step 10 phase20 network transport planning signoff packet"',
    "git push -u origin phase22-step10-phase20-network-transport-planning-signoff-packet"
)

Add-ChecklistItem $checklist "planning_signoff" "Current branch detected" (-not [string]::IsNullOrWhiteSpace($currentBranch)) "current_branch=$currentBranch"
Add-ChecklistItem $checklist "planning_signoff" "At least one planning-signoff section exists" (@($signoffSections).Count -gt 0) "planning_signoff_sections=$(@($signoffSections).Count)"
Add-ChecklistItem $checklist "planning_signoff" "Exactly four stage-now entries exist" (@($signoffEntries | Where-Object { $_.category -eq "stage_now" }).Count -eq 4) "stage_now_entries=$(@($signoffEntries | Where-Object { $_.category -eq "stage_now" }).Count)"
Add-ChecklistItem $checklist "planning_signoff" "Signoff areas were generated" (@($signoffAreas).Count -ge 6) "signoff_areas=$(@($signoffAreas).Count)" "recommended"
Add-ChecklistItem $checklist "planning_signoff" "Archive decisions were carried forward" (@($archiveDecisions).Count -eq @($artifactFamilies).Count) "archive_decisions=$(@($archiveDecisions).Count); artifact_families=$(@($artifactFamilies).Count)" "recommended"
Add-ChecklistItem $checklist "planning_signoff" "Commit commands exist" (@($nextCommitCommands).Count -gt 0) "commit_commands=$(@($nextCommitCommands).Count)"
Add-ChecklistItem $checklist "planning_signoff" "No forbidden paths are currently staged" ($stagedForbiddenCount -eq 0) "staged_forbidden_count=$stagedForbiddenCount"
Add-ChecklistItem $checklist "planning_signoff" "No review-other paths are currently staged" ($stagedReviewOtherCount -eq 0) "staged_review_other_count=$stagedReviewOtherCount" "recommended"
Add-ChecklistItem $checklist "planning_signoff" "Workspace review-other files count is zero or acceptable" ($reviewOtherCount -eq 0) "review_other_count=$reviewOtherCount" "recommended"

$planningFreezeStatus = [string]$planningFreeze.planning_freeze_packet.status
if ($planningFreezeStatus -eq "phase20_planning_freeze_packet_blocked") {
    Add-Issue $issues "blocker" "planning_freeze_packet_blocked" "Planning freeze packet is blocked." "planning_freeze"
} elseif ($planningFreezeStatus -eq "phase20_planning_freeze_packet_review_required_no_write") {
    Add-Issue $issues "review" "planning_freeze_packet_review_required" "Planning freeze packet has review items." "planning_freeze"
}

if ([string]::IsNullOrWhiteSpace($currentBranch)) {
    Add-Issue $issues "blocker" "current_branch_missing" "Current branch was not detected for planning signoff." "workspace"
}
...
[void]$sourceArtifacts.Add([ordered]@{ key="planning_review_packet"; json_path=$planningReviewPath; sha256=Get-FileSha256 $planningReviewPath })
...
$packetStatus = if ($blockerCount -eq 0 -and $reviewCount -eq 0) {
    "phase20_planning_signoff_packet_ready_no_write"
} elseif ($blockerCount -eq 0) {
    "phase20_planning_signoff_packet_review_required_no_write"
} else {
    "phase20_planning_signoff_packet_blocked"
}

if ([string]::IsNullOrWhiteSpace($OutputDir)) {
    $stamp = Get-Date -Format "yyyyMMdd-HHmmss"
    $OutputDir = Join-Path $BackupDir "phase22_phase20_network_transport_planning_signoff_packet_$stamp"
}
...
$report = [ordered]@{
    ...
    phase = "Phase 22 Step 10"
    purpose = "Phase 20 bridge routing network transport planning signoff packet"
    workspace = $Workspace
    platform_api = $PlatformApi
    bridge_url = $BridgeUrl
    safety = [ordered]@{
        phase20_network_transport_planning_signoff_packet_only = $true
        planning_signoff_only = $true
        planning_only = $true
    ...
    planning_signoff_packet = [ordered]@{
        status = $packetStatus
        blocker_count = $blockerCount
        review_count = $reviewCount
        current_branch = $currentBranch
        section_count = @($signoffSections).Count
        entry_count = @($signoffEntries).Count
        signoff_area_count = @($signoffAreas).Count
        ...
        packet_label = "phase22_phase20_network_transport_planning_signoff_no_write"
        reason = if ($blockerCount -eq 0) {
            "Phase 22 Step 10 created a no-write planning signoff packet for the Phase 20 network transport chain. It does not authorize signoff records, implementation, transport, sockets, bridge POST, response capture, mutations, LACRM calls, or live writes."
        } else {
            "Phase 22 Step 10 found blockers in the Phase 20 planning signoff packet. Resolve blockers before continuing planning."
        }
    }
    planning_sections = @($signoffSections)
    planning_entries = @($signoffEntries)
    signoff_areas = @($signoffAreas)
    ...
    counts = [ordered]@{
        sections = @($signoffSections).Count
        entries = @($signoffEntries).Count
        signoff_areas = @($signoffAreas).Count
    ...
    next_recommended_actions = @(
        "Review this planning signoff packet before any Phase 22 signoff-review, approval, or closure packet is drafted.",
        "Keep the latest packet folder in each family and only review older generated folders for manual archive actions.",
        "Use the stage-now entries as the only files to add for the Phase 22 Step 10 commit.",
        "Carry the do-not-stage entries forward as the hard commit boundary.",
        "Do not add requests, httpx, socket use, bridge POST calls, or routing write endpoints from Phase 22 Step 10.",
        "Do not call interface execution methods from Phase 22 Step 10.",
        "Do not record operator signoffs from Phase 22 Step 10.",
        "Do not record final approvals from Phase 22 Step 10.",
        "Do not start an implementation phase from Phase 22 Step 10.",
        "Review signoff-ready planning only: execution envelope, transport injection, bridge gate, response storage, cutover, and audit/rollback signoff areas."
    )
}

$jsonPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_signoff_packet.json"
$sectionsCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_signoff_packet_sections.csv"
$entriesCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_signoff_packet_entries.csv"
$areasCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_signoff_packet_signoff_areas.csv"
$decisionsCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_signoff_packet_decisions.csv"
$familiesCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_signoff_packet_artifact_families.csv"
$gitStatusCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_signoff_packet_git_status.csv"
$stagedCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_signoff_packet_staged.csv"
$sourceArtifactsCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_signoff_packet_source_artifacts.csv"
$checklistCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_signoff_packet_checklist.csv"
$issuesCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_signoff_packet_issues.csv"
$mdPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_signoff_packet.md"

$report | ConvertTo-Json -Depth 100 | Set-Content -LiteralPath $jsonPath -Encoding UTF8
$signoffSections | Export-Csv -LiteralPath $sectionsCsvPath -NoTypeInformation -Encoding UTF8
$signoffEntries | Export-Csv -LiteralPath $entriesCsvPath -NoTypeInformation -Encoding UTF8
$signoffAreas | Export-Csv -LiteralPath $areasCsvPath -NoTypeInformation -Encoding UTF8
...
$sectionText = (@($signoffSections) | ForEach-Object { "- $($_.section): $($_.status); $($_.summary)" }) -join "`n"
$entryText = (@($signoffEntries) | ForEach-Object { "- [$($_.category)] $($_.action) $($_.item): $($_.evidence)" }) -join "`n"
$signoffText = (@($signoffAreas) | ForEach-Object { "- $($_.item): depends_on=$($_.depends_on); status=$($_.status)" }) -join "`n"
...
$md = @"
# Phase 22 Step 10 - Phase 20 Network Transport Planning Signoff Packet

Generated: $($report.generated_at)

Source planning freeze packet:
...
- Phase 20 network transport planning signoff packet only: true
- Planning signoff only: true
- Planning only: true
...
- Sections: $(@($signoffSections).Count)
- Entries: $(@($signoffEntries).Count)
- Signoff areas: $(@($signoffAreas).Count)
...
if ($blockerCount -eq 0) {
    Write-Host "PASS | phase20_network_transport_planning_signoff_packet=$OutputDir | status=$packetStatus | bridge_post_called=False | execution_implementation_created=False | implementation_phase_started=False"
} else {
    Write-Host "CHECK | phase20_network_transport_planning_signoff_packet=$OutputDir | status=$packetStatus | blockers=$blockerCount | bridge_post_called=False | execution_implementation_created=False | implementation_phase_started=False"
}

Write-Host ""
Write-Host "Planning signoff packet files:"
