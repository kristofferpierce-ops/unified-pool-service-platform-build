...
$StepNumber = 96
$StepTitle = "Phase 27 Step 96 - Phase 20 Network Transport Implementation Staging Runtime Final Release Hold Planning Approval Boundary Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Staging Runtime Final Release Hold Planning Approval Boundary Packet"
$ExpectedBranch = "phase27-step96-implementation-staging-runtime-final-release-hold-planning-approval-boundary"
$PriorStep = "Phase 27 Step 95 - Phase 20 Network Transport Implementation Staging Runtime Final Release Hold Planning Safety Disposition Review Packet"
$Slug = "implementation_staging_runtime_final_release_hold_planning_approval_boundary"

if (-not $RepoRoot) { $RepoRoot = (Resolve-Path (Join-Path $PSScriptRoot ".." )).Path }

$StepFiles = @("scripts/phase27_step96_implementation_staging_runtime_final_release_hold_planning_approval_boundary_packet.ps1", "ui/pages/712_Phase27_Step96_Implementation_Staging_Runtime_Final_Release_Hold_Planning_Approval_Boundary_Packet.py", "docs/PHASE27_STEP96_IMPLEMENTATION_STAGING_RUNTIME_FINAL_RELEASE_HOLD_PLANNING_APPROVAL_BOUNDARY_PACKET.md", "tests/test_phase27_step96_implementation_staging_runtime_final_release_hold_planning_approval_boundary_packet.py")
...
$SafetyPosture = [ordered]@{
    ...
    phase27_boundary = "implementation_staging_runtime_final_release_hold_planning_approval_boundary_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 27 Step 96 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 27 Step 96 Phase 20 Network Transport Implementation Staging Runtime Final Release Hold Planning Approval Boundary Packet is present and planning-only."
}

function New-Packet {
    $Timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
    $BackupRoot = Join-Path $RepoRoot "backups"
    $PacketDir = Join-Path $BackupRoot ("phase27_step96_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase27_step96_{0}_packet.json" -f $Slug)

    $Payload = [ordered]@{ phase = 27; step = 96; title = $StepTitle; prior_step = $PriorStep; safety_posture = $SafetyPosture; files = $StepFiles; generated_at = (Get-Date).ToString("s") }
    ...
    Write-Host "PASS: phase27_boundary=implementation_staging_runtime_final_release_hold_planning_approval_boundary_opened_by_packet"
    ...
    Write-Host "PASS: implementation_staging_runtime_final_release_hold_planning_approval_boundary_mode=reference_only"
    Write-Host "PASS: implementation_staging_runtime_final_release_hold_planning_approval_boundary_write=false"
    Write-Host "PASS: implementation_staging_runtime_final_release_hold_planning_approval_boundary_record_creation=false"
