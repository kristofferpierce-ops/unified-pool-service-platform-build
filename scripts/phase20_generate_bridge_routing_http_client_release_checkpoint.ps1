...
if ([string]::IsNullOrWhiteSpace($OperatorSignoffDir)) {
    $operatorSignoffArtifact = Get-LatestArtifact -FolderFilter "phase20_bridge_routing_http_client_operator_signoff_*" -JsonName "phase20_bridge_routing_http_client_operator_signoff.json" -Required $true
} else {
    $signoffJson = Join-Path $OperatorSignoffDir "phase20_bridge_routing_http_client_operator_signoff.json"
    if (!(Test-Path -LiteralPath $signoffJson)) {
        throw "HTTP client operator signoff JSON not found: $signoffJson"
    }
    $operatorSignoffArtifact = [ordered]@{
        ...
        json_name = "phase20_bridge_routing_http_client_operator_signoff.json"
        last_write_time = (Get-Item -LiteralPath $signoffJson).LastWriteTime.ToString("o")
    }
}

$artifactSpecs = @(
    @{ key="phase19_no_post_release_checkpoint"; filter="phase19_bridge_routing_write_release_checkpoint_*"; json="phase19_bridge_routing_write_release_checkpoint.json"; required=$true },
    @{ key="http_client_stub"; filter="phase20_bridge_routing_http_client_stub_*"; json="phase20_bridge_routing_http_client_stub.json"; required=$true },
    @{ key="http_client_dry_run"; filter="phase20_bridge_routing_http_client_dry_run_*"; json="phase20_bridge_routing_http_client_dry_run.json"; required=$true },
    @{ key="http_client_dry_run_validation"; filter="phase20_bridge_routing_http_client_dry_run_validation_*"; json="phase20_bridge_routing_http_client_dry_run_validation.json"; required=$true },
    @{ key="http_client_preflight_matrix"; filter="phase20_bridge_routing_http_client_preflight_matrix_*"; json="phase20_bridge_routing_http_client_preflight_matrix.json"; required=$true },
    @{ key="http_client_operator_signoff"; artifact=$operatorSignoffArtifact; required=$true }
)
...
if ($stubStatus.ok) {
    ...
    Add-Gate $gates "runtime_stub_no_bridge_post" ($stubStatus.value.bridge_post_called -eq $false -and $stubStatus.value.bridge_post_call_implemented -eq $false) "blocker" "bridge_post_called=$($stubStatus.value.bridge_post_called); bridge_post_call_implemented=$($stubStatus.value.bridge_post_call_implemented)" "runtime"
} else {
...
# Artifact invariants across the Phase 20 no-network chain.
...
if ($artifactData.Contains("http_client_preflight_matrix")) {
    $preflight = $artifactData.http_client_preflight_matrix
    Add-Gate $gates "preflight_can_execute_false" ($preflight.preflight.can_execute_bridge_write_now -eq $false) "blocker" "can_execute_bridge_write_now=$($preflight.preflight.can_execute_bridge_write_now)" "http_client_preflight_matrix"
    Add-Gate $gates "preflight_can_add_network_false" ($preflight.preflight.can_add_network_transport_now -eq $false) "blocker" "can_add_network_transport_now=$($preflight.preflight.can_add_network_transport_now)" "http_client_preflight_matrix"
    Add-Gate $gates "preflight_can_add_real_client_false" ($preflight.preflight.can_add_real_bridge_http_client_now -eq $false) "blocker" "can_add_real_bridge_http_client_now=$($preflight.preflight.can_add_real_bridge_http_client_now)" "http_client_preflight_matrix"
}

if ($artifactData.Contains("http_client_operator_signoff")) {
    $signoff = $artifactData.http_client_operator_signoff
    Add-Gate $gates "operator_signoff_can_execute_false" ($signoff.signoff.can_execute_bridge_write_now -eq $false) "blocker" "can_execute_bridge_write_now=$($signoff.signoff.can_execute_bridge_write_now)" "http_client_operator_signoff"
    Add-Gate $gates "operator_signoff_can_add_network_false" ($signoff.signoff.can_add_network_transport_now -eq $false) "blocker" "can_add_network_transport_now=$($signoff.signoff.can_add_network_transport_now)" "http_client_operator_signoff"
    Add-Gate $gates "operator_signoff_can_add_real_client_false" ($signoff.signoff.can_add_real_bridge_http_client_now -eq $false) "blocker" "can_add_real_bridge_http_client_now=$($signoff.signoff.can_add_real_bridge_http_client_now)" "http_client_operator_signoff"
    Add-Gate $gates "operator_signoff_not_live_approved" ($signoff.signoff.approved_for_live_bridge_write -ne $true) "blocker" "approved_for_live_bridge_write=$($signoff.signoff.approved_for_live_bridge_write)" "http_client_operator_signoff"
    Add-Gate $gates "operator_signoff_not_network_approved" ($signoff.signoff.approved_for_real_network_transport -ne $true) "blocker" "approved_for_real_network_transport=$($signoff.signoff.approved_for_real_network_transport)" "http_client_operator_signoff"
}

if ($artifactData.Contains("http_client_dry_run")) {
    $dryRun = $artifactData.http_client_dry_run
    Add-Gate $gates "dry_run_simulated_would_not_send" ($dryRun.simulation.simulated_response.would_send -eq $false) "blocker" "would_send=$($dryRun.simulation.simulated_response.would_send)" "http_client_dry_run"
    Add-Gate $gates "dry_run_transport_no_network" ($dryRun.simulation.simulated_request.transport -eq "dry_run_no_network") "blocker" "transport=$($dryRun.simulation.simulated_request.transport)" "http_client_dry_run"
}
...
$checkpointStatus = if ($blockerCount -eq 0 -and $reviewCount -eq 0) {
    "release_checkpoint_clean_no_network"
} elseif ($blockerCount -eq 0) {
    "release_checkpoint_review_required_no_network"
} else {
...
if ([string]::IsNullOrWhiteSpace($OutputDir)) {
    ...
    $OutputDir = Join-Path $BackupDir "phase20_bridge_routing_http_client_release_checkpoint_$stamp"
}
...
$checkpoint = [ordered]@{
    ...
    phase = "Phase 20 Step 6"
    purpose = "Bridge routing HTTP client no-network release checkpoint"
    workspace = $Workspace
    platform_api = $PlatformApi
    bridge_url = $BridgeUrl
    safety = [ordered]@{
        bridge_routing_http_client_release_checkpoint_only = $true
    ...
    release_checkpoint = [ordered]@{
        ...
        release_freeze_label = "phase20_bridge_routing_http_client_no_network_checkpoint"
        reason = if ($blockerCount -eq 0) {
            "HTTP client release checkpoint is suitable for review. Phase 20 Step 6 still implements no real network transport and authorizes no execution."
        } else {
            "HTTP client release checkpoint has blockers. Resolve blockers before any future real network transport design."
        }
    }
    ...
    next_recommended_actions = @(
        ...
        "Do not execute bridge writes from Phase 20 Step 6.",
        "Future real transport work must begin from a new explicitly guarded step and keep dry-run as default.",
        "Keep bridge routing controls as the operational write path until explicit cutover."
    )
}

$jsonPath = Join-Path $OutputDir "phase20_bridge_routing_http_client_release_checkpoint.json"
$artifactsCsvPath = Join-Path $OutputDir "phase20_bridge_routing_http_client_release_checkpoint_artifacts.csv"
$gatesCsvPath = Join-Path $OutputDir "phase20_bridge_routing_http_client_release_checkpoint_gates.csv"
$issuesCsvPath = Join-Path $OutputDir "phase20_bridge_routing_http_client_release_checkpoint_issues.csv"
$mdPath = Join-Path $OutputDir "phase20_bridge_routing_http_client_release_checkpoint.md"
...
$md = @"
# Phase 20 Bridge Routing HTTP Client Release Checkpoint

Generated: $($checkpoint.generated_at)

## Safety

- Bridge routing HTTP client release checkpoint only: true
...
if ($blockerCount -eq 0) {
    Write-Host "PASS | bridge_routing_http_client_release_checkpoint=$OutputDir | status=$checkpointStatus | bridge_post_called=False"
} else {
    Write-Host "CHECK | bridge_routing_http_client_release_checkpoint=$OutputDir | status=$checkpointStatus | blockers=$blockerCount | bridge_post_called=False"
}

Write-Host ""
Write-Host "HTTP client release checkpoint files:"
