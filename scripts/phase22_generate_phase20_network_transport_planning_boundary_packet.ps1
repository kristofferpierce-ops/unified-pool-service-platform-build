...
function Classify-RepoPath {
    ...
    if ($CurrentStepFiles -contains $Path) {
        return [ordered]@{
            ...
            should_stage = $true
            reason = "Current Phase 22 Step 2 file."
        }
    }
...
if ([string]::IsNullOrWhiteSpace($PlanningEntryPacketDir)) {
    $PlanningEntryPacketDir = Get-LatestArtifactDir -FolderFilter "phase22_phase20_network_transport_planning_entry_packet_*" -JsonName "phase22_phase20_network_transport_planning_entry_packet.json" -Required $true
}
...
$planningEntryPath = Join-Path $PlanningEntryPacketDir "phase22_phase20_network_transport_planning_entry_packet.json"
...
if (!(Test-Path -LiteralPath $planningEntryPath)) {
    throw "Planning entry packet JSON not found: $planningEntryPath"
}

$planningEntry = Read-JsonFile $planningEntryPath
...
$boundarySections = New-Object System.Collections.ArrayList
$boundaryEntries = New-Object System.Collections.ArrayList
$boundaryReviews = New-Object System.Collections.ArrayList
...
Add-ChecklistItem $checklist "artifact" "Planning entry packet exists" $true $planningEntryPath
...
Add-ChecklistItem $checklist "planning_entry" "Planning entry packet is boundary-safe" ($planningEntry.safety.phase20_network_transport_planning_entry_packet_only -eq $true) "phase20_network_transport_planning_entry_packet_only=$($planningEntry.safety.phase20_network_transport_planning_entry_packet_only)"
Add-ChecklistItem $checklist "planning_entry" "Planning entry recorded no closure record" ($planningEntry.safety.design_closure_record_created -eq $false) "design_closure_record_created=$($planningEntry.safety.design_closure_record_created)"
Add-ChecklistItem $checklist "planning_entry" "Planning entry recorded no final approval" ($planningEntry.safety.final_approval_recorded -eq $false) "final_approval_recorded=$($planningEntry.safety.final_approval_recorded)"
Add-ChecklistItem $checklist "planning_entry" "Planning entry started no implementation phase" ($planningEntry.safety.implementation_phase_started -eq $false) "implementation_phase_started=$($planningEntry.safety.implementation_phase_started)"
Add-ChecklistItem $checklist "planning_entry" "Planning entry has no execution implementation" ($planningEntry.safety.execution_implementation_created -eq $false) "execution_implementation_created=$($planningEntry.safety.execution_implementation_created)"
Add-ChecklistItem $checklist "planning_entry" "Planning entry has no network transport" ($planningEntry.safety.network_transport_implemented -eq $false) "network_transport_implemented=$($planningEntry.safety.network_transport_implemented)"
Add-ChecklistItem $checklist "planning_entry" "Planning entry has no socket opened" ($planningEntry.safety.network_socket_opened -eq $false) "network_socket_opened=$($planningEntry.safety.network_socket_opened)"
Add-ChecklistItem $checklist "planning_entry" "Planning entry has no bridge POST" ($planningEntry.safety.bridge_post_called -eq $false -and $planningEntry.safety.bridge_post_call_implemented -eq $false) "bridge_post_called=$($planningEntry.safety.bridge_post_called); bridge_post_call_implemented=$($planningEntry.safety.bridge_post_call_implemented)"
Add-ChecklistItem $checklist "planning_entry" "Planning entry has no platform DB mutation" ($planningEntry.safety.platform_db_mutation_performed -eq $false) "platform_db_mutation_performed=$($planningEntry.safety.platform_db_mutation_performed)"
Add-ChecklistItem $checklist "planning_entry" "Planning entry has no bridge mutation" ($planningEntry.safety.bridge_mutation_performed -eq $false) "bridge_mutation_performed=$($planningEntry.safety.bridge_mutation_performed)"
Add-ChecklistItem $checklist "planning_entry" "Planning entry has no LACRM call" ($planningEntry.safety.lacrm_call_performed -eq $false) "lacrm_call_performed=$($planningEntry.safety.lacrm_call_performed)"
Add-ChecklistItem $checklist "planning_entry" "Planning entry has no blockers" ([int]$planningEntry.planning_entry_packet.blocker_count -eq 0) "blocker_count=$($planningEntry.planning_entry_packet.blocker_count)"
...
$planningPrefixes = @($planningEntry.planning_entries | Where-Object { $_.category -eq "do_not_stage" } | ForEach-Object { [string]$_.item })
if (@($planningPrefixes).Count -gt 0) {
    $doNotStagePrefixes = $planningPrefixes
}
...
# Planning boundary uses git status --short against the platform repo.
...
$artifactFamilies = @($planningEntry.artifact_families | Sort-Object @{Expression = {[int]$_.older_generated_folder_count}; Descending = $true}, @{Expression = {[string]$_.folder_filter}; Descending = $false})
$archiveDecisions = @($planningEntry.archive_decisions | Sort-Object @{Expression = {[int]$_.archive_order}; Descending = $false}, @{Expression = {[string]$_.folder_filter}; Descending = $false})
...
$boundarySpecs = @(
    [ordered]@{ key="execution_boundary"; label="Execution boundary review"; summary="Review execution boundary only. Do not create execution implementation and do not call interface execution methods."; required="recommended" },
    [ordered]@{ key="transport_boundary"; label="Transport adapter boundary review"; summary="Review transport adapter boundary only. Do not add real network transport, do not enable it, do not arm it, and do not open sockets."; required="recommended" },
    [ordered]@{ key="bridge_post_boundary"; label="Bridge POST boundary review"; summary="Review bridge POST request boundary only. Do not implement or call bridge POST endpoints."; required="recommended" },
    [ordered]@{ key="response_capture_boundary"; label="Response capture boundary review"; summary="Review response capture boundary only. Do not capture bridge responses and do not create response capture records."; required="recommended" },
    [ordered]@{ key="cutover_boundary"; label="Cutover boundary review"; summary="Review cutover prerequisites only. Do not create cutover packets and do not record cutover approvals."; required="recommended" },
    [ordered]@{ key="audit_rollback_boundary"; label="Audit and rollback boundary review"; summary="Review audit and rollback prerequisites only. Do not create audit rows, rollback rows, or rollback snapshots."; required="recommended" }
)

foreach ($spec in $boundarySpecs) {
    [void]$boundaryReviews.Add([ordered]@{
        boundary_key = [string]$spec.key
        boundary_label = [string]$spec.label
        status = "review_only"
        summary = [string]$spec.summary
        required = [string]$spec.required
    })
}
...
[void]$boundarySections.Add([ordered]@{
    section = "Phase 22 planning boundary state"
    status = "review"
    summary = "Phase 22 remains planning-only. This packet does not authorize implementation, transport, sockets, bridge POST, response capture, mutations, or live writes."
})
[void]$boundarySections.Add([ordered]@{
    section = "Boundary review topics"
    status = "review"
    summary = "Execution, transport adapter, bridge POST, response capture, cutover, and audit/rollback boundaries remain review-only."
})
[void]$boundarySections.Add([ordered]@{
...
foreach ($file in $currentStepFiles) {
    [void]$boundaryEntries.Add([ordered]@{
        category = "stage_now"
        action = "stage"
        item = $file
        evidence = "Current Phase 22 Step 2 file."
        required = "yes"
    })
}
...
foreach ($review in @($boundaryReviews)) {
    [void]$boundaryEntries.Add([ordered]@{
        category = "planning_boundaries"
        action = "review_only"
        item = [string]$review.boundary_label
        evidence = [string]$review.summary
...
$nextCommitCommands = @(
    ...
    "git status --short",
    "git add scripts/phase22_generate_phase20_network_transport_planning_boundary_packet.ps1",
    "git add ui/pages/108_Phase20_Network_Transport_Planning_Boundary_Packet.py",
    "git add docs/PHASE22_STEP2_PHASE20_NETWORK_TRANSPORT_PLANNING_BOUNDARY_PACKET.md",
    "git add tests/test_phase22_phase20_network_transport_planning_boundary_packet.py",
    "git diff --cached --name-only",
    'git commit -m "Phase 22 Step 2 phase20 network transport planning boundary packet"',
    "git push -u origin phase22-step2-phase20-network-transport-planning-boundary-packet"
)

Add-ChecklistItem $checklist "planning_boundary" "Current branch detected" (-not [string]::IsNullOrWhiteSpace($currentBranch)) "current_branch=$currentBranch"
Add-ChecklistItem $checklist "planning_boundary" "At least one planning-boundary section exists" (@($boundarySections).Count -gt 0) "planning_boundary_sections=$(@($boundarySections).Count)"
Add-ChecklistItem $checklist "planning_boundary" "Exactly four stage-now entries exist" (@($boundaryEntries | Where-Object { $_.category -eq "stage_now" }).Count -eq 4) "stage_now_entries=$(@($boundaryEntries | Where-Object { $_.category -eq "stage_now" }).Count)"
Add-ChecklistItem $checklist "planning_boundary" "Boundary review topics were added" (@($boundaryReviews).Count -ge 6) "boundary_review_topics=$(@($boundaryReviews).Count)" "recommended"
Add-ChecklistItem $checklist "planning_boundary" "Archive decisions were carried forward" (@($archiveDecisions).Count -eq @($artifactFamilies).Count) "archive_decisions=$(@($archiveDecisions).Count); artifact_families=$(@($artifactFamilies).Count)" "recommended"
Add-ChecklistItem $checklist "planning_boundary" "Commit commands exist" (@($nextCommitCommands).Count -gt 0) "commit_commands=$(@($nextCommitCommands).Count)"
Add-ChecklistItem $checklist "planning_boundary" "No forbidden paths are currently staged" ($stagedForbiddenCount -eq 0) "staged_forbidden_count=$stagedForbiddenCount"
Add-ChecklistItem $checklist "planning_boundary" "No review-other paths are currently staged" ($stagedReviewOtherCount -eq 0) "staged_review_other_count=$stagedReviewOtherCount" "recommended"
Add-ChecklistItem $checklist "planning_boundary" "Workspace review-other files count is zero or acceptable" ($reviewOtherCount -eq 0) "review_other_count=$reviewOtherCount" "recommended"

$planningEntryStatus = [string]$planningEntry.planning_entry_packet.status
if ($planningEntryStatus -eq "phase20_planning_entry_packet_blocked") {
    Add-Issue $issues "blocker" "planning_entry_packet_blocked" "Planning entry packet is blocked." "planning_entry"
} elseif ($planningEntryStatus -eq "phase20_planning_entry_packet_review_required_no_write") {
    Add-Issue $issues "review" "planning_entry_packet_review_required" "Planning entry packet has review items." "planning_entry"
}
...
if ([string]::IsNullOrWhiteSpace($currentBranch)) {
    Add-Issue $issues "blocker" "current_branch_missing" "Current branch was not detected for planning boundary review." "workspace"
}
...
$packetStatus = if ($blockerCount -eq 0 -and $reviewCount -eq 0) {
    "phase20_planning_boundary_packet_ready_no_write"
} elseif ($blockerCount -eq 0) {
    "phase20_planning_boundary_packet_review_required_no_write"
} else {
    "phase20_planning_boundary_packet_blocked"
}

if ([string]::IsNullOrWhiteSpace($OutputDir)) {
    $stamp = Get-Date -Format "yyyyMMdd-HHmmss"
    $OutputDir = Join-Path $BackupDir "phase22_phase20_network_transport_planning_boundary_packet_$stamp"
}
...
$report = [ordered]@{
    ...
    phase = "Phase 22 Step 2"
    purpose = "Phase 20 bridge routing network transport planning boundary packet"
    workspace = $Workspace
    platform_api = $PlatformApi
    bridge_url = $BridgeUrl
    safety = [ordered]@{
        phase20_network_transport_planning_boundary_packet_only = $true
        planning_boundary_only = $true
        planning_only = $true
    ...
    planning_boundary_packet = [ordered]@{
        status = $packetStatus
        blocker_count = $blockerCount
        review_count = $reviewCount
        current_branch = $currentBranch
        section_count = @($boundarySections).Count
        entry_count = @($boundaryEntries).Count
        boundary_review_count = @($boundaryReviews).Count
        ...
        packet_label = "phase22_phase20_network_transport_planning_boundary_no_write"
        reason = if ($blockerCount -eq 0) {
            "Phase 22 Step 2 created a no-write planning boundary packet for the Phase 20 network transport chain. It does not authorize implementation, transport, sockets, bridge POST, response capture, mutations, LACRM calls, or live writes."
        } else {
            "Phase 22 Step 2 found blockers in the Phase 20 planning boundary packet. Resolve blockers before continuing planning."
        }
    }
    boundary_sections = @($boundarySections)
    boundary_entries = @($boundaryEntries)
    boundary_reviews = @($boundaryReviews)
    ...
    counts = [ordered]@{
        sections = @($boundarySections).Count
        entries = @($boundaryEntries).Count
        boundary_reviews = @($boundaryReviews).Count
    ...
    next_recommended_actions = @(
        "Review this planning boundary packet before any Phase 22 implementation planning details are drafted.",
        "Keep the latest packet folder in each family and only review older generated folders for manual archive actions.",
        "Use the stage-now entries as the only files to add for the Phase 22 Step 2 commit.",
        "Carry the do-not-stage entries forward as the hard commit boundary.",
        "Do not add requests, httpx, socket use, bridge POST calls, or routing write endpoints from Phase 22 Step 2.",
        "Do not call interface execution methods from Phase 22 Step 2.",
        "Do not record final approvals from Phase 22 Step 2.",
        "Do not start an implementation phase from Phase 22 Step 2.",
        "Review boundary topics only: execution, transport adapter, bridge POST, response capture, cutover, and audit/rollback."
    )
}

$jsonPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_boundary_packet.json"
$sectionsCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_boundary_packet_sections.csv"
$entriesCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_boundary_packet_entries.csv"
$boundaryCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_boundary_packet_boundaries.csv"
$decisionsCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_boundary_packet_decisions.csv"
$familiesCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_boundary_packet_artifact_families.csv"
$gitStatusCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_boundary_packet_git_status.csv"
$stagedCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_boundary_packet_staged.csv"
$sourceArtifactsCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_boundary_packet_source_artifacts.csv"
$checklistCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_boundary_packet_checklist.csv"
$issuesCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_boundary_packet_issues.csv"
$mdPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_boundary_packet.md"

$report | ConvertTo-Json -Depth 100 | Set-Content -LiteralPath $jsonPath -Encoding UTF8
$boundarySections | Export-Csv -LiteralPath $sectionsCsvPath -NoTypeInformation -Encoding UTF8
$boundaryEntries | Export-Csv -LiteralPath $entriesCsvPath -NoTypeInformation -Encoding UTF8
$boundaryReviews | Export-Csv -LiteralPath $boundaryCsvPath -NoTypeInformation -Encoding UTF8
...
$sectionText = (@($boundarySections) | ForEach-Object { "- $($_.section): $($_.status); $($_.summary)" }) -join "`n"
$entryText = (@($boundaryEntries) | ForEach-Object { "- [$($_.category)] $($_.action) $($_.item): $($_.evidence)" }) -join "`n"
$boundaryText = (@($boundaryReviews) | ForEach-Object { "- $($_.boundary_label): $($_.summary)" }) -join "`n"
...
- Phase 20 network transport planning boundary packet only: true
- Planning boundary only: true
- Planning only: true
...
- Sections: $(@($boundarySections).Count)
- Entries: $(@($boundaryEntries).Count)
- Boundary review topics: $(@($boundaryReviews).Count)
...
if ($blockerCount -eq 0) {
    Write-Host "PASS | phase20_network_transport_planning_boundary_packet=$OutputDir | status=$packetStatus | bridge_post_called=False | execution_implementation_created=False | implementation_phase_started=False"
} else {
    Write-Host "CHECK | phase20_network_transport_planning_boundary_packet=$OutputDir | status=$packetStatus | blockers=$blockerCount | bridge_post_called=False | execution_implementation_created=False | implementation_phase_started=False"
}

Write-Host ""
Write-Host "Planning boundary packet files:"
