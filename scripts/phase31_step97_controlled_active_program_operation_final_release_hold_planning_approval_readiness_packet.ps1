...
$StepNumber = 97
$StepTitle = "Phase 31 Step 97 - Phase 20 Network Transport Implementation Controlled Active Program Operation Final Release Hold Planning Approval Readiness Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Controlled Active Program Operation Final Release Hold Planning Approval Readiness Packet"
$ExpectedBranch = "phase31-step97-controlled-active-program-operation-final-release-hold-planning-approval-readiness"
$PriorStep = "Phase 31 Step 96 - Phase 20 Network Transport Implementation Controlled Active Program Operation Final Release Hold Planning Approval Boundary Packet"
$Slug = "implementation_controlled_active_program_operation_final_release_hold_planning_approval_readiness"
...
$StepFiles = @(
    "scripts/phase31_step97_controlled_active_program_operation_final_release_hold_planning_approval_readiness_packet.ps1",
    "ui/pages/1193_Phase31_Step97_Implementation_Controlled_Active_Program_Operation_Final_Release_Hold_Planning_Approval_Readiness_Packet.py",
    "docs/PHASE31_STEP97_CONTROLLED_ACTIVE_PROGRAM_OPERATION_FINAL_RELEASE_HOLD_PLANNING_APPROVAL_READINESS_PACKET.md",
    "tests/test_phase31_step97_controlled_active_program_operation_final_release_hold_planning_approval_readiness_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase31_boundary = "implementation_controlled_active_program_operation_final_release_hold_planning_approval_readiness_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 31 Step 97 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 31 Step 97 Phase 20 Network Transport Implementation Controlled Active Program Operation Final Release Hold Planning Approval Readiness Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase31_step97_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase31_step97_{0}_packet.json" -f $Slug)
