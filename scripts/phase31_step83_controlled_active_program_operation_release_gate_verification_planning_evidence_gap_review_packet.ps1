...
$StepNumber = 83
$StepTitle = "Phase 31 Step 83 - Phase 20 Network Transport Implementation Controlled Active Program Operation Release Gate Verification Planning Evidence Gap Review Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Controlled Active Program Operation Release Gate Verification Planning Evidence Gap Review Packet"
$ExpectedBranch = "phase31-step83-controlled-active-program-operation-release-gate-verification-planning-evidence-gap-review"
$PriorStep = "Phase 31 Step 82 - Phase 20 Network Transport Implementation Controlled Active Program Operation Release Gate Verification Planning Evidence Index Packet"
$Slug = "implementation_controlled_active_program_operation_release_gate_verification_planning_evidence_gap_review"
...
$StepFiles = @(
    "scripts/phase31_step83_controlled_active_program_operation_release_gate_verification_planning_evidence_gap_review_packet.ps1",
    "ui/pages/1179_Phase31_Step83_Implementation_Controlled_Active_Program_Operation_Release_Gate_Verification_Planning_Evidence_Gap_Review_Packet.py",
    "docs/PHASE31_STEP83_CONTROLLED_ACTIVE_PROGRAM_OPERATION_RELEASE_GATE_VERIFICATION_PLANNING_EVIDENCE_GAP_REVIEW_PACKET.md",
    "tests/test_phase31_step83_controlled_active_program_operation_release_gate_verification_planning_evidence_gap_review_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase31_boundary = "implementation_controlled_active_program_operation_release_gate_verification_planning_evidence_gap_review_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 31 Step 83 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 31 Step 83 Phase 20 Network Transport Implementation Controlled Active Program Operation Release Gate Verification Planning Evidence Gap Review Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase31_step83_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase31_step83_{0}_packet.json" -f $Slug)
