...
$StepNumber = 96
$StepTitle = "Phase 33 Step 96 - Phase 20 Network Transport Implementation Trusted Production Rollout Final Release Hold Planning Approval Boundary Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Trusted Production Rollout Final Release Hold Planning Approval Boundary Packet"
$ExpectedBranch = "phase33-step96-trusted-production-rollout-final-release-hold-planning-approval-boundary"
$PriorStep = "Phase 33 Step 95 - Phase 20 Network Transport Implementation Trusted Production Rollout Final Release Hold Planning Safety Disposition Review Packet"
$Slug = "implementation_trusted_production_rollout_final_release_hold_planning_approval_boundary"
...
$StepFiles = @(
    "scripts/phase33_step96_trusted_production_rollout_final_release_hold_planning_approval_boundary_packet.ps1",
    "ui/pages/1432_Phase33_Step96_Implementation_Trusted_Production_Rollout_Final_Release_Hold_Planning_Approval_Boundary_Packet.py",
    "docs/PHASE33_STEP96_TRUSTED_PRODUCTION_ROLLOUT_FINAL_RELEASE_HOLD_PLANNING_APPROVAL_BOUNDARY_PACKET.md",
    "tests/test_phase33_step96_trusted_production_rollout_final_release_hold_planning_approval_boundary_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase33_boundary = "implementation_trusted_production_rollout_final_release_hold_planning_approval_boundary_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 33 Step 96 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 33 Step 96 Phase 20 Network Transport Implementation Trusted Production Rollout Final Release Hold Planning Approval Boundary Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase33_step96_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase33_step96_{0}_packet.json" -f $Slug)
