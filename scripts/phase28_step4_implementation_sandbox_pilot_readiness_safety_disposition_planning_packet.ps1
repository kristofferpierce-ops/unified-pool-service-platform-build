...
$StepNumber = 4
$StepTitle = "Phase 28 Step 4 - Phase 20 Network Transport Implementation Sandbox Pilot Readiness Safety Disposition Planning Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Sandbox Pilot Readiness Safety Disposition Planning Packet"
$ExpectedBranch = "phase28-step4-implementation-sandbox-pilot-readiness-safety-disposition-planning"
$PriorStep = "Phase 28 Step 3 - Phase 20 Network Transport Implementation Sandbox Pilot Readiness Evidence Gap Review Packet"
$Slug = "implementation_sandbox_pilot_readiness_safety_disposition_planning"

if (-not $RepoRoot) { $RepoRoot = (Resolve-Path (Join-Path $PSScriptRoot ".." )).Path }

$StepFiles = @("scripts/phase28_step4_implementation_sandbox_pilot_readiness_safety_disposition_planning_packet.ps1", "ui/pages/740_Phase28_Step4_Implementation_Sandbox_Pilot_Readiness_Safety_Disposition_Planning_Packet.py", "docs/PHASE28_STEP4_IMPLEMENTATION_SANDBOX_PILOT_READINESS_SAFETY_DISPOSITION_PLANNING_PACKET.md", "tests/test_phase28_step4_implementation_sandbox_pilot_readiness_safety_disposition_planning_packet.py")
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 28 Step 4 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 28 Step 4 Phase 20 Network Transport Implementation Sandbox Pilot Readiness Safety Disposition Planning Packet is present and planning-only."
}

function New-Packet {
    $Timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
    $BackupRoot = Join-Path $RepoRoot "backups"
    $PacketDir = Join-Path $BackupRoot ("phase28_step4_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase28_step4_{0}_packet.json" -f $Slug)
    ...
    Write-Host "PASS: phase28_boundary=implementation_sandbox_pilot_readiness_safety_disposition_planning_opened_by_packet"
    ...
    Write-Host "PASS: implementation_sandbox_pilot_readiness_safety_disposition_planning_mode=reference_only"
    Write-Host "PASS: implementation_sandbox_pilot_readiness_safety_disposition_planning_write=false"
    Write-Host "PASS: implementation_sandbox_pilot_readiness_safety_disposition_planning_record_creation=false"
