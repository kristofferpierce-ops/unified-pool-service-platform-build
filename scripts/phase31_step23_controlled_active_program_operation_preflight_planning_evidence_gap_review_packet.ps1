...
$StepNumber = 23
$StepTitle = "Phase 31 Step 23 - Phase 20 Network Transport Implementation Controlled Active Program Operation Preflight Planning Evidence Gap Review Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Controlled Active Program Operation Preflight Planning Evidence Gap Review Packet"
$ExpectedBranch = "phase31-step23-controlled-active-program-operation-preflight-planning-evidence-gap-review"
$PriorStep = "Phase 31 Step 22 - Phase 20 Network Transport Implementation Controlled Active Program Operation Preflight Planning Evidence Index Packet"
$Slug = "implementation_controlled_active_program_operation_preflight_planning_evidence_gap_review"
...
$StepFiles = @(
    "scripts/phase31_step23_controlled_active_program_operation_preflight_planning_evidence_gap_review_packet.ps1",
    "ui/pages/1119_Phase31_Step23_Implementation_Controlled_Active_Program_Operation_Preflight_Planning_Evidence_Gap_Review_Packet.py",
    "docs/PHASE31_STEP23_CONTROLLED_ACTIVE_PROGRAM_OPERATION_PREFLIGHT_PLANNING_EVIDENCE_GAP_REVIEW_PACKET.md",
    "tests/test_phase31_step23_controlled_active_program_operation_preflight_planning_evidence_gap_review_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase31_boundary = "implementation_controlled_active_program_operation_preflight_planning_evidence_gap_review_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 31 Step 23 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 31 Step 23 Phase 20 Network Transport Implementation Controlled Active Program Operation Preflight Planning Evidence Gap Review Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase31_step23_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase31_step23_{0}_packet.json" -f $Slug)
