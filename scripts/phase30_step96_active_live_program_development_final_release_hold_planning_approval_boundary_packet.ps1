...
$StepNumber = 96
$StepTitle = "Phase 30 Step 96 - Phase 20 Network Transport Implementation Active Live Program Development Final Release Hold Planning Approval Boundary Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Active Live Program Development Final Release Hold Planning Approval Boundary Packet"
$ExpectedBranch = "phase30-step96-active-live-program-development-final-release-hold-planning-approval-boundary"
$PriorStep = "Phase 30 Step 95 - Phase 20 Network Transport Implementation Active Live Program Development Final Release Hold Planning Safety Disposition Review Packet"
$Slug = "implementation_active_live_program_development_final_release_hold_planning_approval_boundary"
...
$StepFiles = @(
    "scripts/phase30_step96_active_live_program_development_final_release_hold_planning_approval_boundary_packet.ps1",
    "ui/pages/1072_Phase30_Step96_Implementation_Active_Live_Program_Development_Final_Release_Hold_Planning_Approval_Boundary_Packet.py",
    "docs/PHASE30_STEP96_ACTIVE_LIVE_PROGRAM_DEVELOPMENT_FINAL_RELEASE_HOLD_PLANNING_APPROVAL_BOUNDARY_PACKET.md",
    "tests/test_phase30_step96_active_live_program_development_final_release_hold_planning_approval_boundary_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase30_boundary = "implementation_active_live_program_development_final_release_hold_planning_approval_boundary_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 30 Step 96 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 30 Step 96 Phase 20 Network Transport Implementation Active Live Program Development Final Release Hold Planning Approval Boundary Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase30_step96_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase30_step96_{0}_packet.json" -f $Slug)
