...
function Classify-RepoPath {
    ...
    if ($CurrentStepFiles -contains $Path) {
        return [ordered]@{
            ...
            should_stage = $true
            reason = "Current Phase 22 Step 6 file."
        }
    }
...
if ([string]::IsNullOrWhiteSpace($PlanningPrerequisitePacketDir)) {
    $PlanningPrerequisitePacketDir = Get-LatestArtifactDir -FolderFilter "phase22_phase20_network_transport_planning_prerequisite_packet_*" -JsonName "phase22_phase20_network_transport_planning_prerequisite_packet.json" -Required $true
}
if ([string]::IsNullOrWhiteSpace($PlanningDependencyPacketDir)) {
    $PlanningDependencyPacketDir = Get-LatestArtifactDir -FolderFilter "phase22_phase20_network_transport_planning_dependency_packet_*" -JsonName "phase22_phase20_network_transport_planning_dependency_packet.json" -Required $false
}
...
$planningPrerequisitePath = Join-Path $PlanningPrerequisitePacketDir "phase22_phase20_network_transport_planning_prerequisite_packet.json"
$planningDependencyPath = if (![string]::IsNullOrWhiteSpace($PlanningDependencyPacketDir)) { Join-Path $PlanningDependencyPacketDir "phase22_phase20_network_transport_planning_dependency_packet.json" } else { "" }
...
if (!(Test-Path -LiteralPath $planningPrerequisitePath)) {
    throw "Planning prerequisite packet JSON not found: $planningPrerequisitePath"
}

$planningPrerequisite = Read-JsonFile $planningPrerequisitePath
$planningDependency = if (![string]::IsNullOrWhiteSpace($planningDependencyPath) -and (Test-Path -LiteralPath $planningDependencyPath)) { Read-JsonFile $planningDependencyPath } else { $null }
...
$sequenceSections = New-Object System.Collections.ArrayList
$sequenceEntries = New-Object System.Collections.ArrayList
$sequenceAreas = New-Object System.Collections.ArrayList
...
Add-ChecklistItem $checklist "artifact" "Planning prerequisite packet exists" $true $planningPrerequisitePath
Add-ChecklistItem $checklist "artifact" "Planning dependency packet exists" ($planningDependency -ne $null) $planningDependencyPath "recommended"
Add-ChecklistItem $checklist "artifact" "Cleanup exit packet exists" ($cleanupExit -ne $null) $cleanupExitPath "recommended"

Add-ChecklistItem $checklist "planning_prerequisite" "Planning prerequisite packet is sequence-safe" ($planningPrerequisite.safety.phase20_network_transport_planning_prerequisite_packet_only -eq $true) "phase20_network_transport_planning_prerequisite_packet_only=$($planningPrerequisite.safety.phase20_network_transport_planning_prerequisite_packet_only)"
Add-ChecklistItem $checklist "planning_prerequisite" "Planning prerequisite created no closure record" ($planningPrerequisite.safety.design_closure_record_created -eq $false) "design_closure_record_created=$($planningPrerequisite.safety.design_closure_record_created)"
Add-ChecklistItem $checklist "planning_prerequisite" "Planning prerequisite recorded no final approval" ($planningPrerequisite.safety.final_approval_recorded -eq $false) "final_approval_recorded=$($planningPrerequisite.safety.final_approval_recorded)"
Add-ChecklistItem $checklist "planning_prerequisite" "Planning prerequisite started no implementation phase" ($planningPrerequisite.safety.implementation_phase_started -eq $false) "implementation_phase_started=$($planningPrerequisite.safety.implementation_phase_started)"
Add-ChecklistItem $checklist "planning_prerequisite" "Planning prerequisite has no execution implementation" ($planningPrerequisite.safety.execution_implementation_created -eq $false) "execution_implementation_created=$($planningPrerequisite.safety.execution_implementation_created)"
Add-ChecklistItem $checklist "planning_prerequisite" "Planning prerequisite has no network transport" ($planningPrerequisite.safety.network_transport_implemented -eq $false) "network_transport_implemented=$($planningPrerequisite.safety.network_transport_implemented)"
Add-ChecklistItem $checklist "planning_prerequisite" "Planning prerequisite has no socket opened" ($planningPrerequisite.safety.network_socket_opened -eq $false) "network_socket_opened=$($planningPrerequisite.safety.network_socket_opened)"
Add-ChecklistItem $checklist "planning_prerequisite" "Planning prerequisite has no bridge POST" ($planningPrerequisite.safety.bridge_post_called -eq $false -and $planningPrerequisite.safety.bridge_post_call_implemented -eq $false) "bridge_post_called=$($planningPrerequisite.safety.bridge_post_called); bridge_post_call_implemented=$($planningPrerequisite.safety.bridge_post_call_implemented)"
Add-ChecklistItem $checklist "planning_prerequisite" "Planning prerequisite has no platform DB mutation" ($planningPrerequisite.safety.platform_db_mutation_performed -eq $false) "platform_db_mutation_performed=$($planningPrerequisite.safety.platform_db_mutation_performed)"
Add-ChecklistItem $checklist "planning_prerequisite" "Planning prerequisite has no bridge mutation" ($planningPrerequisite.safety.bridge_mutation_performed -eq $false) "bridge_mutation_performed=$($planningPrerequisite.safety.bridge_mutation_performed)"
Add-ChecklistItem $checklist "planning_prerequisite" "Planning prerequisite has no LACRM call" ($planningPrerequisite.safety.lacrm_call_performed -eq $false) "lacrm_call_performed=$($planningPrerequisite.safety.lacrm_call_performed)"
Add-ChecklistItem $checklist "planning_prerequisite" "Planning prerequisite has no blockers" ([int]$planningPrerequisite.planning_prerequisite_packet.blocker_count -eq 0) "blocker_count=$($planningPrerequisite.planning_prerequisite_packet.blocker_count)"

if ($planningDependency -ne $null) {
    Add-ChecklistItem $checklist "planning_dependency" "Planning dependency packet is planning-only" ($planningDependency.safety.planning_only -eq $true) "planning_only=$($planningDependency.safety.planning_only)" "recommended"
}
...
$prerequisitePrefixes = @($planningPrerequisite.planning_entries | Where-Object { $_.category -eq "do_not_stage" } | ForEach-Object { [string]$_.item })
if (@($prerequisitePrefixes).Count -gt 0) {
    $doNotStagePrefixes = $prerequisitePrefixes
}

$currentStepFiles = @(
    "scripts/phase22_generate_phase20_network_transport_planning_sequence_packet.ps1",
    "ui/pages/112_Phase20_Network_Transport_Planning_Sequence_Packet.py",
    "docs/PHASE22_STEP6_PHASE20_NETWORK_TRANSPORT_PLANNING_SEQUENCE_PACKET.md",
    "tests/test_phase22_phase20_network_transport_planning_sequence_packet.py"
)

$currentBranch = (& git -C $PlatformDir branch --show-current 2>$null | Out-String).Trim()

# Planning sequence uses git status --short against the platform repo.
...
if ($planningPrerequisite.artifact_families) {
    foreach ($family in @($planningPrerequisite.artifact_families)) {
        [void]$artifactFamilies.Add($family)
    }
}
if ($planningPrerequisite.archive_decisions) {
    foreach ($decision in @($planningPrerequisite.archive_decisions)) {
        [void]$archiveDecisions.Add($decision)
    }
}

$sequenceSpecs = @(
    [ordered]@{ key="execution_envelope_sequence"; item="Execution envelope sequence"; depends_on="Execution envelope prerequisite"; summary="Review execution request, result, error, and dry-run envelope ordering before any future implementation work."; implementation_allowed=$false; bridge_post_allowed=$false; socket_allowed=$false; sequence_order=1 },
    [ordered]@{ key="transport_injection_sequence"; item="Transport injection sequence"; depends_on="Transport injection prerequisite"; summary="Review dependency injection ordering and transport-off defaults before any future transport work."; implementation_allowed=$false; bridge_post_allowed=$false; socket_allowed=$false; sequence_order=2 },
    [ordered]@{ key="bridge_gate_sequence"; item="Bridge gate sequence"; depends_on="Bridge gate prerequisite"; summary="Review confirmation phrase, armed-state, environment gate, and explicit POST precondition ordering before any future bridge POST work."; implementation_allowed=$false; bridge_post_allowed=$false; socket_allowed=$false; sequence_order=3 },
    [ordered]@{ key="response_storage_sequence"; item="Response storage sequence"; depends_on="Response storage prerequisite"; summary="Review response record, storage, and idempotent capture ordering before any future response capture work."; implementation_allowed=$false; bridge_post_allowed=$false; socket_allowed=$false; sequence_order=4 },
    [ordered]@{ key="cutover_review_sequence"; item="Cutover review sequence"; depends_on="Cutover review prerequisite"; summary="Review cutover prerequisite ordering, verification checkpoints, and rollback review before any future cutover work."; implementation_allowed=$false; bridge_post_allowed=$false; socket_allowed=$false; sequence_order=5 },
    [ordered]@{ key="audit_rollback_sequence"; item="Audit and rollback sequence"; depends_on="Audit and rollback prerequisite"; summary="Review audit record, rollback record, and snapshot ordering before any future live-write work."; implementation_allowed=$false; bridge_post_allowed=$false; socket_allowed=$false; sequence_order=6 }
)

foreach ($spec in $sequenceSpecs) {
    [void]$sequenceAreas.Add([ordered]@{
        key = $spec.key
        item = $spec.item
    ...
    [void]$sequenceEntries.Add([ordered]@{
        category = "sequence_areas"
        action = "review_only"
        item = $spec.item
...
[void]$sequenceSections.Add([ordered]@{
    section = "Planning sequence state"
    status = "review"
    summary = "Phase 22 Step 6 remains planning-only and converts prerequisite areas into explicit review-only sequence areas."
})
[void]$sequenceSections.Add([ordered]@{
    section = "Sequence areas"
    status = "review"
    summary = "Execution envelope, transport injection, bridge gate, response storage, cutover review, and audit/rollback sequence areas remain review-only."
})
[void]$sequenceSections.Add([ordered]@{
...
foreach ($file in $currentStepFiles) {
    [void]$sequenceEntries.Add([ordered]@{
        ...
        action = "stage"
        item = $file
        evidence = "Current Phase 22 Step 6 file."
        required = "yes"
    })
}
...
$nextCommitCommands = @(
    ...
    "git status --short",
    "git add scripts/phase22_generate_phase20_network_transport_planning_sequence_packet.ps1",
    "git add ui/pages/112_Phase20_Network_Transport_Planning_Sequence_Packet.py",
    "git add docs/PHASE22_STEP6_PHASE20_NETWORK_TRANSPORT_PLANNING_SEQUENCE_PACKET.md",
    "git add tests/test_phase22_phase20_network_transport_planning_sequence_packet.py",
    "git diff --cached --name-only",
    'git commit -m "Phase 22 Step 6 phase20 network transport planning sequence packet"',
    "git push -u origin phase22-step6-phase20-network-transport-planning-sequence-packet"
)

Add-ChecklistItem $checklist "planning_sequence" "Current branch detected" (-not [string]::IsNullOrWhiteSpace($currentBranch)) "current_branch=$currentBranch"
Add-ChecklistItem $checklist "planning_sequence" "At least one planning-sequence section exists" (@($sequenceSections).Count -gt 0) "planning_sequence_sections=$(@($sequenceSections).Count)"
Add-ChecklistItem $checklist "planning_sequence" "Exactly four stage-now entries exist" (@($sequenceEntries | Where-Object { $_.category -eq "stage_now" }).Count -eq 4) "stage_now_entries=$(@($sequenceEntries | Where-Object { $_.category -eq "stage_now" }).Count)"
Add-ChecklistItem $checklist "planning_sequence" "Sequence areas were generated" (@($sequenceAreas).Count -ge 6) "sequence_areas=$(@($sequenceAreas).Count)" "recommended"
Add-ChecklistItem $checklist "planning_sequence" "Archive decisions were carried forward" (@($archiveDecisions).Count -eq @($artifactFamilies).Count) "archive_decisions=$(@($archiveDecisions).Count); artifact_families=$(@($artifactFamilies).Count)" "recommended"
Add-ChecklistItem $checklist "planning_sequence" "Commit commands exist" (@($nextCommitCommands).Count -gt 0) "commit_commands=$(@($nextCommitCommands).Count)"
Add-ChecklistItem $checklist "planning_sequence" "No forbidden paths are currently staged" ($stagedForbiddenCount -eq 0) "staged_forbidden_count=$stagedForbiddenCount"
Add-ChecklistItem $checklist "planning_sequence" "No review-other paths are currently staged" ($stagedReviewOtherCount -eq 0) "staged_review_other_count=$stagedReviewOtherCount" "recommended"
Add-ChecklistItem $checklist "planning_sequence" "Workspace review-other files count is zero or acceptable" ($reviewOtherCount -eq 0) "review_other_count=$reviewOtherCount" "recommended"

$planningPrerequisiteStatus = [string]$planningPrerequisite.planning_prerequisite_packet.status
if ($planningPrerequisiteStatus -eq "phase20_planning_prerequisite_packet_blocked") {
    Add-Issue $issues "blocker" "planning_prerequisite_packet_blocked" "Planning prerequisite packet is blocked." "planning_prerequisite"
} elseif ($planningPrerequisiteStatus -eq "phase20_planning_prerequisite_packet_review_required_no_write") {
    Add-Issue $issues "review" "planning_prerequisite_packet_review_required" "Planning prerequisite packet has review items." "planning_prerequisite"
}
...
if ([string]::IsNullOrWhiteSpace($currentBranch)) {
    Add-Issue $issues "blocker" "current_branch_missing" "Current branch was not detected for planning sequence." "workspace"
}
...
$packetStatus = if ($blockerCount -eq 0 -and $reviewCount -eq 0) {
    "phase20_planning_sequence_packet_ready_no_write"
} elseif ($blockerCount -eq 0) {
    "phase20_planning_sequence_packet_review_required_no_write"
} else {
    "phase20_planning_sequence_packet_blocked"
}

if ([string]::IsNullOrWhiteSpace($OutputDir)) {
    $stamp = Get-Date -Format "yyyyMMdd-HHmmss"
    $OutputDir = Join-Path $BackupDir "phase22_phase20_network_transport_planning_sequence_packet_$stamp"
}
...
$report = [ordered]@{
    ...
    phase = "Phase 22 Step 6"
    purpose = "Phase 20 bridge routing network transport planning sequence packet"
    workspace = $Workspace
    platform_api = $PlatformApi
    bridge_url = $BridgeUrl
    safety = [ordered]@{
        phase20_network_transport_planning_sequence_packet_only = $true
        planning_sequence_only = $true
        planning_only = $true
    ...
    planning_sequence_packet = [ordered]@{
        status = $packetStatus
        blocker_count = $blockerCount
        review_count = $reviewCount
        current_branch = $currentBranch
        section_count = @($sequenceSections).Count
        entry_count = @($sequenceEntries).Count
        sequence_area_count = @($sequenceAreas).Count
        ...
        packet_label = "phase22_phase20_network_transport_planning_sequence_no_write"
        reason = if ($blockerCount -eq 0) {
            "Phase 22 Step 6 created a no-write planning sequence packet for the Phase 20 network transport chain. It does not authorize implementation, transport, sockets, bridge POST, response capture, mutations, LACRM calls, or live writes."
        } else {
            "Phase 22 Step 6 found blockers in the Phase 20 planning sequence packet. Resolve blockers before continuing planning."
        }
    }
    planning_sections = @($sequenceSections)
    planning_entries = @($sequenceEntries)
    sequence_areas = @($sequenceAreas)
    ...
    counts = [ordered]@{
        sections = @($sequenceSections).Count
        entries = @($sequenceEntries).Count
        sequence_areas = @($sequenceAreas).Count
    ...
    next_recommended_actions = @(
        "Review this planning sequence packet before any Phase 22 design-detail packets are drafted.",
        "Keep the latest packet folder in each family and only review older generated folders for manual archive actions.",
        "Use the stage-now entries as the only files to add for the Phase 22 Step 6 commit.",
        "Carry the do-not-stage entries forward as the hard commit boundary.",
        "Do not add requests, httpx, socket use, bridge POST calls, or routing write endpoints from Phase 22 Step 6.",
        "Do not call interface execution methods from Phase 22 Step 6.",
        "Do not record final approvals from Phase 22 Step 6.",
        "Do not start an implementation phase from Phase 22 Step 6.",
        "Review sequence areas only: execution envelope, transport injection, bridge gate, response storage, cutover review, and audit/rollback sequence."
    )
}

$jsonPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_sequence_packet.json"
$sectionsCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_sequence_packet_sections.csv"
$entriesCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_sequence_packet_entries.csv"
$sequenceCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_sequence_packet_sequence_areas.csv"
$decisionsCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_sequence_packet_decisions.csv"
$familiesCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_sequence_packet_artifact_families.csv"
$gitStatusCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_sequence_packet_git_status.csv"
$stagedCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_sequence_packet_staged.csv"
$sourceArtifactsCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_sequence_packet_source_artifacts.csv"
$checklistCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_sequence_packet_checklist.csv"
$issuesCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_sequence_packet_issues.csv"
$mdPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_sequence_packet.md"

$report | ConvertTo-Json -Depth 100 | Set-Content -LiteralPath $jsonPath -Encoding UTF8
$sequenceSections | Export-Csv -LiteralPath $sectionsCsvPath -NoTypeInformation -Encoding UTF8
$sequenceEntries | Export-Csv -LiteralPath $entriesCsvPath -NoTypeInformation -Encoding UTF8
$sequenceAreas | Export-Csv -LiteralPath $sequenceCsvPath -NoTypeInformation -Encoding UTF8
...
$sectionText = (@($sequenceSections) | ForEach-Object { "- $($_.section): $($_.status); $($_.summary)" }) -join "`n"
$entryText = (@($sequenceEntries) | ForEach-Object { "- [$($_.category)] $($_.action) $($_.item): $($_.evidence)" }) -join "`n"
$sequenceText = (@($sequenceAreas) | ForEach-Object { "- $($_.item): depends_on=$($_.depends_on); status=$($_.status); sequence_order=$($_.sequence_order)" }) -join "`n"
...
$md = @"
# Phase 22 Step 6 - Phase 20 Network Transport Planning Sequence Packet

Generated: $($report.generated_at)

Source planning prerequisite packet:

``````
$planningPrerequisitePath
``````

## Safety

- Phase 20 network transport planning sequence packet only: true
- Planning sequence only: true
- Planning only: true
...
- Sections: $(@($sequenceSections).Count)
- Entries: $(@($sequenceEntries).Count)
- Sequence areas: $(@($sequenceAreas).Count)
...
if ($blockerCount -eq 0) {
    Write-Host "PASS | phase20_network_transport_planning_sequence_packet=$OutputDir | status=$packetStatus | bridge_post_called=False | execution_implementation_created=False | implementation_phase_started=False"
} else {
    Write-Host "CHECK | phase20_network_transport_planning_sequence_packet=$OutputDir | status=$packetStatus | blockers=$blockerCount | bridge_post_called=False | execution_implementation_created=False | implementation_phase_started=False"
}

Write-Host ""
Write-Host "Planning sequence packet files:"
