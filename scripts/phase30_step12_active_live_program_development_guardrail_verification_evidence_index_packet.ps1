...
$StepNumber = 12
$StepTitle = "Phase 30 Step 12 - Phase 20 Network Transport Implementation Active Live Program Development Guardrail Verification Evidence Index Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Active Live Program Development Guardrail Verification Evidence Index Packet"
$ExpectedBranch = "phase30-step12-active-live-program-development-guardrail-verification-evidence-index"
$PriorStep = "Phase 30 Step 11 - Phase 20 Network Transport Implementation Active Live Program Development Guardrail Verification Boundary Packet"
$Slug = "implementation_active_live_program_development_guardrail_verification_evidence_index"
...
$StepFiles = @(
    "scripts/phase30_step12_active_live_program_development_guardrail_verification_evidence_index_packet.ps1",
    "ui/pages/988_Phase30_Step12_Implementation_Active_Live_Program_Development_Guardrail_Verification_Evidence_Index_Packet.py",
    "docs/PHASE30_STEP12_ACTIVE_LIVE_PROGRAM_DEVELOPMENT_GUARDRAIL_VERIFICATION_EVIDENCE_INDEX_PACKET.md",
    "tests/test_phase30_step12_active_live_program_development_guardrail_verification_evidence_index_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase30_boundary = "implementation_active_live_program_development_guardrail_verification_evidence_index_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 30 Step 12 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 30 Step 12 Phase 20 Network Transport Implementation Active Live Program Development Guardrail Verification Evidence Index Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase30_step12_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase30_step12_{0}_packet.json" -f $Slug)
