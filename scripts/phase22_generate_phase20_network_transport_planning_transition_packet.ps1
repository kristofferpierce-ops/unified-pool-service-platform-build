...
function Classify-RepoPath {
    ...
    if ($CurrentStepFiles -contains $Path) {
        return [ordered]@{
            ...
            should_stage = $true
            reason = "Current Phase 22 Step 14 file."
        }
    }
...
if ([string]::IsNullOrWhiteSpace($PlanningExitPacketDir)) {
    $PlanningExitPacketDir = Get-LatestArtifactDir -FolderFilter "phase22_phase20_network_transport_planning_exit_packet_*" -JsonName "phase22_phase20_network_transport_planning_exit_packet.json" -Required $true
}
if ([string]::IsNullOrWhiteSpace($PlanningClosurePacketDir)) {
    $PlanningClosurePacketDir = Get-LatestArtifactDir -FolderFilter "phase22_phase20_network_transport_planning_closure_packet_*" -JsonName "phase22_phase20_network_transport_planning_closure_packet.json" -Required $false
}
...
$planningExitPath = Join-Path $PlanningExitPacketDir "phase22_phase20_network_transport_planning_exit_packet.json"
$planningClosurePath = if (![string]::IsNullOrWhiteSpace($PlanningClosurePacketDir)) { Join-Path $PlanningClosurePacketDir "phase22_phase20_network_transport_planning_closure_packet.json" } else { "" }
...
if (!(Test-Path -LiteralPath $planningExitPath)) {
    throw "Planning exit packet JSON not found: $planningExitPath"
}

$planningExit = Read-JsonFile $planningExitPath
$planningClosure = if (![string]::IsNullOrWhiteSpace($planningClosurePath) -and (Test-Path -LiteralPath $planningClosurePath)) { Read-JsonFile $planningClosurePath } else { $null }
...
$transitionSections = New-Object System.Collections.ArrayList
$transitionEntries = New-Object System.Collections.ArrayList
$transitionAreas = New-Object System.Collections.ArrayList
...
Add-ChecklistItem $checklist "artifact" "Planning exit packet exists" $true $planningExitPath
Add-ChecklistItem $checklist "artifact" "Planning closure packet exists" ($planningClosure -ne $null) $planningClosurePath "recommended"
Add-ChecklistItem $checklist "artifact" "Cleanup exit packet exists" ($cleanupExit -ne $null) $cleanupExitPath "recommended"

Add-ChecklistItem $checklist "planning_exit" "Planning exit packet is transition-safe" ($planningExit.safety.phase20_network_transport_planning_exit_packet_only -eq $true) "phase20_network_transport_planning_exit_packet_only=$($planningExit.safety.phase20_network_transport_planning_exit_packet_only)"
Add-ChecklistItem $checklist "planning_exit" "Planning exit created no closure record" ($planningExit.safety.design_closure_record_created -eq $false) "design_closure_record_created=$($planningExit.safety.design_closure_record_created)"
Add-ChecklistItem $checklist "planning_exit" "Planning exit recorded no final approval" ($planningExit.safety.final_approval_recorded -eq $false) "final_approval_recorded=$($planningExit.safety.final_approval_recorded)"
Add-ChecklistItem $checklist "planning_exit" "Planning exit recorded no operator signoff" ($planningExit.safety.operator_signoff_recorded -eq $false) "operator_signoff_recorded=$($planningExit.safety.operator_signoff_recorded)"
Add-ChecklistItem $checklist "planning_exit" "Planning exit recorded no operator approval" ($planningExit.safety.operator_approval_recorded -eq $false) "operator_approval_recorded=$($planningExit.safety.operator_approval_recorded)"
Add-ChecklistItem $checklist "planning_exit" "Planning exit started no implementation phase" ($planningExit.safety.implementation_phase_started -eq $false) "implementation_phase_started=$($planningExit.safety.implementation_phase_started)"
Add-ChecklistItem $checklist "planning_exit" "Planning exit has no execution implementation" ($planningExit.safety.execution_implementation_created -eq $false) "execution_implementation_created=$($planningExit.safety.execution_implementation_created)"
Add-ChecklistItem $checklist "planning_exit" "Planning exit has no network transport" ($planningExit.safety.network_transport_implemented -eq $false) "network_transport_implemented=$($planningExit.safety.network_transport_implemented)"
Add-ChecklistItem $checklist "planning_exit" "Planning exit has no socket opened" ($planningExit.safety.network_socket_opened -eq $false) "network_socket_opened=$($planningExit.safety.network_socket_opened)"
Add-ChecklistItem $checklist "planning_exit" "Planning exit has no bridge POST" ($planningExit.safety.bridge_post_called -eq $false -and $planningExit.safety.bridge_post_call_implemented -eq $false) "bridge_post_called=$($planningExit.safety.bridge_post_called); bridge_post_call_implemented=$($planningExit.safety.bridge_post_call_implemented)"
Add-ChecklistItem $checklist "planning_exit" "Planning exit has no platform DB mutation" ($planningExit.safety.platform_db_mutation_performed -eq $false) "platform_db_mutation_performed=$($planningExit.safety.platform_db_mutation_performed)"
Add-ChecklistItem $checklist "planning_exit" "Planning exit has no bridge mutation" ($planningExit.safety.bridge_mutation_performed -eq $false) "bridge_mutation_performed=$($planningExit.safety.bridge_mutation_performed)"
Add-ChecklistItem $checklist "planning_exit" "Planning exit has no LACRM call" ($planningExit.safety.lacrm_call_performed -eq $false) "lacrm_call_performed=$($planningExit.safety.lacrm_call_performed)"
Add-ChecklistItem $checklist "planning_exit" "Planning exit has no blockers" ([int]$planningExit.planning_exit_packet.blocker_count -eq 0) "blocker_count=$($planningExit.planning_exit_packet.blocker_count)"

if ($planningClosure -ne $null) {
    Add-ChecklistItem $checklist "planning_closure" "Planning closure packet is planning-only" ($planningClosure.safety.planning_only -eq $true) "planning_only=$($planningClosure.safety.planning_only)" "recommended"
}
...
$exitPrefixes = @($planningExit.planning_entries | Where-Object { $_.category -eq "do_not_stage" } | ForEach-Object { [string]$_.item })
...
if ($planningExit.artifact_families) {
    foreach ($family in @($planningExit.artifact_families)) {
        [void]$artifactFamilies.Add($family)
    }
}
if ($planningExit.archive_decisions) {
    foreach ($decision in @($planningExit.archive_decisions)) {
        [void]$archiveDecisions.Add($decision)
    }
}

$transitionSpecs = @(
    [ordered]@{ key="execution_envelope_transition"; item="Execution envelope transition review"; depends_on="Execution envelope exit review"; summary="Review execution request, result, error, and dry-run envelope completeness as transition-ready planning material only."; implementation_allowed=$false; bridge_post_allowed=$false; socket_allowed=$false; transition_level="planning_only" },
    [ordered]@{ key="transport_injection_transition"; item="Transport injection transition review"; depends_on="Transport injection exit review"; summary="Review dependency injection seams and transport-off defaults as transition-ready planning material only."; implementation_allowed=$false; bridge_post_allowed=$false; socket_allowed=$false; transition_level="planning_only" },
    [ordered]@{ key="bridge_gate_transition"; item="Bridge gate transition review"; depends_on="Bridge gate exit review"; summary="Review confirmation phrase, armed-state, environment gate, and explicit POST precondition coverage as transition-ready planning material only."; implementation_allowed=$false; bridge_post_allowed=$false; socket_allowed=$false; transition_level="planning_only" },
    [ordered]@{ key="response_storage_transition"; item="Response storage transition review"; depends_on="Response storage exit review"; summary="Review response record, storage, and idempotent capture coverage as transition-ready planning material only."; implementation_allowed=$false; bridge_post_allowed=$false; socket_allowed=$false; transition_level="planning_only" },
    [ordered]@{ key="cutover_transition"; item="Cutover transition review"; depends_on="Cutover exit review"; summary="Review cutover prerequisites, verification checkpoints, and rollback review coverage as transition-ready planning material only."; implementation_allowed=$false; bridge_post_allowed=$false; socket_allowed=$false; transition_level="planning_only" },
    [ordered]@{ key="audit_rollback_transition"; item="Audit and rollback transition review"; depends_on="Audit and rollback exit review"; summary="Review audit record, rollback record, and snapshot coverage as transition-ready planning material only."; implementation_allowed=$false; bridge_post_allowed=$false; socket_allowed=$false; transition_level="planning_only" }
)

foreach ($spec in $transitionSpecs) {
    [void]$transitionAreas.Add([ordered]@{
        key = $spec.key
        item = $spec.item
    ...
    [void]$transitionEntries.Add([ordered]@{
        category = "transition_areas"
        action = "review_only"
        item = $spec.item
...
[void]$transitionSections.Add([ordered]@{
    section = "Planning transition state"
    status = "review"
    summary = "Phase 22 Step 14 remains planning-only and converts planning exit areas into explicit transition-review areas."
})
[void]$transitionSections.Add([ordered]@{
    section = "Transition areas"
    status = "review"
    summary = "Execution envelope, transport injection, bridge gate, response storage, cutover, and audit/rollback transition areas remain review-only."
})
[void]$transitionSections.Add([ordered]@{
...
foreach ($file in @($currentStepFiles)) {
    [void]$transitionEntries.Add([ordered]@{
        ...
        action = "stage"
        item = $file
        evidence = "Current Phase 22 Step 14 file."
        required = "yes"
    })
}
...
$nextCommitCommands = @(
    ...
    "git status --short",
    "git add scripts/phase22_generate_phase20_network_transport_planning_transition_packet.ps1",
    "git add ui/pages/120_Phase20_Network_Transport_Planning_Transition_Packet.py",
    "git add docs/PHASE22_STEP14_PHASE20_NETWORK_TRANSPORT_PLANNING_TRANSITION_PACKET.md",
    "git add tests/test_phase22_phase20_network_transport_planning_transition_packet.py",
    "git diff --cached --name-only",
    'git commit -m "Phase 22 Step 14 phase20 network transport planning transition packet"',
    "git push -u origin phase22-step14-phase20-network-transport-planning-transition-packet"
)

Add-ChecklistItem $checklist "planning_transition" "Current branch detected" (-not [string]::IsNullOrWhiteSpace($currentBranch)) "current_branch=$currentBranch"
Add-ChecklistItem $checklist "planning_transition" "At least one planning-transition section exists" (@($transitionSections).Count -gt 0) "planning_transition_sections=$(@($transitionSections).Count)"
Add-ChecklistItem $checklist "planning_transition" "Exactly four stage-now entries exist" (@($transitionEntries | Where-Object { $_.category -eq "stage_now" }).Count -eq 4) "stage_now_entries=$(@($transitionEntries | Where-Object { $_.category -eq "stage_now" }).Count)"
Add-ChecklistItem $checklist "planning_transition" "Transition areas were generated" (@($transitionAreas).Count -ge 6) "transition_areas=$(@($transitionAreas).Count)" "recommended"
Add-ChecklistItem $checklist "planning_transition" "Archive decisions were carried forward" (@($archiveDecisions).Count -eq @($artifactFamilies).Count) "archive_decisions=$(@($archiveDecisions).Count); artifact_families=$(@($artifactFamilies).Count)" "recommended"
Add-ChecklistItem $checklist "planning_transition" "Commit commands exist" (@($nextCommitCommands).Count -gt 0) "commit_commands=$(@($nextCommitCommands).Count)"
Add-ChecklistItem $checklist "planning_transition" "No forbidden paths are currently staged" ($stagedForbiddenCount -eq 0) "staged_forbidden_count=$stagedForbiddenCount"
Add-ChecklistItem $checklist "planning_transition" "No review-other paths are currently staged" ($stagedReviewOtherCount -eq 0) "staged_review_other_count=$stagedReviewOtherCount" "recommended"
Add-ChecklistItem $checklist "planning_transition" "Workspace review-other files count is zero or acceptable" ($reviewOtherCount -eq 0) "review_other_count=$reviewOtherCount" "recommended"

$planningExitStatus = [string]$planningExit.planning_exit_packet.status
if ($planningExitStatus -eq "phase20_planning_exit_packet_blocked") {
    Add-Issue $issues "blocker" "planning_exit_packet_blocked" "Planning exit packet is blocked." "planning_exit"
} elseif ($planningExitStatus -eq "phase20_planning_exit_packet_review_required_no_write") {
    Add-Issue $issues "review" "planning_exit_packet_review_required" "Planning exit packet has review items." "planning_exit"
}

if ([string]::IsNullOrWhiteSpace($currentBranch)) {
    Add-Issue $issues "blocker" "current_branch_missing" "Current branch was not detected for planning transition." "workspace"
}
...
[void]$sourceArtifacts.Add([ordered]@{ key="planning_exit_packet"; json_path=$planningExitPath; sha256=Get-FileSha256 $planningExitPath })
...
$packetStatus = if ($blockerCount -eq 0 -and $reviewCount -eq 0) {
    "phase20_planning_transition_packet_ready_no_write"
} elseif ($blockerCount -eq 0) {
    "phase20_planning_transition_packet_review_required_no_write"
} else {
    "phase20_planning_transition_packet_blocked"
}

if ([string]::IsNullOrWhiteSpace($OutputDir)) {
    $stamp = Get-Date -Format "yyyyMMdd-HHmmss"
    $OutputDir = Join-Path $BackupDir "phase22_phase20_network_transport_planning_transition_packet_$stamp"
}
...
$report = [ordered]@{
    ...
    phase = "Phase 22 Step 14"
    purpose = "Phase 20 bridge routing network transport planning transition packet"
    workspace = $Workspace
    platform_api = $PlatformApi
    bridge_url = $BridgeUrl
    safety = [ordered]@{
        phase20_network_transport_planning_transition_packet_only = $true
    ...
    planning_transition_packet = [ordered]@{
        status = $packetStatus
        blocker_count = $blockerCount
        review_count = $reviewCount
        current_branch = $currentBranch
        section_count = @($transitionSections).Count
        entry_count = @($transitionEntries).Count
        transition_area_count = @($transitionAreas).Count
        ...
        packet_label = "phase22_phase20_network_transport_planning_transition_no_write"
        reason = if ($blockerCount -eq 0) {
            "Phase 22 Step 14 created a no-write planning transition packet for the Phase 20 network transport chain. It does not authorize transition records, approvals, implementation, transport, sockets, bridge POST, response capture, mutations, LACRM calls, or live writes."
        } else {
            "Phase 22 Step 14 found blockers in the Phase 20 planning transition packet. Resolve blockers before continuing planning."
        }
    }
    planning_sections = @($transitionSections)
    planning_entries = @($transitionEntries)
    transition_areas = @($transitionAreas)
    ...
    counts = [ordered]@{
        sections = @($transitionSections).Count
        entries = @($transitionEntries).Count
        transition_areas = @($transitionAreas).Count
    ...
    next_recommended_actions = @(
        "Review this planning transition packet before any future implementation-gate or Phase 23 transition work is drafted.",
        "Keep the latest packet folder in each family and only review older generated folders for manual archive actions.",
        "Use the stage-now entries as the only files to add for the Phase 22 Step 14 commit.",
        "Carry the do-not-stage entries forward as the hard commit boundary.",
        "Do not add requests, httpx, socket use, bridge POST calls, or routing write endpoints from Phase 22 Step 14.",
        "Do not call interface execution methods from Phase 22 Step 14.",
        "Do not record operator signoffs, operator approvals, final approvals, or design closure records from Phase 22 Step 14.",
        "Do not start an implementation phase from Phase 22 Step 14.",
        "Review transition-ready planning only: execution envelope, transport injection, bridge gate, response storage, cutover, and audit/rollback transition areas."
    )
}

$jsonPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_transition_packet.json"
$sectionsCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_transition_packet_sections.csv"
$entriesCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_transition_packet_entries.csv"
$areasCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_transition_packet_transition_areas.csv"
$decisionsCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_transition_packet_decisions.csv"
$familiesCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_transition_packet_artifact_families.csv"
$gitStatusCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_transition_packet_git_status.csv"
$stagedCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_transition_packet_staged.csv"
$sourceArtifactsCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_transition_packet_source_artifacts.csv"
$checklistCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_transition_packet_checklist.csv"
$issuesCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_transition_packet_issues.csv"
$mdPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_transition_packet.md"

$report | ConvertTo-Json -Depth 100 | Set-Content -LiteralPath $jsonPath -Encoding UTF8
$transitionSections | Export-Csv -LiteralPath $sectionsCsvPath -NoTypeInformation -Encoding UTF8
$transitionEntries | Export-Csv -LiteralPath $entriesCsvPath -NoTypeInformation -Encoding UTF8
$transitionAreas | Export-Csv -LiteralPath $areasCsvPath -NoTypeInformation -Encoding UTF8
...
$sectionText = (@($transitionSections) | ForEach-Object { "- $($_.section): $($_.status); $($_.summary)" }) -join "`n"
$entryText = (@($transitionEntries) | ForEach-Object { "- [$($_.category)] $($_.action) $($_.item): $($_.evidence)" }) -join "`n"
$transitionText = (@($transitionAreas) | ForEach-Object { "- $($_.item): depends_on=$($_.depends_on); status=$($_.status)" }) -join "`n"
...
- Phase 20 network transport planning transition packet only: true
- Planning transition only: true
- Planning only: true
...
- Sections: $(@($transitionSections).Count)
- Entries: $(@($transitionEntries).Count)
- Transition areas: $(@($transitionAreas).Count)
...
if ($blockerCount -eq 0) {
    Write-Host "PASS | phase20_network_transport_planning_transition_packet=$OutputDir | status=$packetStatus | bridge_post_called=False | execution_implementation_created=False | implementation_phase_started=False"
} else {
    Write-Host "CHECK | phase20_network_transport_planning_transition_packet=$OutputDir | status=$packetStatus | blockers=$blockerCount | bridge_post_called=False | execution_implementation_created=False | implementation_phase_started=False"
}

Write-Host ""
Write-Host "Planning transition packet files:"
