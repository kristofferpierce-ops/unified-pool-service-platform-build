...
$StepNumber = 29
$StepTitle = "Phase 31 Step 29 - Phase 20 Network Transport Implementation Controlled Active Program Operation Preflight Planning Closeout Index Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Controlled Active Program Operation Preflight Planning Closeout Index Packet"
$ExpectedBranch = "phase31-step29-controlled-active-program-operation-preflight-planning-closeout-index"
$PriorStep = "Phase 31 Step 28 - Phase 20 Network Transport Implementation Controlled Active Program Operation Preflight Planning Operator Hold Point Packet"
$Slug = "implementation_controlled_active_program_operation_preflight_planning_closeout_index"
...
$StepFiles = @(
    "scripts/phase31_step29_controlled_active_program_operation_preflight_planning_closeout_index_packet.ps1",
    "ui/pages/1125_Phase31_Step29_Implementation_Controlled_Active_Program_Operation_Preflight_Planning_Closeout_Index_Packet.py",
    "docs/PHASE31_STEP29_CONTROLLED_ACTIVE_PROGRAM_OPERATION_PREFLIGHT_PLANNING_CLOSEOUT_INDEX_PACKET.md",
    "tests/test_phase31_step29_controlled_active_program_operation_preflight_planning_closeout_index_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase31_boundary = "implementation_controlled_active_program_operation_preflight_planning_closeout_index_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 31 Step 29 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 31 Step 29 Phase 20 Network Transport Implementation Controlled Active Program Operation Preflight Planning Closeout Index Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase31_step29_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase31_step29_{0}_packet.json" -f $Slug)
