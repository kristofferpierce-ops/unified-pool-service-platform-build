...
$StepNumber = 3
$StepTitle = "Phase 28 Step 3 - Phase 20 Network Transport Implementation Sandbox Pilot Readiness Evidence Gap Review Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Sandbox Pilot Readiness Evidence Gap Review Packet"
$ExpectedBranch = "phase28-step3-implementation-sandbox-pilot-readiness-evidence-gap-review"
$PriorStep = "Phase 28 Step 2 - Phase 20 Network Transport Implementation Sandbox Pilot Readiness Evidence Index Packet"
$Slug = "implementation_sandbox_pilot_readiness_evidence_gap_review"

if (-not $RepoRoot) { $RepoRoot = (Resolve-Path (Join-Path $PSScriptRoot ".." )).Path }

$StepFiles = @("scripts/phase28_step3_implementation_sandbox_pilot_readiness_evidence_gap_review_packet.ps1", "ui/pages/739_Phase28_Step3_Implementation_Sandbox_Pilot_Readiness_Evidence_Gap_Review_Packet.py", "docs/PHASE28_STEP3_IMPLEMENTATION_SANDBOX_PILOT_READINESS_EVIDENCE_GAP_REVIEW_PACKET.md", "tests/test_phase28_step3_implementation_sandbox_pilot_readiness_evidence_gap_review_packet.py")
...
$SafetyPosture = [ordered]@{
    ...
    phase28_boundary = "implementation_sandbox_pilot_readiness_evidence_gap_review_opened_by_packet"
    ...
    implementation_sandbox_pilot_readiness_evidence_gap_review_mode = "reference_only"
    implementation_sandbox_pilot_readiness_evidence_gap_review_write = $false
    implementation_sandbox_pilot_readiness_evidence_gap_review_record_creation = $false
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 28 Step 3 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 28 Step 3 Phase 20 Network Transport Implementation Sandbox Pilot Readiness Evidence Gap Review Packet is present and planning-only."
}

function New-Packet {
    $Timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
    $BackupRoot = Join-Path $RepoRoot "backups"
    $PacketDir = Join-Path $BackupRoot ("phase28_step3_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase28_step3_{0}_packet.json" -f $Slug)
    ...
    Write-Host "PASS: phase28_boundary=implementation_sandbox_pilot_readiness_evidence_gap_review_opened_by_packet"
    ...
    Write-Host "PASS: implementation_sandbox_pilot_readiness_evidence_gap_review_mode=reference_only"
    Write-Host "PASS: implementation_sandbox_pilot_readiness_evidence_gap_review_write=false"
    Write-Host "PASS: implementation_sandbox_pilot_readiness_evidence_gap_review_record_creation=false"
