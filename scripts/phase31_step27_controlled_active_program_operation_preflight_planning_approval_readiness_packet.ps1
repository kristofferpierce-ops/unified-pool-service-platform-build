...
$StepNumber = 27
$StepTitle = "Phase 31 Step 27 - Phase 20 Network Transport Implementation Controlled Active Program Operation Preflight Planning Approval Readiness Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Controlled Active Program Operation Preflight Planning Approval Readiness Packet"
$ExpectedBranch = "phase31-step27-controlled-active-program-operation-preflight-planning-approval-readiness"
$PriorStep = "Phase 31 Step 26 - Phase 20 Network Transport Implementation Controlled Active Program Operation Preflight Planning Approval Boundary Packet"
$Slug = "implementation_controlled_active_program_operation_preflight_planning_approval_readiness"
...
$SafetyPosture = [ordered]@{
    ...
    phase31_boundary = "implementation_controlled_active_program_operation_preflight_planning_approval_readiness_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 31 Step 27 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 31 Step 27 Phase 20 Network Transport Implementation Controlled Active Program Operation Preflight Planning Approval Readiness Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase31_step27_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase31_step27_{0}_packet.json" -f $Slug)
