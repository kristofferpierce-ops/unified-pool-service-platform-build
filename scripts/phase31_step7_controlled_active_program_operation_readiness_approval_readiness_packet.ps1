...
$StepNumber = 7
$StepTitle = "Phase 31 Step 7 - Phase 20 Network Transport Implementation Controlled Active Program Operation Readiness Approval Readiness Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Controlled Active Program Operation Readiness Approval Readiness Packet"
$ExpectedBranch = "phase31-step7-controlled-active-program-operation-readiness-approval-readiness"
$PriorStep = "Phase 31 Step 6 - Phase 20 Network Transport Implementation Controlled Active Program Operation Readiness Approval Boundary Packet"
$Slug = "implementation_controlled_active_program_operation_readiness_approval_readiness"
...
$StepFiles = @(
    "scripts/phase31_step7_controlled_active_program_operation_readiness_approval_readiness_packet.ps1",
    "ui/pages/1103_Phase31_Step7_Implementation_Controlled_Active_Program_Operation_Readiness_Approval_Readiness_Packet.py",
    "docs/PHASE31_STEP7_CONTROLLED_ACTIVE_PROGRAM_OPERATION_READINESS_APPROVAL_READINESS_PACKET.md",
    "tests/test_phase31_step7_controlled_active_program_operation_readiness_approval_readiness_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase31_boundary = "implementation_controlled_active_program_operation_readiness_approval_readiness_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 31 Step 7 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 31 Step 7 Phase 20 Network Transport Implementation Controlled Active Program Operation Readiness Approval Readiness Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase31_step7_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase31_step7_{0}_packet.json" -f $Slug)
