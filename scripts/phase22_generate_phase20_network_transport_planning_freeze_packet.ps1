...
function Classify-RepoPath {
    ...
    if ($CurrentStepFiles -contains $Path) {
        return [ordered]@{
            ...
            should_stage = $true
            reason = "Current Phase 22 Step 9 file."
        }
    }
...
if ([string]::IsNullOrWhiteSpace($PlanningReviewPacketDir)) {
    $PlanningReviewPacketDir = Get-LatestArtifactDir -FolderFilter "phase22_phase20_network_transport_planning_review_packet_*" -JsonName "phase22_phase20_network_transport_planning_review_packet.json" -Required $true
}
if ([string]::IsNullOrWhiteSpace($PlanningReadinessPacketDir)) {
    $PlanningReadinessPacketDir = Get-LatestArtifactDir -FolderFilter "phase22_phase20_network_transport_planning_readiness_packet_*" -JsonName "phase22_phase20_network_transport_planning_readiness_packet.json" -Required $false
}
...
$planningReviewPath = Join-Path $PlanningReviewPacketDir "phase22_phase20_network_transport_planning_review_packet.json"
$planningReadinessPath = if (![string]::IsNullOrWhiteSpace($PlanningReadinessPacketDir)) { Join-Path $PlanningReadinessPacketDir "phase22_phase20_network_transport_planning_readiness_packet.json" } else { "" }
...
if (!(Test-Path -LiteralPath $planningReviewPath)) {
    throw "Planning review packet JSON not found: $planningReviewPath"
}

$planningReview = Read-JsonFile $planningReviewPath
$planningReadiness = if (![string]::IsNullOrWhiteSpace($planningReadinessPath) -and (Test-Path -LiteralPath $planningReadinessPath)) { Read-JsonFile $planningReadinessPath } else { $null }
...
$freezeSections = New-Object System.Collections.ArrayList
$freezeEntries = New-Object System.Collections.ArrayList
$freezeAreas = New-Object System.Collections.ArrayList
...
Add-ChecklistItem $checklist "artifact" "Planning review packet exists" $true $planningReviewPath
Add-ChecklistItem $checklist "artifact" "Planning readiness packet exists" ($planningReadiness -ne $null) $planningReadinessPath "recommended"
Add-ChecklistItem $checklist "artifact" "Cleanup exit packet exists" ($cleanupExit -ne $null) $cleanupExitPath "recommended"

Add-ChecklistItem $checklist "planning_review" "Planning review packet is freeze-safe" ($planningReview.safety.phase20_network_transport_planning_review_packet_only -eq $true) "phase20_network_transport_planning_review_packet_only=$($planningReview.safety.phase20_network_transport_planning_review_packet_only)"
Add-ChecklistItem $checklist "planning_review" "Planning review created no closure record" ($planningReview.safety.design_closure_record_created -eq $false) "design_closure_record_created=$($planningReview.safety.design_closure_record_created)"
Add-ChecklistItem $checklist "planning_review" "Planning review recorded no final approval" ($planningReview.safety.final_approval_recorded -eq $false) "final_approval_recorded=$($planningReview.safety.final_approval_recorded)"
Add-ChecklistItem $checklist "planning_review" "Planning review started no implementation phase" ($planningReview.safety.implementation_phase_started -eq $false) "implementation_phase_started=$($planningReview.safety.implementation_phase_started)"
Add-ChecklistItem $checklist "planning_review" "Planning review has no execution implementation" ($planningReview.safety.execution_implementation_created -eq $false) "execution_implementation_created=$($planningReview.safety.execution_implementation_created)"
Add-ChecklistItem $checklist "planning_review" "Planning review has no network transport" ($planningReview.safety.network_transport_implemented -eq $false) "network_transport_implemented=$($planningReview.safety.network_transport_implemented)"
Add-ChecklistItem $checklist "planning_review" "Planning review has no socket opened" ($planningReview.safety.network_socket_opened -eq $false) "network_socket_opened=$($planningReview.safety.network_socket_opened)"
Add-ChecklistItem $checklist "planning_review" "Planning review has no bridge POST" ($planningReview.safety.bridge_post_called -eq $false -and $planningReview.safety.bridge_post_call_implemented -eq $false) "bridge_post_called=$($planningReview.safety.bridge_post_called); bridge_post_call_implemented=$($planningReview.safety.bridge_post_call_implemented)"
Add-ChecklistItem $checklist "planning_review" "Planning review has no platform DB mutation" ($planningReview.safety.platform_db_mutation_performed -eq $false) "platform_db_mutation_performed=$($planningReview.safety.platform_db_mutation_performed)"
Add-ChecklistItem $checklist "planning_review" "Planning review has no bridge mutation" ($planningReview.safety.bridge_mutation_performed -eq $false) "bridge_mutation_performed=$($planningReview.safety.bridge_mutation_performed)"
Add-ChecklistItem $checklist "planning_review" "Planning review has no LACRM call" ($planningReview.safety.lacrm_call_performed -eq $false) "lacrm_call_performed=$($planningReview.safety.lacrm_call_performed)"
Add-ChecklistItem $checklist "planning_review" "Planning review has no blockers" ([int]$planningReview.planning_review_packet.blocker_count -eq 0) "blocker_count=$($planningReview.planning_review_packet.blocker_count)"

if ($planningReadiness -ne $null) {
    Add-ChecklistItem $checklist "planning_readiness" "Planning readiness packet is planning-only" ($planningReadiness.safety.planning_only -eq $true) "planning_only=$($planningReadiness.safety.planning_only)" "recommended"
}
...
$reviewPrefixes = @($planningReview.planning_entries | Where-Object { $_.category -eq "do_not_stage" } | ForEach-Object { [string]$_.item })
if (@($reviewPrefixes).Count -gt 0) {
    $doNotStagePrefixes = $reviewPrefixes
}

$currentStepFiles = @(
    "scripts/phase22_generate_phase20_network_transport_planning_freeze_packet.ps1",
    "ui/pages/115_Phase20_Network_Transport_Planning_Freeze_Packet.py",
    "docs/PHASE22_STEP9_PHASE20_NETWORK_TRANSPORT_PLANNING_FREEZE_PACKET.md",
    "tests/test_phase22_phase20_network_transport_planning_freeze_packet.py"
)
...
if ($planningReview.artifact_families) {
    foreach ($family in @($planningReview.artifact_families)) {
        [void]$artifactFamilies.Add($family)
    }
}
if ($planningReview.archive_decisions) {
    foreach ($decision in @($planningReview.archive_decisions)) {
        [void]$archiveDecisions.Add($decision)
    }
}

$freezeSpecs = @(
    [ordered]@{ key="execution_envelope_freeze"; item="Execution envelope freeze review"; depends_on="Execution envelope review"; summary="Review execution request, result, error, and dry-run envelope completeness as freeze-ready planning material only."; implementation_allowed=$false; bridge_post_allowed=$false; socket_allowed=$false; freeze_level="planning_only" },
    [ordered]@{ key="transport_injection_freeze"; item="Transport injection freeze review"; depends_on="Transport injection review"; summary="Review dependency injection seams and transport-off defaults as freeze-ready planning material only."; implementation_allowed=$false; bridge_post_allowed=$false; socket_allowed=$false; freeze_level="planning_only" },
    [ordered]@{ key="bridge_gate_freeze"; item="Bridge gate freeze review"; depends_on="Bridge gate review"; summary="Review confirmation phrase, armed-state, environment gate, and explicit POST precondition coverage as freeze-ready planning material only."; implementation_allowed=$false; bridge_post_allowed=$false; socket_allowed=$false; freeze_level="planning_only" },
    [ordered]@{ key="response_storage_freeze"; item="Response storage freeze review"; depends_on="Response storage review"; summary="Review response record, storage, and idempotent capture coverage as freeze-ready planning material only."; implementation_allowed=$false; bridge_post_allowed=$false; socket_allowed=$false; freeze_level="planning_only" },
    [ordered]@{ key="cutover_freeze"; item="Cutover freeze review"; depends_on="Cutover review"; summary="Review cutover prerequisites, verification checkpoints, and rollback review coverage as freeze-ready planning material only."; implementation_allowed=$false; bridge_post_allowed=$false; socket_allowed=$false; freeze_level="planning_only" },
    [ordered]@{ key="audit_rollback_freeze"; item="Audit and rollback freeze review"; depends_on="Audit and rollback review"; summary="Review audit record, rollback record, and snapshot coverage as freeze-ready planning material only."; implementation_allowed=$false; bridge_post_allowed=$false; socket_allowed=$false; freeze_level="planning_only" }
)

foreach ($spec in $freezeSpecs) {
    [void]$freezeAreas.Add([ordered]@{
        key = $spec.key
        item = $spec.item
    ...
    [void]$freezeEntries.Add([ordered]@{
        category = "freeze_areas"
        action = "review_only"
        item = $spec.item
...
[void]$freezeSections.Add([ordered]@{
    section = "Planning freeze state"
    status = "review"
    summary = "Phase 22 Step 9 remains planning-only and converts planning review areas into explicit freeze-review areas."
})
[void]$freezeSections.Add([ordered]@{
    section = "Freeze areas"
    status = "review"
    summary = "Execution envelope, transport injection, bridge gate, response storage, cutover, and audit/rollback freeze areas remain review-only."
})
[void]$freezeSections.Add([ordered]@{
...
foreach ($file in @($currentStepFiles)) {
    [void]$freezeEntries.Add([ordered]@{
        ...
        action = "stage"
        item = $file
        evidence = "Current Phase 22 Step 9 file."
        required = "yes"
    })
}
...
$nextCommitCommands = @(
    ...
    "git status --short",
    "git add scripts/phase22_generate_phase20_network_transport_planning_freeze_packet.ps1",
    "git add ui/pages/115_Phase20_Network_Transport_Planning_Freeze_Packet.py",
    "git add docs/PHASE22_STEP9_PHASE20_NETWORK_TRANSPORT_PLANNING_FREEZE_PACKET.md",
    "git add tests/test_phase22_phase20_network_transport_planning_freeze_packet.py",
    "git diff --cached --name-only",
    'git commit -m "Phase 22 Step 9 phase20 network transport planning freeze packet"',
    "git push -u origin phase22-step9-phase20-network-transport-planning-freeze-packet"
)

Add-ChecklistItem $checklist "planning_freeze" "Current branch detected" (-not [string]::IsNullOrWhiteSpace($currentBranch)) "current_branch=$currentBranch"
Add-ChecklistItem $checklist "planning_freeze" "At least one planning-freeze section exists" (@($freezeSections).Count -gt 0) "planning_freeze_sections=$(@($freezeSections).Count)"
Add-ChecklistItem $checklist "planning_freeze" "Exactly four stage-now entries exist" (@($freezeEntries | Where-Object { $_.category -eq "stage_now" }).Count -eq 4) "stage_now_entries=$(@($freezeEntries | Where-Object { $_.category -eq "stage_now" }).Count)"
Add-ChecklistItem $checklist "planning_freeze" "Freeze areas were generated" (@($freezeAreas).Count -ge 6) "freeze_areas=$(@($freezeAreas).Count)" "recommended"
Add-ChecklistItem $checklist "planning_freeze" "Archive decisions were carried forward" (@($archiveDecisions).Count -eq @($artifactFamilies).Count) "archive_decisions=$(@($archiveDecisions).Count); artifact_families=$(@($artifactFamilies).Count)" "recommended"
Add-ChecklistItem $checklist "planning_freeze" "Commit commands exist" (@($nextCommitCommands).Count -gt 0) "commit_commands=$(@($nextCommitCommands).Count)"
Add-ChecklistItem $checklist "planning_freeze" "No forbidden paths are currently staged" ($stagedForbiddenCount -eq 0) "staged_forbidden_count=$stagedForbiddenCount"
Add-ChecklistItem $checklist "planning_freeze" "No review-other paths are currently staged" ($stagedReviewOtherCount -eq 0) "staged_review_other_count=$stagedReviewOtherCount" "recommended"
Add-ChecklistItem $checklist "planning_freeze" "Workspace review-other files count is zero or acceptable" ($reviewOtherCount -eq 0) "review_other_count=$reviewOtherCount" "recommended"

$planningReviewStatus = [string]$planningReview.planning_review_packet.status
if ($planningReviewStatus -eq "phase20_planning_review_packet_blocked") {
    Add-Issue $issues "blocker" "planning_review_packet_blocked" "Planning review packet is blocked." "planning_review"
} elseif ($planningReviewStatus -eq "phase20_planning_review_packet_review_required_no_write") {
    Add-Issue $issues "review" "planning_review_packet_review_required" "Planning review packet has review items." "planning_review"
}

if ([string]::IsNullOrWhiteSpace($currentBranch)) {
    Add-Issue $issues "blocker" "current_branch_missing" "Current branch was not detected for planning freeze." "workspace"
}
...
$packetStatus = if ($blockerCount -eq 0 -and $reviewCount -eq 0) {
    "phase20_planning_freeze_packet_ready_no_write"
} elseif ($blockerCount -eq 0) {
    "phase20_planning_freeze_packet_review_required_no_write"
} else {
    "phase20_planning_freeze_packet_blocked"
}

if ([string]::IsNullOrWhiteSpace($OutputDir)) {
    $stamp = Get-Date -Format "yyyyMMdd-HHmmss"
    $OutputDir = Join-Path $BackupDir "phase22_phase20_network_transport_planning_freeze_packet_$stamp"
}
...
$report = [ordered]@{
    ...
    phase = "Phase 22 Step 9"
    purpose = "Phase 20 bridge routing network transport planning freeze packet"
    workspace = $Workspace
    platform_api = $PlatformApi
    bridge_url = $BridgeUrl
    safety = [ordered]@{
        phase20_network_transport_planning_freeze_packet_only = $true
        planning_freeze_only = $true
        planning_only = $true
    ...
    planning_freeze_packet = [ordered]@{
        status = $packetStatus
        blocker_count = $blockerCount
        review_count = $reviewCount
        current_branch = $currentBranch
        section_count = @($freezeSections).Count
        entry_count = @($freezeEntries).Count
        freeze_area_count = @($freezeAreas).Count
        ...
        packet_label = "phase22_phase20_network_transport_planning_freeze_no_write"
        reason = if ($blockerCount -eq 0) {
            "Phase 22 Step 9 created a no-write planning freeze packet for the Phase 20 network transport chain. It does not authorize implementation, transport, sockets, bridge POST, response capture, mutations, LACRM calls, or live writes."
        } else {
            "Phase 22 Step 9 found blockers in the Phase 20 planning freeze packet. Resolve blockers before continuing planning."
        }
    }
    planning_sections = @($freezeSections)
    planning_entries = @($freezeEntries)
    freeze_areas = @($freezeAreas)
    ...
    counts = [ordered]@{
        sections = @($freezeSections).Count
        entries = @($freezeEntries).Count
        freeze_areas = @($freezeAreas).Count
    ...
    next_recommended_actions = @(
        "Review this planning freeze packet before any Phase 22 freeze-review, signoff, or approval packet is drafted.",
        "Keep the latest packet folder in each family and only review older generated folders for manual archive actions.",
        "Use the stage-now entries as the only files to add for the Phase 22 Step 9 commit.",
        "Carry the do-not-stage entries forward as the hard commit boundary.",
        "Do not add requests, httpx, socket use, bridge POST calls, or routing write endpoints from Phase 22 Step 9.",
        "Do not call interface execution methods from Phase 22 Step 9.",
        "Do not record final approvals from Phase 22 Step 9.",
        "Do not start an implementation phase from Phase 22 Step 9.",
        "Review freeze-ready planning only: execution envelope, transport injection, bridge gate, response storage, cutover, and audit/rollback freeze areas."
    )
}

$jsonPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_freeze_packet.json"
$sectionsCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_freeze_packet_sections.csv"
$entriesCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_freeze_packet_entries.csv"
$areasCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_freeze_packet_freeze_areas.csv"
$decisionsCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_freeze_packet_decisions.csv"
$familiesCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_freeze_packet_artifact_families.csv"
$gitStatusCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_freeze_packet_git_status.csv"
$stagedCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_freeze_packet_staged.csv"
$sourceArtifactsCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_freeze_packet_source_artifacts.csv"
$checklistCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_freeze_packet_checklist.csv"
$issuesCsvPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_freeze_packet_issues.csv"
$mdPath = Join-Path $OutputDir "phase22_phase20_network_transport_planning_freeze_packet.md"

$report | ConvertTo-Json -Depth 100 | Set-Content -LiteralPath $jsonPath -Encoding UTF8
$freezeSections | Export-Csv -LiteralPath $sectionsCsvPath -NoTypeInformation -Encoding UTF8
$freezeEntries | Export-Csv -LiteralPath $entriesCsvPath -NoTypeInformation -Encoding UTF8
$freezeAreas | Export-Csv -LiteralPath $areasCsvPath -NoTypeInformation -Encoding UTF8
...
$sectionText = (@($freezeSections) | ForEach-Object { "- $($_.section): $($_.status); $($_.summary)" }) -join "`n"
$entryText = (@($freezeEntries) | ForEach-Object { "- [$($_.category)] $($_.action) $($_.item): $($_.evidence)" }) -join "`n"
$freezeText = (@($freezeAreas) | ForEach-Object { "- $($_.item): depends_on=$($_.depends_on); status=$($_.status)" }) -join "`n"
...
$md = @"
# Phase 22 Step 9 - Phase 20 Network Transport Planning Freeze Packet

Generated: $($report.generated_at)

Source planning review packet:
...
- Phase 20 network transport planning freeze packet only: true
- Planning freeze only: true
- Planning only: true
...
- Sections: $(@($freezeSections).Count)
- Entries: $(@($freezeEntries).Count)
- Freeze areas: $(@($freezeAreas).Count)
...
if ($blockerCount -eq 0) {
    Write-Host "PASS | phase20_network_transport_planning_freeze_packet=$OutputDir | status=$packetStatus | bridge_post_called=False | execution_implementation_created=False | implementation_phase_started=False"
} else {
    Write-Host "CHECK | phase20_network_transport_planning_freeze_packet=$OutputDir | status=$packetStatus | blockers=$blockerCount | bridge_post_called=False | execution_implementation_created=False | implementation_phase_started=False"
}

Write-Host ""
Write-Host "Planning freeze packet files:"
