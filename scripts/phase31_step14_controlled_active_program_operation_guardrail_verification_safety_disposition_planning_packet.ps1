...
$StepNumber = 14
$StepTitle = "Phase 31 Step 14 - Phase 20 Network Transport Implementation Controlled Active Program Operation Guardrail Verification Safety Disposition Planning Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Controlled Active Program Operation Guardrail Verification Safety Disposition Planning Packet"
$ExpectedBranch = "phase31-step14-controlled-active-program-operation-guardrail-verification-safety-disposition-planning"
$PriorStep = "Phase 31 Step 13 - Phase 20 Network Transport Implementation Controlled Active Program Operation Guardrail Verification Evidence Gap Review Packet"
$Slug = "implementation_controlled_active_program_operation_guardrail_verification_safety_disposition_planning"
$ContextKey = "implementation_controlled_active_program_operation_guardrail_verification"
$PhaseContext = "implementation_controlled_active_program_operation_guardrail_verification_planning_only"

if (-not $RepoRoot) { $RepoRoot = (Resolve-Path (Join-Path $PSScriptRoot ".." )).Path }

$StepFiles = @(
    "scripts/phase31_step14_controlled_active_program_operation_guardrail_verification_safety_disposition_planning_packet.ps1",
    "ui/pages/1110_Phase31_Step14_Implementation_Controlled_Active_Program_Operation_Guardrail_Verification_Safety_Disposition_Planning_Packet.py",
    "docs/PHASE31_STEP14_CONTROLLED_ACTIVE_PROGRAM_OPERATION_GUARDRAIL_VERIFICATION_SAFETY_DISPOSITION_PLANNING_PACKET.md",
    "tests/test_phase31_step14_controlled_active_program_operation_guardrail_verification_safety_disposition_planning_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase31_boundary = "implementation_controlled_active_program_operation_guardrail_verification_safety_disposition_planning_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 31 Step 14 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 31 Step 14 Phase 20 Network Transport Implementation Controlled Active Program Operation Guardrail Verification Safety Disposition Planning Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase31_step14_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase31_step14_{0}_packet.json" -f $Slug)
