...
$StepNumber = 21
$StepTitle = "Phase 32 Step 21 - Phase 20 Network Transport Implementation Production-Like Rollout Preflight Planning Boundary Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Production-Like Rollout Preflight Planning Boundary Packet"
$ExpectedBranch = "phase32-step21-production-like-rollout-preflight-planning-boundary"
$PriorStep = "Phase 32 Step 20 - Phase 20 Network Transport Implementation Production-Like Rollout Guardrail Verification Final Boundary Confirmation Packet"
$Slug = "implementation_production_like_rollout_preflight_planning_boundary"
...
$StepFiles = @(
    "scripts/phase32_step21_production_like_rollout_preflight_planning_boundary_packet.ps1",
    "ui/pages/1237_Phase32_Step21_Implementation_Production_Like_Rollout_Preflight_Planning_Boundary_Packet.py",
    "docs/PHASE32_STEP21_PRODUCTION_LIKE_ROLLOUT_PREFLIGHT_PLANNING_BOUNDARY_PACKET.md",
    "tests/test_phase32_step21_production_like_rollout_preflight_planning_boundary_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase32_boundary = "implementation_production_like_rollout_preflight_planning_boundary_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 32 Step 21 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 32 Step 21 Phase 20 Network Transport Implementation Production-Like Rollout Preflight Planning Boundary Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase32_step21_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase32_step21_{0}_packet.json" -f $Slug)
