...
$StepNumber = 41
$StepTitle = "Phase 28 Step 41 - Phase 20 Network Transport Implementation Sandbox Pilot No-Write Dry Run Verification Planning Boundary Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Sandbox Pilot No-Write Dry Run Verification Planning Boundary Packet"
$ExpectedBranch = "phase28-step41-implementation-sandbox-pilot-no-write-dry-run-verification-planning-boundary"
$PriorStep = "Phase 28 Step 40 - Phase 20 Network Transport Implementation Sandbox Pilot No-Write Dry Run Readiness Final Boundary Confirmation Packet"
$Slug = "implementation_sandbox_pilot_no_write_dry_run_verification_planning_boundary"

if (-not $RepoRoot) { $RepoRoot = (Resolve-Path (Join-Path $PSScriptRoot ".." )).Path }

$StepFiles = @(
    "scripts/phase28_step41_implementation_sandbox_pilot_no_write_dry_run_verification_planning_boundary_packet.ps1",
    "ui/pages/777_Phase28_Step41_Implementation_Sandbox_Pilot_No_Write_Dry_Run_Verification_Planning_Boundary_Packet.py",
    "docs/PHASE28_STEP41_IMPLEMENTATION_SANDBOX_PILOT_NO_WRITE_DRY_RUN_VERIFICATION_PLANNING_BOUNDARY_PACKET.md",
    "tests/test_phase28_step41_implementation_sandbox_pilot_no_write_dry_run_verification_planning_boundary_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase28_boundary = "implementation_sandbox_pilot_no_write_dry_run_verification_planning_boundary_opened_by_packet"
    ...
    implementation_sandbox_pilot_no_write_dry_run_verification_planning_boundary_mode = "reference_only"
    implementation_sandbox_pilot_no_write_dry_run_verification_planning_boundary_write = $false
    implementation_sandbox_pilot_no_write_dry_run_verification_planning_boundary_record_creation = $false
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 28 Step 41 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 28 Step 41 Phase 20 Network Transport Implementation Sandbox Pilot No-Write Dry Run Verification Planning Boundary Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase28_step41_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase28_step41_{0}_packet.json" -f $Slug)
    ...
    foreach ($Key in @(
        ...
        "cross_repo_mutation",
        "external_repo_push",
        "implementation_sandbox_pilot_no_write_dry_run_verification_planning_boundary_mode",
        "implementation_sandbox_pilot_no_write_dry_run_verification_planning_boundary_write",
        "implementation_sandbox_pilot_no_write_dry_run_verification_planning_boundary_record_creation",
