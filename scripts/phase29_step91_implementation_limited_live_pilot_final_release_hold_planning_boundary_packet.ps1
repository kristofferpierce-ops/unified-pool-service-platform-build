...
$StepNumber = 91
$StepTitle = "Phase 29 Step 91 - Phase 20 Network Transport Implementation Limited Live Pilot Final Release Hold Planning Boundary Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Limited Live Pilot Final Release Hold Planning Boundary Packet"
$ExpectedBranch = "phase29-step91-implementation-limited-live-pilot-final-release-hold-planning-boundary"
$PriorStep = "Phase 29 Step 90 - Phase 20 Network Transport Implementation Limited Live Pilot Release Gate Verification Planning Final Boundary Confirmation Packet"
$Slug = "implementation_limited_live_pilot_final_release_hold_planning_boundary"

if (-not $RepoRoot) { $RepoRoot = (Resolve-Path (Join-Path $PSScriptRoot ".." )).Path }

$StepFiles = @(
    "scripts/phase29_step91_implementation_limited_live_pilot_final_release_hold_planning_boundary_packet.ps1",
    "ui/pages/947_Phase29_Step91_Implementation_Limited_Live_Pilot_Final_Release_Hold_Planning_Boundary_Packet.py",
    "docs/PHASE29_STEP91_IMPLEMENTATION_LIMITED_LIVE_PILOT_FINAL_RELEASE_HOLD_PLANNING_BOUNDARY_PACKET.md",
    "tests/test_phase29_step91_implementation_limited_live_pilot_final_release_hold_planning_boundary_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase29_boundary = "implementation_limited_live_pilot_final_release_hold_planning_boundary_opened_by_packet"
    ...
    implementation_limited_live_pilot_final_release_hold_planning_boundary_mode = "reference_only"
    implementation_limited_live_pilot_final_release_hold_planning_boundary_write = $false
    implementation_limited_live_pilot_final_release_hold_planning_boundary_record_creation = $false
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 29 Step 91 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 29 Step 91 Phase 20 Network Transport Implementation Limited Live Pilot Final Release Hold Planning Boundary Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase29_step91_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase29_step91_{0}_packet.json" -f $Slug)
    ...
    foreach ($Key in @(
        ...
        "cross_repo_mutation",
        "external_repo_push",
        "implementation_limited_live_pilot_final_release_hold_planning_boundary_mode",
        "implementation_limited_live_pilot_final_release_hold_planning_boundary_write",
        "implementation_limited_live_pilot_final_release_hold_planning_boundary_record_creation",
