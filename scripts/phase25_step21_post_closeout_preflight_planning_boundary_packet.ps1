...
$PhaseNumber = 25
$StepNumber = 21
$StepName = "Phase 25 Step 21 - Phase 20 Network Transport Implementation Post-Closeout Preflight Planning Boundary Packet"
$PriorStepName = "Phase 25 Step 20 - Phase 20 Network Transport Implementation Post-Closeout Guardrail Verification Final Boundary Confirmation Packet"
$ExpectedBranch = "phase25-step21-post-closeout-preflight-planning-boundary"
$ModeKey = "implementation_post_closeout_preflight_planning_boundary_mode"
$WriteKey = "implementation_post_closeout_preflight_planning_boundary_write"
$RecordKey = "implementation_post_closeout_preflight_planning_boundary_record_creation"
$PhaseContext = "implementation_post_closeout_preflight_planning_boundary_planning_only"
$PacketSlug = "phase25_step21_post_closeout_preflight_planning_boundary_packet"

$StepFiles = @(
    "scripts/phase25_step21_post_closeout_preflight_planning_boundary_packet.ps1",
    "ui/pages/397_Phase25_Step21_Implementation_PostCloseout_Preflight_Planning_Boundary_Packet.py",
    "docs/PHASE25_STEP21_POST_CLOSEOUT_PREFLIGHT_PLANNING_BOUNDARY_PACKET.md",
    "tests/test_phase25_step21_post_closeout_preflight_planning_boundary_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase25_boundary = "implementation_post_closeout_preflight_planning_boundary_opened_by_packet"
    ...
    implementation_post_closeout_preflight_planning_boundary_mode = "reference_only"
    implementation_post_closeout_preflight_planning_boundary_write = $false
    implementation_post_closeout_preflight_planning_boundary_record_creation = $false
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 25 Step 21 files copied or already present."
}
...
function Invoke-SmokeTest {
    ...
    $scriptText = Get-Content -LiteralPath (Join-SafePath -Root $Repo -RelativePath "scripts/phase25_step21_post_closeout_preflight_planning_boundary_packet.ps1") -Raw -Encoding UTF8
    ...
    Write-Host "SMOKE TEST PASS: Phase 25 Step 21 Implementation Post-Closeout Preflight Planning Boundary Packet is present and planning-only."
}
...
function Write-Packet {
    ...
    Write-Host "PASS: phase25_boundary=implementation_post_closeout_preflight_planning_boundary_opened_by_packet"
    ...
    Write-Host "PASS: implementation_post_closeout_preflight_planning_boundary_mode=reference_only"
    Write-Host "PASS: implementation_post_closeout_preflight_planning_boundary_write=false"
    Write-Host "PASS: implementation_post_closeout_preflight_planning_boundary_record_creation=false"
...
while ($true) {
    Write-Host ""
    Write-Host "Phase 25 Step 21 menu"
    Write-Host "1. Show status / verify paths"
    Write-Host "2. Apply Phase 25 Step 21 Implementation Post-Closeout Preflight Planning Boundary Packet files"
    Write-Host "3. Smoke test Phase 25 Step 21"
    Write-Host "4. Show server start placeholder only"
    Write-Host "5. Generate Phase 25 Step 21 Implementation Post-Closeout Preflight Planning Boundary Packet"
    Write-Host "6. Exit"
