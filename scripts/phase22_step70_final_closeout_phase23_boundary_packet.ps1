...
$PhaseNumber = 22
$StepNumber = 70
$StepName = "Phase 22 Step 70 - Phase 20 Network Transport Planning Final Closeout Phase 23 Boundary Packet"
$PriorStepName = "Phase 22 Step 69 - Phase 20 Network Transport Planning Final Cross-Repo Handoff Closeout Index Packet"
$ExpectedBranch = "phase22-step70-final-closeout-phase23-boundary"

$StepFiles = @(
    "scripts/phase22_step70_final_closeout_phase23_boundary_packet.ps1",
    "ui/pages/176_Phase20_Network_Transport_Planning_Final_Closeout_Phase23_Boundary_Packet.py",
    "docs/PHASE22_STEP70_FINAL_CLOSEOUT_PHASE23_BOUNDARY_PACKET.md",
    "tests/test_phase22_step70_final_closeout_phase23_boundary_packet.py"
)
...
function Get-SafetyPosture {
    return [ordered]@{
        ...
        final_closeout_phase23_boundary_record_creation = $false
        ...
        final_closeout_phase23_boundary_write = $false
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 22 Step 70 files copied or already present."
}
...
function Test-Smoke {
    ...
    if ($safety["final_closeout_phase23_boundary_write"]) { throw "SMOKE TEST FAIL: final closeout phase 23 boundary write must remain disabled" }
    ...
    Write-Host "SMOKE TEST PASS: Phase 22 Step 70 Phase 20 Network Transport Planning Final Closeout Phase 23 Boundary Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $packetPath = Join-SafePath -Root $backupDir -RelativePath ("phase22_step70_final_closeout_phase23_boundary_packet_{0}.json" -f $stamp)
    ...
    $packet = [ordered]@{
        ...
        planning_only = $true
        final_closeout_phase23_boundary_mode = "reference_only"
        handoff_state = "post_step68_gap_disposition_approval_readiness_reference_only"
        phase23_start = "not_started"
        implementation_phase_start = "not_started"
        safety_posture = Get-SafetyPosture
        final_closeout_phase23_boundary_scope = @(
            "Review reference-only closeout criteria prepared after Step 69",
        ...
        final_closeout_phase23_boundary_buckets = @(
    ...
    Write-Host "PASS: final_closeout_phase23_boundary_mode=reference_only"
    Write-Host "PASS: final_closeout_phase23_boundary_write=false"
    ...
    Write-Host "CHECK: prior_step=Phase 22 Step 69"
    Write-Host "CHECK: phase23_start=not_started"
    Write-Host "CHECK: implementation_phase_start=not_started"
    Write-Host "CHECK: final_closeout_phase23_boundary_mode=reference_only"
...
function Show-ServerPlaceholder {
    ...
    Write-Host "CHECK: Final closeout Phase 23 boundary review is reference-only and does not decide, collect, remediate, mutate, or validate sibling repositories."
}

function Show-Menu {
    while ($true) {
        Write-Host ""
        Write-Host "Phase 22 Step 70 menu"
        Write-Host "1. Show status / verify paths"
        Write-Host "2. Apply Phase 22 Step 70 Phase 20 Network Transport Planning Final Closeout Phase 23 Boundary Packet files"
        Write-Host "3. Smoke test Phase 22 Step 70"
        Write-Host "4. Show server start placeholder only"
        Write-Host "5. Generate Phase 22 Step 70 final closeout phase 23 boundary packet"
        Write-Host "6. Exit"
