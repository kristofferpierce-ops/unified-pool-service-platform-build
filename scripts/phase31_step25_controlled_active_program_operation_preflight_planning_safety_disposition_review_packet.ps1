...
$StepNumber = 25
$StepTitle = "Phase 31 Step 25 - Phase 20 Network Transport Implementation Controlled Active Program Operation Preflight Planning Safety Disposition Review Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Controlled Active Program Operation Preflight Planning Safety Disposition Review Packet"
$ExpectedBranch = "phase31-step25-controlled-active-program-operation-preflight-planning-safety-disposition-review"
$PriorStep = "Phase 31 Step 24 - Phase 20 Network Transport Implementation Controlled Active Program Operation Preflight Planning Safety Disposition Planning Packet"
$Slug = "implementation_controlled_active_program_operation_preflight_planning_safety_disposition_review"
...
$StepFiles = @(
    "scripts/phase31_step25_controlled_active_program_operation_preflight_planning_safety_disposition_review_packet.ps1",
    "ui/pages/1121_Phase31_Step25_Implementation_Controlled_Active_Program_Operation_Preflight_Planning_Safety_Disposition_Review_Packet.py",
    "docs/PHASE31_STEP25_CONTROLLED_ACTIVE_PROGRAM_OPERATION_PREFLIGHT_PLANNING_SAFETY_DISPOSITION_REVIEW_PACKET.md",
    "tests/test_phase31_step25_controlled_active_program_operation_preflight_planning_safety_disposition_review_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase31_boundary = "implementation_controlled_active_program_operation_preflight_planning_safety_disposition_review_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 31 Step 25 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 31 Step 25 Phase 20 Network Transport Implementation Controlled Active Program Operation Preflight Planning Safety Disposition Review Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase31_step25_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase31_step25_{0}_packet.json" -f $Slug)
