...
$StepNumber = 17
$StepTitle = "Phase 31 Step 17 - Phase 20 Network Transport Implementation Controlled Active Program Operation Guardrail Verification Approval Readiness Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Controlled Active Program Operation Guardrail Verification Approval Readiness Packet"
$ExpectedBranch = "phase31-step17-controlled-active-program-operation-guardrail-verification-approval-readiness"
$PriorStep = "Phase 31 Step 16 - Phase 20 Network Transport Implementation Controlled Active Program Operation Guardrail Verification Approval Boundary Packet"
$Slug = "implementation_controlled_active_program_operation_guardrail_verification_approval_readiness"
...
$StepFiles = @(
    "scripts/phase31_step17_controlled_active_program_operation_guardrail_verification_approval_readiness_packet.ps1",
    "ui/pages/1113_Phase31_Step17_Implementation_Controlled_Active_Program_Operation_Guardrail_Verification_Approval_Readiness_Packet.py",
    "docs/PHASE31_STEP17_CONTROLLED_ACTIVE_PROGRAM_OPERATION_GUARDRAIL_VERIFICATION_APPROVAL_READINESS_PACKET.md",
    "tests/test_phase31_step17_controlled_active_program_operation_guardrail_verification_approval_readiness_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase31_boundary = "implementation_controlled_active_program_operation_guardrail_verification_approval_readiness_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 31 Step 17 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 31 Step 17 Phase 20 Network Transport Implementation Controlled Active Program Operation Guardrail Verification Approval Readiness Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase31_step17_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase31_step17_{0}_packet.json" -f $Slug)
