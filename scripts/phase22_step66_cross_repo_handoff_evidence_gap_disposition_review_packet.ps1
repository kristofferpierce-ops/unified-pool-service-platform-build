...
$PhaseNumber = 22
$StepNumber = 66
$StepName = "Phase 22 Step 66 - Phase 20 Network Transport Planning Cross-Repo Handoff Evidence Gap Disposition Review Packet"
$PriorStepName = "Phase 22 Step 65 - Phase 20 Network Transport Planning Cross-Repo Handoff Evidence Gap Disposition Planning Packet"
$ExpectedBranch = "phase22-step66-cross-repo-handoff-evidence-gap-disposition-review"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 22 Step 66 files copied or already present."
}
...
function Test-Smoke {
    ...
    if ($safety["handoff_evidence_gap_disposition_review_write"]) { throw "SMOKE TEST FAIL: handoff evidence gap disposition review write must remain disabled" }
    ...
    Write-Host "SMOKE TEST PASS: Phase 22 Step 66 Phase 20 Network Transport Planning Cross-Repo Handoff Evidence Gap Disposition Review Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $packetPath = Join-SafePath -Root $backupDir -RelativePath ("phase22_step66_cross_repo_handoff_evidence_gap_disposition_review_packet_{0}.json" -f $stamp)
    ...
    $packet = [ordered]@{
        ...
        planning_only = $true
        evidence_gap_disposition_review_mode = "reference_only"
        handoff_state = "post_step65_gap_disposition_planning_reference_only"
        ...
        evidence_gap_disposition_review_scope = @(
            "Review reference-only disposition categories prepared in Step 65",
        ...
        evidence_gap_disposition_review_buckets = @(
        ...
        non_actions = @(
            ...
            "No evidence gap disposition decision creation",
    ...
    Write-Host "PASS: evidence_gap_disposition_review_mode=reference_only"
    Write-Host "PASS: handoff_evidence_gap_disposition_review_write=false"
    ...
    Write-Host "CHECK: prior_step=Phase 22 Step 65"
    Write-Host "CHECK: phase23_start=not_started"
    Write-Host "CHECK: implementation_phase_start=not_started"
    Write-Host "CHECK: evidence_gap_disposition_review_mode=reference_only"
...
function Show-Menu {
    while ($true) {
        Write-Host ""
        Write-Host "Phase 22 Step 66 menu"
        Write-Host "1. Show status / verify paths"
        Write-Host "2. Apply Phase 22 Step 66 Phase 20 Network Transport Planning Cross-Repo Handoff Evidence Gap Disposition Review Packet files"
        Write-Host "3. Smoke test Phase 22 Step 66"
        Write-Host "4. Show server start placeholder only"
        Write-Host "5. Generate Phase 22 Step 66 cross-repo handoff evidence gap disposition review packet"
        Write-Host "6. Exit"
