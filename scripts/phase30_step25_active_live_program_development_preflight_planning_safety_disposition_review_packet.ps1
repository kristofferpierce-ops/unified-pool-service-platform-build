...
$StepNumber = 25
$StepTitle = "Phase 30 Step 25 - Phase 20 Network Transport Implementation Active Live Program Development Preflight Planning Safety Disposition Review Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Active Live Program Development Preflight Planning Safety Disposition Review Packet"
$ExpectedBranch = "phase30-step25-active-live-program-development-preflight-planning-safety-disposition-review"
$PriorStep = "Phase 30 Step 24 - Phase 20 Network Transport Implementation Active Live Program Development Preflight Planning Safety Disposition Planning Packet"
$Slug = "implementation_active_live_program_development_preflight_planning_safety_disposition_review"
...
$StepFiles = @(
    "scripts/phase30_step25_active_live_program_development_preflight_planning_safety_disposition_review_packet.ps1",
    "ui/pages/1001_Phase30_Step25_Implementation_Active_Live_Program_Development_Preflight_Planning_Safety_Disposition_Review_Packet.py",
    "docs/PHASE30_STEP25_ACTIVE_LIVE_PROGRAM_DEVELOPMENT_PREFLIGHT_PLANNING_SAFETY_DISPOSITION_REVIEW_PACKET.md",
    "tests/test_phase30_step25_active_live_program_development_preflight_planning_safety_disposition_review_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase30_boundary = "implementation_active_live_program_development_preflight_planning_safety_disposition_review_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 30 Step 25 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 30 Step 25 Phase 20 Network Transport Implementation Active Live Program Development Preflight Planning Safety Disposition Review Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase30_step25_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase30_step25_{0}_packet.json" -f $Slug)
