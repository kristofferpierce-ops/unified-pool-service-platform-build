...
$PhaseNumber = 36
$StepNumber = 1
$StepTitle = "Phase 36 Step 1 - Phase 20 Network Transport Implementation Trusted Production Controlled Live Write Expansion Readiness Boundary Packet"
$ExpectedBranch = "phase36-step1-readiness-boundary"
$PriorCompletedStep = "Phase 35 Step 120 - Phase 20 Network Transport Implementation Phase 35 Final No-Write Closeout Hold Packet"
...
$StepFiles = @(
    "scripts/phase36_step1_controlled_live_write_expansion_readiness_boundary_packet.ps1",
    "ui/pages/1697_Phase36_Step1_Readiness_Boundary.py",
    "docs/PHASE36_STEP1_CONTROLLED_LIVE_WRITE_EXPANSION_READINESS_BOUNDARY_PACKET.md",
    "tests/test_phase36_step1_controlled_live_write_expansion_readiness_boundary_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase36_boundary = "trusted_production_controlled_live_write_expansion_readiness_boundary_opened_by_packet"
...
function Assert-StepFiles {
    foreach ($Rel in $StepFiles) {
        ...
        if (-not (Test-Path $Path)) {
            throw ("Missing expected Phase 36 Step 1 file: {0}" -f $Rel)
        }
    }
}
...
function Invoke-Apply {
    Assert-StepFiles
    Write-Host "APPLY PASS: Phase 36 Step 1 planning-only packet verified without live-write activation, live-write apply, bridge POST, sockets, DB mutation, server launch, or Phase 37 boundary creation."
}
...
function Invoke-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 36 Step 1 safety markers confirm planning-only/no-write/no-live-write-activation/no-live-write-apply/no-runtime/no-phase37 posture."
}
