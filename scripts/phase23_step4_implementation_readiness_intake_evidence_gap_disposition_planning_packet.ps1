...
$PhaseNumber = 23
$StepNumber = 4
$StepName = "Phase 23 Step 4 - Phase 20 Network Transport Implementation Readiness Intake Evidence Gap Disposition Planning Packet"
$PriorStepName = "Phase 23 Step 3 - Phase 20 Network Transport Implementation Readiness Intake Evidence Gap Review Packet"
$ExpectedBranch = "phase23-step4-readiness-intake-evidence-gap-disposition-planning"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 23 Step 4 files copied or already present."
}
...
function Test-Smoke {
    ...
    if ($safety["readiness_intake_evidence_gap_disposition_planning_write"]) { throw "SMOKE TEST FAIL: readiness intake evidence gap disposition planning writes must remain disabled" }
    if ($safety["readiness_intake_evidence_gap_disposition_planning_record_creation"]) { throw "SMOKE TEST FAIL: readiness intake evidence gap disposition planning record creation must remain disabled" }
    ...
    Write-Host "SMOKE TEST PASS: Phase 23 Step 4 Phase 20 Network Transport Implementation Readiness Intake Evidence Gap Disposition Planning Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $packetDir = Join-SafePath -Root $backupRoot -RelativePath ("phase23_step4_implementation_readiness_intake_evidence_gap_disposition_planning_packet_{0}" -f $stamp)
    if (-not (Test-SafePath -Path $packetDir)) { New-Item -ItemType Directory -Path $packetDir -Force | Out-Null }
    $packetPath = Join-SafePath -Root $packetDir -RelativePath "phase23_step4_implementation_readiness_intake_evidence_gap_disposition_planning_packet.json"
    $packet = [ordered]@{
        phase = 23
        step = 4
        step_name = $StepName
        ...
        intake_scope = @(
            "Carry forward the Phase 23 Step 3 readiness intake evidence gap review as the prior reference",
            "Open Phase 23 as a planning intake evidence gap disposition planning only",
    ...
    Write-Host "PASS: readiness_intake_evidence_gap_disposition_planning_mode=reference_only"
    Write-Host "PASS: readiness_intake_evidence_gap_disposition_planning_write=false"
    Write-Host "PASS: readiness_intake_evidence_gap_disposition_planning_record_creation=false"
    ...
    Write-Host "CHECK: prior_step=Phase 23 Step 3"
...
function Show-ServerPlaceholder {
    ...
    Write-Host "CHECK: Phase 23 Step 4 is planning intake only and does not start network transport implementation."
...
function Show-Menu {
    while ($true) {
        Write-Host ""
        Write-Host "Phase 23 Step 4 menu"
        Write-Host "1. Show status / verify paths"
        Write-Host "2. Apply Phase 23 Step 4 Phase 20 Network Transport Implementation Readiness Intake Evidence Gap Disposition Planning Packet files"
        Write-Host "3. Smoke test Phase 23 Step 4"
        Write-Host "4. Show server start placeholder only"
        Write-Host "5. Generate Phase 23 Step 4 implementation readiness intake evidence gap disposition planning packet"
        Write-Host "6. Exit"
