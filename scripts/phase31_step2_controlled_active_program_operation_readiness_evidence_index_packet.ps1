...
$StepNumber = 2
$StepTitle = "Phase 31 Step 2 - Phase 20 Network Transport Implementation Controlled Active Program Operation Readiness Evidence Index Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Controlled Active Program Operation Readiness Evidence Index Packet"
$ExpectedBranch = "phase31-step2-controlled-active-program-operation-readiness-evidence-index"
$PriorStep = "Phase 31 Step 1 - Phase 20 Network Transport Implementation Controlled Active Program Operation Readiness Boundary Packet"
$Slug = "implementation_controlled_active_program_operation_readiness_evidence_index"
...
$SafetyPosture = [ordered]@{
    ...
    phase31_boundary = "implementation_controlled_active_program_operation_readiness_evidence_index_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 31 Step 2 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 31 Step 2 Phase 20 Network Transport Implementation Controlled Active Program Operation Readiness Evidence Index Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase31_step2_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase31_step2_{0}_packet.json" -f $Slug)
