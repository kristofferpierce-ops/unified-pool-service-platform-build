...
$StepNumber = 18
$StepTitle = "Phase 30 Step 18 - Phase 20 Network Transport Implementation Active Live Program Development Guardrail Verification Operator Hold Point Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Active Live Program Development Guardrail Verification Operator Hold Point Packet"
$ExpectedBranch = "phase30-step18-active-live-program-development-guardrail-verification-operator-hold-point"
$PriorStep = "Phase 30 Step 17 - Phase 20 Network Transport Implementation Active Live Program Development Guardrail Verification Approval Readiness Packet"
$Slug = "implementation_active_live_program_development_guardrail_verification_operator_hold_point"
...
$StepFiles = @(
    "scripts/phase30_step18_active_live_program_development_guardrail_verification_operator_hold_point_packet.ps1",
    "ui/pages/994_Phase30_Step18_Implementation_Active_Live_Program_Development_Guardrail_Verification_Operator_Hold_Point_Packet.py",
    "docs/PHASE30_STEP18_ACTIVE_LIVE_PROGRAM_DEVELOPMENT_GUARDRAIL_VERIFICATION_OPERATOR_HOLD_POINT_PACKET.md",
    "tests/test_phase30_step18_active_live_program_development_guardrail_verification_operator_hold_point_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase30_boundary = "implementation_active_live_program_development_guardrail_verification_operator_hold_point_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 30 Step 18 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 30 Step 18 Phase 20 Network Transport Implementation Active Live Program Development Guardrail Verification Operator Hold Point Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase30_step18_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase30_step18_{0}_packet.json" -f $Slug)
