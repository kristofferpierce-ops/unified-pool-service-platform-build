...
function Classify-RepoPath {
    ...
    if ($CurrentStepFiles -contains $Path) {
        return [ordered]@{
            ...
            should_stage = $true
            reason = "Current Phase 21 Step 11 file."
        }
    }
...
if ([string]::IsNullOrWhiteSpace($ArchivePlanPacketDir)) {
    $ArchivePlanPacketDir = Get-LatestArtifactDir -FolderFilter "phase21_phase20_network_transport_archive_plan_packet_*" -JsonName "phase21_phase20_network_transport_archive_plan_packet.json" -Required $true
}
...
$archivePlanPath = Join-Path $ArchivePlanPacketDir "phase21_phase20_network_transport_archive_plan_packet.json"
...
if (!(Test-Path -LiteralPath $archivePlanPath)) {
    throw "Archive plan packet JSON not found: $archivePlanPath"
}

$archivePlan = Read-JsonFile $archivePlanPath
...
$decisionSections = New-Object System.Collections.ArrayList
$decisionEntries = New-Object System.Collections.ArrayList
...
Add-ChecklistItem $checklist "artifact" "Archive plan packet exists" $true $archivePlanPath
...
Add-ChecklistItem $checklist "archive_plan" "Archive plan packet is archive-decision-safe" ($archivePlan.safety.phase20_network_transport_archive_plan_packet_only -eq $true) "phase20_network_transport_archive_plan_packet_only=$($archivePlan.safety.phase20_network_transport_archive_plan_packet_only)"
Add-ChecklistItem $checklist "archive_plan" "Archive plan created no closure record" ($archivePlan.safety.design_closure_record_created -eq $false) "design_closure_record_created=$($archivePlan.safety.design_closure_record_created)"
Add-ChecklistItem $checklist "archive_plan" "Archive plan recorded no final approval" ($archivePlan.safety.final_approval_recorded -eq $false) "final_approval_recorded=$($archivePlan.safety.final_approval_recorded)"
Add-ChecklistItem $checklist "archive_plan" "Archive plan started no implementation phase" ($archivePlan.safety.implementation_phase_started -eq $false) "implementation_phase_started=$($archivePlan.safety.implementation_phase_started)"
Add-ChecklistItem $checklist "archive_plan" "Archive plan has no execution implementation" ($archivePlan.safety.execution_implementation_created -eq $false) "execution_implementation_created=$($archivePlan.safety.execution_implementation_created)"
Add-ChecklistItem $checklist "archive_plan" "Archive plan has no network transport" ($archivePlan.safety.network_transport_implemented -eq $false) "network_transport_implemented=$($archivePlan.safety.network_transport_implemented)"
Add-ChecklistItem $checklist "archive_plan" "Archive plan has no socket opened" ($archivePlan.safety.network_socket_opened -eq $false) "network_socket_opened=$($archivePlan.safety.network_socket_opened)"
Add-ChecklistItem $checklist "archive_plan" "Archive plan has no bridge POST" ($archivePlan.safety.bridge_post_called -eq $false -and $archivePlan.safety.bridge_post_call_implemented -eq $false) "bridge_post_called=$($archivePlan.safety.bridge_post_called); bridge_post_call_implemented=$($archivePlan.safety.bridge_post_call_implemented)"
Add-ChecklistItem $checklist "archive_plan" "Archive plan has no platform DB mutation" ($archivePlan.safety.platform_db_mutation_performed -eq $false) "platform_db_mutation_performed=$($archivePlan.safety.platform_db_mutation_performed)"
Add-ChecklistItem $checklist "archive_plan" "Archive plan has no bridge mutation" ($archivePlan.safety.bridge_mutation_performed -eq $false) "bridge_mutation_performed=$($archivePlan.safety.bridge_mutation_performed)"
Add-ChecklistItem $checklist "archive_plan" "Archive plan has no LACRM call" ($archivePlan.safety.lacrm_call_performed -eq $false) "lacrm_call_performed=$($archivePlan.safety.lacrm_call_performed)"
Add-ChecklistItem $checklist "archive_plan" "Archive plan has no blockers" ([int]$archivePlan.archive_plan_packet.blocker_count -eq 0) "blocker_count=$($archivePlan.archive_plan_packet.blocker_count)"
...
# Archive decision uses git status --short against the platform repo.
...
$artifactFamilies = @($archivePlan.artifact_families | Sort-Object @{Expression = {[int]$_.older_generated_folder_count}; Descending = $true}, @{Expression = {[string]$_.folder_filter}; Descending = $false})
...
[void]$decisionSections.Add([ordered]@{
    section = "Archive decisions"
    status = if ($archiveCandidateFamilyCount -gt 0) { "review" } else { "clean" }
    summary = "Each packet family is labeled keep-latest-only or manual-archive-review."
})
[void]$decisionSections.Add([ordered]@{
    section = "Keep latest packet folders"
    status = "required"
...
[void]$decisionSections.Add([ordered]@{
    ...
    status = "required"
    summary = "Carry forward the do-not-stage paths while archive operations remain manual only."
})
[void]$decisionSections.Add([ordered]@{
    section = "Review-before-stage boundary"
    status = if ($reviewOtherCount -eq 0 -and $stagedReviewOtherCount -eq 0) { "clean" } else { "review" }
    summary = "Any non-Step-11 paths outside the do-not-stage list require review before staging."
})
[void]$decisionSections.Add([ordered]@{
...
foreach ($file in $currentStepFiles) {
    [void]$decisionEntries.Add([ordered]@{
        category = "stage_now"
        action = "stage"
        item = $file
        evidence = "Current Phase 21 Step 11 file."
        required = "yes"
    })
}
...
foreach ($family in @($artifactFamilies)) {
    ...
    if ($olderCount -gt 0) {
        $decision = "manual_archive_review"
        $action = "review_archive_older_generated_folders"
        $reason = "Older generated folders exist; keep latest packet folder and review older folders manually."
        $order = $decisionOrder
        $decisionOrder += 1
    } else {
        $decision = "keep_latest_only"
        $action = "keep_latest"
        $reason = "Only the latest packet folder exists."
        $order = 0
    }

    [void]$archiveDecisions.Add([ordered]@{
        folder_filter = [string]$family.folder_filter
        json_name = [string]$family.json_name
        folder_count = [int]$family.folder_count
        older_generated_folder_count = [int]$family.older_generated_folder_count
        latest_folder = [string]$family.latest_folder
        latest_json_path = [string]$family.latest_json_path
        decision = $decision
        action = $action
        decision_reason = $reason
        archive_order = $order
        keep_latest_folder = $true
        required = "recommended"
    })

    [void]$decisionEntries.Add([ordered]@{
        category = "archive_decisions"
        action = $action
        item = [string]$family.folder_filter
        evidence = "decision=$decision; older_generated_folders=$olderCount; latest=$($family.latest_folder)"
        required = "recommended"
    })
}
...
$nextCommitCommands = @(
    ...
    "git status --short",
    "git add scripts/phase21_generate_phase20_network_transport_archive_decision_packet.ps1",
    "git add ui/pages/103_Phase20_Network_Transport_Archive_Decision_Packet.py",
    "git add docs/PHASE21_STEP11_PHASE20_NETWORK_TRANSPORT_ARCHIVE_DECISION_PACKET.md",
    "git add tests/test_phase21_phase20_network_transport_archive_decision_packet.py",
    "git diff --cached --name-only",
    'git commit -m "Phase 21 Step 11 phase20 network transport archive decision packet"',
    "git push -u origin phase21-step11-phase20-network-transport-archive-decision-packet"
)

Add-ChecklistItem $checklist "archive_decision" "Current branch detected" (-not [string]::IsNullOrWhiteSpace($currentBranch)) "current_branch=$currentBranch"
Add-ChecklistItem $checklist "archive_decision" "At least one archive-decision section exists" (@($decisionSections).Count -gt 0) "archive_decision_sections=$(@($decisionSections).Count)"
Add-ChecklistItem $checklist "archive_decision" "Exactly four stage-now entries exist" (@($decisionEntries | Where-Object { $_.category -eq "stage_now" }).Count -eq 4) "stage_now_entries=$(@($decisionEntries | Where-Object { $_.category -eq "stage_now" }).Count)"
Add-ChecklistItem $checklist "archive_decision" "Archive decisions were generated" (@($archiveDecisions).Count -eq @($artifactFamilies).Count) "archive_decisions=$(@($archiveDecisions).Count); artifact_families=$(@($artifactFamilies).Count)" "recommended"
Add-ChecklistItem $checklist "archive_decision" "Commit commands exist" (@($nextCommitCommands).Count -gt 0) "commit_commands=$(@($nextCommitCommands).Count)"
Add-ChecklistItem $checklist "archive_decision" "No forbidden paths are currently staged" ($stagedForbiddenCount -eq 0) "staged_forbidden_count=$stagedForbiddenCount"
Add-ChecklistItem $checklist "archive_decision" "No review-other paths are currently staged" ($stagedReviewOtherCount -eq 0) "staged_review_other_count=$stagedReviewOtherCount" "recommended"
Add-ChecklistItem $checklist "archive_decision" "Workspace review-other files count is zero or acceptable" ($reviewOtherCount -eq 0) "review_other_count=$reviewOtherCount" "recommended"

$archivePlanStatus = [string]$archivePlan.archive_plan_packet.status
if ($archivePlanStatus -eq "phase20_archive_plan_packet_blocked") {
    Add-Issue $issues "blocker" "archive_plan_packet_blocked" "Archive plan packet is blocked." "archive_plan"
} elseif ($archivePlanStatus -eq "phase20_archive_plan_packet_review_required_no_write") {
    Add-Issue $issues "review" "archive_plan_packet_review_required" "Archive plan packet has review items." "archive_plan"
}

if ([string]::IsNullOrWhiteSpace($currentBranch)) {
    Add-Issue $issues "blocker" "current_branch_missing" "Current branch was not detected for archive decision." "workspace"
}
...
[void]$sourceArtifacts.Add([ordered]@{ key="archive_plan_packet"; json_path=$archivePlanPath; sha256=Get-FileSha256 $archivePlanPath })
...
$packetStatus = if ($blockerCount -eq 0 -and $reviewCount -eq 0) {
    "phase20_archive_decision_packet_ready_no_write"
} elseif ($blockerCount -eq 0) {
    "phase20_archive_decision_packet_review_required_no_write"
} else {
    "phase20_archive_decision_packet_blocked"
}

if ([string]::IsNullOrWhiteSpace($OutputDir)) {
    $stamp = Get-Date -Format "yyyyMMdd-HHmmss"
    $OutputDir = Join-Path $BackupDir "phase21_phase20_network_transport_archive_decision_packet_$stamp"
}
...
$report = [ordered]@{
    ...
    phase = "Phase 21 Step 11"
    purpose = "Phase 20 bridge routing network transport archive decision packet"
    workspace = $Workspace
    platform_api = $PlatformApi
    bridge_url = $BridgeUrl
    safety = [ordered]@{
        phase20_network_transport_archive_decision_packet_only = $true
        archive_decision_only = $true
    ...
    archive_decision_packet = [ordered]@{
        status = $packetStatus
        blocker_count = $blockerCount
        review_count = $reviewCount
        current_branch = $currentBranch
        section_count = @($decisionSections).Count
        entry_count = @($decisionEntries).Count
        ...
        packet_label = "phase21_phase20_network_transport_archive_decision_no_write"
        reason = if ($blockerCount -eq 0) {
            "Phase 21 Step 11 created a no-write archive decision packet for the Phase 20 network transport cleanup chain. It does not authorize approvals, implementation, transport, sockets, bridge POST, response capture, mutations, LACRM calls, or live writes."
        } else {
            "Phase 21 Step 11 found blockers in the Phase 20 archive decision packet. Resolve blockers before further cleanup work."
        }
    }
    decision_sections = @($decisionSections)
    ...
    counts = [ordered]@{
        sections = @($decisionSections).Count
        entries = @($decisionEntries).Count
    ...
    next_recommended_actions = @(
        "Review this archive decision packet before any manual backup-folder archiving.",
        "Keep the latest packet folder in each family and only review older generated folders for manual archive actions.",
        "Use the stage-now entries as the only files to add for the Step 11 commit.",
        "Carry the do-not-stage entries forward as the hard commit boundary.",
        "Do not add requests, httpx, socket use, bridge POST calls, or routing write endpoints from Phase 21 Step 11.",
        "Do not call interface execution methods from Phase 21 Step 11.",
        "Do not record final approvals from Phase 21 Step 11.",
        "Do not start an implementation phase from Phase 21 Step 11.",
        "Keep bridge routing controls as the operational write path until explicit cutover."
    )
}

$jsonPath = Join-Path $OutputDir "phase21_phase20_network_transport_archive_decision_packet.json"
$sectionsCsvPath = Join-Path $OutputDir "phase21_phase20_network_transport_archive_decision_packet_sections.csv"
$entriesCsvPath = Join-Path $OutputDir "phase21_phase20_network_transport_archive_decision_packet_entries.csv"
$decisionsCsvPath = Join-Path $OutputDir "phase21_phase20_network_transport_archive_decision_packet_decisions.csv"
$familiesCsvPath = Join-Path $OutputDir "phase21_phase20_network_transport_archive_decision_packet_artifact_families.csv"
$gitStatusCsvPath = Join-Path $OutputDir "phase21_phase20_network_transport_archive_decision_packet_git_status.csv"
$stagedCsvPath = Join-Path $OutputDir "phase21_phase20_network_transport_archive_decision_packet_staged.csv"
$sourceArtifactsCsvPath = Join-Path $OutputDir "phase21_phase20_network_transport_archive_decision_packet_source_artifacts.csv"
$checklistCsvPath = Join-Path $OutputDir "phase21_phase20_network_transport_archive_decision_packet_checklist.csv"
$issuesCsvPath = Join-Path $OutputDir "phase21_phase20_network_transport_archive_decision_packet_issues.csv"
$mdPath = Join-Path $OutputDir "phase21_phase20_network_transport_archive_decision_packet.md"

$report | ConvertTo-Json -Depth 100 | Set-Content -LiteralPath $jsonPath -Encoding UTF8
$decisionSections | Export-Csv -LiteralPath $sectionsCsvPath -NoTypeInformation -Encoding UTF8
$decisionEntries | Export-Csv -LiteralPath $entriesCsvPath -NoTypeInformation -Encoding UTF8
...
$sectionText = (@($decisionSections) | ForEach-Object { "- $($_.section): $($_.status); $($_.summary)" }) -join "`n"
$entryText = (@($decisionEntries) | ForEach-Object { "- [$($_.category)] $($_.action) $($_.item): $($_.evidence)" }) -join "`n"
...
- Phase 20 network transport archive decision packet only: true
- Archive decision only: true
...
if ($blockerCount -eq 0) {
    Write-Host "PASS | phase20_network_transport_archive_decision_packet=$OutputDir | status=$packetStatus | bridge_post_called=False | execution_implementation_created=False | implementation_phase_started=False"
} else {
    Write-Host "CHECK | phase20_network_transport_archive_decision_packet=$OutputDir | status=$packetStatus | blockers=$blockerCount | bridge_post_called=False | execution_implementation_created=False | implementation_phase_started=False"
}

Write-Host ""
Write-Host "Archive decision packet files:"
