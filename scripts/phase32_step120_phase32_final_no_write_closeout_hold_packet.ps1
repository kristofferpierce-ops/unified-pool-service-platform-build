...
$StepNumber = 120
$StepTitle = "Phase 32 Step 120 - Phase 20 Network Transport Implementation Phase 32 Final No-Write Closeout Hold Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Phase 32 Final No-Write Closeout Hold Packet"
$ExpectedBranch = "phase32-step120-final-no-write-closeout-hold"
$PriorStep = "Phase 32 Step 119 - Phase 20 Network Transport Implementation Production-Like Rollout Final Release Result Review Planning Closeout Index Packet"
$Slug = "phase32_final_no_write_closeout_hold"
$ContextKey = "phase32_final_no_write_closeout_hold"
$PhaseContext = "phase32_final_no_write_closeout_hold_planning_only"

if (-not $RepoRoot) { $RepoRoot = (Resolve-Path (Join-Path $PSScriptRoot ".." )).Path }

$StepFiles = @(
    "scripts/phase32_step120_phase32_final_no_write_closeout_hold_packet.ps1",
    "ui/pages/1336_Phase32_Step120_Phase32_Final_No_Write_Closeout_Hold_Packet.py",
    "docs/PHASE32_STEP120_PHASE32_FINAL_NO_WRITE_CLOSEOUT_HOLD_PACKET.md",
    "tests/test_phase32_step120_phase32_final_no_write_closeout_hold_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase32_boundary = "phase32_final_no_write_closeout_hold_packet_only"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 32 Step 120 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 32 Step 120 Phase 20 Network Transport Implementation Phase 32 Final No-Write Closeout Hold Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase32_step120_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase32_step120_{0}_packet.json" -f $Slug)
