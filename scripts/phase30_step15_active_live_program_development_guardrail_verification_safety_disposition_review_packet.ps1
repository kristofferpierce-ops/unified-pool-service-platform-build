...
$StepNumber = 15
$StepTitle = "Phase 30 Step 15 - Phase 20 Network Transport Implementation Active Live Program Development Guardrail Verification Safety Disposition Review Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Active Live Program Development Guardrail Verification Safety Disposition Review Packet"
$ExpectedBranch = "phase30-step15-active-live-program-development-guardrail-verification-safety-disposition-review"
$PriorStep = "Phase 30 Step 14 - Phase 20 Network Transport Implementation Active Live Program Development Guardrail Verification Safety Disposition Planning Packet"
$Slug = "implementation_active_live_program_development_guardrail_verification_safety_disposition_review"
...
$StepFiles = @(
    "scripts/phase30_step15_active_live_program_development_guardrail_verification_safety_disposition_review_packet.ps1",
    "ui/pages/991_Phase30_Step15_Implementation_Active_Live_Program_Development_Guardrail_Verification_Safety_Disposition_Review_Packet.py",
    "docs/PHASE30_STEP15_ACTIVE_LIVE_PROGRAM_DEVELOPMENT_GUARDRAIL_VERIFICATION_SAFETY_DISPOSITION_REVIEW_PACKET.md",
    "tests/test_phase30_step15_active_live_program_development_guardrail_verification_safety_disposition_review_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase30_boundary = "implementation_active_live_program_development_guardrail_verification_safety_disposition_review_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 30 Step 15 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 30 Step 15 Phase 20 Network Transport Implementation Active Live Program Development Guardrail Verification Safety Disposition Review Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase30_step15_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase30_step15_{0}_packet.json" -f $Slug)
