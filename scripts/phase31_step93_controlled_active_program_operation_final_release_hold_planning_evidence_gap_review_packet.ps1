...
$StepNumber = 93
$StepTitle = "Phase 31 Step 93 - Phase 20 Network Transport Implementation Controlled Active Program Operation Final Release Hold Planning Evidence Gap Review Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Controlled Active Program Operation Final Release Hold Planning Evidence Gap Review Packet"
$ExpectedBranch = "phase31-step93-controlled-active-program-operation-final-release-hold-planning-evidence-gap-review"
$PriorStep = "Phase 31 Step 92 - Phase 20 Network Transport Implementation Controlled Active Program Operation Final Release Hold Planning Evidence Index Packet"
$Slug = "implementation_controlled_active_program_operation_final_release_hold_planning_evidence_gap_review"
...
$StepFiles = @(
    "scripts/phase31_step93_controlled_active_program_operation_final_release_hold_planning_evidence_gap_review_packet.ps1",
    "ui/pages/1189_Phase31_Step93_Implementation_Controlled_Active_Program_Operation_Final_Release_Hold_Planning_Evidence_Gap_Review_Packet.py",
    "docs/PHASE31_STEP93_CONTROLLED_ACTIVE_PROGRAM_OPERATION_FINAL_RELEASE_HOLD_PLANNING_EVIDENCE_GAP_REVIEW_PACKET.md",
    "tests/test_phase31_step93_controlled_active_program_operation_final_release_hold_planning_evidence_gap_review_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase31_boundary = "implementation_controlled_active_program_operation_final_release_hold_planning_evidence_gap_review_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 31 Step 93 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 31 Step 93 Phase 20 Network Transport Implementation Controlled Active Program Operation Final Release Hold Planning Evidence Gap Review Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase31_step93_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase31_step93_{0}_packet.json" -f $Slug)
