...
$PhaseNumber = 23
$StepNumber = 36
$StepName = "Phase 23 Step 36 - Phase 20 Network Transport Implementation Patch Review Planning Evidence Index Packet"
$PriorStepName = "Phase 23 Step 35 - Phase 20 Network Transport Implementation Patch Review Planning Boundary Packet"
$ExpectedBranch = "phase23-step36-implementation-patch-review-planning-evidence-index"
$ModeKey = "implementation_patch_review_planning_evidence_index_mode"
$WriteKey = "implementation_patch_review_planning_evidence_index_write"
$RecordKey = "implementation_patch_review_planning_evidence_index_record_creation"
$PhaseContext = "implementation_patch_review_planning_evidence_index_planning_only"
$PacketSlug = "phase23_step36_implementation_patch_review_planning_evidence_index_packet"

$StepFiles = @(
    "scripts/phase23_step36_implementation_patch_review_planning_evidence_index_packet.ps1",
    "ui/pages/212_Phase23_Step36_Implementation_Patch_Review_Planning_Evidence_Index_Packet.py",
    "docs/PHASE23_STEP36_IMPLEMENTATION_PATCH_REVIEW_PLANNING_EVIDENCE_INDEX_PACKET.md",
    "tests/test_phase23_step36_implementation_patch_review_planning_evidence_index_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase23_planning_boundary = "implementation_patch_review_planning_evidence_index_opened_by_packet"
    phase23_implementation_start = $false
    implementation_phase_start = $false
    cross_repo_write = $false
    cross_repo_mutation = $false
    external_repo_push = $false
    implementation_patch_review_planning_evidence_index_mode = "reference_only"
    implementation_patch_review_planning_evidence_index_write = $false
    implementation_patch_review_planning_evidence_index_record_creation = $false
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 23 Step 36 files copied or already present."
}
...
function Invoke-SmokeTest {
    ...
    $scriptText = Get-Content -LiteralPath (Join-SafePath -Root $Repo -RelativePath "scripts/phase23_step36_implementation_patch_review_planning_evidence_index_packet.ps1") -Raw -Encoding UTF8
    ...
    Write-Host "SMOKE TEST PASS: Phase 23 Step 36 Implementation Patch Review Planning Evidence Index Packet is present and planning-only."
}
...
function Write-Packet {
    ...
    Write-Host "PASS: phase23_planning_boundary=implementation_patch_review_planning_evidence_index_opened_by_packet"
    ...
    Write-Host "PASS: implementation_patch_review_planning_evidence_index_mode=reference_only"
    Write-Host "PASS: implementation_patch_review_planning_evidence_index_write=false"
    Write-Host "PASS: implementation_patch_review_planning_evidence_index_record_creation=false"
...
while ($true) {
    Write-Host ""
    Write-Host "Phase 23 Step 36 menu"
    Write-Host "1. Show status / verify paths"
    Write-Host "2. Apply Phase 23 Step 36 Implementation Patch Review Planning Evidence Index Packet files"
    Write-Host "3. Smoke test Phase 23 Step 36"
    Write-Host "4. Show server start placeholder only"
    Write-Host "5. Generate Phase 23 Step 36 Implementation Patch Review Planning Evidence Index Packet"
    Write-Host "6. Exit"
