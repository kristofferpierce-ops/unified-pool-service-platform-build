...
$StepNumber = 17
$StepTitle = "Phase 30 Step 17 - Phase 20 Network Transport Implementation Active Live Program Development Guardrail Verification Approval Readiness Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Active Live Program Development Guardrail Verification Approval Readiness Packet"
$ExpectedBranch = "phase30-step17-active-live-program-development-guardrail-verification-approval-readiness"
$PriorStep = "Phase 30 Step 16 - Phase 20 Network Transport Implementation Active Live Program Development Guardrail Verification Approval Boundary Packet"
$Slug = "implementation_active_live_program_development_guardrail_verification_approval_readiness"
...
$StepFiles = @(
    "scripts/phase30_step17_active_live_program_development_guardrail_verification_approval_readiness_packet.ps1",
    "ui/pages/993_Phase30_Step17_Implementation_Active_Live_Program_Development_Guardrail_Verification_Approval_Readiness_Packet.py",
    "docs/PHASE30_STEP17_ACTIVE_LIVE_PROGRAM_DEVELOPMENT_GUARDRAIL_VERIFICATION_APPROVAL_READINESS_PACKET.md",
    "tests/test_phase30_step17_active_live_program_development_guardrail_verification_approval_readiness_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase30_boundary = "implementation_active_live_program_development_guardrail_verification_approval_readiness_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 30 Step 17 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 30 Step 17 Phase 20 Network Transport Implementation Active Live Program Development Guardrail Verification Approval Readiness Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase30_step17_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase30_step17_{0}_packet.json" -f $Slug)
