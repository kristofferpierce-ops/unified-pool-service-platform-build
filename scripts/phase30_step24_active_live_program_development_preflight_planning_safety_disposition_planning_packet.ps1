...
$StepNumber = 24
$StepTitle = "Phase 30 Step 24 - Phase 20 Network Transport Implementation Active Live Program Development Preflight Planning Safety Disposition Planning Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Active Live Program Development Preflight Planning Safety Disposition Planning Packet"
$ExpectedBranch = "phase30-step24-active-live-program-development-preflight-planning-safety-disposition-planning"
$PriorStep = "Phase 30 Step 23 - Phase 20 Network Transport Implementation Active Live Program Development Preflight Planning Evidence Gap Review Packet"
$Slug = "implementation_active_live_program_development_preflight_planning_safety_disposition_planning"
$ContextKey = "implementation_active_live_program_development_preflight_planning"
$PhaseContext = "implementation_active_live_program_development_preflight_planning_planning_only"

if (-not $RepoRoot) { $RepoRoot = (Resolve-Path (Join-Path $PSScriptRoot ".." )).Path }

$StepFiles = @(
    "scripts/phase30_step24_active_live_program_development_preflight_planning_safety_disposition_planning_packet.ps1",
    "ui/pages/1000_Phase30_Step24_Implementation_Active_Live_Program_Development_Preflight_Planning_Safety_Disposition_Planning_Packet.py",
    "docs/PHASE30_STEP24_ACTIVE_LIVE_PROGRAM_DEVELOPMENT_PREFLIGHT_PLANNING_SAFETY_DISPOSITION_PLANNING_PACKET.md",
    "tests/test_phase30_step24_active_live_program_development_preflight_planning_safety_disposition_planning_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase30_boundary = "implementation_active_live_program_development_preflight_planning_safety_disposition_planning_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 30 Step 24 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 30 Step 24 Phase 20 Network Transport Implementation Active Live Program Development Preflight Planning Safety Disposition Planning Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase30_step24_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase30_step24_{0}_packet.json" -f $Slug)
