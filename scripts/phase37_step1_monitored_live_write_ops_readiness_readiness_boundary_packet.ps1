...
$PhaseNumber = 37
$StepNumber = 1
$StepTitle = "Phase 37 Step 1 - Phase 20 Network Transport Implementation Trusted Production Monitored Live Write Operations Readiness Boundary Packet"
$PriorCompletedStep = "Phase 36 Step 120 - Phase 20 Network Transport Implementation Phase 36 Final No-Write Closeout Hold Packet"
...
$StepFiles = @(
    "scripts/phase37_step1_monitored_live_write_ops_readiness_readiness_boundary_packet.ps1",
    "ui/pages/1817_Phase37_Step1_Monitored_Live_Write_Ops_Readiness_Boundary.py",
    "docs/PHASE37_STEP1_MONITORED_LIVE_WRITE_OPS_READINESS_READINESS_BOUNDARY_PACKET.md",
    "tests/test_phase37_step1_monitored_live_write_ops_readiness_readiness_boundary_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase37_boundary = "trusted_production_monitored_live_write_operations_readiness_boundary_opened_by_packet"
...
function Assert-StepFiles {
    foreach ($Rel in $StepFiles) {
        ...
        if (-not (Test-Path $Path)) {
            throw ("Missing expected Phase 37 Step 1 file: {0}" -f $Rel)
        }
    }
}
...
function Invoke-Apply {
    Assert-StepFiles
    Write-Host "APPLY PASS: Phase 37 Step 1 planning-only packet verified without live-write activation, live-write apply, live-user access, bridge POST, sockets, DB mutation, server launch, or Phase 38 boundary creation."
}
...
function Invoke-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 37 Step 1 safety markers confirm planning-only/no-write/no-live-write-activation/no-live-write-apply/no-runtime/no-phase38 posture."
}
