...
$StepNumber = 6
$StepTitle = "Phase 35 Step 6 - Phase 20 Network Transport Implementation Trusted Production Limited Live Write Pilot Readiness Approval Boundary Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Trusted Production Limited Live Write Pilot Readiness Approval Boundary Packet"
$ExpectedBranch = "phase35-step6-limited-live-write-pilot-readiness-approval-boundary"
$PriorStep = "Phase 35 Step 5 - Phase 20 Network Transport Implementation Trusted Production Limited Live Write Pilot Readiness Safety Disposition Review Packet"
$Slug = "trusted_production_limited_live_write_pilot_readiness_approval_boundary"
...
$StepFiles = @(
    "scripts/phase35_step6_limited_live_write_pilot_readiness_approval_boundary_packet.ps1",
    "ui/pages/1582_Phase35_Step6_Limited_Live_Write_Pilot_Readiness_Approval_Boundary.py",
    "docs/PHASE35_STEP6_LIMITED_LIVE_WRITE_PILOT_READINESS_APPROVAL_BOUNDARY_PACKET.md",
    "tests/test_phase35_step6_limited_live_write_pilot_readiness_approval_boundary_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase35_boundary = "trusted_production_limited_live_write_pilot_readiness_approval_boundary_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 35 Step 6 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 35 Step 6 Phase 20 Network Transport Implementation Trusted Production Limited Live Write Pilot Readiness Approval Boundary Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase35_step6_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase35_step6_{0}_packet.json" -f $Slug)
