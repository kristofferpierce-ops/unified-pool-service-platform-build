...
$PhaseNumber = 22
$StepNumber = 69
$StepName = "Phase 22 Step 69 - Phase 20 Network Transport Planning Final Cross-Repo Handoff Closeout Index Packet"
$PriorStepName = "Phase 22 Step 68 - Phase 20 Network Transport Planning Cross-Repo Handoff Evidence Gap Disposition Approval Readiness Packet"
$ExpectedBranch = "phase22-step69-final-handoff-closeout-index"

$StepFiles = @(
    "scripts/phase22_step69_cross_repo_handoff_closeout_index_packet.ps1",
    "ui/pages/175_Phase20_Network_Transport_Planning_Final_Cross_Repo_Handoff_Closeout_Index_Packet.py",
    "docs/PHASE22_STEP69_FINAL_CROSS_REPO_HANDOFF_CLOSEOUT_INDEX_PACKET.md",
    "tests/test_phase22_step69_cross_repo_handoff_closeout_index_packet.py"
)
...
function Get-SafetyPosture {
    return [ordered]@{
        ...
        handoff_closeout_index_record_creation = $false
        ...
        handoff_closeout_index_write = $false
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 22 Step 69 files copied or already present."
}
...
function Test-Smoke {
    ...
    if ($safety["handoff_closeout_index_write"]) { throw "SMOKE TEST FAIL: handoff closeout index write must remain disabled" }
...
function New-Packet {
    ...
    $packetPath = Join-SafePath -Root $backupDir -RelativePath ("phase22_step69_cross_repo_handoff_closeout_index_packet_{0}.json" -f $stamp)
    ...
    $packet = [ordered]@{
        ...
        planning_only = $true
        final_cross_repo_handoff_closeout_index_mode = "reference_only"
        ...
        final_cross_repo_handoff_closeout_index_scope = @(
            "Review reference-only closeout criteria prepared after Step 68",
        ...
        final_cross_repo_handoff_closeout_index_buckets = @(
    ...
    Write-Host "PASS: final_cross_repo_handoff_closeout_index_mode=reference_only"
    Write-Host "PASS: handoff_closeout_index_write=false"
    ...
    Write-Host "CHECK: prior_step=Phase 22 Step 68"
    Write-Host "CHECK: phase23_start=not_started"
    Write-Host "CHECK: implementation_phase_start=not_started"
    Write-Host "CHECK: final_cross_repo_handoff_closeout_index_mode=reference_only"
...
function Show-Menu {
    while ($true) {
        Write-Host ""
        Write-Host "Phase 22 Step 69 menu"
        Write-Host "1. Show status / verify paths"
        Write-Host "2. Apply Phase 22 Step 69 Phase 20 Network Transport Planning Final Cross-Repo Handoff Closeout Index Packet files"
        Write-Host "3. Smoke test Phase 22 Step 69"
        Write-Host "4. Show server start placeholder only"
        Write-Host "5. Generate Phase 22 Step 69 final cross-repo handoff closeout index packet"
        Write-Host "6. Exit"
