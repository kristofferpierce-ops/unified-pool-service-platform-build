...
$StepNumber = 84
$StepTitle = "Phase 30 Step 84 - Phase 20 Network Transport Implementation Active Live Program Development Release Gate Verification Planning Safety Disposition Planning Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Active Live Program Development Release Gate Verification Planning Safety Disposition Planning Packet"
$ExpectedBranch = "phase30-step84-active-live-program-development-release-gate-verification-planning-safety-disposition-planning"
$PriorStep = "Phase 30 Step 83 - Phase 20 Network Transport Implementation Active Live Program Development Release Gate Verification Planning Evidence Gap Review Packet"
$Slug = "implementation_active_live_program_development_release_gate_verification_planning_safety_disposition_planning"
$ContextKey = "implementation_active_live_program_development_release_gate_verification_planning"
$PhaseContext = "implementation_active_live_program_development_release_gate_verification_planning_planning_only"

if (-not $RepoRoot) { $RepoRoot = (Resolve-Path (Join-Path $PSScriptRoot ".." )).Path }

$StepFiles = @(
    "scripts/phase30_step84_active_live_program_development_release_gate_verification_planning_safety_disposition_planning_packet.ps1",
    "ui/pages/1060_Phase30_Step84_Implementation_Active_Live_Program_Development_Release_Gate_Verification_Planning_Safety_Disposition_Planning_Packet.py",
    "docs/PHASE30_STEP84_ACTIVE_LIVE_PROGRAM_DEVELOPMENT_RELEASE_GATE_VERIFICATION_PLANNING_SAFETY_DISPOSITION_PLANNING_PACKET.md",
    "tests/test_phase30_step84_active_live_program_development_release_gate_verification_planning_safety_disposition_planning_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase30_boundary = "implementation_active_live_program_development_release_gate_verification_planning_safety_disposition_planning_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 30 Step 84 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 30 Step 84 Phase 20 Network Transport Implementation Active Live Program Development Release Gate Verification Planning Safety Disposition Planning Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase30_step84_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase30_step84_{0}_packet.json" -f $Slug)
