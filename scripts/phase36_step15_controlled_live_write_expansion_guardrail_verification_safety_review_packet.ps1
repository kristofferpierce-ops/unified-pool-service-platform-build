...
$PhaseNumber = 36
$StepNumber = 15
$StepTitle = "Phase 36 Step 15 - Phase 20 Network Transport Implementation Trusted Production Controlled Live Write Expansion Guardrail Verification Safety Disposition Review Packet"
$ExpectedBranch = "phase36-step15-guardrail-verification-safety-review"
$PriorCompletedStep = "Phase 36 Step 14 - Phase 20 Network Transport Implementation Trusted Production Controlled Live Write Expansion Guardrail Verification Safety Disposition Planning Packet"
...
$StepFiles = @(
    "scripts/phase36_step15_controlled_live_write_expansion_guardrail_verification_safety_review_packet.ps1",
    "ui/pages/1711_Phase36_Step15_Guardrail_Verification_Safety_Review.py",
    "docs/PHASE36_STEP15_CONTROLLED_LIVE_WRITE_EXPANSION_GUARDRAIL_VERIFICATION_SAFETY_REVIEW_PACKET.md",
    "tests/test_phase36_step15_controlled_live_write_expansion_guardrail_verification_safety_review_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase36_boundary = "trusted_production_controlled_live_write_expansion_guardrail_verification_safety_review_opened_by_packet"
...
function Assert-StepFiles {
    foreach ($Rel in $StepFiles) {
        ...
        if (-not (Test-Path $Path)) {
            throw ("Missing expected Phase 36 Step 15 file: {0}" -f $Rel)
        }
    }
}
...
function Invoke-Apply {
    Assert-StepFiles
    Write-Host "APPLY PASS: Phase 36 Step 15 planning-only packet verified without live-write activation, live-write apply, bridge POST, sockets, DB mutation, server launch, or Phase 37 boundary creation."
}
...
function Invoke-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 36 Step 15 safety markers confirm planning-only/no-write/no-live-write-activation/no-live-write-apply/no-runtime/no-phase37 posture."
}
