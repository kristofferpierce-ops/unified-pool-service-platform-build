...
$StepNumber = 89
$StepTitle = "Phase 31 Step 89 - Phase 20 Network Transport Implementation Controlled Active Program Operation Release Gate Verification Planning Closeout Index Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Controlled Active Program Operation Release Gate Verification Planning Closeout Index Packet"
$ExpectedBranch = "phase31-step89-controlled-active-program-operation-release-gate-verification-planning-closeout-index"
$PriorStep = "Phase 31 Step 88 - Phase 20 Network Transport Implementation Controlled Active Program Operation Release Gate Verification Planning Operator Hold Point Packet"
$Slug = "implementation_controlled_active_program_operation_release_gate_verification_planning_closeout_index"
...
$StepFiles = @(
    "scripts/phase31_step89_controlled_active_program_operation_release_gate_verification_planning_closeout_index_packet.ps1",
    "ui/pages/1185_Phase31_Step89_Implementation_Controlled_Active_Program_Operation_Release_Gate_Verification_Planning_Closeout_Index_Packet.py",
    "docs/PHASE31_STEP89_CONTROLLED_ACTIVE_PROGRAM_OPERATION_RELEASE_GATE_VERIFICATION_PLANNING_CLOSEOUT_INDEX_PACKET.md",
    "tests/test_phase31_step89_controlled_active_program_operation_release_gate_verification_planning_closeout_index_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase31_boundary = "implementation_controlled_active_program_operation_release_gate_verification_planning_closeout_index_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 31 Step 89 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 31 Step 89 Phase 20 Network Transport Implementation Controlled Active Program Operation Release Gate Verification Planning Closeout Index Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase31_step89_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase31_step89_{0}_packet.json" -f $Slug)
