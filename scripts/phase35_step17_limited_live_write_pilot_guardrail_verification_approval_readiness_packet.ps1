...
$StepNumber = 17
$StepTitle = "Phase 35 Step 17 - Phase 20 Network Transport Implementation Trusted Production Limited Live Write Pilot Guardrail Verification Approval Readiness Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Trusted Production Limited Live Write Pilot Guardrail Verification Approval Readiness Packet"
$ExpectedBranch = "phase35-step17-limited-live-write-pilot-guardrail-verification-approval-readiness"
$PriorStep = "Phase 35 Step 16 - Phase 20 Network Transport Implementation Trusted Production Limited Live Write Pilot Guardrail Verification Approval Boundary Packet"
$Slug = "trusted_production_limited_live_write_pilot_guardrail_verification_approval_readiness"
...
$StepFiles = @(
    "scripts/phase35_step17_limited_live_write_pilot_guardrail_verification_approval_readiness_packet.ps1",
    "ui/pages/1593_Phase35_Step17_Live_Write_Pilot_Guardrail_Approval_Readiness.py",
    "docs/PHASE35_STEP17_LIMITED_LIVE_WRITE_PILOT_GUARDRAIL_VERIFICATION_APPROVAL_READINESS_PACKET.md",
    "tests/test_phase35_step17_limited_live_write_pilot_guardrail_verification_approval_readiness_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase35_boundary = "trusted_production_limited_live_write_pilot_guardrail_verification_approval_readiness_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 35 Step 17 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 35 Step 17 Phase 20 Network Transport Implementation Trusted Production Limited Live Write Pilot Guardrail Verification Approval Readiness Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase35_step17_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase35_step17_{0}_packet.json" -f $Slug)
