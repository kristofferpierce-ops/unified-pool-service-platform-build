...
$PhaseNumber = 24
$StepNumber = 4
$StepName = "Phase 24 Step 4 - Phase 20 Network Transport Implementation Controlled Activation Safety Disposition Planning Packet"
$PriorStepName = "Phase 24 Step 3 - Phase 20 Network Transport Implementation Controlled Activation Evidence Gap Review Packet"
$ExpectedBranch = "phase24-step4-controlled-activation-safety-disposition-planning"
$ModeKey = "implementation_controlled_activation_safety_disposition_planning_mode"
$WriteKey = "implementation_controlled_activation_safety_disposition_planning_write"
$RecordKey = "implementation_controlled_activation_safety_disposition_planning_record_creation"
$PhaseContext = "implementation_controlled_activation_safety_disposition_planning_planning_only"
$PacketSlug = "phase24_step4_controlled_activation_safety_disposition_planning_packet"

$StepFiles = @(
    "scripts/phase24_step4_controlled_activation_safety_disposition_planning_packet.ps1",
    "ui/pages/260_Phase24_Step4_Implementation_Controlled_Activation_Safety_Disposition_Planning_Packet.py",
    "docs/PHASE24_STEP4_CONTROLLED_ACTIVATION_SAFETY_DISPOSITION_PLANNING_PACKET.md",
    "tests/test_phase24_step4_controlled_activation_safety_disposition_planning_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase24_boundary = "implementation_controlled_activation_safety_disposition_planning_opened_by_packet"
    ...
    implementation_controlled_activation_safety_disposition_planning_mode = "reference_only"
    implementation_controlled_activation_safety_disposition_planning_write = $false
    implementation_controlled_activation_safety_disposition_planning_record_creation = $false
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 24 Step 4 files copied or already present."
}
...
function Invoke-SmokeTest {
    ...
    $scriptText = Get-Content -LiteralPath (Join-SafePath -Root $Repo -RelativePath "scripts/phase24_step4_controlled_activation_safety_disposition_planning_packet.ps1") -Raw -Encoding UTF8
    ...
    Write-Host "SMOKE TEST PASS: Phase 24 Step 4 Implementation Controlled Activation Safety Disposition Planning Packet is present and planning-only."
}
...
function Write-Packet {
    ...
    Write-Host "PASS: phase24_boundary=implementation_controlled_activation_safety_disposition_planning_opened_by_packet"
    ...
    Write-Host "PASS: implementation_controlled_activation_safety_disposition_planning_mode=reference_only"
    Write-Host "PASS: implementation_controlled_activation_safety_disposition_planning_write=false"
    Write-Host "PASS: implementation_controlled_activation_safety_disposition_planning_record_creation=false"
...
while ($true) {
    Write-Host ""
    Write-Host "Phase 24 Step 4 menu"
    Write-Host "1. Show status / verify paths"
    Write-Host "2. Apply Phase 24 Step 4 Implementation Controlled Activation Safety Disposition Planning Packet files"
    Write-Host "3. Smoke test Phase 24 Step 4"
    Write-Host "4. Show server start placeholder only"
    Write-Host "5. Generate Phase 24 Step 4 Implementation Controlled Activation Safety Disposition Planning Packet"
    Write-Host "6. Exit"
