...
$StepNumber = 17
$StepName = "Phase 22 Step 17 - Phase 20 Network Transport Planning Implementation Preflight Packet"
$PacketSlug = "phase22_phase20_network_transport_planning_implementation_preflight_packet"
$BranchName = "phase22-step17-phase20-network-transport-planning-implementation-preflight-packet"
$ExpectedPriorStep = "Phase 22 Step 16 - Phase 20 Network Transport Planning Implementation Readiness Packet"
...
$StepFiles = @(
    ...
    "tests\test_phase22_phase20_network_transport_planning_implementation_preflight_packet.py"
)
...
function Copy-Step17Files {
    ...
    Write-Host "APPLY PASS: Phase 22 Step 17 files copied or already present."
}

function Test-Step17Preflight {
    $Root = Resolve-Phase22RepoRoot
    $Missing = Test-StepFileSetPresent -Root $Root
    if ($Missing.Count -gt 0) {
        throw ("Missing Step 17 files: " + ($Missing -join ", "))
    }
    ...
    Write-Host "SMOKE TEST PASS: Phase 22 Step 17 Phase 20 Network Transport Planning Implementation Preflight Packet is present and planning-only."
}
...
function Write-ImplementationPreflightPacket {
    $Root = Resolve-Phase22RepoRoot
    ...
    $Packet = [ordered]@{
        ...
        purpose = "Planning-only implementation preflight packet. It inventories readiness boundaries without starting implementation, approving writes, or creating closure records."
...
function Show-Menu {
    Write-Host ""
    Write-Host "Phase 22 Step 17 menu"
    Write-Host "1. Show status / verify paths"
    Write-Host "2. Apply Phase 22 Step 17 Phase 20 Network Transport Planning Implementation Preflight Packet files"
    Write-Host "3. Smoke test Phase 22 Step 17"
    Write-Host "4. Show server start placeholder only"
    Write-Host "5. Generate Phase 20 network transport planning implementation preflight packet"
    Write-Host "6. Exit"
}

function Invoke-Step17Action {
    param([string]$SelectedAction)
    switch ($SelectedAction) {
        "status" { Show-StepStatus }
        "apply" { Copy-Step17Files }
        "smoke" { Test-Step17Preflight }
        "server-placeholder" { Show-ServerStartPlaceholder }
        "packet" { Write-ImplementationPreflightPacket }
        "all" {
            Show-StepStatus
            Copy-Step17Files
            Test-Step17Preflight
            Write-ImplementationPreflightPacket
        }
...
if (-not [string]::IsNullOrWhiteSpace($CleanAction)) {
    Invoke-Step17Action -SelectedAction $CleanAction
    return
}
...
while ($true) {
    ...
    switch ($Choice) {
        "1" { Show-StepStatus }
        "2" { Copy-Step17Files }
        "3" { Test-Step17Preflight }
        "4" { Show-ServerStartPlaceholder }
        "5" { Write-ImplementationPreflightPacket }
        "6" { break }
