...
$StepNumber = 91
$StepTitle = "Phase 31 Step 91 - Phase 20 Network Transport Implementation Controlled Active Program Operation Final Release Hold Planning Boundary Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Controlled Active Program Operation Final Release Hold Planning Boundary Packet"
$ExpectedBranch = "phase31-step91-controlled-active-program-operation-final-release-hold-planning-boundary"
$PriorStep = "Phase 31 Step 90 - Phase 20 Network Transport Implementation Controlled Active Program Operation Release Gate Verification Planning Final Boundary Confirmation Packet"
$Slug = "implementation_controlled_active_program_operation_final_release_hold_planning_boundary"
...
$StepFiles = @(
    "scripts/phase31_step91_controlled_active_program_operation_final_release_hold_planning_boundary_packet.ps1",
    "ui/pages/1187_Phase31_Step91_Implementation_Controlled_Active_Program_Operation_Final_Release_Hold_Planning_Boundary_Packet.py",
    "docs/PHASE31_STEP91_CONTROLLED_ACTIVE_PROGRAM_OPERATION_FINAL_RELEASE_HOLD_PLANNING_BOUNDARY_PACKET.md",
    "tests/test_phase31_step91_controlled_active_program_operation_final_release_hold_planning_boundary_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase31_boundary = "implementation_controlled_active_program_operation_final_release_hold_planning_boundary_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 31 Step 91 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 31 Step 91 Phase 20 Network Transport Implementation Controlled Active Program Operation Final Release Hold Planning Boundary Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase31_step91_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase31_step91_{0}_packet.json" -f $Slug)
