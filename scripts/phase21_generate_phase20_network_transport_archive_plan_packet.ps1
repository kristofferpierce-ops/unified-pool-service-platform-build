param(
    ...
    [string]$ArchiveReadinessPacketDir = "",
...
function Classify-RepoPath {
    ...
    if ($CurrentStepFiles -contains $Path) {
        return [ordered]@{
            ...
            should_stage = $true
            reason = "Current Phase 21 Step 10 file."
        }
    }
...
if ([string]::IsNullOrWhiteSpace($ArchiveReadinessPacketDir)) {
    $ArchiveReadinessPacketDir = Get-LatestArtifactDir -FolderFilter "phase21_phase20_network_transport_archive_readiness_packet_*" -JsonName "phase21_phase20_network_transport_archive_readiness_packet.json" -Required $true
}
...
$archiveReadinessPath = Join-Path $ArchiveReadinessPacketDir "phase21_phase20_network_transport_archive_readiness_packet.json"
...
if (!(Test-Path -LiteralPath $archiveReadinessPath)) {
    throw "Archive readiness packet JSON not found: $archiveReadinessPath"
}

$archiveReadiness = Read-JsonFile $archiveReadinessPath
...
$archivePlanSections = New-Object System.Collections.ArrayList
$archivePlanEntries = New-Object System.Collections.ArrayList
...
Add-ChecklistItem $checklist "artifact" "Archive readiness packet exists" $true $archiveReadinessPath
...
Add-ChecklistItem $checklist "archive_readiness" "Archive readiness packet is archive-plan-safe" ($archiveReadiness.safety.phase20_network_transport_archive_readiness_packet_only -eq $true) "phase20_network_transport_archive_readiness_packet_only=$($archiveReadiness.safety.phase20_network_transport_archive_readiness_packet_only)"
Add-ChecklistItem $checklist "archive_readiness" "Archive readiness created no closure record" ($archiveReadiness.safety.design_closure_record_created -eq $false) "design_closure_record_created=$($archiveReadiness.safety.design_closure_record_created)"
Add-ChecklistItem $checklist "archive_readiness" "Archive readiness recorded no final approval" ($archiveReadiness.safety.final_approval_recorded -eq $false) "final_approval_recorded=$($archiveReadiness.safety.final_approval_recorded)"
Add-ChecklistItem $checklist "archive_readiness" "Archive readiness started no implementation phase" ($archiveReadiness.safety.implementation_phase_started -eq $false) "implementation_phase_started=$($archiveReadiness.safety.implementation_phase_started)"
Add-ChecklistItem $checklist "archive_readiness" "Archive readiness has no execution implementation" ($archiveReadiness.safety.execution_implementation_created -eq $false) "execution_implementation_created=$($archiveReadiness.safety.execution_implementation_created)"
Add-ChecklistItem $checklist "archive_readiness" "Archive readiness has no network transport" ($archiveReadiness.safety.network_transport_implemented -eq $false) "network_transport_implemented=$($archiveReadiness.safety.network_transport_implemented)"
Add-ChecklistItem $checklist "archive_readiness" "Archive readiness has no socket opened" ($archiveReadiness.safety.network_socket_opened -eq $false) "network_socket_opened=$($archiveReadiness.safety.network_socket_opened)"
Add-ChecklistItem $checklist "archive_readiness" "Archive readiness has no bridge POST" ($archiveReadiness.safety.bridge_post_called -eq $false -and $archiveReadiness.safety.bridge_post_call_implemented -eq $false) "bridge_post_called=$($archiveReadiness.safety.bridge_post_called); bridge_post_call_implemented=$($archiveReadiness.safety.bridge_post_call_implemented)"
Add-ChecklistItem $checklist "archive_readiness" "Archive readiness has no platform DB mutation" ($archiveReadiness.safety.platform_db_mutation_performed -eq $false) "platform_db_mutation_performed=$($archiveReadiness.safety.platform_db_mutation_performed)"
Add-ChecklistItem $checklist "archive_readiness" "Archive readiness has no bridge mutation" ($archiveReadiness.safety.bridge_mutation_performed -eq $false) "bridge_mutation_performed=$($archiveReadiness.safety.bridge_mutation_performed)"
Add-ChecklistItem $checklist "archive_readiness" "Archive readiness has no LACRM call" ($archiveReadiness.safety.lacrm_call_performed -eq $false) "lacrm_call_performed=$($archiveReadiness.safety.lacrm_call_performed)"
Add-ChecklistItem $checklist "archive_readiness" "Archive readiness has no blockers" ([int]$archiveReadiness.archive_readiness_packet.blocker_count -eq 0) "blocker_count=$($archiveReadiness.archive_readiness_packet.blocker_count)"
...
# Archive plan uses git status --short against the platform repo.
...
$artifactFamilies = @($archiveReadiness.artifact_families | Sort-Object @{Expression = {[int]$_.older_generated_folder_count}; Descending = $true}, @{Expression = {[string]$_.folder_filter}; Descending = $false})
...
[void]$archivePlanSections.Add([ordered]@{
    section = "Manual archive order"
    status = if ($archiveCandidateFamilyCount -gt 0) { "review" } else { "clean" }
    summary = "Families with older generated folders should be reviewed in order, highest older-folder count first."
})
[void]$archivePlanSections.Add([ordered]@{
...
[void]$archivePlanSections.Add([ordered]@{
    ...
    summary = "Any non-Step-10 paths outside the do-not-stage list require review before staging."
})
[void]$archivePlanSections.Add([ordered]@{
...
foreach ($file in $currentStepFiles) {
    [void]$archivePlanEntries.Add([ordered]@{
        category = "stage_now"
        action = "stage"
        item = $file
        evidence = "Current Phase 21 Step 10 file."
        required = "yes"
        archive_order = 0
    })
}
...
$nextCommitCommands = @(
    ...
    "git status --short",
    "git add scripts/phase21_generate_phase20_network_transport_archive_plan_packet.ps1",
    "git add ui/pages/102_Phase20_Network_Transport_Archive_Plan_Packet.py",
    "git add docs/PHASE21_STEP10_PHASE20_NETWORK_TRANSPORT_ARCHIVE_PLAN_PACKET.md",
    "git add tests/test_phase21_phase20_network_transport_archive_plan_packet.py",
    "git diff --cached --name-only",
    'git commit -m "Phase 21 Step 10 phase20 network transport archive plan packet"',
    "git push -u origin phase21-step10-phase20-network-transport-archive-plan-packet"
)

Add-ChecklistItem $checklist "archive_plan" "Current branch detected" (-not [string]::IsNullOrWhiteSpace($currentBranch)) "current_branch=$currentBranch"
Add-ChecklistItem $checklist "archive_plan" "At least one archive-plan section exists" (@($archivePlanSections).Count -gt 0) "archive_plan_sections=$(@($archivePlanSections).Count)"
Add-ChecklistItem $checklist "archive_plan" "Exactly four stage-now entries exist" (@($archivePlanEntries | Where-Object { $_.category -eq "stage_now" }).Count -eq 4) "stage_now_entries=$(@($archivePlanEntries | Where-Object { $_.category -eq "stage_now" }).Count)"
Add-ChecklistItem $checklist "archive_plan" "At least one do-not-stage entry exists" (@($archivePlanEntries | Where-Object { $_.category -eq "do_not_stage" }).Count -gt 0) "do_not_stage_entries=$(@($archivePlanEntries | Where-Object { $_.category -eq "do_not_stage" }).Count)"
Add-ChecklistItem $checklist "archive_plan" "Artifact families were evaluated" (@($artifactFamilies).Count -gt 0) "artifact_families=$(@($artifactFamilies).Count)" "recommended"
Add-ChecklistItem $checklist "archive_plan" "Commit commands exist" (@($nextCommitCommands).Count -gt 0) "commit_commands=$(@($nextCommitCommands).Count)"
Add-ChecklistItem $checklist "archive_plan" "No forbidden paths are currently staged" ($stagedForbiddenCount -eq 0) "staged_forbidden_count=$stagedForbiddenCount"
Add-ChecklistItem $checklist "archive_plan" "No review-other paths are currently staged" ($stagedReviewOtherCount -eq 0) "staged_review_other_count=$stagedReviewOtherCount" "recommended"
Add-ChecklistItem $checklist "archive_plan" "Workspace review-other files count is zero or acceptable" ($reviewOtherCount -eq 0) "review_other_count=$reviewOtherCount" "recommended"

$archiveReadinessStatus = [string]$archiveReadiness.archive_readiness_packet.status
if ($archiveReadinessStatus -eq "phase20_archive_readiness_packet_blocked") {
    Add-Issue $issues "blocker" "archive_readiness_packet_blocked" "Archive readiness packet is blocked." "archive_readiness"
} elseif ($archiveReadinessStatus -eq "phase20_archive_readiness_packet_review_required_no_write") {
    Add-Issue $issues "review" "archive_readiness_packet_review_required" "Archive readiness packet has review items." "archive_readiness"
}

if ([string]::IsNullOrWhiteSpace($currentBranch)) {
    Add-Issue $issues "blocker" "current_branch_missing" "Current branch was not detected for archive plan." "workspace"
}
...
[void]$sourceArtifacts.Add([ordered]@{ key="archive_readiness_packet"; json_path=$archiveReadinessPath; sha256=Get-FileSha256 $archiveReadinessPath })
...
$packetStatus = if ($blockerCount -eq 0 -and $reviewCount -eq 0) {
    "phase20_archive_plan_packet_ready_no_write"
} elseif ($blockerCount -eq 0) {
    "phase20_archive_plan_packet_review_required_no_write"
} else {
    "phase20_archive_plan_packet_blocked"
}

if ([string]::IsNullOrWhiteSpace($OutputDir)) {
    $stamp = Get-Date -Format "yyyyMMdd-HHmmss"
    $OutputDir = Join-Path $BackupDir "phase21_phase20_network_transport_archive_plan_packet_$stamp"
}
...
$report = [ordered]@{
    ...
    phase = "Phase 21 Step 10"
    purpose = "Phase 20 bridge routing network transport archive plan packet"
    workspace = $Workspace
    platform_api = $PlatformApi
    bridge_url = $BridgeUrl
    safety = [ordered]@{
        phase20_network_transport_archive_plan_packet_only = $true
    ...
    archive_plan_packet = [ordered]@{
        status = $packetStatus
        blocker_count = $blockerCount
        review_count = $reviewCount
        current_branch = $currentBranch
        section_count = @($archivePlanSections).Count
        entry_count = @($archivePlanEntries).Count
        ...
        packet_label = "phase21_phase20_network_transport_archive_plan_no_write"
        reason = if ($blockerCount -eq 0) {
            "Phase 21 Step 10 created a no-write archive plan packet for the Phase 20 network transport cleanup chain. It does not authorize approvals, implementation, transport, sockets, bridge POST, response capture, mutations, LACRM calls, or live writes."
        } else {
            "Phase 21 Step 10 found blockers in the Phase 20 archive plan packet. Resolve blockers before further cleanup work."
        }
    }
    archive_sections = @($archivePlanSections)
    archive_entries = @($archivePlanEntries)
    ...
    counts = [ordered]@{
        sections = @($archivePlanSections).Count
        entries = @($archivePlanEntries).Count
    ...
    next_recommended_actions = @(
        "Review this archive plan packet before any manual backup-folder archiving.",
        "Keep the latest packet folder in each family and only archive older generated folders after review.",
        "Use the stage-now entries as the only files to add for the Step 10 commit.",
        "Carry the do-not-stage entries forward as the hard commit boundary.",
        "Do not add requests, httpx, socket use, bridge POST calls, or routing write endpoints from Phase 21 Step 10.",
        "Do not call interface execution methods from Phase 21 Step 10.",
        "Do not record final approvals from Phase 21 Step 10.",
        "Do not start an implementation phase from Phase 21 Step 10.",
        "Keep bridge routing controls as the operational write path until explicit cutover."
    )
}

$jsonPath = Join-Path $OutputDir "phase21_phase20_network_transport_archive_plan_packet.json"
$sectionsCsvPath = Join-Path $OutputDir "phase21_phase20_network_transport_archive_plan_packet_sections.csv"
$entriesCsvPath = Join-Path $OutputDir "phase21_phase20_network_transport_archive_plan_packet_entries.csv"
$familiesCsvPath = Join-Path $OutputDir "phase21_phase20_network_transport_archive_plan_packet_artifact_families.csv"
$gitStatusCsvPath = Join-Path $OutputDir "phase21_phase20_network_transport_archive_plan_packet_git_status.csv"
$stagedCsvPath = Join-Path $OutputDir "phase21_phase20_network_transport_archive_plan_packet_staged.csv"
$sourceArtifactsCsvPath = Join-Path $OutputDir "phase21_phase20_network_transport_archive_plan_packet_source_artifacts.csv"
$checklistCsvPath = Join-Path $OutputDir "phase21_phase20_network_transport_archive_plan_packet_checklist.csv"
$issuesCsvPath = Join-Path $OutputDir "phase21_phase20_network_transport_archive_plan_packet_issues.csv"
$mdPath = Join-Path $OutputDir "phase21_phase20_network_transport_archive_plan_packet.md"

$report | ConvertTo-Json -Depth 100 | Set-Content -LiteralPath $jsonPath -Encoding UTF8
$archivePlanSections | Export-Csv -LiteralPath $sectionsCsvPath -NoTypeInformation -Encoding UTF8
$archivePlanEntries | Export-Csv -LiteralPath $entriesCsvPath -NoTypeInformation -Encoding UTF8
...
$sectionText = (@($archivePlanSections) | ForEach-Object { "- $($_.section): $($_.status); $($_.summary)" }) -join "`n"
$entryText = (@($archivePlanEntries) | ForEach-Object { "- [$($_.category)] $($_.action) $($_.item): $($_.evidence)" }) -join "`n"
...
Source archive readiness packet:

``````
$archiveReadinessPath
``````

## Safety

- Phase 20 network transport archive plan packet only: true
- Archive plan only: true
...
if ($blockerCount -eq 0) {
    Write-Host "PASS | phase20_network_transport_archive_plan_packet=$OutputDir | status=$packetStatus | bridge_post_called=False | execution_implementation_created=False | implementation_phase_started=False"
} else {
    Write-Host "CHECK | phase20_network_transport_archive_plan_packet=$OutputDir | status=$packetStatus | blockers=$blockerCount | bridge_post_called=False | execution_implementation_created=False | implementation_phase_started=False"
}

Write-Host ""
Write-Host "Archive plan packet files:"
