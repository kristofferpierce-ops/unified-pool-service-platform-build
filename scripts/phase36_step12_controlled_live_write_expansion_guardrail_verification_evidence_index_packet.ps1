...
$PhaseNumber = 36
$StepNumber = 12
$StepTitle = "Phase 36 Step 12 - Phase 20 Network Transport Implementation Trusted Production Controlled Live Write Expansion Guardrail Verification Evidence Index Packet"
$ExpectedBranch = "phase36-step12-guardrail-verification-evidence-index"
$PriorCompletedStep = "Phase 36 Step 11 - Phase 20 Network Transport Implementation Trusted Production Controlled Live Write Expansion Guardrail Verification Boundary Packet"
...
$StepFiles = @(
    "scripts/phase36_step12_controlled_live_write_expansion_guardrail_verification_evidence_index_packet.ps1",
    "ui/pages/1708_Phase36_Step12_Guardrail_Verification_Evidence_Index.py",
    "docs/PHASE36_STEP12_CONTROLLED_LIVE_WRITE_EXPANSION_GUARDRAIL_VERIFICATION_EVIDENCE_INDEX_PACKET.md",
    "tests/test_phase36_step12_controlled_live_write_expansion_guardrail_verification_evidence_index_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase36_boundary = "trusted_production_controlled_live_write_expansion_guardrail_verification_evidence_index_opened_by_packet"
...
function Assert-StepFiles {
    foreach ($Rel in $StepFiles) {
        ...
        if (-not (Test-Path $Path)) {
            throw ("Missing expected Phase 36 Step 12 file: {0}" -f $Rel)
        }
    }
}
...
function Invoke-Apply {
    Assert-StepFiles
    Write-Host "APPLY PASS: Phase 36 Step 12 planning-only packet verified without live-write activation, live-write apply, bridge POST, sockets, DB mutation, server launch, or Phase 37 boundary creation."
}
...
function Invoke-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 36 Step 12 safety markers confirm planning-only/no-write/no-live-write-activation/no-live-write-apply/no-runtime/no-phase37 posture."
}
