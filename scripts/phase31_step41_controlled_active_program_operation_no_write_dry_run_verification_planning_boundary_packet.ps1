...
$StepNumber = 41
$StepTitle = "Phase 31 Step 41 - Phase 20 Network Transport Implementation Controlled Active Program Operation No-Write Dry Run Verification Planning Boundary Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Controlled Active Program Operation No-Write Dry Run Verification Planning Boundary Packet"
$ExpectedBranch = "phase31-step41-controlled-active-program-operation-no-write-dry-run-verification-planning-boundary"
$PriorStep = "Phase 31 Step 40 - Phase 20 Network Transport Implementation Controlled Active Program Operation No-Write Dry Run Readiness Final Boundary Confirmation Packet"
$Slug = "implementation_controlled_active_program_operation_no_write_dry_run_verification_planning_boundary"
...
$StepFiles = @(
    "scripts/phase31_step41_controlled_active_program_operation_no_write_dry_run_verification_planning_boundary_packet.ps1",
    "ui/pages/1137_Phase31_Step41_Implementation_Controlled_Active_Program_Operation_No_Write_Dry_Run_Verification_Planning_Boundary_Packet.py",
    "docs/PHASE31_STEP41_CONTROLLED_ACTIVE_PROGRAM_OPERATION_NO_WRITE_DRY_RUN_VERIFICATION_PLANNING_BOUNDARY_PACKET.md",
    "tests/test_phase31_step41_controlled_active_program_operation_no_write_dry_run_verification_planning_boundary_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase31_boundary = "implementation_controlled_active_program_operation_no_write_dry_run_verification_planning_boundary_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 31 Step 41 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 31 Step 41 Phase 20 Network Transport Implementation Controlled Active Program Operation No-Write Dry Run Verification Planning Boundary Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase31_step41_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase31_step41_{0}_packet.json" -f $Slug)
