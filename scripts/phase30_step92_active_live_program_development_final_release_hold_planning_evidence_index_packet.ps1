...
$StepNumber = 92
$StepTitle = "Phase 30 Step 92 - Phase 20 Network Transport Implementation Active Live Program Development Final Release Hold Planning Evidence Index Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Active Live Program Development Final Release Hold Planning Evidence Index Packet"
$ExpectedBranch = "phase30-step92-active-live-program-development-final-release-hold-planning-evidence-index"
$PriorStep = "Phase 30 Step 91 - Phase 20 Network Transport Implementation Active Live Program Development Final Release Hold Planning Boundary Packet"
$Slug = "implementation_active_live_program_development_final_release_hold_planning_evidence_index"
...
$StepFiles = @(
    "scripts/phase30_step92_active_live_program_development_final_release_hold_planning_evidence_index_packet.ps1",
    "ui/pages/1068_Phase30_Step92_Implementation_Active_Live_Program_Development_Final_Release_Hold_Planning_Evidence_Index_Packet.py",
    "docs/PHASE30_STEP92_ACTIVE_LIVE_PROGRAM_DEVELOPMENT_FINAL_RELEASE_HOLD_PLANNING_EVIDENCE_INDEX_PACKET.md",
    "tests/test_phase30_step92_active_live_program_development_final_release_hold_planning_evidence_index_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase30_boundary = "implementation_active_live_program_development_final_release_hold_planning_evidence_index_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 30 Step 92 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 30 Step 92 Phase 20 Network Transport Implementation Active Live Program Development Final Release Hold Planning Evidence Index Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase30_step92_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase30_step92_{0}_packet.json" -f $Slug)
