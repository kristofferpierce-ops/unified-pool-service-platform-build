...
$StepNumber = 13
$StepTitle = "Phase 35 Step 13 - Phase 20 Network Transport Implementation Trusted Production Limited Live Write Pilot Guardrail Verification Evidence Gap Review Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Trusted Production Limited Live Write Pilot Guardrail Verification Evidence Gap Review Packet"
$ExpectedBranch = "phase35-step13-limited-live-write-pilot-guardrail-verification-evidence-gap-review"
$PriorStep = "Phase 35 Step 12 - Phase 20 Network Transport Implementation Trusted Production Limited Live Write Pilot Guardrail Verification Evidence Index Packet"
$Slug = "trusted_production_limited_live_write_pilot_guardrail_verification_evidence_gap_review"
...
$StepFiles = @(
    "scripts/phase35_step13_limited_live_write_pilot_guardrail_verification_evidence_gap_review_packet.ps1",
    "ui/pages/1589_Phase35_Step13_Live_Write_Pilot_Guardrail_Evidence_Gap_Review.py",
    "docs/PHASE35_STEP13_LIMITED_LIVE_WRITE_PILOT_GUARDRAIL_VERIFICATION_EVIDENCE_GAP_REVIEW_PACKET.md",
    "tests/test_phase35_step13_limited_live_write_pilot_guardrail_verification_evidence_gap_review_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase35_boundary = "trusted_production_limited_live_write_pilot_guardrail_verification_evidence_gap_review_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 35 Step 13 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 35 Step 13 Phase 20 Network Transport Implementation Trusted Production Limited Live Write Pilot Guardrail Verification Evidence Gap Review Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase35_step13_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase35_step13_{0}_packet.json" -f $Slug)
