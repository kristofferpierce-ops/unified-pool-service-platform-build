...
$StepNumber = 2
$StepTitle = "Phase 28 Step 2 - Phase 20 Network Transport Implementation Sandbox Pilot Readiness Evidence Index Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Sandbox Pilot Readiness Evidence Index Packet"
$ExpectedBranch = "phase28-step2-implementation-sandbox-pilot-readiness-evidence-index"
$PriorStep = "Phase 28 Step 1 - Phase 20 Network Transport Implementation Sandbox Pilot Readiness Boundary Packet"
$Slug = "implementation_sandbox_pilot_readiness_evidence_index"

if (-not $RepoRoot) { $RepoRoot = (Resolve-Path (Join-Path $PSScriptRoot ".." )).Path }

$StepFiles = @("scripts/phase28_step2_implementation_sandbox_pilot_readiness_evidence_index_packet.ps1", "ui/pages/738_Phase28_Step2_Implementation_Sandbox_Pilot_Readiness_Evidence_Index_Packet.py", "docs/PHASE28_STEP2_IMPLEMENTATION_SANDBOX_PILOT_READINESS_EVIDENCE_INDEX_PACKET.md", "tests/test_phase28_step2_implementation_sandbox_pilot_readiness_evidence_index_packet.py")
...
$SafetyPosture = [ordered]@{
    ...
    phase28_boundary = "implementation_sandbox_pilot_readiness_evidence_index_opened_by_packet"
    ...
    implementation_sandbox_pilot_readiness_evidence_index_mode = "reference_only"
    implementation_sandbox_pilot_readiness_evidence_index_write = $false
    implementation_sandbox_pilot_readiness_evidence_index_record_creation = $false
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 28 Step 2 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 28 Step 2 Phase 20 Network Transport Implementation Sandbox Pilot Readiness Evidence Index Packet is present and planning-only."
}

function New-Packet {
    $Timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
    $BackupRoot = Join-Path $RepoRoot "backups"
    $PacketDir = Join-Path $BackupRoot ("phase28_step2_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase28_step2_{0}_packet.json" -f $Slug)
    ...
    Write-Host "PASS: phase28_boundary=implementation_sandbox_pilot_readiness_evidence_index_opened_by_packet"
    ...
    Write-Host "PASS: implementation_sandbox_pilot_readiness_evidence_index_mode=reference_only"
    Write-Host "PASS: implementation_sandbox_pilot_readiness_evidence_index_write=false"
    Write-Host "PASS: implementation_sandbox_pilot_readiness_evidence_index_record_creation=false"
