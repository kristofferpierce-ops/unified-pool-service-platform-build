...
$StepNumber = 23
$StepTitle = "Phase 30 Step 23 - Phase 20 Network Transport Implementation Active Live Program Development Preflight Planning Evidence Gap Review Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Active Live Program Development Preflight Planning Evidence Gap Review Packet"
$ExpectedBranch = "phase30-step23-active-live-program-development-preflight-planning-evidence-gap-review"
$PriorStep = "Phase 30 Step 22 - Phase 20 Network Transport Implementation Active Live Program Development Preflight Planning Evidence Index Packet"
$Slug = "implementation_active_live_program_development_preflight_planning_evidence_gap_review"
...
$StepFiles = @(
    "scripts/phase30_step23_active_live_program_development_preflight_planning_evidence_gap_review_packet.ps1",
    "ui/pages/999_Phase30_Step23_Implementation_Active_Live_Program_Development_Preflight_Planning_Evidence_Gap_Review_Packet.py",
    "docs/PHASE30_STEP23_ACTIVE_LIVE_PROGRAM_DEVELOPMENT_PREFLIGHT_PLANNING_EVIDENCE_GAP_REVIEW_PACKET.md",
    "tests/test_phase30_step23_active_live_program_development_preflight_planning_evidence_gap_review_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase30_boundary = "implementation_active_live_program_development_preflight_planning_evidence_gap_review_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 30 Step 23 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 30 Step 23 Phase 20 Network Transport Implementation Active Live Program Development Preflight Planning Evidence Gap Review Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase30_step23_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase30_step23_{0}_packet.json" -f $Slug)
