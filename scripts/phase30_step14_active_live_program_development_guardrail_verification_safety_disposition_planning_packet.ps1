...
$StepNumber = 14
$StepTitle = "Phase 30 Step 14 - Phase 20 Network Transport Implementation Active Live Program Development Guardrail Verification Safety Disposition Planning Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Active Live Program Development Guardrail Verification Safety Disposition Planning Packet"
$ExpectedBranch = "phase30-step14-active-live-program-development-guardrail-verification-safety-disposition-planning"
$PriorStep = "Phase 30 Step 13 - Phase 20 Network Transport Implementation Active Live Program Development Guardrail Verification Evidence Gap Review Packet"
$Slug = "implementation_active_live_program_development_guardrail_verification_safety_disposition_planning"
$ContextKey = "implementation_active_live_program_development_guardrail_verification"
$PhaseContext = "implementation_active_live_program_development_guardrail_verification_planning_only"

if (-not $RepoRoot) { $RepoRoot = (Resolve-Path (Join-Path $PSScriptRoot ".." )).Path }

$StepFiles = @(
    "scripts/phase30_step14_active_live_program_development_guardrail_verification_safety_disposition_planning_packet.ps1",
    "ui/pages/990_Phase30_Step14_Implementation_Active_Live_Program_Development_Guardrail_Verification_Safety_Disposition_Planning_Packet.py",
    "docs/PHASE30_STEP14_ACTIVE_LIVE_PROGRAM_DEVELOPMENT_GUARDRAIL_VERIFICATION_SAFETY_DISPOSITION_PLANNING_PACKET.md",
    "tests/test_phase30_step14_active_live_program_development_guardrail_verification_safety_disposition_planning_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase30_boundary = "implementation_active_live_program_development_guardrail_verification_safety_disposition_planning_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 30 Step 14 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 30 Step 14 Phase 20 Network Transport Implementation Active Live Program Development Guardrail Verification Safety Disposition Planning Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase30_step14_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase30_step14_{0}_packet.json" -f $Slug)
