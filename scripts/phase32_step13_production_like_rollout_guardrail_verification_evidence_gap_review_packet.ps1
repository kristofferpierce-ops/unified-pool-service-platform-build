...
$StepNumber = 13
$StepTitle = "Phase 32 Step 13 - Phase 20 Network Transport Implementation Production-Like Rollout Guardrail Verification Evidence Gap Review Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Production-Like Rollout Guardrail Verification Evidence Gap Review Packet"
$ExpectedBranch = "phase32-step13-production-like-rollout-guardrail-verification-evidence-gap-review"
$PriorStep = "Phase 32 Step 12 - Phase 20 Network Transport Implementation Production-Like Rollout Guardrail Verification Evidence Index Packet"
$Slug = "implementation_production_like_rollout_guardrail_verification_evidence_gap_review"
...
$StepFiles = @(
    "scripts/phase32_step13_production_like_rollout_guardrail_verification_evidence_gap_review_packet.ps1",
    "ui/pages/1229_Phase32_Step13_Implementation_Production_Like_Rollout_Guardrail_Verification_Evidence_Gap_Review_Packet.py",
    "docs/PHASE32_STEP13_PRODUCTION_LIKE_ROLLOUT_GUARDRAIL_VERIFICATION_EVIDENCE_GAP_REVIEW_PACKET.md",
    "tests/test_phase32_step13_production_like_rollout_guardrail_verification_evidence_gap_review_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase32_boundary = "implementation_production_like_rollout_guardrail_verification_evidence_gap_review_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 32 Step 13 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 32 Step 13 Phase 20 Network Transport Implementation Production-Like Rollout Guardrail Verification Evidence Gap Review Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase32_step13_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase32_step13_{0}_packet.json" -f $Slug)
