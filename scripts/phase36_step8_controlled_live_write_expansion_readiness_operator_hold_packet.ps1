...
$PhaseNumber = 36
$StepNumber = 8
$StepTitle = "Phase 36 Step 8 - Phase 20 Network Transport Implementation Trusted Production Controlled Live Write Expansion Readiness Operator Hold Point Packet"
$ExpectedBranch = "phase36-step8-readiness-operator-hold"
$PriorCompletedStep = "Phase 36 Step 7 - Phase 20 Network Transport Implementation Trusted Production Controlled Live Write Expansion Readiness Approval Readiness Packet"
...
$StepFiles = @(
    "scripts/phase36_step8_controlled_live_write_expansion_readiness_operator_hold_packet.ps1",
    "ui/pages/1704_Phase36_Step8_Readiness_Operator_Hold.py",
    "docs/PHASE36_STEP8_CONTROLLED_LIVE_WRITE_EXPANSION_READINESS_OPERATOR_HOLD_PACKET.md",
    "tests/test_phase36_step8_controlled_live_write_expansion_readiness_operator_hold_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase36_boundary = "trusted_production_controlled_live_write_expansion_readiness_operator_hold_opened_by_packet"
...
function Assert-StepFiles {
    foreach ($Rel in $StepFiles) {
        ...
        if (-not (Test-Path $Path)) {
            throw ("Missing expected Phase 36 Step 8 file: {0}" -f $Rel)
        }
    }
}
...
function Invoke-Apply {
    Assert-StepFiles
    Write-Host "APPLY PASS: Phase 36 Step 8 planning-only packet verified without live-write activation, live-write apply, bridge POST, sockets, DB mutation, server launch, or Phase 37 boundary creation."
}
...
function Invoke-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 36 Step 8 safety markers confirm planning-only/no-write/no-live-write-activation/no-live-write-apply/no-runtime/no-phase37 posture."
}
