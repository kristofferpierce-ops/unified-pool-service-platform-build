...
$PhaseNumber = 24
$StepNumber = 2
$StepName = "Phase 24 Step 2 - Phase 20 Network Transport Implementation Controlled Activation Evidence Index Packet"
$PriorStepName = "Phase 24 Step 1 - Phase 20 Network Transport Implementation Controlled Activation Boundary Packet"
$ExpectedBranch = "phase24-step2-controlled-activation-evidence-index"
$ModeKey = "implementation_controlled_activation_evidence_index_mode"
$WriteKey = "implementation_controlled_activation_evidence_index_write"
$RecordKey = "implementation_controlled_activation_evidence_index_record_creation"
$PhaseContext = "implementation_controlled_activation_evidence_index_planning_only"
$PacketSlug = "phase24_step2_controlled_activation_evidence_index_packet"

$StepFiles = @(
    "scripts/phase24_step2_controlled_activation_evidence_index_packet.ps1",
    "ui/pages/258_Phase24_Step2_Implementation_Controlled_Activation_Evidence_Index_Packet.py",
    "docs/PHASE24_STEP2_CONTROLLED_ACTIVATION_EVIDENCE_INDEX_PACKET.md",
    "tests/test_phase24_step2_controlled_activation_evidence_index_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase24_boundary = "implementation_controlled_activation_evidence_index_opened_by_packet"
    ...
    implementation_controlled_activation_evidence_index_mode = "reference_only"
    implementation_controlled_activation_evidence_index_write = $false
    implementation_controlled_activation_evidence_index_record_creation = $false
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 24 Step 2 files copied or already present."
}
...
function Invoke-SmokeTest {
    ...
    $scriptText = Get-Content -LiteralPath (Join-SafePath -Root $Repo -RelativePath "scripts/phase24_step2_controlled_activation_evidence_index_packet.ps1") -Raw -Encoding UTF8
    ...
    Write-Host "SMOKE TEST PASS: Phase 24 Step 2 Implementation Controlled Activation Evidence Index Packet is present and planning-only."
}
...
function Write-Packet {
    ...
    Write-Host "PASS: phase24_boundary=implementation_controlled_activation_evidence_index_opened_by_packet"
    ...
    Write-Host "PASS: implementation_controlled_activation_evidence_index_mode=reference_only"
    Write-Host "PASS: implementation_controlled_activation_evidence_index_write=false"
    Write-Host "PASS: implementation_controlled_activation_evidence_index_record_creation=false"
...
while ($true) {
    Write-Host ""
    Write-Host "Phase 24 Step 2 menu"
    Write-Host "1. Show status / verify paths"
    Write-Host "2. Apply Phase 24 Step 2 Implementation Controlled Activation Evidence Index Packet files"
    Write-Host "3. Smoke test Phase 24 Step 2"
    Write-Host "4. Show server start placeholder only"
    Write-Host "5. Generate Phase 24 Step 2 Implementation Controlled Activation Evidence Index Packet"
    Write-Host "6. Exit"
