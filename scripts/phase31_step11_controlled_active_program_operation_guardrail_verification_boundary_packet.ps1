...
$StepNumber = 11
$StepTitle = "Phase 31 Step 11 - Phase 20 Network Transport Implementation Controlled Active Program Operation Guardrail Verification Boundary Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Controlled Active Program Operation Guardrail Verification Boundary Packet"
$ExpectedBranch = "phase31-step11-controlled-active-program-operation-guardrail-verification-boundary"
$PriorStep = "Phase 31 Step 10 - Phase 20 Network Transport Implementation Controlled Active Program Operation Readiness Final Boundary Confirmation Packet"
$Slug = "implementation_controlled_active_program_operation_guardrail_verification_boundary"
...
$StepFiles = @(
    "scripts/phase31_step11_controlled_active_program_operation_guardrail_verification_boundary_packet.ps1",
    "ui/pages/1107_Phase31_Step11_Implementation_Controlled_Active_Program_Operation_Guardrail_Verification_Boundary_Packet.py",
    "docs/PHASE31_STEP11_CONTROLLED_ACTIVE_PROGRAM_OPERATION_GUARDRAIL_VERIFICATION_BOUNDARY_PACKET.md",
    "tests/test_phase31_step11_controlled_active_program_operation_guardrail_verification_boundary_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase31_boundary = "implementation_controlled_active_program_operation_guardrail_verification_boundary_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 31 Step 11 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 31 Step 11 Phase 20 Network Transport Implementation Controlled Active Program Operation Guardrail Verification Boundary Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase31_step11_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase31_step11_{0}_packet.json" -f $Slug)
