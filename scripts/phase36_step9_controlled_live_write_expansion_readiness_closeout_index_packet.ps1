...
$PhaseNumber = 36
$StepNumber = 9
$StepTitle = "Phase 36 Step 9 - Phase 20 Network Transport Implementation Trusted Production Controlled Live Write Expansion Readiness Closeout Index Packet"
$ExpectedBranch = "phase36-step9-readiness-closeout-index"
$PriorCompletedStep = "Phase 36 Step 8 - Phase 20 Network Transport Implementation Trusted Production Controlled Live Write Expansion Readiness Operator Hold Point Packet"
...
$StepFiles = @(
    "scripts/phase36_step9_controlled_live_write_expansion_readiness_closeout_index_packet.ps1",
    "ui/pages/1705_Phase36_Step9_Readiness_Closeout_Index.py",
    "docs/PHASE36_STEP9_CONTROLLED_LIVE_WRITE_EXPANSION_READINESS_CLOSEOUT_INDEX_PACKET.md",
    "tests/test_phase36_step9_controlled_live_write_expansion_readiness_closeout_index_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase36_boundary = "trusted_production_controlled_live_write_expansion_readiness_closeout_index_opened_by_packet"
...
function Assert-StepFiles {
    foreach ($Rel in $StepFiles) {
        ...
        if (-not (Test-Path $Path)) {
            throw ("Missing expected Phase 36 Step 9 file: {0}" -f $Rel)
        }
    }
}
...
function Invoke-Apply {
    Assert-StepFiles
    Write-Host "APPLY PASS: Phase 36 Step 9 planning-only packet verified without live-write activation, live-write apply, bridge POST, sockets, DB mutation, server launch, or Phase 37 boundary creation."
}
...
function Invoke-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 36 Step 9 safety markers confirm planning-only/no-write/no-live-write-activation/no-live-write-apply/no-runtime/no-phase37 posture."
}
