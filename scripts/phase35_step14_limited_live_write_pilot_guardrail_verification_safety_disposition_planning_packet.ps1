...
$StepNumber = 14
$StepTitle = "Phase 35 Step 14 - Phase 20 Network Transport Implementation Trusted Production Limited Live Write Pilot Guardrail Verification Safety Disposition Planning Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Trusted Production Limited Live Write Pilot Guardrail Verification Safety Disposition Planning Packet"
$ExpectedBranch = "phase35-step14-limited-live-write-pilot-guardrail-verification-safety-planning"
$PriorStep = "Phase 35 Step 13 - Phase 20 Network Transport Implementation Trusted Production Limited Live Write Pilot Guardrail Verification Evidence Gap Review Packet"
$Slug = "trusted_production_limited_live_write_pilot_guardrail_verification_safety_disposition_planning"
$ContextKey = "trusted_production_limited_live_write_pilot_guardrail_verification"
$PhaseContext = "trusted_production_limited_live_write_pilot_guardrail_verification_planning_only"

if (-not $RepoRoot) { $RepoRoot = (Resolve-Path (Join-Path $PSScriptRoot ".." )).Path }

$StepFiles = @(
    "scripts/phase35_step14_limited_live_write_pilot_guardrail_verification_safety_disposition_planning_packet.ps1",
    "ui/pages/1590_Phase35_Step14_Live_Write_Pilot_Guardrail_Safety_Planning.py",
    "docs/PHASE35_STEP14_LIMITED_LIVE_WRITE_PILOT_GUARDRAIL_VERIFICATION_SAFETY_PLANNING_PACKET.md",
    "tests/test_phase35_step14_limited_live_write_pilot_guardrail_verification_safety_disposition_planning_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase35_boundary = "trusted_production_limited_live_write_pilot_guardrail_verification_safety_disposition_planning_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 35 Step 14 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 35 Step 14 Phase 20 Network Transport Implementation Trusted Production Limited Live Write Pilot Guardrail Verification Safety Disposition Planning Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase35_step14_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase35_step14_{0}_packet.json" -f $Slug)
