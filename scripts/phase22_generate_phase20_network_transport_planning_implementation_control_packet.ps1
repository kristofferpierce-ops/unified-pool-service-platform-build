...
$StepNumber = 18
$StepName = "Phase 22 Step 18 - Phase 20 Network Transport Planning Implementation Control Packet"
$PacketSlug = "phase22_phase20_network_transport_planning_implementation_control_packet"
$BranchName = "phase22-step18-phase20-network-transport-planning-implementation-control-packet"
$ExpectedPriorStep = "Phase 22 Step 17 - Phase 20 Network Transport Planning Implementation Preflight Packet"
...
$StepFiles = @(
    ...
    "tests\test_phase22_phase20_network_transport_planning_implementation_control_packet.py"
)
...
function Copy-Step18Files {
    ...
    Write-Host "APPLY PASS: Phase 22 Step 18 files copied or already present."
}

function Test-Step18Control {
    $Root = Resolve-Phase22RepoRoot
    $Missing = Test-StepFileSetPresent -Root $Root
    if ($Missing.Count -gt 0) {
        throw ("Missing Step 18 files: " + ($Missing -join ", "))
    }
    ...
    Write-Host "SMOKE TEST PASS: Phase 22 Step 18 Phase 20 Network Transport Planning Implementation Control Packet is present and planning-only."
}
...
function Write-ImplementationControlPacket {
    $Root = Resolve-Phase22RepoRoot
    ...
    $Packet = [ordered]@{
        ...
        purpose = "Planning-only implementation control packet. It inventories readiness boundaries without starting implementation, approving writes, or creating closure records."
        safety_posture = $SafetyPosture
        control_checks = @(
            "Step 16 readiness packet is treated as the latest completed committed step.",
            ...
            "Any future implementation work must be a separately named and authorized phase."
        )
...
function Show-Menu {
    Write-Host ""
    Write-Host "Phase 22 Step 18 menu"
    Write-Host "1. Show status / verify paths"
    Write-Host "2. Apply Phase 22 Step 18 Phase 20 Network Transport Planning Implementation Control Packet files"
    Write-Host "3. Smoke test Phase 22 Step 18"
    Write-Host "4. Show server start placeholder only"
    Write-Host "5. Generate Phase 20 network transport planning implementation control packet"
    Write-Host "6. Exit"
}

function Invoke-Step18Action {
    param([string]$SelectedAction)
    switch ($SelectedAction) {
        "status" { Show-StepStatus }
        "apply" { Copy-Step18Files }
        "smoke" { Test-Step18Control }
        "server-placeholder" { Show-ServerStartPlaceholder }
        "packet" { Write-ImplementationControlPacket }
        "all" {
            Show-StepStatus
            Copy-Step18Files
            Test-Step18Control
            Write-ImplementationControlPacket
        }
...
if (-not [string]::IsNullOrWhiteSpace($CleanAction)) {
    Invoke-Step18Action -SelectedAction $CleanAction
    return
}
...
while ($true) {
    ...
    switch ($Choice) {
        "1" { Show-StepStatus }
        "2" { Copy-Step18Files }
        "3" { Test-Step18Control }
        "4" { Show-ServerStartPlaceholder }
        "5" { Write-ImplementationControlPacket }
        "6" { break }
