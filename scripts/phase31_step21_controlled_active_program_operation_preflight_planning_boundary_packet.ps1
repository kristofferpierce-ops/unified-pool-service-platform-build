...
$StepNumber = 21
$StepTitle = "Phase 31 Step 21 - Phase 20 Network Transport Implementation Controlled Active Program Operation Preflight Planning Boundary Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Controlled Active Program Operation Preflight Planning Boundary Packet"
$ExpectedBranch = "phase31-step21-controlled-active-program-operation-preflight-planning-boundary"
$PriorStep = "Phase 31 Step 20 - Phase 20 Network Transport Implementation Controlled Active Program Operation Guardrail Verification Final Boundary Confirmation Packet"
$Slug = "implementation_controlled_active_program_operation_preflight_planning_boundary"
...
$StepFiles = @(
    "scripts/phase31_step21_controlled_active_program_operation_preflight_planning_boundary_packet.ps1",
    "ui/pages/1117_Phase31_Step21_Implementation_Controlled_Active_Program_Operation_Preflight_Planning_Boundary_Packet.py",
    "docs/PHASE31_STEP21_CONTROLLED_ACTIVE_PROGRAM_OPERATION_PREFLIGHT_PLANNING_BOUNDARY_PACKET.md",
    "tests/test_phase31_step21_controlled_active_program_operation_preflight_planning_boundary_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase31_boundary = "implementation_controlled_active_program_operation_preflight_planning_boundary_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 31 Step 21 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 31 Step 21 Phase 20 Network Transport Implementation Controlled Active Program Operation Preflight Planning Boundary Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase31_step21_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase31_step21_{0}_packet.json" -f $Slug)
