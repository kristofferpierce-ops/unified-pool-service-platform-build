...
$PhaseNumber = 22
$StepNumber = 63
$StepName = "Phase 22 Step 63 - Phase 20 Network Transport Planning Cross-Repo Handoff Evidence Index Packet"
$PriorStepName = "Phase 22 Step 62 - Phase 20 Network Transport Planning Cross-Repo Handoff Verification Packet"
$ExpectedBranch = "phase22-step63-cross-repo-handoff-evidence-index"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 22 Step 63 files copied or already present."
}
...
function Test-Smoke {
    ...
    if ($safety["handoff_evidence_index_write"]) { throw "SMOKE TEST FAIL: handoff evidence index write must remain disabled" }
    if ($safety["evidence_capture_mutation"]) { throw "SMOKE TEST FAIL: evidence capture mutation must remain disabled" }
    ...
    Write-Host "SMOKE TEST PASS: Phase 22 Step 63 Phase 20 Network Transport Planning Cross-Repo Handoff Evidence Index Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $packetPath = Join-SafePath -Root $backupDir -RelativePath ("phase22_step63_cross_repo_handoff_evidence_index_packet_{0}.json" -f $stamp)
    ...
    $packet = [ordered]@{
        ...
        planning_only = $true
        evidence_index_mode = "reference_only"
        handoff_state = "post_step62_reference_verified"
        phase23_start = "not_started"
        implementation_phase_start = "not_started"
        safety_posture = Get-SafetyPosture
        evidence_index_scope = @(
            "Index Step 61 and Step 62 handoff artifacts by reference only",
    ...
    Write-Host "PASS: evidence_index_mode=reference_only"
    Write-Host "PASS: handoff_evidence_index_write=false"
...
function Show-ServerPlaceholder {
    ...
    Write-Host "CHECK: Evidence indexing is reference-only and does not collect, mutate, or validate sibling repositories."
}

function Show-Menu {
    while ($true) {
        Write-Host ""
        Write-Host "Phase 22 Step 63 menu"
        Write-Host "1. Show status / verify paths"
        Write-Host "2. Apply Phase 22 Step 63 Phase 20 Network Transport Planning Cross-Repo Handoff Evidence Index Packet files"
        Write-Host "3. Smoke test Phase 22 Step 63"
        Write-Host "4. Show server start placeholder only"
        Write-Host "5. Generate Phase 22 Step 63 cross-repo handoff evidence index packet"
        Write-Host "6. Exit"
