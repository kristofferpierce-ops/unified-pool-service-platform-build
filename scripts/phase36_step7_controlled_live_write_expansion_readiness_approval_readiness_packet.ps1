...
$PhaseNumber = 36
$StepNumber = 7
$StepTitle = "Phase 36 Step 7 - Phase 20 Network Transport Implementation Trusted Production Controlled Live Write Expansion Readiness Approval Readiness Packet"
$ExpectedBranch = "phase36-step7-readiness-approval-readiness"
$PriorCompletedStep = "Phase 36 Step 6 - Phase 20 Network Transport Implementation Trusted Production Controlled Live Write Expansion Readiness Approval Boundary Packet"
...
$StepFiles = @(
    "scripts/phase36_step7_controlled_live_write_expansion_readiness_approval_readiness_packet.ps1",
    "ui/pages/1703_Phase36_Step7_Readiness_Approval_Readiness.py",
    "docs/PHASE36_STEP7_CONTROLLED_LIVE_WRITE_EXPANSION_READINESS_APPROVAL_READINESS_PACKET.md",
    "tests/test_phase36_step7_controlled_live_write_expansion_readiness_approval_readiness_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase36_boundary = "trusted_production_controlled_live_write_expansion_readiness_approval_readiness_opened_by_packet"
...
function Assert-StepFiles {
    foreach ($Rel in $StepFiles) {
        ...
        if (-not (Test-Path $Path)) {
            throw ("Missing expected Phase 36 Step 7 file: {0}" -f $Rel)
        }
    }
}
...
function Invoke-Apply {
    Assert-StepFiles
    Write-Host "APPLY PASS: Phase 36 Step 7 planning-only packet verified without live-write activation, live-write apply, bridge POST, sockets, DB mutation, server launch, or Phase 37 boundary creation."
}
...
function Invoke-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 36 Step 7 safety markers confirm planning-only/no-write/no-live-write-activation/no-live-write-apply/no-runtime/no-phase37 posture."
}
