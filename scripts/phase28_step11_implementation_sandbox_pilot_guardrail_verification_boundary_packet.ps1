...
$StepNumber = 11
$StepTitle = "Phase 28 Step 11 - Phase 20 Network Transport Implementation Sandbox Pilot Guardrail Verification Boundary Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Sandbox Pilot Guardrail Verification Boundary Packet"
$ExpectedBranch = "phase28-step11-implementation-sandbox-pilot-guardrail-verification-boundary"
$PriorStep = "Phase 28 Step 10 - Phase 20 Network Transport Implementation Sandbox Pilot Readiness Final Boundary Confirmation Packet"
$Slug = "implementation_sandbox_pilot_guardrail_verification_boundary"

if (-not $RepoRoot) { $RepoRoot = (Resolve-Path (Join-Path $PSScriptRoot ".." )).Path }

$StepFiles = @("scripts/phase28_step11_implementation_sandbox_pilot_guardrail_verification_boundary_packet.ps1", "ui/pages/747_Phase28_Step11_Implementation_Sandbox_Pilot_Guardrail_Verification_Boundary_Packet.py", "docs/PHASE28_STEP11_IMPLEMENTATION_SANDBOX_PILOT_GUARDRAIL_VERIFICATION_BOUNDARY_PACKET.md", "tests/test_phase28_step11_implementation_sandbox_pilot_guardrail_verification_boundary_packet.py")
...
$SafetyPosture = [ordered]@{
    ...
    phase28_boundary = "implementation_sandbox_pilot_guardrail_verification_boundary_opened_by_packet"
    ...
    implementation_sandbox_pilot_guardrail_verification_boundary_mode = "reference_only"
    implementation_sandbox_pilot_guardrail_verification_boundary_write = $false
    implementation_sandbox_pilot_guardrail_verification_boundary_record_creation = $false
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 28 Step 11 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 28 Step 11 Phase 20 Network Transport Implementation Sandbox Pilot Guardrail Verification Boundary Packet is present and planning-only."
}

function New-Packet {
    $Timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
    $BackupRoot = Join-Path $RepoRoot "backups"
    $PacketDir = Join-Path $BackupRoot ("phase28_step11_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase28_step11_{0}_packet.json" -f $Slug)

    $Payload = [ordered]@{ phase = 28; step = 11; title = $StepTitle; prior_step = $PriorStep; safety_posture = $SafetyPosture; files = $StepFiles; generated_at = (Get-Date).ToString("s") }
    ...
    Write-Host "PASS: phase28_boundary=implementation_sandbox_pilot_guardrail_verification_boundary_opened_by_packet"
    ...
    Write-Host "PASS: implementation_sandbox_pilot_guardrail_verification_boundary_mode=reference_only"
    Write-Host "PASS: implementation_sandbox_pilot_guardrail_verification_boundary_write=false"
    Write-Host "PASS: implementation_sandbox_pilot_guardrail_verification_boundary_record_creation=false"
