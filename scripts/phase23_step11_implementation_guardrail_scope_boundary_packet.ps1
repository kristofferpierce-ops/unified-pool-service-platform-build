...
$PhaseNumber = 23
$StepNumber = 11
$StepName = "Phase 23 Step 11 - Phase 20 Network Transport Implementation Guardrail Scope Boundary Packet"
$PriorStepName = "Phase 23 Step 10 - Phase 20 Network Transport Implementation Guardrail Baseline Packet"
$ExpectedBranch = "phase23-step11-implementation-guardrail-scope-boundary"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 23 Step 11 files copied or already present."
}
...
function Test-Smoke {
    ...
    if ($safety["implementation_guardrail_scope_boundary_write"]) { throw "SMOKE TEST FAIL: implementation guardrail scope boundary writes must remain disabled" }
    if ($safety["implementation_guardrail_scope_boundary_record_creation"]) { throw "SMOKE TEST FAIL: implementation guardrail scope boundary record creation must remain disabled" }
    ...
    Write-Host "SMOKE TEST PASS: Phase 23 Step 11 Phase 20 Network Transport Implementation Guardrail Scope Boundary Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $packetDir = Join-SafePath -Root $backupRoot -RelativePath ("phase23_step11_implementation_guardrail_scope_boundary_packet_{0}" -f $stamp)
    if (-not (Test-SafePath -Path $packetDir)) { New-Item -ItemType Directory -Path $packetDir -Force | Out-Null }
    $packetPath = Join-SafePath -Root $packetDir -RelativePath "phase23_step11_implementation_guardrail_scope_boundary_packet.json"
    $packet = [ordered]@{
        phase = 23
        step = 11
        step_name = $StepName
        ...
        intake_scope = @(
            "Carry forward the Phase 23 Step 10 implementation guardrail baseline as the prior reference",
    ...
    Write-Host "PASS: phase23_planning_boundary=guardrail_scope_boundary_opened_by_packet"
    ...
    Write-Host "PASS: implementation_guardrail_scope_boundary_mode=reference_only"
    Write-Host "PASS: implementation_guardrail_scope_boundary_write=false"
    Write-Host "PASS: implementation_guardrail_scope_boundary_record_creation=false"
    ...
    Write-Host "CHECK: prior_step=Phase 23 Step 10"
...
function Show-Menu {
    while ($true) {
        Write-Host ""
        Write-Host "Phase 23 Step 11 menu"
        Write-Host "1. Show status / verify paths"
        Write-Host "2. Apply Phase 23 Step 11 Phase 20 Network Transport Implementation Guardrail Scope Boundary Packet files"
        Write-Host "3. Smoke test Phase 23 Step 11"
        Write-Host "4. Show server start placeholder only"
        Write-Host "5. Generate Phase 23 Step 11 implementation guardrail scope boundary packet"
        Write-Host "6. Exit"
