...
$PhaseNumber = 36
$StepNumber = 4
$StepTitle = "Phase 36 Step 4 - Phase 20 Network Transport Implementation Trusted Production Controlled Live Write Expansion Readiness Safety Disposition Planning Packet"
$ExpectedBranch = "phase36-step4-readiness-safety-planning"
$PriorCompletedStep = "Phase 36 Step 3 - Phase 20 Network Transport Implementation Trusted Production Controlled Live Write Expansion Readiness Evidence Gap Review Packet"
...
$SafetyPosture = [ordered]@{
    ...
    phase36_boundary = "trusted_production_controlled_live_write_expansion_readiness_safety_planning_opened_by_packet"
...
function Assert-StepFiles {
    foreach ($Rel in $StepFiles) {
        ...
        if (-not (Test-Path $Path)) {
            throw ("Missing expected Phase 36 Step 4 file: {0}" -f $Rel)
        }
    }
}
...
function Invoke-Apply {
    Assert-StepFiles
    Write-Host "APPLY PASS: Phase 36 Step 4 planning-only packet verified without live-write activation, live-write apply, bridge POST, sockets, DB mutation, server launch, or Phase 37 boundary creation."
}
...
function Invoke-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 36 Step 4 safety markers confirm planning-only/no-write/no-live-write-activation/no-live-write-apply/no-runtime/no-phase37 posture."
}
