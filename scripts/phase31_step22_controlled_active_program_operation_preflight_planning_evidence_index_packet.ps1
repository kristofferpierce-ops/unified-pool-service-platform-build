...
$StepNumber = 22
$StepTitle = "Phase 31 Step 22 - Phase 20 Network Transport Implementation Controlled Active Program Operation Preflight Planning Evidence Index Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Controlled Active Program Operation Preflight Planning Evidence Index Packet"
$ExpectedBranch = "phase31-step22-controlled-active-program-operation-preflight-planning-evidence-index"
$PriorStep = "Phase 31 Step 21 - Phase 20 Network Transport Implementation Controlled Active Program Operation Preflight Planning Boundary Packet"
$Slug = "implementation_controlled_active_program_operation_preflight_planning_evidence_index"
...
$StepFiles = @(
    "scripts/phase31_step22_controlled_active_program_operation_preflight_planning_evidence_index_packet.ps1",
    "ui/pages/1118_Phase31_Step22_Implementation_Controlled_Active_Program_Operation_Preflight_Planning_Evidence_Index_Packet.py",
    "docs/PHASE31_STEP22_CONTROLLED_ACTIVE_PROGRAM_OPERATION_PREFLIGHT_PLANNING_EVIDENCE_INDEX_PACKET.md",
    "tests/test_phase31_step22_controlled_active_program_operation_preflight_planning_evidence_index_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase31_boundary = "implementation_controlled_active_program_operation_preflight_planning_evidence_index_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 31 Step 22 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 31 Step 22 Phase 20 Network Transport Implementation Controlled Active Program Operation Preflight Planning Evidence Index Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase31_step22_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase31_step22_{0}_packet.json" -f $Slug)
