...
$StepNumber = 4
$StepTitle = "Phase 32 Step 4 - Phase 20 Network Transport Implementation Production-Like Rollout Readiness Safety Disposition Planning Packet"
$ShortTitle = "Phase 20 Network Transport Implementation Production-Like Rollout Readiness Safety Disposition Planning Packet"
$ExpectedBranch = "phase32-step4-production-like-rollout-readiness-safety-disposition-planning"
$PriorStep = "Phase 32 Step 3 - Phase 20 Network Transport Implementation Production-Like Rollout Readiness Evidence Gap Review Packet"
$Slug = "implementation_production_like_rollout_readiness_safety_disposition_planning"
$ContextKey = "implementation_production_like_rollout_readiness"
$PhaseContext = "implementation_production_like_rollout_readiness_planning_only"

if (-not $RepoRoot) { $RepoRoot = (Resolve-Path (Join-Path $PSScriptRoot ".." )).Path }

$StepFiles = @(
    "scripts/phase32_step4_production_like_rollout_readiness_safety_disposition_planning_packet.ps1",
    "ui/pages/1220_Phase32_Step4_Implementation_Production_Like_Rollout_Readiness_Safety_Disposition_Planning_Packet.py",
    "docs/PHASE32_STEP4_PRODUCTION_LIKE_ROLLOUT_READINESS_SAFETY_DISPOSITION_PLANNING_PACKET.md",
    "tests/test_phase32_step4_production_like_rollout_readiness_safety_disposition_planning_packet.py"
)
...
$SafetyPosture = [ordered]@{
    ...
    phase32_boundary = "implementation_production_like_rollout_readiness_safety_disposition_planning_opened_by_packet"
...
function Apply-StepFiles {
    ...
    Write-Host "APPLY PASS: Phase 32 Step 4 files copied or already present."
}
...
function Test-Smoke {
    ...
    Write-Host "SMOKE TEST PASS: Phase 32 Step 4 Phase 20 Network Transport Implementation Production-Like Rollout Readiness Safety Disposition Planning Packet is present and planning-only."
}
...
function New-Packet {
    ...
    $PacketDir = Join-Path $BackupRoot ("phase32_step4_{0}_packet_{1}" -f $Slug, $Timestamp)
    New-Item -ItemType Directory -Path $PacketDir -Force | Out-Null
    $PacketJson = Join-Path $PacketDir ("phase32_step4_{0}_packet.json" -f $Slug)
