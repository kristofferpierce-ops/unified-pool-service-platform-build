...
class EstimateOutput:
    ...
    chemical_costs: dict
    service_costs: dict
    monthly_chemical_real_cost: float
    annual_chemical_real_cost: float
    monthly_chemical_sell_price: float
    annual_chemical_sell_price: float
    monthly_service_real_cost: float
    annual_service_real_cost: float
    monthly_service_sell_price: float
    annual_service_sell_price: float
...
def _stored_labor_settings(session: Session) -> LaborSettings:
    payload = get_setting(session, 'labor_settings', None)
    if not payload:
...
def burdened_labor_rate(settings: LaborSettings) -> float:
    return settings.tech_hourly_wage * (
        1 + settings.payroll_tax_burden_pct / 100.0 + settings.benefits_burden_pct / 100.0
    )


def service_time_multiplier(
    *,
    bath_score: int,
    debris_score: int,
    filtration_score: int,
...
def _margin_divisor(target_margin_pct: float) -> float:
    return max(0.01, 1.0 - float(target_margin_pct) / 100.0)


def _as_output_dict(value: EstimateOutput | dict[str, Any] | Any) -> dict[str, Any]:
    ...
    if callable(model_dump):
        return model_dump()
    raise TypeError('Estimate output must be a dataclass or dictionary')


def commercial_breakout_dict(
    estimate_output: EstimateOutput | dict[str, Any],
    target_margin_pct: float | None = None,
) -> dict[str, float]:
    output_data = _as_output_dict(estimate_output)
    chemical_costs = output_data.get('chemical_costs', {}) or {}
    service_costs = output_data.get('service_costs', {}) or {}
    margin_divisor = _margin_divisor(target_margin_pct if target_margin_pct is not None else output_data.get('target_margin_pct', 35.0))

    monthly_chemical_real_cost = float(
        output_data.get('monthly_chemical_real_cost')
        or sum(float(item.get('monthly_cost', 0.0) or 0.0) for item in chemical_costs.values())
    )
    annual_chemical_real_cost = float(
        output_data.get('annual_chemical_real_cost')
        or sum(float(item.get('annual_cost', 0.0) or 0.0) for item in chemical_costs.values())
    )
    monthly_service_real_cost = float(
        output_data.get('monthly_service_real_cost')
        or service_costs.get('monthly_service_cost')
        or max(0.0, float(output_data.get('monthly_real_cost', 0.0) or 0.0) - monthly_chemical_real_cost)
    )
    annual_service_real_cost = float(
        output_data.get('annual_service_real_cost')
        or service_costs.get('annual_service_cost')
        or max(0.0, float(output_data.get('annual_real_cost', 0.0) or 0.0) - annual_chemical_real_cost)
    )

    monthly_chemical_sell_price = float(
        output_data.get('monthly_chemical_sell_price') or monthly_chemical_real_cost / margin_divisor
    )
    annual_chemical_sell_price = float(
        output_data.get('annual_chemical_sell_price') or annual_chemical_real_cost / margin_divisor
    )
    monthly_service_sell_price = float(
        output_data.get('monthly_service_sell_price') or monthly_service_real_cost / margin_divisor
    )
    annual_service_sell_price = float(
        output_data.get('annual_service_sell_price') or annual_service_real_cost / margin_divisor
    )

    return {
        'monthly_chemical_real_cost': monthly_chemical_real_cost,
        'annual_chemical_real_cost': annual_chemical_real_cost,
        'monthly_chemical_sell_price': monthly_chemical_sell_price,
        'annual_chemical_sell_price': annual_chemical_sell_price,
        'monthly_service_real_cost': monthly_service_real_cost,
        'annual_service_real_cost': annual_service_real_cost,
        'monthly_service_sell_price': monthly_service_sell_price,
        'annual_service_sell_price': annual_service_sell_price,
    }
...
def calculate_estimate(
    ...
    labor_settings: LaborSettings | None = None,
) -> EstimateOutput:
    labor_settings = labor_settings or _stored_labor_settings(session)
    ...
    products = {
        product.sku.lower().replace('-', '_'): product
        for product in session.exec(select(ChemicalProduct).where(ChemicalProduct.is_active == True)).all()
    }
    alias_map = {
        ...
        'chem_clean_express_oz': 'chem_clean_exp',
    }

    chemical_costs: dict[str, dict[str, float | str]] = {}
    monthly_chemical_cost = 0.0
    annual_chemical_cost = 0.0
    margin_divisor = _margin_divisor(estimate_input.target_margin_pct)
    ...
    for chemical_name, values in chemical_quantities.items():
        ...
        annual_cost = float(values['annual_quantity']) * unit_cost
        monthly_cost = annual_cost / 12.0
        annual_sell_price = annual_cost / margin_divisor
        monthly_sell_price = annual_sell_price / 12.0
    ...
    monthly_service_real_cost = (direct_labor_cost_per_visit + overhead_cost_per_visit) * estimate_input.visits_per_month
    annual_service_real_cost = monthly_service_real_cost * 12.0
    monthly_service_sell_price = monthly_service_real_cost / margin_divisor
    annual_service_sell_price = annual_service_real_cost / margin_divisor
    ...
    service_costs = {
        ...
        'monthly_service_cost': monthly_service_real_cost,
        'annual_service_cost': annual_service_real_cost,
    }

    monthly_chemical_sell_price = monthly_chemical_cost / margin_divisor
    annual_chemical_sell_price = annual_chemical_cost / margin_divisor
    monthly_real_cost = monthly_chemical_cost + monthly_service_real_cost
    annual_real_cost = annual_chemical_cost + annual_service_real_cost
    monthly_sell_price = monthly_chemical_sell_price + monthly_service_sell_price
    annual_sell_price = annual_chemical_sell_price + annual_service_sell_price
    ...
    return EstimateOutput(
        ...
        monthly_chemical_real_cost=monthly_chemical_cost,
        annual_chemical_real_cost=annual_chemical_cost,
        monthly_chemical_sell_price=monthly_chemical_sell_price,
        annual_chemical_sell_price=annual_chemical_sell_price,
        monthly_service_real_cost=monthly_service_real_cost,
        annual_service_real_cost=annual_service_real_cost,
...
def save_estimate_run(
    ...
    scenario_id: int,
    estimate_input: EstimateInput,
    estimate_output: EstimateOutput,
) -> EstimateRun:
    record = EstimateRun(
