...
Phase 21 Step 7 continues the Phase 21 cleanup and consolidation phase.

It creates a no-write commit guard for the Phase 20 bridge routing network transport cleanup chain. It reads the latest Phase 21 Step 6 workspace hygiene packet and turns the current repo state into a commit guard that shows what to stage now, what must not be staged, what should be reviewed before staging, and what is already staged.
...
## Generate commit guard packet

From the platform repo, with FastAPI and bridge running:

```powershell
powershell -ExecutionPolicy Bypass -File scripts\phase21_generate_phase20_network_transport_commit_guard_packet.ps1
```

Output:

```text
C:\Users\krist\Desktop\unified_pool_service_platform_build\backups\phase21_phase20_network_transport_commit_guard_packet_<timestamp>
```
...
phase21_phase20_network_transport_commit_guard_packet_sections.csv
phase21_phase20_network_transport_commit_guard_packet_entries.csv
phase21_phase20_network_transport_commit_guard_packet_git_status.csv
phase21_phase20_network_transport_commit_guard_packet_staged.csv
phase21_phase20_network_transport_commit_guard_packet_checklist.csv
phase21_phase20_network_transport_commit_guard_packet_issues.csv
phase21_phase20_network_transport_commit_guard_packet.md
```
...
```text
Phase20 Network Transport Commit Guard Packet
```

## Commit guard

Stage only the four Phase 21 Step 7 files. Do not stage generated commit-guard files, local databases, `.env`, `.venv`, bridge folders, backups, evidence packs, import artifacts, or unrelated Replaster Quote files.
