...
- Phase: 23
- Step: 30
- Prior step: Phase 23 Step 29 - Phase 20 Network Transport Implementation Patch Assembly Planning Safety Disposition Planning Packet
...
Set-Location $Parent
powershell.exe -NoProfile -ExecutionPolicy Bypass -File (Join-Path $Parent "unified_pool_service_platform_build\scripts/phase23_step30_implementation_patch_assembly_planning_safety_disposition_review_packet.ps1") -Action all
```

## Expected smoke-test text

```text
SMOKE TEST PASS: Phase 23 Step 30 Implementation Patch Assembly Planning Safety Disposition Review Packet is present and planning-only.
```
...
PASS: phase23_planning_boundary=implementation_patch_assembly_planning_safety_disposition_review_opened_by_packet
...
PASS: implementation_patch_assembly_planning_safety_disposition_review_mode=reference_only
PASS: implementation_patch_assembly_planning_safety_disposition_review_write=false
PASS: implementation_patch_assembly_planning_safety_disposition_review_record_creation=false
...
CHECK: prior_step=Phase 23 Step 29 - Phase 20 Network Transport Implementation Patch Assembly Planning Safety Disposition Planning Packet
CHECK: phase23_context=implementation_patch_assembly_planning_safety_disposition_review_planning_only
