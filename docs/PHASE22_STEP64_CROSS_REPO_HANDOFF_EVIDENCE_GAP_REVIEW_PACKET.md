...
This packet adds a planning-only evidence gap review after the Step 63 cross-repo handoff evidence index packet.

It reviews the handoff evidence references for gaps without collecting evidence from sibling repositories, mutating any repository outside the platform working tree, starting Phase 23, or introducing network transport implementation work.

## Evidence gap review scope

- Review the Step 61 cross-repo handoff reference by name only.
- Review the Step 62 cross-repo handoff verification reference by name only.
...
- Keep the one-file installer flow using `INSTALL_PHASE22_STEP64_SINGLE_FILE.ps1`.
...
```text
SMOKE TEST PASS: Phase 22 Step 64 Phase 20 Network Transport Planning Cross-Repo Handoff Evidence Gap Review Packet is present and planning-only.
```
...
Set-Location $Parent
$Installer = Get-ChildItem "$env:USERPROFILE\Downloads" -Filter "INSTALL_PHASE22_STEP64_SINGLE_FILE*.ps1" |
...
Set-Location $Parent
python -m pytest (Join-Path $Repo "tests\test_phase22_step64_cross_repo_handoff_evidence_gap_review_packet.py") -q
git -C $Repo status
```
...
git -C $Repo commit -m "Phase 22 Step 64 - cross-repo handoff evidence gap review packet"
