...
st.set_page_config(page_title="Phase 20 Network Transport Consolidated Closeout Packet", page_icon="📚", layout="wide")
...
def latest_reports(limit: int = 20) -> list[Path]:
    ...
    return sorted(
        [
            p
            for p in BACKUP_DIR.glob("phase21_phase20_network_transport_consolidated_closeout_packet_*")
            if p.is_dir() and (p / "phase21_phase20_network_transport_consolidated_closeout_packet.json").exists()
        ],
...
st.title("📚 Phase 21 Step 2 - Phase 20 Network Transport Consolidated Closeout Packet")
st.caption("No-write closeout packet for the Phase 20 bridge routing network transport design chain.")

st.warning(
    "This page is Phase 20 network transport consolidated-closeout-only. It does not create design closure records, "
...
if not reports:
    st.info(
        "No Phase 20 network transport consolidated closeout packet found yet. Run "
        "`scripts\\phase21_generate_phase20_network_transport_consolidated_closeout_packet.ps1` from the platform repo."
    )
    st.stop()
...
json_path = report_dir / "phase21_phase20_network_transport_consolidated_closeout_packet.json"
buckets_csv_path = report_dir / "phase21_phase20_network_transport_consolidated_closeout_packet_buckets.csv"
artifacts_csv_path = report_dir / "phase21_phase20_network_transport_consolidated_closeout_packet_artifacts.csv"
source_csv_path = report_dir / "phase21_phase20_network_transport_consolidated_closeout_packet_source_audits.csv"
checklist_csv_path = report_dir / "phase21_phase20_network_transport_consolidated_closeout_packet_checklist.csv"
issues_csv_path = report_dir / "phase21_phase20_network_transport_consolidated_closeout_packet_issues.csv"
md_path = report_dir / "phase21_phase20_network_transport_consolidated_closeout_packet.md"

report = load_json(json_path)
safety = report.get("safety", {})
closeout = report.get("closeout_packet", {})
...
c1.metric("Closeout status", closeout.get("status", "unknown"))
c2.metric("Present artifacts", closeout.get("present_artifact_count", 0))
c3.metric("Review items", closeout.get("review_count", 0))
c4.metric("Implementation phase", "Not started" if not safety.get("implementation_phase_started") else "CHECK")

if (
    safety.get("phase20_network_transport_consolidated_closeout_packet_only")
    and safety.get("consolidated_closeout_packet_only")
    ...
    st.success("Safe consolidated closeout packet confirmed: no-write closeout only, bridge GET only, no approval, no implementation phase, no transport, no bridge POST, no socket.")
else:
    st.error("Review consolidated closeout packet safety flags before continuing.")

if closeout.get("status") == "phase20_closeout_packet_blocked":
    st.error("Consolidated closeout packet is blocked. Resolve blockers before creating additional closeout or cleanup artifacts.")
elif closeout.get("status") == "phase20_closeout_packet_review_required_no_write":
    st.warning("Consolidated closeout packet has review items. This still does not authorize approvals, implementation, or network transport.")
else:
    st.success("Consolidated closeout packet is ready for review. This still does not authorize approvals, implementation, or network transport.")

st.subheader("Consolidated closeout summary")
st.json(closeout)

st.subheader("Closeout buckets")
if buckets:
    st.dataframe(buckets, use_container_width=True, hide_index=True)
else:
    st.info("No bucket rows were recorded.")
...
if issues:
    ...
    st.success("No consolidated closeout issues were recorded.")
...
for label, path, filename in [
    ("Download buckets CSV", buckets_csv_path, "phase21_phase20_network_transport_consolidated_closeout_packet_buckets.csv"),
    ("Download artifacts CSV", artifacts_csv_path, "phase21_phase20_network_transport_consolidated_closeout_packet_artifacts.csv"),
    ("Download source audits CSV", source_csv_path, "phase21_phase20_network_transport_consolidated_closeout_packet_source_audits.csv"),
    ("Download checklist CSV", checklist_csv_path, "phase21_phase20_network_transport_consolidated_closeout_packet_checklist.csv"),
    ("Download issues CSV", issues_csv_path, "phase21_phase20_network_transport_consolidated_closeout_packet_issues.csv"),
]:
    if path.exists():
        st.download_button(label, path.read_text(encoding="utf-8-sig"), filename, "text/csv")

st.download_button(
    "Download consolidated closeout packet JSON",
    json.dumps(report, indent=2, default=str),
    "phase21_phase20_network_transport_consolidated_closeout_packet.json",
    "application/json",
)

with st.expander("Raw consolidated closeout packet JSON"):
    st.json(report)
