...
st.set_page_config(page_title="Bridge Routing Write Release Checkpoint", page_icon="🏁", layout="wide")
...
def latest_reports(limit: int = 20) -> list[Path]:
    ...
    return sorted(
        [
            p
            for p in BACKUP_DIR.glob("phase19_bridge_routing_write_release_checkpoint_*")
            if p.is_dir() and (p / "phase19_bridge_routing_write_release_checkpoint.json").exists()
        ],
...
st.title("🏁 Phase 19 Bridge Routing Write Release Checkpoint")
st.caption("No-POST release checkpoint across the full bridge-routing write evidence chain.")

st.warning(
    "This page is release-checkpoint-only. It does not write to the bridge, does not save platform records, "
...
if not reports:
    st.info(
        "No bridge routing write release checkpoint found yet. Run "
        "`scripts\\phase19_generate_bridge_routing_write_release_checkpoint.ps1` from the platform repo."
    )
    st.stop()
...
json_path = report_dir / "phase19_bridge_routing_write_release_checkpoint.json"
artifacts_csv_path = report_dir / "phase19_bridge_routing_write_release_checkpoint_artifacts.csv"
gates_csv_path = report_dir / "phase19_bridge_routing_write_release_checkpoint_gates.csv"
issues_csv_path = report_dir / "phase19_bridge_routing_write_release_checkpoint_issues.csv"
md_path = report_dir / "phase19_bridge_routing_write_release_checkpoint.md"
...
if (
    safety.get("bridge_routing_write_release_checkpoint_only")
    ...
    st.success("Safe release checkpoint confirmed: bridge GET only, no platform DB write, no bridge POST, no HTTP client.")
else:
    st.error("Review release checkpoint safety flags before continuing.")

if checkpoint.get("status") == "release_checkpoint_blocked":
    st.error("Release checkpoint is blocked. Resolve blockers before any future bridge HTTP client design.")
elif checkpoint.get("status") == "release_checkpoint_review_required_no_post":
    st.warning("Release checkpoint has review items. This still does not authorize execution.")
else:
    st.success("Release checkpoint is clean for review. This still does not authorize execution.")
...
for label, path, filename in [
    ("Download artifacts CSV", artifacts_csv_path, "phase19_bridge_routing_write_release_checkpoint_artifacts.csv"),
    ("Download gates CSV", gates_csv_path, "phase19_bridge_routing_write_release_checkpoint_gates.csv"),
    ("Download issues CSV", issues_csv_path, "phase19_bridge_routing_write_release_checkpoint_issues.csv"),
]:
    if path.exists():
...
with st.expander("Raw release checkpoint JSON"):
    st.json(report)
