...
st.set_page_config(page_title="Bridge Routing HTTP Client Preflight Matrix", page_icon="🧮", layout="wide")
...
def latest_reports(limit: int = 20) -> list[Path]:
    ...
    return sorted(
        [
            p
            for p in BACKUP_DIR.glob("phase20_bridge_routing_http_client_preflight_matrix_*")
            if p.is_dir() and (p / "phase20_bridge_routing_http_client_preflight_matrix.json").exists()
        ],
...
st.title("🧮 Phase 20 Bridge Routing HTTP Client Preflight Matrix")
st.caption("No-network preflight matrix for the HTTP client dry-run transport path.")

st.warning(
    "This page is HTTP-client-preflight-matrix-only. It does not write to the bridge, does not save platform records, "
...
if not reports:
    st.info(
        "No bridge routing HTTP client preflight matrix found yet. Run "
        "`scripts\\phase20_generate_bridge_routing_http_client_preflight_matrix.ps1` from the platform repo."
    )
    st.stop()

choice = st.selectbox(
    "HTTP client preflight matrix",
    options=list(range(len(reports))),
    format_func=lambda i: f"{reports[i].name} - {reports[i].stat().st_mtime_ns}",
)
report_dir = reports[choice]
json_path = report_dir / "phase20_bridge_routing_http_client_preflight_matrix.json"
gates_csv_path = report_dir / "phase20_bridge_routing_http_client_preflight_matrix_global_gates.csv"
rows_csv_path = report_dir / "phase20_bridge_routing_http_client_preflight_matrix_rows.csv"
issues_csv_path = report_dir / "phase20_bridge_routing_http_client_preflight_matrix_issues.csv"
md_path = report_dir / "phase20_bridge_routing_http_client_preflight_matrix.md"
...
if (
    safety.get("bridge_routing_http_client_preflight_matrix_only")
    ...
    st.success("Safe preflight matrix confirmed: bridge GET only, no platform DB write, no bridge POST, no network transport.")
else:
    st.error("Review HTTP client preflight safety flags before continuing.")

if preflight.get("status") == "blocked":
    st.error("HTTP client preflight matrix is blocked. Resolve blockers before future network transport design.")
elif preflight.get("status") == "preflight_valid_with_review_items":
    st.warning("HTTP client preflight matrix has review items. This still does not authorize execution.")
else:
    st.success("HTTP client preflight matrix is valid for design review. This still does not authorize execution.")
...
for label, path, filename in [
    ("Download global gates CSV", gates_csv_path, "phase20_bridge_routing_http_client_preflight_matrix_global_gates.csv"),
    ("Download transport rows CSV", rows_csv_path, "phase20_bridge_routing_http_client_preflight_matrix_rows.csv"),
    ("Download issues CSV", issues_csv_path, "phase20_bridge_routing_http_client_preflight_matrix_issues.csv"),
]:
    if path.exists():
        st.download_button(label, path.read_text(encoding="utf-8-sig"), filename, "text/csv")

st.download_button(
    "Download HTTP client preflight matrix JSON",
    json.dumps(report, indent=2, default=str),
    "phase20_bridge_routing_http_client_preflight_matrix.json",
    "application/json",
)

with st.expander("Raw HTTP client preflight matrix JSON"):
    st.json(report)
