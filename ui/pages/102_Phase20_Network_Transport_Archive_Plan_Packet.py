...
st.set_page_config(page_title="Phase 20 Network Transport Archive Plan Packet", page_icon="🗂", layout="wide")
...
def latest_reports(limit: int = 20) -> list[Path]:
    ...
    return sorted(
        [
            p
            for p in BACKUP_DIR.glob("phase21_phase20_network_transport_archive_plan_packet_*")
            if p.is_dir() and (p / "phase21_phase20_network_transport_archive_plan_packet.json").exists()
        ],
...
st.title("🗂 Phase 21 Step 10 - Phase 20 Network Transport Archive Plan Packet")
st.caption("No-write archive plan packet for the Phase 20 bridge routing network transport cleanup chain.")

st.warning(
    "This page is Phase 20 network transport archive-plan-only. It does not create design closure records, "
...
if not reports:
    st.info(
        "No Phase 20 network transport archive plan packet found yet. Run "
        "`scripts\\phase21_generate_phase20_network_transport_archive_plan_packet.ps1` from the platform repo."
    )
    st.stop()
...
json_path = report_dir / "phase21_phase20_network_transport_archive_plan_packet.json"
sections_csv_path = report_dir / "phase21_phase20_network_transport_archive_plan_packet_sections.csv"
entries_csv_path = report_dir / "phase21_phase20_network_transport_archive_plan_packet_entries.csv"
families_csv_path = report_dir / "phase21_phase20_network_transport_archive_plan_packet_artifact_families.csv"
git_status_csv_path = report_dir / "phase21_phase20_network_transport_archive_plan_packet_git_status.csv"
staged_csv_path = report_dir / "phase21_phase20_network_transport_archive_plan_packet_staged.csv"
source_artifacts_csv_path = report_dir / "phase21_phase20_network_transport_archive_plan_packet_source_artifacts.csv"
checklist_csv_path = report_dir / "phase21_phase20_network_transport_archive_plan_packet_checklist.csv"
issues_csv_path = report_dir / "phase21_phase20_network_transport_archive_plan_packet_issues.csv"
md_path = report_dir / "phase21_phase20_network_transport_archive_plan_packet.md"

report = load_json(json_path)
safety = report.get("safety", {})
packet = report.get("archive_plan_packet", {})
...
c1.metric("Archive plan status", packet.get("status", "unknown"))
...
if (
    safety.get("phase20_network_transport_archive_plan_packet_only")
    and safety.get("archive_plan_only")
    ...
    st.success("Safe archive plan confirmed: no-write archive review only, bridge GET only, no approval, no implementation phase, no transport, no bridge POST, no socket.")
else:
    st.error("Review archive-plan safety flags before continuing.")

if packet.get("status") == "phase20_archive_plan_packet_blocked":
    st.error("Archive plan packet is blocked. Resolve blockers before further cleanup work.")
elif packet.get("status") == "phase20_archive_plan_packet_review_required_no_write":
    st.warning("Archive plan packet has review items. This still does not authorize approvals, implementation, or network transport.")
else:
    st.success("Archive plan packet is ready for review. This still does not authorize approvals, implementation, or network transport.")

st.subheader("Archive plan summary")
st.json(packet)
...
if issues:
    ...
    st.success("No archive-plan issues were recorded.")
...
for label, path, filename in [
    ("Download sections CSV", sections_csv_path, "phase21_phase20_network_transport_archive_plan_packet_sections.csv"),
    ("Download entries CSV", entries_csv_path, "phase21_phase20_network_transport_archive_plan_packet_entries.csv"),
    ("Download artifact families CSV", families_csv_path, "phase21_phase20_network_transport_archive_plan_packet_artifact_families.csv"),
    ("Download git status CSV", git_status_csv_path, "phase21_phase20_network_transport_archive_plan_packet_git_status.csv"),
    ("Download staged CSV", staged_csv_path, "phase21_phase20_network_transport_archive_plan_packet_staged.csv"),
    ("Download source artifacts CSV", source_artifacts_csv_path, "phase21_phase20_network_transport_archive_plan_packet_source_artifacts.csv"),
    ("Download checklist CSV", checklist_csv_path, "phase21_phase20_network_transport_archive_plan_packet_checklist.csv"),
    ("Download issues CSV", issues_csv_path, "phase21_phase20_network_transport_archive_plan_packet_issues.csv"),
]:
    if path.exists():
        st.download_button(label, path.read_text(encoding="utf-8-sig"), filename, "text/csv")

st.download_button(
    "Download archive plan packet JSON",
    json.dumps(report, indent=2, default=str),
    "phase21_phase20_network_transport_archive_plan_packet.json",
    "application/json",
)

with st.expander("Raw archive plan packet JSON"):
    st.json(report)
