...
st.set_page_config(page_title="Phase 20 Network Transport Cleanup Completion Packet", page_icon="✅", layout="wide")
...
def latest_reports(limit: int = 20) -> list[Path]:
    ...
    return sorted(
        [
            p
            for p in BACKUP_DIR.glob("phase21_phase20_network_transport_cleanup_completion_packet_*")
            if p.is_dir() and (p / "phase21_phase20_network_transport_cleanup_completion_packet.json").exists()
        ],
...
st.title("✅ Phase 21 Step 12 - Phase 20 Network Transport Cleanup Completion Packet")
st.caption("No-write cleanup completion packet for the Phase 20 bridge routing network transport cleanup chain.")

st.warning(
    "This page is Phase 20 network transport cleanup-completion-only. It does not create design closure records, "
...
if not reports:
    st.info(
        "No Phase 20 network transport cleanup completion packet found yet. Run "
        "`scripts\\phase21_generate_phase20_network_transport_cleanup_completion_packet.ps1` from the platform repo."
    )
    st.stop()
...
json_path = report_dir / "phase21_phase20_network_transport_cleanup_completion_packet.json"
sections_csv_path = report_dir / "phase21_phase20_network_transport_cleanup_completion_packet_sections.csv"
entries_csv_path = report_dir / "phase21_phase20_network_transport_cleanup_completion_packet_entries.csv"
decisions_csv_path = report_dir / "phase21_phase20_network_transport_cleanup_completion_packet_decisions.csv"
families_csv_path = report_dir / "phase21_phase20_network_transport_cleanup_completion_packet_artifact_families.csv"
git_status_csv_path = report_dir / "phase21_phase20_network_transport_cleanup_completion_packet_git_status.csv"
staged_csv_path = report_dir / "phase21_phase20_network_transport_cleanup_completion_packet_staged.csv"
source_artifacts_csv_path = report_dir / "phase21_phase20_network_transport_cleanup_completion_packet_source_artifacts.csv"
checklist_csv_path = report_dir / "phase21_phase20_network_transport_cleanup_completion_packet_checklist.csv"
issues_csv_path = report_dir / "phase21_phase20_network_transport_cleanup_completion_packet_issues.csv"
md_path = report_dir / "phase21_phase20_network_transport_cleanup_completion_packet.md"

report = load_json(json_path)
safety = report.get("safety", {})
packet = report.get("cleanup_completion_packet", {})
sections = report.get("completion_sections", [])
entries = report.get("completion_entries", [])
...
completion_commands = report.get("completion_commands", [])

c1, c2, c3, c4 = st.columns(4)
c1.metric("Cleanup completion status", packet.get("status", "unknown"))
...
if (
    safety.get("phase20_network_transport_cleanup_completion_packet_only")
    and safety.get("cleanup_completion_only")
    ...
    st.success("Safe cleanup completion confirmed: no-write cleanup review only, bridge GET only, no approval, no implementation phase, no transport, no bridge POST, no socket.")
else:
    st.error("Review cleanup-completion safety flags before continuing.")

if packet.get("status") == "phase20_cleanup_completion_packet_blocked":
    st.error("Cleanup completion packet is blocked. Resolve blockers before further cleanup work.")
elif packet.get("status") == "phase20_cleanup_completion_packet_review_required_no_write":
    st.warning("Cleanup completion packet has review items. This still does not authorize approvals, implementation, or network transport.")
else:
    st.success("Cleanup completion packet is ready for review. This still does not authorize approvals, implementation, or network transport.")

st.subheader("Cleanup completion summary")
st.json(packet)

st.subheader("Completion sections")
if sections:
    st.dataframe(sections, use_container_width=True, hide_index=True)
else:
    st.info("No completion sections were recorded.")

st.subheader("Completion entries")
if entries:
    ...
    st.info("No completion entries were recorded.")
...
if completion_commands:
    st.code("\n".join(completion_commands), language="powershell")
...
if issues:
    ...
    st.success("No cleanup-completion issues were recorded.")
...
for label, path, filename in [
    ("Download sections CSV", sections_csv_path, "phase21_phase20_network_transport_cleanup_completion_packet_sections.csv"),
    ("Download entries CSV", entries_csv_path, "phase21_phase20_network_transport_cleanup_completion_packet_entries.csv"),
    ("Download decisions CSV", decisions_csv_path, "phase21_phase20_network_transport_cleanup_completion_packet_decisions.csv"),
    ("Download artifact families CSV", families_csv_path, "phase21_phase20_network_transport_cleanup_completion_packet_artifact_families.csv"),
    ("Download git status CSV", git_status_csv_path, "phase21_phase20_network_transport_cleanup_completion_packet_git_status.csv"),
    ("Download staged CSV", staged_csv_path, "phase21_phase20_network_transport_cleanup_completion_packet_staged.csv"),
    ("Download source artifacts CSV", source_artifacts_csv_path, "phase21_phase20_network_transport_cleanup_completion_packet_source_artifacts.csv"),
    ("Download checklist CSV", checklist_csv_path, "phase21_phase20_network_transport_cleanup_completion_packet_checklist.csv"),
    ("Download issues CSV", issues_csv_path, "phase21_phase20_network_transport_cleanup_completion_packet_issues.csv"),
]:
    if path.exists():
        st.download_button(label, path.read_text(encoding="utf-8-sig"), filename, "text/csv")

st.download_button(
    "Download cleanup completion packet JSON",
    json.dumps(report, indent=2, default=str),
    "phase21_phase20_network_transport_cleanup_completion_packet.json",
    "application/json",
)

with st.expander("Raw cleanup completion packet JSON"):
    st.json(report)
