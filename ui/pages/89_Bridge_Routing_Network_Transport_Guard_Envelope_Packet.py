...
st.set_page_config(page_title="Bridge Routing Network Transport Guard Envelope Packet", page_icon="🛡️", layout="wide")
...
def latest_reports(limit: int = 20) -> list[Path]:
    ...
    return sorted(
        [
            p
            for p in BACKUP_DIR.glob("phase20_bridge_routing_network_transport_guard_envelope_packet_*")
            if p.is_dir() and (p / "phase20_bridge_routing_network_transport_guard_envelope_packet.json").exists()
        ],
...
st.title("🛡️ Phase 20 Bridge Routing Network Transport Guard Envelope Packet")
st.caption("Preview-only guard envelope packet for future bridge routing network transport work.")

st.warning(
    "This page is network-transport-guard-envelope-packet-only. It shows guard-envelope evidence. "
...
if not reports:
    st.info(
        "No bridge routing network transport guard envelope packet found yet. Run "
        "`scripts\\phase20_generate_bridge_routing_network_transport_guard_envelope_packet.ps1` from the platform repo."
    )
    st.stop()
...
json_path = report_dir / "phase20_bridge_routing_network_transport_guard_envelope_packet.json"
methods_csv_path = report_dir / "phase20_bridge_routing_network_transport_guard_envelope_packet_methods.csv"
checklist_csv_path = report_dir / "phase20_bridge_routing_network_transport_guard_envelope_packet_checklist.csv"
artifacts_csv_path = report_dir / "phase20_bridge_routing_network_transport_guard_envelope_packet_artifacts.csv"
issues_csv_path = report_dir / "phase20_bridge_routing_network_transport_guard_envelope_packet_issues.csv"
md_path = report_dir / "phase20_bridge_routing_network_transport_guard_envelope_packet.md"

report = load_json(json_path)
safety = report.get("safety", {})
packet = report.get("guard_envelope_packet", {})
methods = report.get("guard_methods", [])
...
c2.metric("Guard methods", packet.get("guard_method_count", 0))
...
if (
    safety.get("bridge_routing_network_transport_guard_envelope_packet_only")
    and safety.get("guard_envelope_only")
    and safety.get("guard_preview_created")
    ...
    st.success("Safe guard envelope confirmed: preview-only, bridge GET only, no execution implementation, no transport, no bridge POST, no socket.")
else:
    st.error("Review guard envelope packet safety flags before continuing.")

if packet.get("status") == "guard_envelope_packet_blocked":
    st.error("Guard envelope packet is blocked. Resolve blockers before future readiness report design.")
elif packet.get("status") == "guard_envelope_packet_review_required_no_write":
    st.warning("Guard envelope packet has review items. This still does not authorize execution implementation or network transport.")
else:
    st.success("Guard envelope packet is ready for review. This still does not authorize execution implementation or network transport.")

st.subheader("Guard envelope packet summary")
st.json(packet)

st.subheader("Guard methods")
if methods:
    st.dataframe(methods, use_container_width=True, hide_index=True)
else:
    st.info("No guard methods were recorded.")
...
if issues:
    ...
    st.success("No guard envelope packet issues were recorded.")
...
for label, path, filename in [
    ("Download methods CSV", methods_csv_path, "phase20_bridge_routing_network_transport_guard_envelope_packet_methods.csv"),
    ("Download checklist CSV", checklist_csv_path, "phase20_bridge_routing_network_transport_guard_envelope_packet_checklist.csv"),
    ("Download artifacts CSV", artifacts_csv_path, "phase20_bridge_routing_network_transport_guard_envelope_packet_artifacts.csv"),
    ("Download issues CSV", issues_csv_path, "phase20_bridge_routing_network_transport_guard_envelope_packet_issues.csv"),
]:
    if path.exists():
        st.download_button(label, path.read_text(encoding="utf-8-sig"), filename, "text/csv")

st.download_button(
    "Download guard envelope packet JSON",
    json.dumps(report, indent=2, default=str),
    "phase20_bridge_routing_network_transport_guard_envelope_packet.json",
    "application/json",
)

with st.expander("Raw guard envelope packet JSON"):
    st.json(report)
