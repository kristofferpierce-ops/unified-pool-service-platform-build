...
st.set_page_config(page_title="Phase 20 Network Transport Commit Guard Packet", page_icon="🛡️", layout="wide")
...
def latest_reports(limit: int = 20) -> list[Path]:
    ...
    return sorted(
        [
            p
            for p in BACKUP_DIR.glob("phase21_phase20_network_transport_commit_guard_packet_*")
            if p.is_dir() and (p / "phase21_phase20_network_transport_commit_guard_packet.json").exists()
        ],
...
st.title("🛡️ Phase 21 Step 7 - Phase 20 Network Transport Commit Guard Packet")
st.caption("No-write commit guard for the Phase 20 bridge routing network transport cleanup chain.")

st.warning(
    "This page is Phase 20 network transport commit-guard-only. It does not create design closure records, "
...
if not reports:
    st.info(
        "No Phase 20 network transport commit guard packet found yet. Run "
        "`scripts\\phase21_generate_phase20_network_transport_commit_guard_packet.ps1` from the platform repo."
    )
    st.stop()
...
json_path = report_dir / "phase21_phase20_network_transport_commit_guard_packet.json"
sections_csv_path = report_dir / "phase21_phase20_network_transport_commit_guard_packet_sections.csv"
entries_csv_path = report_dir / "phase21_phase20_network_transport_commit_guard_packet_entries.csv"
git_status_csv_path = report_dir / "phase21_phase20_network_transport_commit_guard_packet_git_status.csv"
staged_csv_path = report_dir / "phase21_phase20_network_transport_commit_guard_packet_staged.csv"
checklist_csv_path = report_dir / "phase21_phase20_network_transport_commit_guard_packet_checklist.csv"
issues_csv_path = report_dir / "phase21_phase20_network_transport_commit_guard_packet_issues.csv"
md_path = report_dir / "phase21_phase20_network_transport_commit_guard_packet.md"

report = load_json(json_path)
safety = report.get("safety", {})
packet = report.get("commit_guard_packet", {})
sections = report.get("commit_guard_sections", [])
entries = report.get("commit_guard_entries", [])
git_status_entries = report.get("git_status_entries", [])
staged_entries = report.get("staged_entries", [])
checklist = report.get("checklist", [])
issues = report.get("issues", [])
commit_commands = report.get("commit_commands", [])

c1, c2, c3, c4 = st.columns(4)
c1.metric("Guard status", packet.get("status", "unknown"))
c2.metric("Stage-now entries", len([row for row in entries if str(row.get("category")) == "stage_now"]))
c3.metric("Review items", packet.get("review_count", 0))
c4.metric("Staged forbidden", packet.get("staged_forbidden_count", 0))

if (
    safety.get("phase20_network_transport_commit_guard_packet_only")
    and safety.get("commit_guard_only")
    ...
    st.success("Safe commit guard confirmed: no-write guard only, bridge GET only, no approval, no implementation phase, no transport, no bridge POST, no socket.")
else:
    st.error("Review commit-guard safety flags before continuing.")

if packet.get("status") == "phase20_commit_guard_packet_blocked":
    st.error("Commit guard packet is blocked. Resolve blockers before staging files.")
elif packet.get("status") == "phase20_commit_guard_packet_review_required_no_write":
    st.warning("Commit guard packet has review items. This still does not authorize approvals, implementation, or network transport.")
else:
    st.success("Commit guard packet is ready for review. This still does not authorize approvals, implementation, or network transport.")

st.subheader("Commit guard summary")
st.json(packet)

st.subheader("Commit guard sections")
if sections:
    st.dataframe(sections, use_container_width=True, hide_index=True)
else:
    st.info("No commit guard sections were recorded.")

st.subheader("Commit guard entries")
if entries:
    ...
    st.info("No commit guard entries were recorded.")
...
if commit_commands:
    st.code("\n".join(commit_commands), language="powershell")
...
if issues:
    ...
    st.success("No commit-guard issues were recorded.")
...
for label, path, filename in [
    ("Download sections CSV", sections_csv_path, "phase21_phase20_network_transport_commit_guard_packet_sections.csv"),
    ("Download entries CSV", entries_csv_path, "phase21_phase20_network_transport_commit_guard_packet_entries.csv"),
    ("Download git status CSV", git_status_csv_path, "phase21_phase20_network_transport_commit_guard_packet_git_status.csv"),
    ("Download staged CSV", staged_csv_path, "phase21_phase20_network_transport_commit_guard_packet_staged.csv"),
    ("Download checklist CSV", checklist_csv_path, "phase21_phase20_network_transport_commit_guard_packet_checklist.csv"),
    ("Download issues CSV", issues_csv_path, "phase21_phase20_network_transport_commit_guard_packet_issues.csv"),
]:
    if path.exists():
        st.download_button(label, path.read_text(encoding="utf-8-sig"), filename, "text/csv")

st.download_button(
    "Download commit guard packet JSON",
    json.dumps(report, indent=2, default=str),
    "phase21_phase20_network_transport_commit_guard_packet.json",
    "application/json",
)

with st.expander("Raw commit guard packet JSON"):
    st.json(report)
