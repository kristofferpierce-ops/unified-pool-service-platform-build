...
st.set_page_config(page_title="Bridge Routing Write Operator Signoff", page_icon="🖊️", layout="wide")
...
def latest_reports(limit: int = 20) -> list[Path]:
    ...
    return sorted(
        [
            p
            for p in BACKUP_DIR.glob("phase19_bridge_routing_write_operator_signoff_*")
            if p.is_dir() and (p / "phase19_bridge_routing_write_operator_signoff.json").exists()
        ],
...
st.title("🖊️ Phase 19 Bridge Routing Write Operator Signoff")
st.caption("Operator signoff dossier for future design review. This does not authorize bridge writes.")

st.warning(
    "This page is operator-signoff-only. It does not write to the bridge, does not save platform records, "
...
if not reports:
    st.info(
        "No bridge routing write operator signoff dossier found yet. Run "
        "`scripts\\phase19_generate_bridge_routing_write_operator_signoff.ps1` from the platform repo."
    )
    st.stop()
...
json_path = report_dir / "phase19_bridge_routing_write_operator_signoff.json"
checklist_csv_path = report_dir / "phase19_bridge_routing_write_operator_signoff_checklist.csv"
issues_csv_path = report_dir / "phase19_bridge_routing_write_operator_signoff_issues.csv"
md_path = report_dir / "phase19_bridge_routing_write_operator_signoff.md"
...
if (
    safety.get("bridge_routing_write_operator_signoff_only")
    ...
    st.success("Safe operator signoff confirmed: bridge GET only, no platform DB write, no bridge POST, no HTTP client.")
else:
    st.error("Review operator signoff safety flags before continuing.")

if signoff.get("status") == "blocked":
    st.error("Operator signoff dossier is blocked. Resolve blockers before any future bridge HTTP client design.")
    ...
    st.success("Dossier is ready for operator design review. This still does not authorize execution.")
...
for label, path, filename in [
    ("Download signoff checklist CSV", checklist_csv_path, "phase19_bridge_routing_write_operator_signoff_checklist.csv"),
    ("Download signoff issues CSV", issues_csv_path, "phase19_bridge_routing_write_operator_signoff_issues.csv"),
]:
    if path.exists():
        st.download_button(label, path.read_text(encoding="utf-8-sig"), filename, "text/csv")

st.download_button(
    "Download operator signoff JSON",
    json.dumps(report, indent=2, default=str),
    "phase19_bridge_routing_write_operator_signoff.json",
    "application/json",
)

with st.expander("Raw operator signoff JSON"):
    st.json(report)
