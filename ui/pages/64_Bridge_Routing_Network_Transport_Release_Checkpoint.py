...
st.set_page_config(page_title="Bridge Routing Network Transport Release Checkpoint", page_icon="🏁", layout="wide")
...
def latest_reports(limit: int = 20) -> list[Path]:
    ...
    return sorted(
        [
            p
            for p in BACKUP_DIR.glob("phase20_bridge_routing_network_transport_release_checkpoint_*")
            if p.is_dir() and (p / "phase20_bridge_routing_network_transport_release_checkpoint.json").exists()
        ],
...
st.title("🏁 Phase 20 Bridge Routing Network Transport Release Checkpoint")
st.caption("No-socket release checkpoint across the Phase 20 network-transport evidence chain.")

st.warning(
    "This page is network-transport-release-checkpoint-only. It does not write to the bridge, does not save platform records, "
...
if not reports:
    st.info(
        "No bridge routing network transport release checkpoint found yet. Run "
        "`scripts\\phase20_generate_bridge_routing_network_transport_release_checkpoint.ps1` from the platform repo."
    )
    st.stop()

choice = st.selectbox(
    "Network transport release checkpoint",
    options=list(range(len(reports))),
    format_func=lambda i: f"{reports[i].name} - {reports[i].stat().st_mtime_ns}",
)
report_dir = reports[choice]
json_path = report_dir / "phase20_bridge_routing_network_transport_release_checkpoint.json"
artifacts_csv_path = report_dir / "phase20_bridge_routing_network_transport_release_checkpoint_artifacts.csv"
gates_csv_path = report_dir / "phase20_bridge_routing_network_transport_release_checkpoint_gates.csv"
issues_csv_path = report_dir / "phase20_bridge_routing_network_transport_release_checkpoint_issues.csv"
md_path = report_dir / "phase20_bridge_routing_network_transport_release_checkpoint.md"
...
if (
    safety.get("bridge_routing_network_transport_release_checkpoint_only")
    ...
    st.error("Review network transport release checkpoint safety flags before continuing.")

if checkpoint.get("status") == "release_checkpoint_blocked":
    st.error("Network transport release checkpoint is blocked. Resolve blockers before future real transport design.")
elif checkpoint.get("status") == "release_checkpoint_review_required_no_socket":
    st.warning("Network transport release checkpoint has review items. This still does not authorize execution.")
else:
    st.success("Network transport release checkpoint is clean for review. This still does not authorize execution.")
...
for label, path, filename in [
    ("Download artifacts CSV", artifacts_csv_path, "phase20_bridge_routing_network_transport_release_checkpoint_artifacts.csv"),
    ("Download gates CSV", gates_csv_path, "phase20_bridge_routing_network_transport_release_checkpoint_gates.csv"),
    ("Download issues CSV", issues_csv_path, "phase20_bridge_routing_network_transport_release_checkpoint_issues.csv"),
]:
    if path.exists():
        st.download_button(label, path.read_text(encoding="utf-8-sig"), filename, "text/csv")

st.download_button(
    "Download network transport release checkpoint JSON",
    json.dumps(report, indent=2, default=str),
    "phase20_bridge_routing_network_transport_release_checkpoint.json",
    "application/json",
)

with st.expander("Raw network transport release checkpoint JSON"):
    st.json(report)
