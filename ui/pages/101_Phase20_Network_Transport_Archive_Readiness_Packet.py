...
st.set_page_config(page_title="Phase 20 Network Transport Archive Readiness Packet", page_icon="🗃️", layout="wide")
...
def latest_reports(limit: int = 20) -> list[Path]:
    ...
    return sorted(
        [
            p
            for p in BACKUP_DIR.glob("phase21_phase20_network_transport_archive_readiness_packet_*")
            if p.is_dir() and (p / "phase21_phase20_network_transport_archive_readiness_packet.json").exists()
        ],
...
st.title("🗃️ Phase 21 Step 9 - Phase 20 Network Transport Archive Readiness Packet")
st.caption("No-write archive readiness packet for the Phase 20 bridge routing network transport cleanup chain.")

st.warning(
    "This page is Phase 20 network transport archive-readiness-only. It does not create design closure records, "
...
if not reports:
    st.info(
        "No Phase 20 network transport archive readiness packet found yet. Run "
        "`scripts\\phase21_generate_phase20_network_transport_archive_readiness_packet.ps1` from the platform repo."
    )
    st.stop()
...
json_path = report_dir / "phase21_phase20_network_transport_archive_readiness_packet.json"
sections_csv_path = report_dir / "phase21_phase20_network_transport_archive_readiness_packet_sections.csv"
entries_csv_path = report_dir / "phase21_phase20_network_transport_archive_readiness_packet_entries.csv"
families_csv_path = report_dir / "phase21_phase20_network_transport_archive_readiness_packet_artifact_families.csv"
git_status_csv_path = report_dir / "phase21_phase20_network_transport_archive_readiness_packet_git_status.csv"
staged_csv_path = report_dir / "phase21_phase20_network_transport_archive_readiness_packet_staged.csv"
source_artifacts_csv_path = report_dir / "phase21_phase20_network_transport_archive_readiness_packet_source_artifacts.csv"
checklist_csv_path = report_dir / "phase21_phase20_network_transport_archive_readiness_packet_checklist.csv"
issues_csv_path = report_dir / "phase21_phase20_network_transport_archive_readiness_packet_issues.csv"
md_path = report_dir / "phase21_phase20_network_transport_archive_readiness_packet.md"

report = load_json(json_path)
safety = report.get("safety", {})
packet = report.get("archive_readiness_packet", {})
sections = report.get("archive_sections", [])
...
c1.metric("Archive status", packet.get("status", "unknown"))
c2.metric("Archive candidate families", packet.get("archive_candidate_family_count", 0))
c3.metric("Review items", packet.get("review_count", 0))
c4.metric("Staged forbidden", packet.get("staged_forbidden_count", 0))

if (
    safety.get("phase20_network_transport_archive_readiness_packet_only")
    and safety.get("archive_readiness_only")
    ...
    st.success("Safe archive readiness confirmed: no-write archive review only, bridge GET only, no approval, no implementation phase, no transport, no bridge POST, no socket.")
else:
    st.error("Review archive-readiness safety flags before continuing.")

if packet.get("status") == "phase20_archive_readiness_packet_blocked":
    st.error("Archive readiness packet is blocked. Resolve blockers before further cleanup work.")
elif packet.get("status") == "phase20_archive_readiness_packet_review_required_no_write":
    st.warning("Archive readiness packet has review items. This still does not authorize approvals, implementation, or network transport.")
else:
    st.success("Archive readiness packet is ready for review. This still does not authorize approvals, implementation, or network transport.")

st.subheader("Archive readiness summary")
st.json(packet)
...
if sections:
    ...
    st.info("No archive sections were recorded.")
...
if entries:
    ...
    st.info("No archive entries were recorded.")
...
if issues:
    ...
    st.success("No archive-readiness issues were recorded.")
...
for label, path, filename in [
    ("Download sections CSV", sections_csv_path, "phase21_phase20_network_transport_archive_readiness_packet_sections.csv"),
    ("Download entries CSV", entries_csv_path, "phase21_phase20_network_transport_archive_readiness_packet_entries.csv"),
    ("Download artifact families CSV", families_csv_path, "phase21_phase20_network_transport_archive_readiness_packet_artifact_families.csv"),
    ("Download git status CSV", git_status_csv_path, "phase21_phase20_network_transport_archive_readiness_packet_git_status.csv"),
    ("Download staged CSV", staged_csv_path, "phase21_phase20_network_transport_archive_readiness_packet_staged.csv"),
    ("Download source artifacts CSV", source_artifacts_csv_path, "phase21_phase20_network_transport_archive_readiness_packet_source_artifacts.csv"),
    ("Download checklist CSV", checklist_csv_path, "phase21_phase20_network_transport_archive_readiness_packet_checklist.csv"),
    ("Download issues CSV", issues_csv_path, "phase21_phase20_network_transport_archive_readiness_packet_issues.csv"),
]:
    if path.exists():
        st.download_button(label, path.read_text(encoding="utf-8-sig"), filename, "text/csv")

st.download_button(
    "Download archive readiness packet JSON",
    json.dumps(report, indent=2, default=str),
    "phase21_phase20_network_transport_archive_readiness_packet.json",
    "application/json",
)

with st.expander("Raw archive readiness packet JSON"):
    st.json(report)
