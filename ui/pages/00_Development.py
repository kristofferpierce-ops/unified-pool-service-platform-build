...
DEFAULT_LOG_ENTRIES = [
    ...
    ('2026.08.01-c', 'shipped', 'Purchasing', 'Distributor invoice importer: portal paste -> per-vendor price ledger',
     'app/services/invoice_import.py ingests a Heritage Pool Supply online Invoice-History copy-paste (one paste = many invoices) into the product price ledger, so historical invoices backfill with NO API key (the LLM general-extractor for other vendors/PDFs plugs in later behind the same ParsedInvoice contract). parse_heritage_paste splits on Invoice #, anchors on stable markers (ITEM #/MFG #, Invoice Summary, $x /UOM) not exact spacing, and self-checks arithmetic (qty x unit == ext) + totals. Products are keyed on the MANUFACTURER PART NUMBER (MFG #) -- the cross-vendor key, so a Jandy FHPM1.0 from Heritage or Key West Chemical is ONE product with comparable prices; ChemicalProduct reused as the catalog. Credit memos / returns (invoice ...CM or negative qty/price) are recorded but EXCLUDED from pricing (a -$1,900 pump is not a deal). Idempotent per (vendor, invoice #); wires the Tier 2 vendor spine. Feeds the existing best_source/anomaly purchasing engine. 7 tests on real invoice shapes incl MFG # 0, /FT units, ITEM #-with-no-MFG, multi-line, service-prefix (176 -> 183). Next: LLM general extractor + Key West Chemical (chemicals need pack-size $/unit normalization) + cross-vendor item/invoice search.'),
    ('2026.08.01-b', 'shipped', 'QuickBooks', 'Tier 2 vendor identity spine (357 vendors) + no-loss vault fix',
     'Vendor/VendorProfile/VendorMatch (app/models/vendor_tables.py) + resolve_vendor (app/services/vendor_matching.py) mirror the customer spine that unified 447 customers -- built parallel, live customer code untouched. Tax-id is Pass 1 (keyed-hash match, never plaintext); name-only matches FLAG as possible duplicates, never auto-merge (VENDOR_STRONG_NAME_AUTOLINKS=False) because the real data has genuine >=0.90 false-positive pairs (City of Key West vs Kia of Key West). VendorListTranslator (quickbooks/vendors.py) ingests !VEND with a bank-artifact denylist. Validated on the real 357 vendors: 350 created, 7 excluded (OUTGOING WIRE etc.), 0 orphaned, 91 flagged for optional review, false-positive lookalikes kept distinct, idempotent, 0 PII leak. Design pass (4-agent workflow) caught a REAL no-loss bug: base.py declared the RawSourceCell PII vault but never wrote it -- a sensitive value would have been silently dropped; now cipher_value (stdlib encrypt-then-MAC, app/core/secrets.py) + keyed value_hash are written. Additive nullable vendor_id FKs on ProductPriceHistory/ChemicalProduct/AssetRecord/CommercialVendorOrder + a generic additive-column migration in create_db_and_tables (self-heals nullable columns added to shipped models; real DB migrated). SCOPE: item/material unit costs NOT in this QB file (0 items) -- deferred to Skimmer/Tier 3. 21 tests (154 -> 175).'),
    ('2026.08.01-a', 'shipped', 'QuickBooks', 'Gated apply: real QB cost base -> live cost-per-hour engine',
     'apply_quickbooks_costing (quickbooks_costing.py) is the owner-signed-off apply: it replaces the seeded ExpenseItem overhead spine with the real QB overhead + depreciation accounts (QB is the authoritative cost base), sets LaborSettings from QB labor (already fully burdened -> 0 extra burden; the "wage" is loaded labor cost per billable hour) + the owner billable-hours capacity, and records an ApplyEvent for audit/reversibility. COGS/materials excluded (attribute per-job, Tier 2/3). Idempotent (delete-then-write). Owner capacity: 6 cleaning + 1 repair tech, ~12,000 billable hrs/yr (PROVISIONAL, pending Skimmer visit durations). FY2025 result: overhead+depreciation $253,498.30, labor $629,317.61 -> true cost-per-hour lifts off the seeded $25.30 guess to the real QB-derived figure. 2 tests (152 -> 154).'),
    ('2026.06.30-z', 'shipped', 'QuickBooks', 'Tier 1 P&L translator: real cost base reconciled to the penny',
     'CSV report tokenizer (csv_report.py, handles quoted-comma account names + parenthesized negatives + exact-Decimal money) + ProfitAndLossTranslator (pnl.py) land each P&L cost account as an ExpenseActual at (period, basis) with role overhead/cogs/labor/depreciation; revenue excluded (FreshBooks owns it); section/parent/Total control rows retained but never summed; unknown accounts quarantined. New ExpenseActual model (provenance report_provisional, supersede-ready, verbatim amount_raw for exact tie-outs). Owner-confirmed role overrides applied (apply_confirmed_account_roles: Payroll Processing->overhead, Ask My Accountant->excluded). New quickbooks_costing.py reconciles imported cost to the P&L own control totals in exact Decimal + previews cost-per-hour beside the seeded number (no live ExpenseItem apply -- gated on clean-period file + owner sign-off). Validated end-to-end on the real books: chart + P&L, 0 quarantined, 0 byte-round-trip failures, COGS ties $3,188,061.09 exactly, Expense ties $5,987,076.71 exactly. Reconciliation caught a real IIF quoted-name bug ("Licenses, Permits,Other Taxes") mid-validation -- fixed via _unquote. 7 tests (145 -> 152). Cost-per-hour NUMBER still pending real billable-capacity + a known 12-month P&L period.'),
    ('2026.06.30-y', 'shipped', 'QuickBooks', 'Chart-of-Accounts translator (Tier 0) validated on real books',
     'IIF tokenizer (app/connectors/quickbooks/iif.py) turns an Intuit Interchange File into verbatim, byte-addressed rows with per-band column headers; ChartOfAccountsTranslator (coa.py) maps !ACCNT rows to LedgerAccount. account_type + normal_balance are deterministic from the QuickBooks ACCNTTYPE code (BANK/AR/FIXASSET/COGS/EXP/INC/...); account_role + is_labor_account + is_depreciation_account are PROPOSED by keyword and await owner confirmation before any Tier 1 cost applies. Non-account rows (357 vendors, headers, blanks) are retained and reason-coded (deferred_to_tier2_vendors, iif_header), never dropped. Validated against the owner real qb_lists.IIF: 495 rows, 79 accounts interpreted, 0 quarantined, 0 byte-round-trip failures, 0 accounts missing a type/balance. Real cost base surfaced: 5 COGS + 28 overhead + 4 proposed-labor (Payroll Expenses/Taxes/Processing/Bonus -- Payroll Processing is really a fee, owner to reclassify) + 2 depreciation; income accounts excluded (FreshBooks owns revenue). 7 tests (138 -> 145). Next: owner confirms labor/depreciation tags, then Tier 1 P&L overhead -> cost-per-hour.'),
