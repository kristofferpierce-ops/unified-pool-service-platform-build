...
st.set_page_config(page_title="Bridge Routing Network Transport Interface Scaffold Operator Signoff", page_icon="🖊️", layout="wide")
...
def latest_reports(limit: int = 20) -> list[Path]:
    ...
    return sorted(
        [
            p
            for p in BACKUP_DIR.glob("phase20_bridge_routing_network_transport_interface_scaffold_operator_signoff_*")
            if p.is_dir() and (p / "phase20_bridge_routing_network_transport_interface_scaffold_operator_signoff.json").exists()
        ],
...
st.title("🖊️ Phase 20 Bridge Routing Network Transport Interface Scaffold Operator Signoff")
st.caption("Operator signoff dossier for future interface-adapter design review. This does not authorize bridge writes.")

st.warning(
    "This page is network-transport-interface-scaffold-operator-signoff-only. It does not write to the bridge, "
...
if not reports:
    st.info(
        "No bridge routing network transport interface scaffold operator signoff dossier found yet. Run "
        "`scripts\\phase20_generate_bridge_routing_network_transport_interface_scaffold_operator_signoff.ps1` from the platform repo."
    )
    st.stop()

choice = st.selectbox(
    "Interface scaffold operator signoff dossier",
    options=list(range(len(reports))),
    format_func=lambda i: f"{reports[i].name} - {reports[i].stat().st_mtime_ns}",
)
report_dir = reports[choice]
json_path = report_dir / "phase20_bridge_routing_network_transport_interface_scaffold_operator_signoff.json"
checklist_csv_path = report_dir / "phase20_bridge_routing_network_transport_interface_scaffold_operator_signoff_checklist.csv"
issues_csv_path = report_dir / "phase20_bridge_routing_network_transport_interface_scaffold_operator_signoff_issues.csv"
md_path = report_dir / "phase20_bridge_routing_network_transport_interface_scaffold_operator_signoff.md"
...
if (
    safety.get("bridge_routing_network_transport_interface_scaffold_operator_signoff_only")
    ...
    st.error("Review interface scaffold operator signoff safety flags before continuing.")

if signoff.get("status") == "blocked":
    st.error("Interface scaffold operator signoff dossier is blocked. Resolve blockers before future real transport design.")
    ...
    st.success("Dossier is ready for operator interface-adapter design review. This still does not authorize execution.")
...
for label, path, filename in [
    ("Download signoff checklist CSV", checklist_csv_path, "phase20_bridge_routing_network_transport_interface_scaffold_operator_signoff_checklist.csv"),
    ("Download signoff issues CSV", issues_csv_path, "phase20_bridge_routing_network_transport_interface_scaffold_operator_signoff_issues.csv"),
]:
    if path.exists():
        st.download_button(label, path.read_text(encoding="utf-8-sig"), filename, "text/csv")

st.download_button(
    "Download interface scaffold operator signoff JSON",
    json.dumps(report, indent=2, default=str),
    "phase20_bridge_routing_network_transport_interface_scaffold_operator_signoff.json",
    "application/json",
)

with st.expander("Raw interface scaffold operator signoff JSON"):
    st.json(report)
